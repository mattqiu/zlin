...
class supplier_albumControl extends BaseSupplierControl {
...
//      Tpl::output('show_page',$page->show());
        ...
        Tpl::showpage('supplier_album.list');
    }
    /**
     * 相册分类添加
     *
    ...
    public function album_addOp(){
        ...
        Tpl::showpage('supplier_album.class_add','null_layout');
    }
    /**
     * 相册保存
     *
    ...
    public function album_add_saveOp(){
        if (chksubmit()){
            ...
            if($class_count['count'] >= 20){
                showDialog(Language::get('album_class_save_max_20'),'index.php?act=supplier_album','error',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
            /**
             * 实例化相册模型
             */
            ...
            if($return){
                showDialog(Language::get('album_class_save_succeed'),'index.php?act=supplier_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
        }
    ...
    public function album_editOp(){
        ...
        Tpl::showpage('supplier_album.class_edit','null_layout');
    }
    /**
     * 相册分类编辑保存
     */
    public function album_edit_saveOp(){
        ...
        if($return){
            ...
            if($re){
                showDialog(Language::get('album_class_edit_succeed'),'index.php?act=supplier_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
        }else{
    ...
    public function album_delOp(){
        ...
        if($return){
            showDialog(Language::get('album_class_file_del_succeed'),'index.php?act=supplier_album','succ');
        }else{
            showDialog(Language::get('album_class_file_del_lose'));
        }
    }
    ...
    public function album_pic_listOp(){
        ...
        Tpl::showpage('supplier_album.pic_list');
    }
    /**
     * 图片列表，外部调用
     */
    public function pic_listOp(){
        ...
        switch($_GET['item']) {
        case 'goods':
            Tpl::showpage('supplier_goods_add.step2_master_image','null_layout');
            break;
        case 'des':
            Tpl::showpage('supplier_goods_add.step2_desc_image','null_layout');
            break;
        case 'groupbuy':
            Tpl::showpage('supplier_groupbuy.album','null_layout');
            break;
        case 'store_sns_normal':
            Tpl::showpage('supplier_sns_add.album', 'null_layout');
            break;
        case 'goods_image':
            Tpl::output('color_id', $_GET['color_id']);
            Tpl::showpage('supplier_goods_add.step3_goods_image', 'null_layout');
            break;
        case 'mobile':
            Tpl::output('type', $_GET['type']);
            Tpl::showpage('supplier_goods_add.step2_mobile_image', 'null_layout');
            break;
        }
    }
    /**
    ...
    public function change_album_coverOp(){
        ...
        if($return){
            ...
            if($re){
                showDialog(Language::get('im_common_op_succ'),'reload','succ');
            }
        }else{
            showDialog(Language::get('im_common_op_fail'));
        }
    ...
    public function album_pic_infoOp(){
        ...
        Tpl::showpage('supplier_album.pic_info');
    }
    ...
    public function album_ad_ajaxOp(){
        ...
        Tpl::showpage('supplier_album.pic_scroll_ajax','null_layout');
    }
    ...
    public function album_pic_moveOp(){
        ...
        Tpl::output('class_list',$class_list);
        Tpl::showpage('supplier_album.move','null_layout');
    }
	...
	public function album_pic_watermarkOp(){
		...
		if ($store_wm_info['wm_image_name'] == '' && $store_wm_info['wm_text'] == ''){
			showDialog(Language::get('album_class_setting_wm'),"index.php?act=supplier_album&op=store_watermark",'error','CUR_DIALOG.close();');//"请先设置水印"
		}
    ...
    private function profile_menu($menu_type,$menu_key=''){
        ...
        switch ($menu_type) {
            case 'album':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('im_member_path_my_album'),'menu_url'=>'index.php?act=supplier_album'),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('im_member_path_watermark'),'menu_url'=>'index.php?act=supplier_album&op=store_watermark')
                );
                break;
            case 'album_pic':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('im_member_path_my_album'),'menu_url'=>'index.php?act=supplier_album'),
                3=>array('menu_key'=>'pic_list','menu_name'=>Language::get('im_member_path_album_pic_list'),'menu_url'=>'index.php?act=supplier_album&op=album_pic_list&id='.intval($_GET['id'])),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('im_member_path_watermark'),'menu_url'=>'index.php?act=supplier_album&op=store_watermark')
                );
                break;
            case 'album_pic_info':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('im_member_path_my_album'),'menu_url'=>'index.php?act=supplier_album'),
                3=>array('menu_key'=>'pic_info','menu_name'=>Language::get('im_member_path_album_pic_info'),'menu_url'=>'index.php?act=supplier_album&op=album_pic_info&id='.intval($_GET['id']).'&class_id='.intval($_GET['class_id'])),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('im_member_path_watermark'),'menu_url'=>'index.php?act=supplier_album&op=store_watermark')
                );
                break;
        }
