...
class channelControl extends BaseHomeControl {
    ...
    public function indexOp() {		
		...
		Tpl::output('channel_style',$channel['channel_style']);
        if ($channel['gc_id'] > 0) {
            $gc_id = $channel['gc_id'];
		...
		if ($condition['channel_id'] == 603){
			Tpl::output('index_sign','qcshop');
		}
		if ($condition['channel_id'] == 604){
			Tpl::output('index_sign','mmtshop');
		}		
    ...
    public function get_right_listOp(){
        ...
        $goods_list = array();
        $list = $model_goods->getGoodsListByCommonidDistinct($condition,$field,'goods_edittime desc',99);
