...
class buyControl extends BaseBuyControl {
    ...
    public function buy_step1Op() {
        ...
        foreach ($result['store_cart_list'] as $key => $value) {
        	//店铺商品云币抵扣金额
        	$store_vip_points_total = $result['store_point_total'][$key]['store_vip_points_total'];
        	$points_amount += $store_vip_points_total;
        	//返利云币数
        	$rebate_points = $result['store_other_total'][$key]['order_pointscount'];
        	$order_pointscount += $rebate_points;
        }
        $points_total = ceil($points_amount/C("points_trade"));
        //获取会员等级
        if(!empty($_SESSION['M_grade_level'])){
        	$member_grade = $_SESSION['M_grade_level'];
        }else{
        	$model_member = Model('member');
        	$member_info = $model_member->getMemberInfoByID($_SESSION['member_id'],'mc_id,member_points,member_exppoints');
        	$member_grade = $model_member->getOneMemberGradeLevel($member_info['member_exppoints']);
        }
        Tpl::output('member_grade', $member_grade); //会员等级
        //计算订单可使用云币总和
        Tpl::output('points_total', $points_total); //云币数
        Tpl::output('points_amount', $points_amount);//云币可抵用的额度
        Tpl::output('order_pointscount', $order_pointscount); //赠送云币数
        //商品金额计算(分别对每个商品/优惠套装小计、每个店铺小计)、云币汇总
        Tpl::output('store_cart_list', $result['store_cart_list']);
        Tpl::output('store_goods_total', $result['store_goods_total']);		
        Tpl::output('store_point_total', $result['store_point_total']);
    ...
    public function payOp() {
        ...
        $order_list = $model_order->getOrderList($condition,'','order_id,order_state,payment_code,order_amount,rcb_amount,pd_amount,order_sn,store_id,store_name,add_time,shipping_fee,pay_sn,pay_method, pay_store_id','','',array(),true);
        if (empty($order_list)) {
            ...
            if ($order_info['payment_code'] == 'offline') {
                ...
                if ($payed_amount > 0) {
                    ...
                    if (floatval($order_info['pd_amount']) > 0) {
                        $payed_tips .= ' 积分已支付：￥'.$order_info['pd_amount'];
                    }
        ...
        if (empty($pay_amount_online) && empty($pay_amount_offline)) {
            redirect('index.php?act=buy&op=pay_ok&pay_sn='.$pay_sn.'&pay_amount='.imPriceFormat($pay_amount));
        }
        ...
        Tpl::output('pay_amount_online',imPriceFormat($pay_amount_online));
        Tpl::output('pd_amount',imPriceFormat($pd_amount));
		
        //显示支付接口列表
        if ($pay_amount_online > 0) {
            $model_payment = Model('payment');
            $condition = array();
			$payment_list = $model_payment->getPaymentOpenList($condition,$Payment_info['pay_store_id'],$Payment_info['token']);//支付方式_待修改
     	...
     	if (chksubmit()){
     		...
     		if ($_POST['invoice_type'] == 2) {
     		    if (empty($_POST['inv_company']) || empty($_POST['inv_code']) || empty($_POST['inv_reg_addr'])) {
     		        exit(json_encode(array('state'=>false,'msg'=>Language::get('im_common_save_fail','UTF-8'))));
     		    }
     		}
			$data = array();
			...
			if ($insert_id) {
				...
				exit(json_encode(array('state'=>'fail','msg'=>Language::get('im_common_save_fail','UTF-8'))));
			}
     	} else {
     		Tpl::showpage('buy_address.add','null_layout');
     	}
     ...
     * 推送跨境淘 支付成功的订单
     */
    public function push_ikjtao_okOp() {
    	$pay_sn	= $_GET['pay_sn'];
    	if (!preg_match('/^\d{18}$/',$pay_sn)){
    		showMessage(Language::get('cart_order_pay_not_exists'),'index.php?act=member_order','html','error');
    	}
