...
class goodsControl extends BaseGoodsControl {
	...
	public function goodsaddOp(){
		...
		$model_goods = Model('goods');
		//获取商品信息
		$goods = $model_goods->field('goods_id,goods_commonid,goods_name,is_market,store_id,store_name,goods_image')->find($goods_id);  //查询商品
	...
	public function save_consultOp(){
        ...
        if (process::islock('commit')){
            showDialog(L('im_common_op_repeat'));
        }else{
