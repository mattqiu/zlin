...
class store_navigationControl extends BaseSellerControl {
    ...
    public function navigation_saveOp() {
        ...
        showDialog(L('im_common_op_succ'), urlShop('store_navigation', 'navigation_list'), 'succ');
    }
    ...
    public function navigation_delOp() {
        ...
        if($sn_id > 0) {
            ...
            showDialog(L('im_common_op_fail'), urlShop('store_navigation', 'navigation_list'), 'error');
        }
    }
	
	public function quick_linkOp() {
		Tpl::output('sn_id', $_GET['sn_id']);
		// 商品分类
        $store_goods_class = Model('store_goods_class')->getClassTree(array('store_id' => $_SESSION['store_id'], 'stc_state' => '1'));
        Tpl::output('store_goods_class', $store_goods_class);
		//展厅列表
		$custom_list = Model('store_decoration')->getStoreDecorationList(array('store_id' => $_SESSION['store_id'],'decoration_type' => array('gt',0)));
        Tpl::output('custom_list', $custom_list);
		
        Tpl::showpage('store_quick_link_select','null_layout');	
    }
