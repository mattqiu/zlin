...
class memberControl extends BaseMemberControl{
    ...
    public function ajax_load_member_infoOp() {		
		...
		//推广员或导购员链接
		if (OPEN_STORE_EXTENSION_STATE > 0 && ($member_info['mc_id']==1 || $member_info['mc_id']==2)){
			$model_extension = Model('extension');
			$extension_info = $model_extension->getExtensionByMemberID($member_info['member_id'],'mc_id,commis_balance,commis_totals');
			...
			$extension_info['extension_link'] = MAIN_SITE_URL.'/index.php?extension='.urlsafe_b64encode($member_info['member_id']);			
			if ($member_info['mc_id']==2){				
			    $extension_info['my_subordinate'] = $model_extension->countPromotionChild($member_info['store_id'], $_SESSION['member_id']);
			    $extension_info['my_subordinate_all'] = $model_extension->countAllPromotionChild($member_info['store_id'], $_SESSION['member_id']);		        
			}
			Tpl::output('extension_info',$extension_info);
		}		
        
        //代金券数量
        $member_info['voucher_count'] = Model('voucher')->getCurrentAvailableVoucherCount($_SESSION['member_id']);
		//推广二维码
		$member_info['extension_qrcode'] = GetExtensionQRcode($_SESSION['member_id']);
