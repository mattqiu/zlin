...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function goods_listOp() {
        ...
        //根据店铺的ID 查出该店铺是否是分店
        $storeInfo = $model_store->getStoreInfo($where);
       	if(!empty($storeInfo['parent_id'])){
       		$parent_str = explode('|', $storeInfo['parent_tree']);
       		$where['store_id'] = $parent_str['1'];
       	}
        if (intval($_GET['stc_id']) > 0) {
            $where['goods_stcids'] = array('like', '%,' . intval($_GET['stc_id']) . ',%');
        }
        ...
        if ($_GET['gtype'] == 'fxgoods') {
        	$goods_list = $model_goods->getGoodsListByCommonidDistinct($where);
        }else{
        $goods_list = $model_goods->getGoodsCommonOnlineList($where);
        }
        Tpl::output('show_page', $model_goods->showpage());
        ...
        if ($_GET['gtype'] == 'fxgoods') {
        	$this->profile_menu('fxgoods_list', 'fxgoods_list');
        	Tpl::showpage('store_goods_list.onlinefx');
        }else{
    ...
    public function edit_goodsOp() {
        ...
        // 取得商品规格的输入值
        $goods_array = $model_goods->getGoodsList($where, 'goods_id,goods_marketprice,goods_tradeprice,goods_price,goods_storage,goods_serial,goods_barcode,goods_storage_alarm,goods_spec');
        $sp_value = array();
        if (is_array($goods_array) && !empty($goods_array)) {
            ...
            foreach ( $goods_array as $k => $v ) {
                ...
                if (!empty($a)) {
					...
					$sp_value ['i_' . $id . '|tradeprice'] = $v['goods_tradeprice'];
		...
		// 会员级别
        $grade_list = Model('member')->getMemberGradeArr();
        Tpl::output('grade_list', $grade_list);
		
		// 佣金提成
        $commis_list = Model('extension_commis_class')->getCommisList($_SESSION['store_id']);
...
//         $price_list = $model_price->getPriceList(DEFAULT_PLATFORM_STORE_ID);
//         Tpl::output('priceclass_list',$price_list);
    ...
    public function edit_save_goodsOp() {
		...
		//$update_common['store_type']         = intval($this->store_info['store_type']);
        $update_common['goods_image']        = $_POST['image_path'];
		$update_common['goods_advpic']       = $_POST['advpic_path'];
        $update_common['goods_price']        = floatval($_POST['g_price']);
        $update_common['goods_marketprice']  = floatval($_POST['g_marketprice']);
		$update_common['goods_tradeprice']   = floatval($_POST['g_tradeprice']);
		...
		$update_common['view_grade']         = intval($_POST['view_grade'])?intval($_POST['view_grade']):0;
		$update_common['promotion_cid']      = intval($_POST['promotion_cid'])?intval($_POST['promotion_cid']):0;
		$update_common['saleman_cid']        = intval($_POST['saleman_cid'])?intval($_POST['saleman_cid']):0;	
		$update_common['rebate_cid']         = intval($_POST['rebate_cid'])?intval($_POST['rebate_cid']):0;	

		//<!-- 分销选入店铺 start zhang -->
		/**
		 * 预售商品不需要设置发货时间，只需设置客户下单后几天发货即可
		 */
		$update_common['presell_deliverdate']= $update_common['is_presell'] == 1? strtotime("+7 days"):'';//strtotime($_POST['g_deliverdate']) : time()+7*24*60*60; // 预售商品的发货时间
		
		$update_common['is_market']       	 = intval($_POST['is_market']);			//是否加入市场
		$update_common['baifen']       		 = $update_common['is_market']==1?floatval(($update_common['goods_price']-$update_common['goods_tradeprice'])/$update_common['goods_tradeprice']) : $_POST['baifen'];			//设置分销百分比率
		$update_common['goods_take']		 = floatval($_POST['g_collect']);// $update_common['is_market'] == 1? floatval($_POST['goods_take']) : '0.00'; // 只有加入市场的商品才可以设置批发价
		$update_common['goods_gain']		 = floatval($_POST['g_gain']);//平台利润
		$update_common['presell_days']		= $update_common['is_presell'] == 1 ? intval($_POST['g_presell']) : ''; // 预售商品的后几天发货
		$update_common['give_points']	 	= intval($_POST['g_points']);//赠送云币
		//只需添加goods-common表即可
		$update_common['fx_minprice']       = $update_common['is_market']==1 ? floatval($_POST['fx_minprice']):0.00;//最低售价
		if(empty($update_common['fx_minprice'] )){
			$update_common['fx_minprice'] = $update_common['goods_price'];
		}
		$update_common['fx_maxprice']       = $update_common['is_market']==1 ? floatval($_POST['fx_maxprice']):"0.00";			//最高价
		
		//众筹商品 zhangchao
		$update_common['is_crowdfunding']    = $update_common['goods_state'] == 1 ? intval($_POST['is_crowdfunding']) : 0;     // 只有出售中的商品可以众筹
		$update_common['crowdfunding_deliverdate']= $update_common['is_crowdfunding'] == 1? strtotime($_POST['g_crowdfundingdate']) : strtotime("+30 days"); // 众筹商品的发货时间
		$update_common['cf_starttime']		= $update_common['is_crowdfunding'] == 1? strtotime($_POST['cf_starttime']) : ''; // 众筹开始时间
		$update_common['cf_endtime']		= $update_common['is_crowdfunding'] == 1? strtotime($_POST['cf_endtime']) : ''; // 众筹商品的结束时间
        ...
        if (is_array ( $_POST ['spec'] )) {
            foreach ($_POST['spec'] as $value) {
                ...
                if (!empty($goods_info)) {
                    ...
                    $update['store_name']        = $_SESSION['store_name'];
				   	//<!-- 分销选入店铺 start zhang -->
					$update['is_market']       	 = $update_common['is_market'];			//是否加入市场
					$update['baifen']       	 = $update_common['baifen'];			//设置分销百分比率
					//众筹商品 zhangchao
					$update['is_crowdfunding']   = $update_common['is_crowdfunding'];     // 只有出售中的商品可以众筹
					$update['goods_take']	 	=  $update_common['goods_take'];//加入市场的商品批发价
					$update['goods_gain']		 = $update_common['goods_gain'];//平台利润
					$update['presell_days']	 = $update_common['presell_days']; // 预售商品的后几天发货
					$update['cf_starttime']		 = $update_common['cf_starttime']; // 众筹开始时间
					$update['cf_endtime']		 = $update_common['cf_endtime'] ; // 众筹商品的结束时间
					$update['give_points']	 	= $update_common['give_points'];//赠送云币
		    		//$update['store_type']        = $update_common['store_type'];
					...
					$update['goods_tradeprice']  = $value['tradeprice'] == 0 ? $update_common['goods_tradeprice'] : $value['tradeprice'];
					...
					$update['goods_advpic']      = $update_common['goods_advpic'];
					...
					$update['view_grade']        = $update_common['view_grade'];
					$update['promotion_cid']     = $update_common['promotion_cid'];
					$update['saleman_cid']       = $update_common['saleman_cid'];
					$update['rebate_cid']        = $update_common['rebate_cid'];					
					$update['give_points']    = $model_commis->getCommisPriceByID($update['rebate_cid'],$update['goods_price'],$update['goods_costprice']);
					
					$update['promotion_amount']  = $model_commis->getCommisPriceByID($update['promotion_cid'],$update['goods_price'],$update['goods_costprice']);
					$update['saleman_amount']    = $model_commis->getCommisPriceByID($update['saleman_cid'],$update['goods_price'],$update['goods_costprice']);
					
                    $model_goods->editGoodsById($update, $goods_id);
					// 生成商品二维码							
					$PhpQRCode->BuildGoodsQRCode($_SESSION['store_id'],$goods_id,$_POST['image_path']);					
                } else {
                    $insert = array();
                    ...
                    //<!-- 分销选入店铺 start zhang -->
                    $insert['is_market']       	 = $update_common['is_market'];			//是否加入市场
                    $insert['baifen']       	 = $update_common['baifen'];			//设置分销百分比率
                    //众筹商品 zhangchao
                    $insert['is_crowdfunding']   =  $update_common['is_crowdfunding'];     // 只有出售中的商品可以众筹
                    $insert['goods_take']	 	= $update_common['goods_take'];//加入市场的商品批发价
                    $insert['goods_gain']		 = $update_common['goods_gain'];//平台利润
                    $update['give_points']	 	= $update_common['give_points'];//赠送云币
                    $insert['presell_days']	 = $update_common['presell_days']; // 预售商品的后几天发货
                    $insert['cf_starttime']		 = $update_common['cf_starttime']; // 众筹开始时间
                    $insert['cf_endtime']		 = $update_common['cf_endtime']; // 众筹商品的结束时间
		    		//$update['store_type']        = $update_common['store_type'];
					...
					$insert['goods_tradeprice']  = $value['tradeprice'] == 0 ? $update_common['goods_tradeprice'] : $value['tradeprice'];
					...
					$insert['goods_advpic']      = $update_common['goods_advpic'];
					...
					$insert['view_grade']        = $update_common['view_grade'];
					
					$insert['promotion_cid']     = $update_common['promotion_cid'];
					$insert['saleman_cid']       = $update_common['saleman_cid'];
					$insert['rebate_cid']        = $update_common['rebate_cid'];
					$insert['give_points']    = $model_commis->getCommisPriceByID($insert['rebate_cid'],$insert['goods_price'],$insert['goods_costprice']);
					
					$insert['promotion_amount']  = $model_commis->getCommisPriceByID($insert['promotion_cid'],$insert['goods_price'],$insert['goods_costprice']);
					$insert['saleman_amount']    = $model_commis->getCommisPriceByID($insert['saleman_cid'],$insert['goods_price'],$insert['goods_costprice']);
            ...
            if (!empty($goods_info)) {
				...
				$update['goods_tradeprice']  = $update_common['goods_tradeprice'];
				$update['goods_costprice']   = $update_common['goods_costprice'];
				...
				$insert['goods_tradeprice']  = $update_common['goods_tradeprice'];
				$insert['goods_costprice']   = $update_common['goods_costprice'];
        ...
        //如果为分销商品则不更新公共表
        if($_POST['type']='fxgoods' && !empty($_POST['type'])){
        	$return = 1;
        }else{
        ...
        if ($return) {
            //提交事务
            Model()->commit();
            showDialog(L('im_common_op_succ'), $_POST['ref_url'], 'succ');
        } else {
            //回滚事务
            Model()->rollback();
            showDialog(L('store_goods_index_goods_edit_fail'), urlShop('store_goods_online', 'index'));
    ...
    public function edit_save_imageOp() {
        if (chksubmit()) {
            ...
            $rs = Logic('goods')->editSaveImage($_POST['img'], $common_id, $_SESSION['store_id'], $_SESSION['store_id'], $_SESSION['store_name']);
            if ($rs) {
            // 添加操作日志
            $this->recordSellerLog('编辑商品，平台货号：'.$common_id);
                showDialog(L('im_common_op_succ'), $_POST['ref_url'], 'succ');
            } else {
                showDialog(L('im_common_save_fail'), urlShop('store_goods_online', 'index'));
            }
        }
    }
    ...
    public function goods_unshowOp() {
        ...
        //下架分销商的商品
        $fxwhere['goods_commonid'] = $where['goods_commonid'];
        $fxwhere['up_id'] = $_SESSION['store_id'];
        Model('goods')->editProducesOffline($fxwhere);
        if ($return) {
            // 更新优惠套餐状态关闭
     ...
     * 分销商 商品下架
     */
    public function fxgoods_unshowOp() {
    	$common_id = $this->checkRequestCommonId($_GET['commonid']);
    	$commonid_array = explode(',', $common_id);
    	$model_goods = Model('goods');
    	$where = array();
    ...
    public function edit_jingleOp() {
        if (chksubmit()) {
            ...
            if ($return) {
                ...
                showDialog(L('im_common_op_fail'), 'reload');
            }
        }
        $common_id = $this->checkRequestCommonId($_GET['commonid']);
     ...
     * 设置商品分类
     */
    public function edit_goodsClassOp() {
    	if (chksubmit()) {
    		$common_id = $this->checkRequestCommonId($_POST['commonid']);
    		$commonid_array = explode(',', $common_id);
    		$gtype = $_POST['gtype'];
    		...
    		if(!empty($gtype) && $gtype == 'fxgoods'){
    			$return = Model('goods')->editGoods($update_common, $where);
    		}else{
	    		Model('goods')->editGoodsCommon($update_common, $where);
	    		$return = Model('goods')->editGoods($update_common, $where);
    		}
    		if ($return) {
    			// 添加操作日志
    			$this->recordSellerLog('设置商品分类，平台货号：'.$common_id);
    			showDialog(L('im_common_op_succ'), 'reload', 'succ');
    		} else {
    			showDialog(L('im_common_op_fail'), 'reload');
    		}
    	...
    	$store_goods_class = Model('store_goods_class')->getClassTree(array('store_id' => $_SESSION['store_id'], 'stc_state' => '1'));
    	Tpl::output('store_goods_class', $store_goods_class);
    	Tpl::showpage('store_goods_list.edit_goodsclass', 'null_layout');
    }
    /**
     * 添加赠品
     */
    ...
    public function save_giftOp() {
        ...
        showDialog(L('im_common_save_succ'), $_POST['ref_url'], 'succ');
    }
     ...
     * 申请补货
     */
    public function replenish_goodsOp() {
		$common_id = $this->checkRequestCommonId($_GET['commonid']);
        $commonid_array = explode(',', $common_id);
        $model_goods = Model('goods');
        $where = array();
        $where['goods_commonid'] = array('in', $commonid_array);
        $where['store_id'] = $_SESSION['store_id'];
        $goods_list = Model('goods')->getGoodsList($where,'goods_id,goods_name,goods_costprice,goods_price,goods_storage,goods_storage_alarm,goods_image,src_id');
        if ($goods_list) {
            Tpl::output('goods_list', $goods_list);
            Tpl::showpage('store_goods_list.replenish', 'null_layout');
        } else {
            showDialog('商品信息缺失，补货申请失败！', getReferer() ? getReferer() : 'index.php?act=store_goods_online&op=goods_list', 'succ', '', 2);
        }		
	}
    ...
    public function replenish_saveOp() {
		...
		if (is_array($data_goods_array) && !empty($data_goods_array)) {
		  ...
		  foreach ($data_goods_array as $k => $v ) {
			...
			$b_id[]=$goods_info[2];
			$data_goods_info[$goods_info[0]]['p_id']=$goods_info[0];
			$data_goods_info[$goods_info[0]]['nums']=$goods_info[1];
			$data_goods_info[$goods_info[0]]['b_id']=$goods_info[2];
		  }
		}
		if (empty($b_id) || !is_array($b_id)){
			showDialog('商品信息缺失，补货申请失败！', getReferer() ? getReferer() : 'index.php?act=store_goods_online&op=goods_list', 'succ', '', 2);
		}
		$parent_id = $_GET['pst_id'];
		$parent_name = Model('store')->getStoreNameByID($parent_id);
		
		$model_apply = Model('branch_apply');		
		$branch_apply = array();
		$branch_apply['bp_store_id'] = $parent_id;
		$branch_apply['bp_store_name'] = $parent_name;
		$branch_apply['bp_branch_id'] = $_SESSION['store_id'];
		$branch_apply['bp_branch_name'] = $_SESSION['store_name'];
		$branch_apply['bp_type'] = 1;
		$branch_apply['bp_addinfo'] = serialize($data_goods_info);
		$branch_apply['bp_dispose'] = 0;
		$branch_apply['bp_views'] = 0;
		$branch_apply['bp_replyinfo'] = '';
		$branch_apply['bp_addtime'] = TIMESTAMP;
		$branch_apply['bp_distime'] = 0;
		$branch_apply['bp_order_sn'] = 0;
		$branch_apply['bp_admin'] = '';
		$return = $model_apply->insert($branch_apply);
     ...
     * 申请退货
     */
    public function returned_goodsOp() {
		$common_id = $this->checkRequestCommonId($_GET['commonid']);
        $commonid_array = explode(',', $common_id);
        $model_goods = Model('goods');
        $where = array();
        ...
        if ($goods_list) {
            Tpl::output('goods_list', $goods_list);
            Tpl::showpage('store_goods_list.returned', 'null_layout');
        } else {
            showDialog('商品信息缺失，退货申请失败！', getReferer() ? getReferer() : 'index.php?act=store_goods_online&op=goods_list', 'succ', '', 2);
        }			
	}
        ...
        switch ($menu_type) {
            ...
            case 'fxgoods_list':
                $menu_array = array(
				   ...
				   //<!-- 分销选入店铺 start zhang -->
				   array('menu_key' => 'fxgoods_list',    'menu_name' => '分销的商品', 'menu_url' => urlShop('store_goods_online', 'fxonline'))
                );
                break;
            case 'edit_detail':
    ...
    //<!-- 分销选入店铺 start zhang -->
    public function fxonlineOp(){
    
    	$condition =array();
    	$condition['store_id'] = $_SESSION['store_id'];
    	$condition['up_id'] = array('gt',0);
    	$model_goods = Model('goods');
    	$goods_list = $model_goods->getGoodsListByCommonidDistinct($condition);
     ...
     * 编辑分销商品
     */
    public function edit_fxgoodsOp() {
    	if (chksubmit()) {
    		$common_id = $this->checkRequestCommonId($_POST['commonid']);
    		$commonid_array = explode(',', $common_id);
    		$where = array('goods_commonid' => array('in', $commonid_array), 'store_id' => $_SESSION['store_id']);
    	...
    	Tpl::showpage('store_goods_list.edit_fxgoods', 'null_layout');
    }
    
    /**
     * 编辑分销商品价格
     */
    public function edit_fxpriceOp() {
    	if (chksubmit()) {
    		
    		$common_id = $this->checkRequestCommonId($_POST['commonid']);
    		$commonid_array = explode(',', $common_id);
    		//$gwhere['store_id'] = $_SESSION['store_id'];
    		//$gwhere['goods_commonid'] = array('in', $commonid_array);
    		foreach ($_POST['spec'] as $skey=>$spec){
    			$goods_id = $spec['goods_id'];
    			$gwhere['goods_id'] = $goods_id;
    			$gupdate['goods_marketprice'] 	= $spec['marketprice'];
    			$gupdate['goods_price'] 		= $spec['price'];
    			$gupdate['goods_tradeprice'] 	= $spec['tradeprice'];
    			$gupdate['goods_storage'] 		= $spec['stock'];
    			$gupdate['goods_serial'] 		= $spec['serial'];
    			$gupdate['goods_barcode'] 		= $spec['barcode'];
    			$return = Model('goods')->editGoods($gupdate,$gwhere);
    		}
    		
    			// 添加操作日志
    			$this->recordSellerLog('修改分销商品价格，平台货号：'.$common_id);
    			showDialog(L('im_common_op_succ'), 'reload', 'succ');
    		...
    		if (is_array($goods_array) && !empty($goods_array)) {
    			...
    			$spec_checked = array();
    			foreach ( $goods_array as $k => $v ) {
    				$gs_arr = unserialize($v['goods_spec']);
    				if (!empty($gs_arr)) {
    					$spec_arr = array_slice($gs_arr,0);
    					$goods_array[$k]['spec_arr'] = $spec_arr;
    				}
    			}
    		}
    		Tpl::output ('goods_array', $goods_array );
    	...
    	Tpl::showpage('store_goods_list.edit_fxprice', 'null_layout');
    }
    ...
    public function del_goodsOp(){
    	...
    	$sql ="DELETE FROM `zlin_goods` WHERE `zlin_goods`.`goods_commonid` = '{$comid}' and `zlin_goods`.`store_id`='{$store_id}'";
    	$result = mysql_query($sql);
    	if($result){
    		echo '<script>alert("删除分销商品");window.history.go(-2);</script>';
    	}
    }
    
    //推荐商品
    
    public function ti_goodsOp(){
    	include('mysql_config.php');
    	$comid = $_GET['commonid'];
    	$store_id = $_SESSION['store_id'];
    	$sql = "UPDATE `zlin_goods` SET `goods_commend` = '1' WHERE `zlin_goods`.`goods_commonid` ='{$comid}' and `zlin_goods`.`store_id`='{$store_id}'";
    	$result = mysql_query($sql);
    	if($result){
    		echo '<script>alert("设置成推荐商品");window.history.go(-1);</script>';
    	}
    }
    //不推荐
    
    public function bti_goodsOp(){
    	include('mysql_config.php');
    	$comid = $_GET['commonid'];
    	$store_id = $_SESSION['store_id'];
    	$sql = "UPDATE `zlin_goods` SET `goods_commend` = '0' WHERE `zlin_goods`.`goods_commonid` ='{$comid}' and `zlin_goods`.`store_id`='{$store_id}'";
    	$result = mysql_query($sql);
    	if($result){
    		echo '<script>alert("设置成不推荐商品");window.history.go(-1);</script>';
    	}
    }
