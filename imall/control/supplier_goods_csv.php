...
class supplier_goods_csvControl extends BaseSupplierControl {
	...
	public function indexOp(){
			...
			if(is_array($records) and count($records) > 0){
				foreach($records as $k=>$record){
					if($remain_num>0 and $k>=$remain_num){
						showMessage($lang['store_goods_index_goods_limit'].$store_grade['sg_goods_limit'].$lang['store_goods_index_goods_limit1'].$lang['store_goods_import_end'].(count($records)-$remain_num).$lang['store_goods_import_products_no_import'],'index.php?act=store_goods&op=supplier_goods_csv&step=4','html','error');
					}
		...
		Tpl::output('menu_sign','supplier_goods_csv');
		Tpl::showpage('supplier_goods_import');
	}
