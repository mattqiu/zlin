...
class store_goods_classControl extends BaseSellerControl {
	...
	public function goods_class_saveOp() {
		...
		if(isset($_POST['stc_id'])) {
			...
			if($state) {
				showDialog(Language::get('im_common_save_succ'),'index.php?act=store_goods_class&op=index','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
			} else {
	...
	public function drop_goods_classOp() {
		...
		if ($drop_state){
			showDialog(Language::get('im_common_del_succ'),'index.php?act=store_goods_class&op=store_goods_class','succ');
		}else{
