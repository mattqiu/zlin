...
class supplier_infoControl extends BaseSupplierControl {
    ...
    public function bind_class_saveOp() {
        ...
        $param = array();
        $param['store_id'] = $_SESSION['store_id']?$_SESSION['store_id']:$_SESSION['supplier_id'];
        $param['state'] = 0;
        ...
        if($result) {
            showDialog('申请成功，请等待系统审核','index.php?act=supplier_info&op=bind_class','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
        }else {
            showDialog($lang['im_common_save_fail']);
        }
    }
    ...
    public function reopenOp(){
        ...
        if(intval($store_info['store_end_time']) > 0) {
        	...
        	$store_info['store_end_time'] 	 = $store_info['store_time'] + 2592000;
        	$store_info['allow_applay_date'] = $store_info['store_end_time'] - 2592000;
        }
...
//              $model_store->editStore(array('store_end_time'=>$data['re_end_time']),array('store_id'=>$_SESSION['store_id']));
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'index':
                $menu_array[] = array('menu_key'=>'bind_class', 'menu_name'=>$lang['im_member_path_bind_class'], 'menu_url'=>'index.php?act=supplier_info&op=bind_class');
                if (!checkPlatformStore()) {
                    $menu_array[] = array('menu_key'=>'index', 'menu_name'=>$lang['im_member_path_store_info'], 'menu_url'=>'index.php?act=supplier_info&op=index');
                    $menu_array[] = array('menu_key'=>'reopen', 'menu_name'=>$lang['im_member_path_store_reopen'], 'menu_url'=>'index.php?act=supplier_info&op=reopen');
                }
                break;
            case 'bind_class':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>$lang['im_member_path_bind_class'], 'menu_url'=>'index.php?act=supplier_bind_class&op=index'),
                );
                break;
            case 'add':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>$lang['im_member_path_bind_class'], 'menu_url'=>'index.php?act=supplier_bind_class&op=index'),
                array('menu_key'=>'add', 'menu_name'=>$lang['im_member_path_bind_class_add'], 'menu_url'=>'index.php?act=supplier_bind_class&op=add')
                );
                break;
        }
        if(!empty($array)) {
