...
class store_snsControl extends BaseSellerControl{
	...
	public function store_sns_saveOp(){
		...
		switch ($_POST['type']){
			case '2':
				...
				if($sns_image != '') $content	= '<div class="fd-media">
									<div class="thumb-image"><a href="javascript:void(0);" im_type="thumb-image"><img src="'.$sns_image.'" /><i></i></a></div>
									<div class="origin-image"><a href="javascript:void(0);" im_type="origin-image"></a></div>
								</div>';
				break;
			case '9':
		...
		if($rs){
			showDialog(Language::get('im_common_op_succ'), 'index.php?act=store_sns', 'succ');
		}else{
	...
	private function profile_menu($menu_key) {
		$menu_array	= array(
				...
				3=>array('menu_key'=>'store_sns_brower', 'menu_name'=>Language::get('store_sns_browse'), 'menu_url'=>urlShop('store_snshome', 'index', array('store_id' => $_SESSION['store_id'])), 'target'=>'_blank')
		);
