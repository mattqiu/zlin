...
class member_connectControl extends BaseMemberControl {
	...
	public function wxbindOp(){
		...
		if (trim($this->member_info['member_wxinfo'])){
			$this->member_info['member_wxinfoarr'] = unserialize($this->member_info['member_wxinfo']);
		}
		Tpl::output('member_info',$this->member_info);
		//信息输出
		self::profile_menu('wx_bind');
		Tpl::showpage('member_wxbind');
	}
	/**
	 * 微信解绑
	 */
	public function wxunbindOp(){
		//修改密码
		...
		session_unset();
		session_destroy();
		showMessage('成功解除微信绑定！','index.php?act=login&ref_url='.urlencode('index.php?act=member_connect&op=wxbind'));
	}
	...
	private function profile_menu($menu_key='',$array=array()) {
		...
		$menu_array		= array();
		$menu_array = array(
			1=>array('menu_key'=>'qq_bind',	'menu_name'=>$lang['im_member_path_qq_bind'],	'menu_url'=>'index.php?act=member_connect&op=qqbind'),
			2=>array('menu_key'=>'wx_bind',	'menu_name'=>'微信绑定',	'menu_url'=>'index.php?act=member_connect&op=wxbind'),
			3=>array('menu_key'=>'sina_bind',	'menu_name'=>$lang['im_member_path_sina_bind'],	'menu_url'=>'index.php?act=member_connect&op=sinabind'),
		);
		if(!empty($array)) {
			$menu_array[] = $array;
		}
