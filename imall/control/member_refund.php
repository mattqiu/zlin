...
class member_refundControl extends BaseMemberControl {
	...
	public function add_refundOp(){
		...
		if (chksubmit() && $goods_id > 0){
			...
			$refund_array['refund_type'] = $_POST['refund_type'];//类型:1为退款不退货,2为即退款也退货
			...
			if ($state) {
				...
				showDialog(Language::get('im_common_save_succ'),$show_url,'succ');
			} else {
	...
	public function add_refund_allOp(){
		...
		if (chksubmit()) {
			...
			if ($state) {
			    $model_refund->editOrderLock($order_id);
				showDialog(Language::get('im_common_save_succ'),'index.php?act=member_refund&op=index','succ');
			} else {
				showDialog(Language::get('im_common_save_fail'),'reload','error');
			}
		}
