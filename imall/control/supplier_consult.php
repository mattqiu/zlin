...
class supplier_consultControl extends BaseSupplierControl {
    ...
    public function reply_saveOp(){
        ...
        if($state){
            ...
            showDialog(Language::get('im_common_op_succ'),'reload','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
        } else {
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'consult':
                $menu_array = array(
                1=>array('menu_key'=>'consult_list',    'menu_name'=>Language::get('im_member_path_all_consult'),           'menu_url'=>'index.php?act=supplier_consult&op=consult_list'),
                2=>array('menu_key'=>'to_reply',    'menu_name'=>Language::get('im_member_path_unreplied_consult'),         'menu_url'=>'index.php?act=supplier_consult&op=consult_list&type=to_reply'),
                3=>array('menu_key'=>'replied', 'menu_name'=>Language::get('im_member_path_replied_consult'),           'menu_url'=>'index.php?act=supplier_consult&op=consult_list&type=replied'));
                break;
        }
        if(!empty($array)) {
            $menu_array[] = $array;
