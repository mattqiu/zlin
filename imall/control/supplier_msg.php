...
class supplier_msgControl extends BaseSupplierControl {
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array();
        $menu_array = array(
            array('menu_key'=>'msg_list',   'menu_name'=>'消息列表',    'menu_url'=>urlShop('supplier_msg', 'index')),
            array('menu_key'=>'sys_list',   'menu_name'=>'系统公告',    'menu_url'=>urlShop('supplier_msg', 'sys_list')),
            array('menu_key'=>'msg_setting','menu_name'=>'消息接收设置',  'menu_url'=>urlShop('supplier_msg', 'msg_setting')),
        );
        if (!$this->checkIsAdmin()) {
            unset($menu_array[2]);
        }
