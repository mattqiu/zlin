...
class show_live_groupbuyControl extends BaseGroupBuyControl {

    public function __construct() {
        parent::__construct();
        //读取语言包
        Language::read('live_groupbuy,home_cart_index');
    }
	...
	public function livegroupbuystep2Op(){
		if(chksubmit()){
			...
			if(isset($_POST['pd_pay']) && $_POST['pd_pay']==1){
				...
				if($params['price']>$member['available_predeposit']){//使用积分支付部分
	...
	public function paymentOp(){
		...
		$payment_info = $model_payment->getPaymentOpenInfo($condition_payment);//支付方式_待修改
