...
class member_snsindexControl extends BaseMemberControl {
	...
	public function indexOp(){
		...
		//信息输出
		Tpl::output('imhash',substr(md5(SHOP_SITE_URL.$_GET['act'].$_GET['op']),0,8));
	...
	public function addtraceOp(){
		...
		if ($result){
			...
			if (cookie('weibonum') != null && intval(cookie('weibonum')) >0){
				setIMCookie('weibonum',intval(cookie('weibonum'))+1,2*3600);//保存2小时
			}else{
				setIMCookie('weibonum',1,2*3600);//保存2小时
			}
			$js = "var obj = $(\"#weiboform\").find(\"[im_type='formprivacytab']\");$(obj).find('span').removeClass('selected');$(obj).find('ul li:nth-child(1)').find('span').addClass('selected');";
	...
	public function sharegoodsOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($result){
				...
				$content_str .= "<div class=\"fd-media\">
					...
					<div class=\"goodsinfo\">
						<dl>
							...
							<dd>".Language::get('sns_sharegoods_price').Language::get('im_colon').Language::get('currency').$goods_info['goods_price']."</dd>
							<dd>".Language::get('sns_sharegoods_freight').Language::get('im_colon').Language::get('currency').$goods_info['goods_freight']."</dd>
	                  		<dd imtype=\"collectbtn_{$goods_info['goods_id']}\"><a href=\"javascript:void(0);\" onclick=\"javascript:collect_goods(\'{$goods_info['goods_id']}\',\'succ\',\'collectbtn_{$goods_info['goods_id']}\');\">".Language::get('sns_sharegoods_collect')."</a></dd>
	                  	</dl>
	                  </div>
	             </div>";
				...
				//输出js
				$js = "DialogManager.close('sharegoods');var countobj=$('[im_type=\'sharecount_{$goods_info['goods_id']}\']');$(countobj).html(parseInt($(countobj).text())+1);";
				$url = '';
				if ($_GET['irefresh']){
					...
					<div class=\"goodsinfo\">
						<dl>
							<dt><a target=\"_blank\" href=\"{$store_info['store_url']}\">".$store_info['store_name']."</a></dt>
	                  		<dd imtype=\"storecollectbtn_{$store_info['store_id']}\"><a href=\"javascript:void(0);\" onclick=\"javascript:collect_store(\'{$store_info['store_id']}\',\'succ\',\'storecollectbtn_{$store_info['store_id']}\');\">".Language::get('sns_sharestore_collect')."</a></dd>
	                  	</dl>
	                  </div>
	             </div>";
				$insert_arr['trace_content'] = $content_str;
	...
	public function deltraceOp(){
		...
		if ($result){
			...
			if ($_GET['type'] == 'href'){
				showDialog(Language::get('im_common_del_succ'),'index.php?act=member_snshome&op=trace&mid='.$_SESSION['member_id'],'succ');
			}else {
				$js = "location.reload();";
				showDialog(Language::get('im_common_del_succ'),'','succ',$js);
			}
		} else {
			showDialog(Language::get('im_common_del_fail'),'','error');
		}
	}
	/**
	 * SNS动态列表
		...
		if (!empty($tracelist)){
			foreach ($tracelist as $k=>$v){
				if ($v['trace_title']){
					...
					$v['trace_title_forward'] = '|| @'.$v['trace_membername'].Language::get('im_colon').preg_replace("/<a(.*?)href=\"(.*?)\"(.*?)>@(.*?)<\/a>([\s|:|：]|$)/is",'@${4}${5}',$v['trace_title']);
				}
	...
	public function editprivacyOp(){
		...
		if ($result){
			...
			$js = "var obj = $(\"#recordone_{$id}\").find(\"[im_type='privacytab']\"); $(obj).find('span').removeClass('selected');$(obj).find('li:nth-child(".$privacy_item.")').find('span').addClass('selected');";
	...
	public function addcommentOp(){
		...
		if ($result){
			...
			if (cookie('commentnum') != null && intval(cookie('commentnum')) >0){
				setIMCookie('commentnum',intval(cookie('commentnum'))+1,2*3600);//保存2小时
			}else{
				setIMCookie('commentnum',1,2*3600);//保存2小时
			}
	...
	public function delcommentOp(){
		...
		if ($result){
			...
			$js .="$('.comment-list [im_type=\"commentrow_{$id}\"]').remove();";
			showDialog(Language::get('im_common_del_succ'),'','succ',$js);
		}else {
			showDialog(Language::get('im_common_del_fail'),'','error');
		}
	...
	public function editlikeOp(){
		...
		if ($result){
			...
			$content_str .= "<div class=\"fd-media\">
				...
				<div class=\"goodsinfo\">
					<dl>
						...
						<dd>".Language::get('sns_sharegoods_price').Language::get('im_colon').Language::get('currency').$goods_info['goods_price']."</dd>
						<dd>".Language::get('sns_sharegoods_freight').Language::get('im_colon').Language::get('currency').$goods_info['goods_freight']."</dd>
                  		<dd imtype=\"collectbtn_{$goods_info['goods_id']}\"><a href=\"javascript:void(0);\" onclick=\"javascript:collect_goods(\'{$goods_info['goods_id']}\',\'succ\',\'collectbtn_{$goods_info['goods_id']}\');\">".Language::get('sns_sharegoods_collect')."</a>&nbsp;&nbsp;(".$goods_info['goods_collect'].Language::get('sns_collecttip').")</dd>
                  	</dl>
                  </div>
             </div>";
			$insert_arr['trace_content'] = $content_str;
			...
			$insert_arr['trace_copycount'] = 0;
			$result = $tracelog_model->tracelogAdd($insert_arr);
			$js = "var obj = $(\"#likestat_{$goods_info['goods_id']}\"); $(\"#likestat_{$goods_info['goods_id']}\").find('i').addClass('noaction');$(obj).find('a').addClass('noaction'); var countobj=$('[im_type=\'likecount_{$goods_info['goods_id']}\']');$(countobj).html(parseInt($(countobj).text())+1);";
			showDialog(Language::get('im_common_op_succ'),'','succ',$js);
		}else {
			showDialog(Language::get('im_common_op_fail'),'','error');
		}
	}
	/**
	 * 添加转发
	...
	public function addforwardOp(){
		...
		if ($tracelog_info['trace_originalid'] > 0 || $tracelog_info['trace_from'] != 1){
			...
			$content_str .= Language::get('im_colon')."{$tracelog_info['trace_title']}</div>";
		...
		if ($result){
			...
			if ($_GET['irefresh']){
				...
				if (cookie('forwardnum') != null && intval(cookie('forwardnum')) >0){
					setIMCookie('forwardnum',intval(cookie('forwardnum'))+1,2*3600);//保存2小时
				}else{
					setIMCookie('forwardnum',1,2*3600);//保存2小时
				}
				if ($_GET['type']=='home'){
	...
	public function sharegoods_oneOp(){
		...
		$webShare = 0;
		if (!empty($app_arr) && is_array($app_arr)){
		...
		Tpl::output('webShare',$webShare);
		//推广处理 导购员或推广员链接
		$$extension_id = '';
		if ((OPEN_STORE_EXTENSION_STATE > 0 && ($_SESSION['M_mc_id']==1 || $_SESSION['M_mc_id']==2))){
		    $extension_id=urlsafe_b64encode($_SESSION['member_id']);
	    }
		$goods_info['goods_url'] = urlShop('goods' , 'index', array('goods_id'=>$goods_info['goods_id'],'extension'=>$extension_id));
		//信息输出
		Tpl::output('imhash',substr(md5(SHOP_SITE_URL.$_GET['act'].$_GET['op']),0,8));
	...
	public function sharestore_oneOp(){
		...
		$store_info['store_url'] = urlShop('show_store', 'index', array('store_id'=>$store_info['store_id'],'extension'=>$extension_id));
		//判断系统是否开启站外分享功能		
		if (C('share_isuse') == 1){
		    //站外分享接口
		    $model = Model('sns_binding');
