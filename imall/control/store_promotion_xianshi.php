...
class store_promotion_xianshiControl extends BaseSellerControl {
    ...
    public function xianshi_edit_saveOp() {
        ...
        if($result && $result) {
            ...
            showDialog(Language::get('im_common_op_succ'),self::LINK_XIANSHI_LIST,'succ','',3);
        }else {
    ...
    public function xianshi_delOp() {
        ...
        if($result) {
            ...
            showDialog(L('im_common_op_succ'), urlShop('store_promotion_xianshi', 'xianshi_list'), 'succ');
        } else {
    ...
    public function xianshi_quota_add_saveOp() {
        ...
        $this->recordSellerLog('购买'.$xianshi_quota_quantity.'份限时折扣套餐，单价'.$current_price.$lang['im_yuan']);
