...
class supplier_orderControl extends BaseSupplierControl {
    ...
    public function indexOp() {
        ...
        if (in_array($_GET['state_type'],$allow_state_array)) {
	     ...
	     //<!-- 分销选入店铺 start zhang -->
	    //订单状态的操作
            $condition['order_state'] = str_replace($allow_state_array,array(ORDER_STATE_NEW,ORDER_STATE_PAY,ORDER_STATE_SEND,ORDER_STATE_SUCCESS,ORDER_STATE_CANCEL), $_GET['state_type']);
			
			if($_GET['state_type'] == $allow_state_array[5]){
				$condition['up_id'] = $condition['store_id'];
    ...
    public function order_printOp() {
        ...
        if (!empty($order_info['extend_order_goods'])){
            ...
            foreach ($order_info['extend_order_goods'] as $k => $v){
                ...
                $v['goods_all_price'] = imPriceFormat($v['goods_num'] * $v['goods_price']);
        ...
        Tpl::output('goods_total_price',imPriceFormat($goods_total_price));
    ...
    private function profile_menu($menu_type='',$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
            $menu_array = array(
            array('menu_key'=>'store_order',        'menu_name'=>Language::get('im_member_path_all_order'), 'menu_url'=>'index.php?act=supplier_order'),
            array('menu_key'=>'state_new',          'menu_name'=>Language::get('im_member_path_wait_pay'),  'menu_url'=>'index.php?act=supplier_order&op=index&state_type=state_new'),
            array('menu_key'=>'state_pay',          'menu_name'=>Language::get('im_member_path_wait_send'), 'menu_url'=>'index.php?act=supplier_order&op=store_order&state_type=state_pay'),
            array('menu_key'=>'state_notakes',      'menu_name'=>'待自提', 'menu_url'=>'index.php?act=supplier_order&op=store_order&state_type=state_notakes'),
            array('menu_key'=>'state_send',         'menu_name'=>Language::get('im_member_path_sent'),      'menu_url'=>'index.php?act=supplier_order&op=index&state_type=state_send'),
            array('menu_key'=>'state_success',      'menu_name'=>Language::get('im_member_path_finished'),  'menu_url'=>'index.php?act=supplier_order&op=index&state_type=state_success'),
            array('menu_key'=>'state_cancel',       'menu_name'=>Language::get('im_member_path_canceled'),  'menu_url'=>'index.php?act=supplier_order&op=index&state_type=state_cancel'),
            );
            break;
        }
