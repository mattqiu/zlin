...
class store_brandControl extends BaseSellerControl {
		...
		if ($_POST['form_submit'] == 'ok' and intval($_POST['brand_id']) != 0) {
			...
			if ($error != ''){
				...
				if ($result){
					...
					showDialog($lang['im_common_save_succ'],'index.php?act=store_brand&op=brand_list','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
				}else {
					showDialog($lang['im_common_save_fail']);
				}
			}
	...
	public function drop_brandOp() {
		...
		if ($brand_id > 0){
			...
			showDialog(Language::get('im_common_del_succ'),'index.php?act=store_brand&op=brand_list','succ');
		}else {
	...
	private function profile_menu($menu_type,$menu_key='',$array=array()) {
		...
		switch ($menu_type) {
			case 'brand_list':
				$menu_array = array(
				    1=>array('menu_key'=>'brand_list', 'menu_name'=>$lang['im_member_path_brand_list'], 'menu_url'=>'index.php?act=store_brand&op=brand_list')
				);
				break;
		}
		if(!empty($array)) {
