...
class store_voucherControl extends BaseSellerControl{
	...
	public function __construct() {
		parent::__construct() ;
		//读取语言包
		Language::read('member_layout,member_voucher,member_store_promotion_bundling');
		//判断系统是否开启代金券功能
	...
	public function quotaaddOp(){
		if (chksubmit()){
            ...
            $this->recordSellerLog('购买'.$quota_quantity.'份代金券套餐，单价'.$current_price.L('im_yuan'));

            if($reault){
                showDialog(Language::get('voucher_apply_buy_succ'),'index.php?act=store_voucher&op=quotalist','succ');
            } else {
                showDialog(Language::get('im_common_op_fail'),'index.php?act=store_voucher&op=quotalist');
            }
        }else {
            //输出导航
    ...
    public function templateaddOp(){
        ...
        if(chksubmit()){
	        ...
	        //不限制金额 if($price>=$limit) $error.=Language::get('voucher_template_price_error');
			//验证卡密代金券发放数量
            $gettype = trim($_POST['gettype_sel']);
            if($gettype == 'pwd'){
                if (intval($_POST['txt_template_total']) > 9999999){
                    $error.= '领取方式为卡密兑换的代金券，发放总数不能超过9999999张';
                }
            }
            ...
            $model->delVoucherGoods(array('vc_id' => intval($_POST['voucher_t_id'])));
            if (!empty($_POST['goods']) && is_array($_POST['goods'])) {
            	foreach ($_POST['goods'] as $key => $val) {
            		// 验证是否为本店铺商品
            		$goods_info = $model_goods->getGoodsCommonInfoByID($val['gid'], 'goods_commonid,goods_name,goods_image,store_id');
            		if (empty($goods_info) || $goods_info['store_id'] != $_SESSION['store_id']) {
            			continue;
            		}
            		$data = array();
            		$data['voucher_t_id'] = isset($_POST['tid']) ? intval($_POST['tid']) : 0;
            		$data['goods_commonid'] = $goods_info['goods_commonid'];
            		$data['goods_name'] = $goods_info['goods_name'];
            		$data['goods_image'] = $goods_info['goods_image'];
            		$data['vc_goods_price'] = imPriceFormat($val['price']);
            		$data['vc_appoint'] = intval($val['appoint']);
            ...
            // 插入数据
            $return = $model->addVoucherGoodsAll($data_goods);
	        ...
	        if ($error){
	        	...
	        	$insert_arr['vc_goods_range'] = intval($_POST['vc_goods_range']);
	        	$insert_arr['vc_goods_gain'] = imPriceFormat($_POST['vc_goods_gain']);
	        	//自定义图片
	            ...
	            if($rs){
	                ...
	                showDialog(Language::get('im_common_save_succ'),'index.php?act=store_voucher&op=templatelist','succ');
	            }else{
	                showDialog(Language::get('im_common_save_fail'),'index.php?act=store_voucher&op=templatelist','error');
	            }
	        }
        }else{
            //店铺分类
    ...
    public function templateeditOp(){
        ...
        if(chksubmit()){
	        ...
	        if ($error){
	        	...
	        	$update_arr['vc_goods_range'] = intval($_POST['vc_goods_range']);
	        	$update_arr['vc_goods_gain'] = imPriceFormat($_POST['vc_goods_gain']);
	        	//自定义图片
		        if (!empty($_FILES['customimg']['name'])){
		        	$upload = new UploadFile();
		        	$upload->set('default_dir',	ATTACH_VOUCHER.DS.$_SESSION['store_id']);
	            ...
	            if($rs){
	                ...
	                showDialog(Language::get('im_common_op_succ'),'index.php?act=store_voucher&op=templatelist','succ');
	            }else{
	                showDialog(Language::get('im_common_op_fail'),'index.php?act=store_voucher&op=templatelist','error');
	            }
	        }
        }else{
	        ...
	        $v_goods_list = $model->getVoucherGoodsList(array('voucher_t_id' => intval($t_id)));
	        if (!empty($v_goods_list)) {
	        	$goodsid_array = array();
	        	foreach ($v_goods_list as $val) {
	        		$goodsid_array[] = $val['goods_commonid'];
	        	}
	        	$goods_list = Model('goods')->getGoodsCommonList(array('goods_commonid' => array('in', $goodsid_array)), 'goods_commonid,goods_price,goods_gain,goods_image,goods_name');
	        	Tpl::output('goods_list', array_under_reset($goods_list, 'goods_commonid'));
	        }
	        Tpl::output('v_goods_list', $v_goods_list);
	        //店铺分类
	        $store_class = rkcache('store_class', true);
	        Tpl::output('store_class', $store_class);
	        //查询店铺详情
    ...
    public function templatedelOp(){
        ...
        if ($rs){
        	...
        	showDialog(Language::get('im_common_del_succ'),'reload','succ');
        }else {
        	showDialog(Language::get('im_common_del_fail'));
        }
    }
    /**
     * 查看代金券详细
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'voucher':
				$menu_array = array(
				1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('im_member_path_store_voucher'), 'menu_url'=>'index.php?act=store_voucher&op=templatelist'),
				);
				break;
			case 'quota_add':
				$menu_array = array(
	 ...
	 * 代金卷活动添加商品
	 */
	public function voucher_add_goodsOp() {
		/**
		 * 实例化模型
		 */
		$model_goods =Model('goods');
		...
		$goods_list = $model_goods->getGoodsCommonOnlineList($where, '*', 8, '');
		...
		*/
		...
		Tpl::showpage('store_voucher_template.add_goods', 'null_layout');
	}
}