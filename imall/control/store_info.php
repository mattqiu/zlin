...
class store_infoControl extends BaseSellerControl {
	...
	public function bind_classOp() {
	    ...
	    $store_bind_class_list = $model_store_bind_class->getStoreBindClassList(array('store_id'=>$_SESSION['store_id']), 15);
	    $goods_class = Model('goods_class')->getGoodsClassIndexedListAll();
	    for($i = 0, $j = count($store_bind_class_list); $i < $j; $i++) {
	        $store_bind_class_list[$i]['class_1_name'] = $goods_class[$store_bind_class_list[$i]['class_1']]['gc_name'];
	        $store_bind_class_list[$i]['class_2_name'] = $goods_class[$store_bind_class_list[$i]['class_2']]['gc_name'];
		...
		Tpl::output('show_page', $model_store_bind_class->showpage());
	...
	public function bind_class_saveOp() {
	    ...
	    if (preg_match('/^[\d,]+$/',$_POST['goods_class'])) {
	        ...
	        showDialog($lang['im_common_save_fail']);
	    }
		...
		$commis_rate = $class_list['data'][$class_1]['commis_rate'];
		if(!empty($class_2)) {
	        $commis_rate = $class_list['data'][$class_2]['commis_rate'];
	    }
	    if(!empty($class_3)) {
	        $commis_rate = $class_list['data'][$class_3]['commis_rate'];
	    }	
		
		$class_data = array();
		$class_data['store_id']    = $_SESSION['store_id'];
		$class_data['state']       = 0;
		$class_data['class_1']     = $class_1;
		$class_data['class_2']     = empty($class_2)?0:$class_2;
		$class_data['class_3']     = empty($class_3)?0:$class_3;
		$class_data['commis_rate'] = $commis_rate;
		
		$result = $model_store_bind_class->addStoreBindClass($class_data);
	    if($result) {
...
// 	            $model_store->editStore(array('store_end_time'=>$data['re_end_time']),array('store_id'=>$_SESSION['store_id']));
	            ...
	            showDialog(Language::get('im_common_save_succ').'，需付款金额'.imPriceFormat($data['re_pay_amount']).'元，请尽快完成付款，付款完成后请上传付款凭证','reload','succ','',5);
	        }
	    } else {
	        showDialog(Language::get('im_common_del_fail'));
	    }
	...
	private function profile_menu($menu_type,$menu_key='',$array=array()) {
	    ...
	    switch ($menu_type) {
	        case 'index':
                $menu_array[] = array('menu_key'=>'bind_class', 'menu_name'=>$lang['im_member_path_bind_class'], 'menu_url'=>'index.php?act=store_info&op=bind_class');
                if (!checkPlatformStore()) {
                    $menu_array[] = array('menu_key'=>'index', 'menu_name'=>$lang['im_member_path_store_info'], 'menu_url'=>'index.php?act=store_info&op=index');
                    $menu_array[] = array('menu_key'=>'reopen', 'menu_name'=>$lang['im_member_path_store_reopen'], 'menu_url'=>'index.php?act=store_info&op=reopen');
                }
	            break;
	    	case 'bind_class':
	    	    $menu_array = array(
	    	    array('menu_key'=>'index', 'menu_name'=>$lang['im_member_path_bind_class'], 'menu_url'=>'index.php?act=store_bind_class&op=index'),
	    	    );
	    	    break;
	    	case 'add':
	    	    $menu_array = array(
	    	    array('menu_key'=>'index', 'menu_name'=>$lang['im_member_path_bind_class'], 'menu_url'=>'index.php?act=store_bind_class&op=index'),
	    	    array('menu_key'=>'add', 'menu_name'=>$lang['im_member_path_bind_class_add'], 'menu_url'=>'index.php?act=store_bind_class&op=add')
	    	    );
	    	    break;
	    }
	    if(!empty($array)) {
