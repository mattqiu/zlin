...
class store_goods_addControl extends BaseSellerControl {
    ...
    public function save_goodsOp() {
        if (chksubmit()) {
			...
			$common_array['store_type']         = intval($this->store_info['store_type']);
            ...
            $common_array['presell_deliverdate']= $common_array['is_presell'] == 1? strtotime("+7 days"):''; //strtotime($_POST['g_deliverdate']) : strtotime("+7 days"); // 预售商品的发货时间
            $common_array['is_own_shop']        = in_array($_SESSION['store_id'], model('store')->getOwnShopIds()) ? 1 : 0;
		    $common_array['view_grade']         = intval($_POST['view_grade'])?intval($_POST['view_grade']):0;
		    $common_array['promotion_cid']      = intval($_POST['promotion_cid'])?intval($_POST['promotion_cid']):0;
		    $common_array['saleman_cid']        = intval($_POST['saleman_cid'])?intval($_POST['saleman_cid']):0;			
		    $common_array['rebate_cid']        = intval($_POST['rebate_cid'])?intval($_POST['rebate_cid']):0;
			
		    //<!-- 分销选入店铺 start zhang -->
		    $common_array['is_market']       	= intval($_POST['is_market']);			//是否加入市场
		    $common_array['baifen']       		 = $common_array['is_market']==1?floatval(($update_common['goods_price']-$update_common['goods_tradeprice'])/$common_array['goods_tradeprice']) : $_POST['baifen']==''?intval($_POST['store_subsidy'][3]):8;			//设置分销百分比率
		    //$common_array['baifen']       		= $_POST['baifen']==''?intval($_POST['store_subsidy'][3]):8;			//设置分销百分比率
		    //众筹商品 zhangchao
		    $common_array['give_points']	= intval($_POST['g_points']);
		    $common_array['is_crowdfunding']    = $common_array['goods_state'] == 1 ? intval($_POST['is_crowdfunding']) : 0;     // 只有出售中的商品可以众筹
		    $common_array['crowdfunding_deliverdate']	= $common_array['is_crowdfunding'] == 1? strtotime($_POST['g_crowdfundingdate']) : '0'; // 众筹商品的发货时间
		    $common_array['goods_take']		= floatval($_POST['g_collect']);// $update_common['is_market'] == 1? floatval($_POST['goods_take']) : '0.00'; // 只有加入市场的商品才可以设置批发价
		    $common_array['goods_gain']		 	= floatval($_POST['g_gain']);//平台利润
		    $common_array['presell_days']		= $common_array['is_presell'] == 1? intval($_POST['g_presell']) : ''; // 预售商品的后几天发货
		    $common_array['cf_starttime']		= $common_array['is_crowdfunding'] == 1? strtotime($_POST['cf_starttime']) : '0'; // 众筹开始时间
		    $common_array['cf_endtime']			= $common_array['is_crowdfunding'] == 1? strtotime($_POST['cf_endtime']) : '0'; // 众筹商品的结束时间
		    //只需添加goods-common表即可
		    $common_array['fx_minprice']       = $common_array['is_market']==1?floatval($_POST['fx_minprice']):0.00;//最低售价
		    if(empty($common_array['fx_minprice'] )){
				$common_array['fx_minprice'] = $common_array['goods_price'];
		    }
		    $common_array['fx_maxprice']       = $common_array['is_market']==1?floatval($_POST['fx_maxprice']):0.00;			//最高价
            ...
            if ($common_id) {
                ...
                if (is_array($_POST['spec'])) {
                    foreach ($_POST['spec'] as $value) {
						...
						$goods['store_type']        = $common_array['store_type'];
						...
						$goods['goods_tradeprice']  = $value['tradeprice'] == 0 ? $common_array['goods_tradeprice'] : $value['tradeprice'];
						...
						$goods['goods_advpic']      = $common_array['goods_advpic'];
						...
						$goods['view_grade']        = $common_array['view_grade'];
						$goods['promotion_cid']     = $common_array['promotion_cid'];
						$goods['saleman_cid']       = $common_array['saleman_cid'];						
						$goods['rebate_cid']       = $common_array['rebate_cid'];
						//<!-- 分销选入店铺 start zhang -->
						$goods['is_market']       	= $common_array['is_market'];			//是否加入市场
						$goods['baifen']       	 	= $common_array['baifen'];			//设置分销百分比率
						//众筹商品 zhangchao
						$goods['is_crowdfunding']   = $common_array['is_crowdfunding'];     // 只有出售中的商品可以众筹
						$goods['crowdfunding_deliverdate']   = $common_array['crowdfunding_deliverdate'];
						$goods['goods_take']	 	= $common_array['goods_take'];//加入市场的商品批发价
						$goods['goods_gain']		= $common_array['goods_gain'];//平台利润
						$goods['presell_days']	 	= $common_array['presell_days']; // 预售商品的后几天发货
						$goods['cf_starttime']		= $common_array['cf_starttime']; // 众筹开始时间
						$goods['cf_endtime']		= $common_array['cf_endtime']; // 众筹商品的结束时间
						$goods['give_points']	 	= $model_commis->getCommisPriceByID($goods['rebate_cid'],$goods['goods_price'],$goods['goods_costprice']);//赠送云币
						$goods['promotion_amount']  = $model_commis->getCommisPriceByID($goods['promotion_cid'],$goods['goods_price'],$goods['goods_costprice']);
					    $goods['saleman_amount']    = $model_commis->getCommisPriceByID($goods['saleman_cid'],$goods['goods_price'],$goods['goods_costprice']);
					...
					$goods['goods_tradeprice']  = $common_array['goods_tradeprice'];
					$goods['goods_costprice']   = $common_array['goods_costprice'];
					...
					$goods['cf_starttime']		= $common_array['cf_starttime']; // 众筹开始时间
					$goods['cf_endtime']		= $common_array['cf_endtime']; // 众筹商品的结束时间
					$goods['give_points']	 	= $common_array['give_points'];//赠送云币
					//众筹商品 zhangchao
					$goods['is_crowdfunding']   = $common_array['is_crowdfunding'];     // 只有出售中的商品可以众筹
					$goods['crowdfunding_deliverdate']   = $common_array['crowdfunding_deliverdate'];
					$goods['promotion_amount']  = $model_commis->getCommisPriceByID($goods['promotion_cid'],$goods['goods_price'],$goods['goods_costprice']);
    ...
    public function add_step_fourOp() {
		...
		//<!-- 分销选入店铺 start zhang -->	
		$data_array['baifen'] = $goods_info['baifen'];
        ...
        if ($gc_id_1 > 0) {
            ...
            foreach ( $class_list as $val ) {
                if ($val ['gc_id'] == $gc_id_1) {
                    $list_array ['one'] .= '<li class="" onclick="selClass($(this));" data-param="{gcid:' . $val ['gc_id'] . ', deep:1, tid:' . $val ['type_id'] . '}" imtype="selClass"> <a class="classDivClick" href="javascript:void(0)"><span class="has_leaf"><i class="fa fa-angle-double-right"></i>' . $val ['gc_name'] . '</span></a> </li>';
                } else {
                    $list_array ['one'] .= '<li class="" onclick="selClass($(this));" data-param="{gcid:' . $val ['gc_id'] . ', deep:1, tid:' . $val ['type_id'] . '}" imtype="selClass"> <a class="" href="javascript:void(0)"><span class="has_leaf"><i class="fa fa-angle-double-right"></i>' . $val ['gc_name'] . '</span></a> </li>';
                }
            }
        }
        // 2级
        if ($gc_id_2 > 0) {
            ...
            foreach ( $class_list as $val ) {
                if ($val ['gc_id'] == $gc_id_2) {
                    $list_array ['two'] .= '<li class="" onclick="selClass($(this));" data-param="{gcid:' . $val ['gc_id'] . ', deep:2, tid:' . $val ['type_id'] . '}" imtype="selClass"> <a class="classDivClick" href="javascript:void(0)"><span class="has_leaf"><i class="fa fa-angle-double-right"></i>' . $val ['gc_name'] . '</span></a> </li>';
                } else {
                    $list_array ['two'] .= '<li class="" onclick="selClass($(this));" data-param="{gcid:' . $val ['gc_id'] . ', deep:2, tid:' . $val ['type_id'] . '}" imtype="selClass"> <a class="" href="javascript:void(0)"><span class="has_leaf"><i class="fa fa-angle-double-right"></i>' . $val ['gc_name'] . '</span></a> </li>';
                }
            }
        }
        // 3级
        if ($gc_id_3 > 0) {
            ...
            foreach ( $class_list as $val ) {
                if ($val ['gc_id'] == $gc_id_3) {
                    $list_array ['three'] .= '<li class="" onclick="selClass($(this));" data-param="{gcid:' . $val ['gc_id'] . ', deep:3, tid:' . $val ['type_id'] . '}" imtype="selClass"> <a class="classDivClick" href="javascript:void(0)"><span class="has_leaf"><i class="fa fa-angle-double-right"></i>' . $val ['gc_name'] . '</span></a> </li>';
                } else {
                    $list_array ['three'] .= '<li class="" onclick="selClass($(this));" data-param="{gcid:' . $val ['gc_id'] . ', deep:3, tid:' . $val ['type_id'] . '}" imtype="selClass"> <a class="" href="javascript:void(0)"><span class="has_leaf"><i class="fa fa-angle-double-right"></i>' . $val ['gc_name'] . '</span></a> </li>';
                }
            }
        }
        // 转码
