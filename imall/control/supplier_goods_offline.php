...
class supplier_goods_offlineControl extends BaseSupplierControl {
    ...
    public function goods_storageOp() {
        ...
        switch ($_GET['type']) {
            // 违规的商品
            case 'lock_up':
                Tpl::showpage('supplier_goods_list.offline_lockup');
                break;
            // 等待审核或审核失败的商品
            case 'wait_verify':
                Tpl::output('verify', array('0' => '未通过', '10' => '等待审核'));
                Tpl::showpage('supplier_goods_list.offline_waitverify');
                break;
            // 仓库中的商品
            default:
                Tpl::showpage('supplier_goods_list.offline');
                break;
        }
    }
    ...
    private function profile_menu($menu_key = '') {
        $menu_array = array(
            array('menu_key' => 'goods_storage',    'menu_name' => L('nc_member_path_goods_storage'),   'menu_url' => urlShop('supplier_goods_offline', 'index')),
            array('menu_key' => 'goods_lockup',     'menu_name' => L('nc_member_path_goods_state'),     'menu_url' => urlShop('supplier_goods_offline', 'index', array('type' => 'lock_up'))),
            array('menu_key' => 'goods_verify',     'menu_name' => L('nc_member_path_goods_verify'),    'menu_url' => urlShop('supplier_goods_offline', 'index', array('type' => 'wait_verify')))
        );
