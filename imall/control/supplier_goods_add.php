...
class supplier_goods_addControl extends BaseSupplierControl {
    /**
     * 三方店铺验证，商品数量，有效期
     */
    private function checkStore(){
        ...
        if ($goodsLimit > 0) {
            ...
            if ($goods_num >= $goodsLimit) {
                showMessage(L('store_goods_index_goods_limit') . $goodsLimit . L('store_goods_index_goods_limit1'), 'index.php?act=supplier_goods_online&op=goods_list', 'html', 'error');
            }
        }
    }
    ...
    public function add_step_twoOp() {
        ...
        Tpl::output('supplier_goods_class', $store_goods_class);
    ...
    public function save_goodsOp() {
        ...
        if(!$result['state']) {
            showMessage(L('error') . $result['msg'], urlShop('supplier_center'), 'html', 'error');
        }

        redirect(urlShop('supplier_goods_add', 'add_step_three', array('commonid' => $result['data'])));
    }
    ...
    public function save_imageOp(){
        if (chksubmit()) {
            ...
            if ($rs) {
                redirect(urlShop('supplier_goods_add', 'add_step_four', array('commonid' => $common_id)));
            } else {
    ...
    public function add_step_fourOp() {
        ...
        Tpl::showpage('supplier_goods_add.step4');
    }
