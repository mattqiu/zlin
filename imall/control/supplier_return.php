...
class supplier_returnControl extends BaseSupplierControl{
    ...
    public function editOp() {
        ...
        if (chksubmit()) {
            $reload = 'index.php?act=supplier_return&lock=1';
            if ($return['order_lock'] == '2') {
                $reload = 'index.php?act=supplier_return&lock=2';
            }
    ...
    public function receiveOp() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showDialog(Language::get('im_common_save_succ'),'reload','succ','CUR_DIALOG.close();');
            } else {
                showDialog(Language::get('im_common_save_fail'),'reload','error','CUR_DIALOG.close();');
            }
        }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'return':
                $menu_array = array(
                    array('menu_key'=>'2','menu_name'=>'售前退货',  'menu_url'=>'index.php?act=supplier_return&lock=2'),
                    array('menu_key'=>'1','menu_name'=>'售后退货','menu_url'=>'index.php?act=supplier_return&lock=1')
                );
                break;
        }
