...
class store_accountControl extends BaseSellerControl {
    ...
    public function account_listOp() {
        ...
        foreach ($seller_list as $skey => $seller){
        	$salemen_list = explode(',', $seller['saleman_id']);
        	$salemen = '';
        	foreach ($salemen_list as $saleman){
        		$condition = array(
        				'store_id' => $_SESSION['store_id'],
        				'seller_id' => $saleman
        		);
        		$seller_info = $model_seller->getSellerInfo($condition);
        		$salemen .= $seller_info['seller_name'].' ';
        	}
        	$seller_list[$skey]['salemen'] = $salemen;
        }
    ...
    public function account_addOp() {
    	...
    	$salemen_list = $model_seller->getSellerList($condition);
    	Tpl::output('salemen_list', $salemen_list);
        $model_seller_group = Model('seller_group');
        $seller_group_list = $model_seller_group->getSellerGroupList(array('store_id' => $_SESSION['store_id']));
    ...
    public function account_editOp() {
        ...
        Tpl::output('saleman', explode(',', $seller_info['saleman_id']));//所属导购
        $condition = array(
        		'store_id' => $_SESSION['store_id'],
        		'is_owner' => 2,//导购员列表
        		'seller_group_id' => array('gt', 0)
        );
        $saleman_list = $model_seller->getSellerList($condition);
        foreach ($saleman_list as $skey=> $salemen){
        	if($salemen['seller_id']!=$seller_info['seller_id']){
        		$salemen_list[$skey] = $saleman_list[$skey];
        	}
        }
        //$salemen_list = array_diff($salemen_list, $seller_info['member_id']); //需要二维极差
        Tpl::output('salemen_list', $salemen_list);
        $model_seller_group = Model('seller_group');
        $seller_group_list = $model_seller_group->getSellerGroupList(array('store_id' => $_SESSION['store_id']));
        if (empty($seller_group_list)) {
    ...
    public function account_saveOp() {
        ...
        $is_owner = intval($_POST['is_owner']);
        ...
        $seller_info = array(
            ...
            'store_id' => $_SESSION['store_id'],
        	'saleman_id' => implode(',', $_POST['saleman_id']),
        	'is_owner' => $is_owner,
        ...
        if($result) {
        	$this->edit_staff_status($seller_info['member_id'],$is_owner); //修改导购对应身份
        	if($_POST['is_default']){
        		$model_seller->editSeller(array('is_default' => 0), array('member_id'=>$member_info['member_id'],'store_id'=>array('neq',$_SESSION['store_id'])));//其他店铺设置非默认
        	}
            $this->recordSellerLog('添加账号成功，账号编号'.$result);
            showDialog(Language::get('im_common_op_succ'), urlShop('store_account', 'account_list'), 'succ');
        } else {
            $this->recordSellerLog('添加账号失败');
            showDialog(Language::get('im_common_save_fail'), urlShop('store_account', 'account_list'), 'error');
        }
    }
    ...
    public function account_edit_saveOp() {
        ...
        if($result) {
        	$this->edit_staff_status($_POST['member_id'],$is_owner);
        	if($_POST['is_default']){
        		$model_seller->editSeller(array('is_default' => 0), array('seller_id'=>array('neq',$_POST['seller_id']),'member_id'=>$_POST['member_id']));//其他店铺设置非默认
        	}
    ...
    public function account_delOp() {
        ...
        if($seller_id > 0) {
            ...
            if($result) {
                ...
                showDialog(Language::get('im_common_save_fail'),'reload','error');
            }
        } else {
    ...
    private function _check_member_password($member_name, $password) {
        $condition = array();
        $condition['member_name|member_mobile']	= $member_name;
    ...
    /**
     * 根据设置导购的身份,将其其他两处的身份同时给修改 zhangc
     * 会员表和推广员导购员表
     */
    function edit_staff_status($member_id,$is_status=0){
    	if ($is_status!=0){
    		$model_member = Model('member');
    		$condition['member_id'] = $member_id;
    		if($is_status==1){
    		...
    		if($is_status==1){
    			$extension['mc_id'] = 10;//店长
    		}elseif($is_status==2){
    			$extension['mc_id'] = 12;//导购员
    		}
    		$model_extension->editExtension($extension, $condition);
    	}    	
    }
}