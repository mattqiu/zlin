...
class buy_virtualControl extends BaseBuyControl {
    ...
    public function payOp() {
        ...
        //计算所需要支付金额
        $diff_pay_amount = imPriceFormat(floatval($order_info['order_amount'])-$payed_amount);
        ...
        if ($diff_pay_amount == 0) {
            redirect('index.php?act=buy_virtual&op=pay_ok&order_sn='.$order_info['order_sn'].'&order_id='.$order_info['order_id'].'&order_amount='.imPriceFormat($order_info['order_amount']));
        }

        Tpl::output('diff_pay_amount',imPriceFormat($diff_pay_amount));

        //显示支付接口列表
        $model_payment = Model('payment');
        $condition = array();
        $payment_list = $model_payment->getPaymentOpenList($condition);//支付方式_待修改
