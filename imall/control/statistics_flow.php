...
class statistics_flowControl extends BaseSellerControl{
    ...
    public function customerflowOp() {
		...
		$data = $model_flow->findPassengers_byShopNameOrCode($this->shopName,"",$beginDate, $endDate, $queryType);
		if ($data['status']==1){
			$total = 0;
			if ($this->search_arr['search_type']=='day'){
			    //构造竖轴数据
		...
		$stat_json = getStatData_LineLabels($stat_arr);		
        Tpl::output('stat_json',$stat_json);
		self::profile_menu('customerflow');
		Tpl::showpage('stat.flow.customer');
	}
	...
	private function profile_menu($menu_key='') {
        $menu_array	= array(
            1=>array('menu_key'=>'storeflow','menu_name'=>'店铺网络流量','menu_url'=>'index.php?act=statistics_flow&op=storeflow'),
			2=>array('menu_key'=>'customerflow','menu_name'=>'实体店客流统计','menu_url'=>'index.php?act=statistics_flow&op=customerflow'),
            3=>array('menu_key'=>'goodsflow','menu_name'=>'商品流量排名','menu_url'=>'index.php?act=statistics_flow&op=goodsflow')
        );
