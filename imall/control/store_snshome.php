...
class store_snshomeControl extends BaseStoreControl{
	...
	public function indexOp(){
		...
		$where['strace_storeid'] = $this->store_id;
		if($_GET['type'] != ''){
...
// 		//查询会员信息
	...
	public function addforwardOp(){
		...
		$insert_arr['trace_content']			= "<dl class=\"fd-wrap\">
														<dt>
															<h3><a href=\"index.php?act=store_snshome&store_id=".$stracelog_info['strace_storeid']."\" target=\"_blank\">".$stracelog_info['strace_storename']."</a>".Language::get('im_colon')."
	...
	public function deltraceOp(){
		...
		if ($result){
			...
			$js = "$('[im_type=\"tracerow_{$stid}\"]').remove();";
	...
	public function delcommentOp(){
		...
		if ($result){
			...
			$js .="$('.comment-list [im_type=\"commentrow_".$scid."\"]').remove();";
			showDialog(L('im_common_del_succ'),'','succ',$js);
		}else {
			showDialog(L('im_common_del_fail'),'','error');
		}
	}
	/**
	 * 一条SNS动态及其评论
