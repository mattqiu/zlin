...
class store_promotion_bundlingControl extends BaseSellerControl {
    ...
    public function bundling_addOp() {
        ...
        if (chksubmit()) {
            ...
            $data['bl_freight'] = intval($_POST['bundling_freight_choose'])=='1' ? '0.00' : $_POST['bundling_freight'];
            $data['bl_is_extension'] = $_POST['bl_is_extension'];
            $data['extension_day'] = intval($_POST['bl_is_extension'])=='0' ? '0' : $_POST['extension_day'];
            $data['bl_state'] = intval($_POST['state']);
            if(isset($_POST['bundling_id'])){
            	$condition = array();
                $condition['bl_id'] = intval($_POST['bundling_id']);
            	$return = $model_bundling->editBundling($data,$condition);
            }else{
	            $return = $model_bundling->addBundling($data);
            }
            if (!$return) {
                showDialog(L('im_common_op_fail'), '', '', 'error');
            }
            ...
            if (!empty($_POST['goods']) && is_array($_POST['goods'])) {
                foreach ($_POST['goods'] as $key => $val) {
                    ...
                    $data['bl_goods_price'] = imPriceFormat($val['price']);
            ...
            showDialog(L('im_common_op_succ'), urlShop('store_promotion_bundling', 'bundling_list'), 'succ');
        }
        ...
        if (intval($_GET['bundling_id']) > 0) {
            ...
            if (!empty($b_goods_list)) {
                ...
                $goods_list = Model('goods')->getGoodsList(array('goods_id' => array('in', $goodsid_array)), 'goods_id,goods_price,goods_gain,goods_image,goods_name');
