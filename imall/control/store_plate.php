...
class store_plateControl extends BaseSellerControl {
    ...
    public function plate_addOp() {
        if (chksubmit()) {
            ...
            if ($result) {
                showDialog(L('im_common_op_succ'), urlShop('store_plate', 'index'),'succ');
            } else {
                showDialog(L('im_common_op_fail'), urlShop('store_plate', 'index'));
            }
        }
        // 是否能使用编辑器
    ...
    public function drop_plateOp() {
        ...
        if ($return) {
            showDialog(L('im_common_del_succ'), 'reload', 'succ');
        } else {
