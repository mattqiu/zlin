...
class Control{	
    ...
    protected function checkMessage() {
        ...
        if (cookie($cookie_name) != null){
            ...
            setIMCookie($cookie_name,"$countnum",2*3600);//保存2小时
        }
        ...
        if($_SESSION['member_id']) {
            ...
            if ($member_info){
                ...
                if(!empty($_SESSION['M_grade_level'])){
                	$member_info['member_grade'] = $_SESSION['M_grade_level'];
                }else{
                	$member_info['member_grade'] = $member_info['level'];
                }
            }
        }
    ...
    protected function checkLogin(){
        if ($_SESSION['is_login'] !== '1'){
            ...
            if ($_GET['inajax']){
                ...
                @header("location: " . urlShop('login', 'index', array('ref_url' => $ref_url)));
            }
            exit;
        }
    }
...
class BaseGroupBuyControl extends Control {
    ...
    public function __construct(){				
        ...
        $nav_link = array(
            ...
            1=>array(
                'title'=>Language::get('im_groupbuy')
            )
        );
...
class BaseBrandControl extends Control {
	...
	public function __construct(){
		...
		Tpl::output('hot_search',@explode(',',C('hot_search')));		
		Tpl::output('nav_list',($nav = rkcache('nav'))? $nav :rkcache('nav',true));		
		//获得品牌列表
        $model = Model('brand');
        $brands = $model->get_all_brandcategory();
        extract($brands);
		Tpl::output('brand_l',$brand_list);
...
class BaseStoreJoinControl extends Control {
...
class BaseMemberControl extends Control {
    ...
    public function __construct(){
        ...
        Language::read('common,member_layout,home_layout');
    ...
    private function _getMenuList() {
        $menu_list = array();
		$child = array();
		$child['member_order']      = array('name' => '实物交易订单', 'url'=>urlShop('member_order', 'index'));
		//$child['member_vr_order']   = array('name' => '虚拟兑码订单', 'url'=>urlShop('member_vr_order','index'));
		//$child['member_flea']       = array('name' => '我的闲置宝贝', 'url'=>urlShop('member_flea','index'));
		$child['member_favorites']  = array('name' => '我的收藏', 'url'=>urlShop('member_favorites', 'fglist'));
		$child['member_evaluate']   = array('name' => '交易评价/晒单', 'url'=>urlShop('member_evaluate', 'list'));
		$child['predeposit']        = array('name' => '账户余额', 'url'=>urlShop('predeposit', 'pd_log_list'));
		$child['member_points']     = array('name' => '我的云币', 'url'=>urlShop('member_points', 'index'));
		$child['member_voucher']    = array('name' => '我的代金券', 'url'=>urlShop('member_voucher', 'index'));
		$menu_list['trade'] = array('name' => '交易管理', 'child' => $child);
		
		$menu_list['serv'] = array('name' => '客户服务', 'child' => array(
		...
		$menu_list['info'] = array('name' => '资料管理', 'child' => array(
		...
		$menu_list['app'] = array('name' => '应用管理', 'child' => array(
                'sns'               => array('name' => '个人主页', 'url'=>urlShop('member_snshome', 'index'))
		));
		if(OPEN_CMS_STATE ==1 && C('cms_isuse') !== null){
		    $menu_list['app']['child']['cms'] = array('name' => '我的CMS', 'url'=>urlCMS('member_article', 'article_list'));
		}
		if(OPEN_CIRCLE_STATE ==1 && C('circle_isuse') !== null){
		    $menu_list['app']['child']['circle'] = array('name' => '我的圈子', 'url'=>urlCircle('p_center', 'index'));
		}
		if(OPEN_MICROSHOP_STATE ==1 && C('microshop_isuse') !== null){
		    $menu_list['app']['child']['circle'] = array('name' => '我的微商城', 'url'=>urlMicroshop('home', 'index', array('member_id' => $_SESSION['member_id'])));
		}
		if(OPEN_MODULE_WEIXIN_STATE == 1){
		    $menu_list['app']['child']['circle'] = array('name' => '微商城', 'url'=>urlWeixin('userindex', 'index'));
		}
...
class BaseSNSControl extends Control {
    ...
    protected function formatDate($time){
        ...
        if ($reference_date_time == $handle_date_time){
            ...
            $month_text = Language::get('im_month');
            $day_text = Language::get('im_day');
    ...
    private function add_visit(){
        ...
        if ($is_increase == true){
            ...
            setIMCookie('visitor',implode('_',$cookie_arr),24*3600);//保存24小时
        }
    }
    //我的关注
...
class BaseGoodsControl extends BaseStoreControl {
	...
	protected $store_theme = 'default';

    public function __construct(){

        Language::read('common,home_layout,store_layout');
...
class BaseSellerControl extends Control {
    ...
    //是否充许开分店
	protected $branch_op = 1;
	//是否充许店铺推广
	protected $extension_op = 0;
	//店铺类型 0:普通店铺 1：总店 2：分店
	protected $branch_tp = 0;
     ...
     * app商家目录列表
     */
   public function getAppSellerMenuList() {
    	$seller_menu = array();
    	$seller_menu = $this->_getAppMenuList();
    	$seller_function_list = $this->_getSellerFunctionList($seller_menu);
    ...
    private function _getMenuList() {
        ...
        $menu_list['goods']  = array('name' => '商品', 'child' => array(
				...
				array('name' => '商品分类', 'act'=>'store_goods_class', 'op'=>'index'),
                array('name' => '图片管理', 'act'=>'store_album', 'op'=>'album_cate'),
        ));
        $menu_list['order']  = array('name' => '订单', 'child' => array(
                ...
                array('name' => '物流配送', 'act'=>'store_transport', 'op'=>'index'),
				
        ));
        $menu_list['promotion'] = array('name' => '促销', 'child' => array(
                array('name' => '预售商品', 'act'=>'store_promotion_book', 'op'=>'index'),
        		array('name' => '众筹管理', 'act'=>'store_promotion_funding', 'op'=>'index'),
	...
	if ($_SESSION['S_extension_op'] == 1){
		//if (OPEN_STORE_EXTENSION_STATE == 1 && $_SESSION['S_extension_op'] == 1){
            $menu_list['extension'] = array('name' => '推广', 'child' => array(
            	array('name' => '会员管理', 'act'=>'seller_member', 'op'=>'index'),
			    array('name' => '推广配置', 'act'=>'seller_extension', 'op'=>'config'),
			    array('name' => '推广员管理', 'act'=>'seller_promotion', 'op'=>'promotion_list'),
				array('name' => '导购员管理', 'act'=>'seller_saleman', 'op'=>'saleman_list'),
				array('name' => '抽佣明细', 'act'=>'seller_commisdetail', 'op'=>'index'),
				array('name' => '佣金结算', 'act'=>'seller_commisputout', 'op'=>'index'),
				array('name' => '抽佣统计', 'act'=>'statistics_commis', 'op'=>'commis_statistics'),
            ));
		}
		if ($_SESSION['S_branch_op'] == 1){
		    $menu_list['branch']  = array('name' => '分店', 'child' => array(
				array('name' => '分店管理', 'act'=>'seller_branch', 'op'=>'branch_list'),	
				array('name' => '营业统计', 'act'=>'seller_branch_statistics', 'op'=>'index'),
				array('name' => '运营结算', 'act'=>'seller_branch_balance', 'op'=>'index'),
				array('name' => '商品调拔', 'act'=>'seller_branch_stubbs', 'op'=>'index'),
				array('name' => '数据同步', 'act'=>'seller_branch_synchro', 'op'=>'index'),					   
            ));
		}
		if ($_SESSION['S_parent_id'] > 0){
			$menu_list['distribution']  = array('name' => '分销', 'child' => array(
				array('name' => '补货管理', 'act'=>'seller_distri_replenish', 'op'=>'index'),	
				array('name' => '退货管理', 'act'=>'seller_distri_returned', 'op'=>'index'),
				array('name' => '分销结算', 'act'=>'seller_distri_balance', 'op'=>'index'),			   
            ));
		}
        $menu_list['settle'] = array('name' => '结算', 'child' => array(
				...
				array('name' => 'PC端支付配置', 'act'=>'seller_payment', 'op'=>'system'),	
				array('name' => '移动支付配置', 'act'=>'mb_payment', 'op'=>'index'),	            
        ));
        $menu_list['store']  = array('name' => '店铺', 'child' => array(
		        array('name' => '店铺信息', 'act'=>'store_info', 'op'=>'bind_class'),
                array('name' => '店铺设置', 'act'=>'store_setting', 'op'=>'store_setting'),
				array('name' => '实体商铺', 'act'=>'store_map', 'op'=>'index'),
        ...
        $menu_list['mobile'] = array('name' => '手机端', 'child' => array(
                array('name' => '店铺装修', 'act'=>'mb_decoration', 'op'=>'index_edit'),
				array('name' => '专辑管理', 'act'=>'mb_special', 'op'=>'special_list'),
				array('name' => '分类导航', 'act'=>'mb_navigation', 'op'=>'navigation_list'),
				array('name' => '首页广告', 'act'=>'mb_ad', 'op'=>'index'),                	   
        ));
        $menu_list['consult'] = array('name' => '售后', 'child' => array(
        ...
        $menu_list['statistics'] = array('name' => '统计', 'child' => array(
                array('name' => '运营概况', 'act'=>'statistics_general', 'op'=>'general'),
				...
				array('name' => '销售明细', 'act'=>'store_sales_record', 'op'=>'index'),						
        ));            
        $menu_list['account'] = array('name' => '账号', 'child' => array(
        ...
        //$menu_list['wechat'] = array('name' => '微信', 'child' => array(
        		//array('name' => '微信公众号', 'url'=>'weixin/index.php?act=userindex&op=index'),
        		//array('name' => '微商城', 'url'=>urlWeixin('userindex', 'index')),
    	//));
        
        $menu_list['fx_market'] = array('name' => '分销管理', 'child' => array(
        		array('name' => '分销市场', 'act'=>'fx_market', 'op'=>'index'),
        		array('name' => '已分销的商品', 'act'=>'store_goods_fx', 'op'=>'fx_goods_list'),
        		array('name' => '分销实物订单', 'act'=>'store_fx_order', 'op'=>'index'),
        		array('name' => '分销虚拟订单', 'act'=>'store_fx_vr_order', 'op'=>'index'),
        		array('name' => '消息自动回复', 'act'=>'store_fx_bill_log', 'op'=>'index'),
        ));
        return $menu_list;
    }
    /**
    ...
    private function _getAppMenuList() {
    	...
    	$menu_list['wxapp']  = array('name' => '小程序权限', 'child' => array(
    			array('name' => '快捷收银', 'power'=>'cashier', 'url'=>'index'),
    	...
    	$menu_list['wsm_app'] = array('name' => '进销存系统', 'child' => array(
    			array('name' => '采购管理', 'power'=>'purchase','url'=> 'index'),
    			array('name' => '入库管理', 'power'=>'storage','url'=> 'index'),
    			array('name' => '出库管理', 'power'=>'storage','url'=> 'index'),
    			array('name' => '仓库管理', 'power'=>'wms','url'=> 'index'),
    	));
    	$menu_list['settle'] = array('name' => '结算', 'child' => array(
    			array('name' => '实物结算', 'power'=>'store_bill', 'url'=>'index'),
    			array('name' => '虚拟结算', 'power'=>'store_vr_bill', 'url'=>'index'),
    			array('name' => '店铺消费', 'power'=>'store_cost', 'url'=>'cost_list'),
    			array('name' => '移动支付配置', 'power'=>'mb_payment', 'url'=>'index'),
    	));
    	$menu_list['store']  = array('name' => '店铺', 'child' => array(
    			array('name' => '店铺信息', 'power'=>'store_info', 'url'=>'bind_class'),
    			array('name' => '店铺设置', 'power'=>'store_setting', 'url'=>'store_setting'),
    			array('name' => '实体商铺', 'power'=>'store_map', 'url'=>'index'),
    			array('name' => '店铺装修', 'power'=>'store_decoration', 'url'=>'decoration_setting'),
    			array('name' => '店铺动态', 'power'=>'store_sns', 'url'=>'index'),
    	));
    	if ($_SESSION['S_parent_id'] > 0){
    		$menu_list['distribution']  = array('name' => '分销', 'child' => array(
    				array('name' => '补货管理', 'power'=>'seller_distri_replenish', 'url'=>'index'),
    				array('name' => '退货管理', 'power'=>'seller_distri_returned', 'url'=>'index'),
    				array('name' => '分销结算', 'power'=>'seller_distri_balance', 'url'=>'index'),
    		));
    	}
    	$menu_list['statistics'] = array('name' => '统计', 'child' => array(
    			array('name' => '运营概况', 'power'=>'statistics_general', 'url'=>'general'),
    			array('name' => '商品分析', 'power'=>'statistics_goods', 'url'=>'goodslist'),
    			array('name' => '运营报告', 'power'=>'statistics_sale', 'url'=>'sale'),
    			array('name' => '行业分析', 'power'=>'statistics_industry', 'url'=>'hot'),
    			array('name' => '流量统计', 'power'=>'statistics_flow', 'url'=>'storeflow'),
    			array('name' => '销售明细', 'power'=>'store_sales_record', 'url'=>'index'),
    	));
    	$menu_list['account'] = array('name' => '账号', 'child' => array(
    			array('name' => '账号列表', 'power'=>'store_account', 'url'=>'account_list'),
    			array('name' => '账号组', 'power'=>'store_account_group', 'url'=>'group_list'),
    			array('name' => '账号日志', 'power'=>'seller_log', 'url'=>'log_list'),
    	));
    	$menu_list['fx_market'] = array('name' => '分销管理', 'child' => array(
    			array('name' => '分销市场', 'power'=>'fx_market', 'url'=>'index'),
    			array('name' => '已分销的商品', 'power'=>'store_goods_fx', 'url'=>'fx_goods_list'),
    			array('name' => '分销实物订单', 'power'=>'store_fx_order', 'url'=>'index'),
    			array('name' => '分销虚拟订单', 'power'=>'store_fx_vr_order', 'url'=>'index'),
    			array('name' => '消息自动回复', 'power'=>'store_fx_bill_log', 'url'=>'index'),
    	));
    	return $menu_list;
    }
    ...
    public function storeAutoShare($data, $type) {
        ...
        if ($auto_setting['sauto_' . $type] == 1) {
            ...
            if ($auto_setting['sauto_' . $type . 'title'] != '') {
                ...
                $auto_title = 'im_store_auto_share_' . $type . rand(1, 5);
    ...
    private function checkStoreMsg() {//判断cookie是否存在
        ...
        if (cookie($cookie_name) != null && intval(cookie($cookie_name)) >=0){
            ...
            setIMCookie($cookie_name,intval($countnum),2*3600);//保存2小时
        }
 ...
 *
 */
class BaseSupplierControl extends Control {
	...
	public function __construct(){
		...
		if(!C('site_status')) halt(C('closed_reason'));
		Tpl::setDir('supplier');
		Tpl::setLayout('supplier_layout');

		Tpl::output('nav_list', rkcache('nav',true));
		if ($_GET['act'] !== 'supplier_login'&&$_GET['act'] !== 'supplier_register') {

			if(empty($_SESSION['supplier_id'])) {
				@header('location: index.php?act=supplier_login&op=show_login');die;
			}
			...
			if ($_SESSION['supplier_is_admin'] !== 1 && $_GET['act'] !== 'supplier_center' && $_GET['act'] !== 'supplier_logout') {
				if (!in_array($_GET['act'], $_SESSION['supplier_limits'])) {
					showMessage('没有权限', '', '', 'error');
				}
			}
			
			// 卖家菜单
			Tpl::output('menu', $_SESSION['supplier_menu']);
			// 当前菜单
			$current_menu = $this->_getCurrentMenu($_SESSION['supplier_function_list']);
			Tpl::output('current_menu', $current_menu);
			// 左侧菜单
			if($_GET['act'] == 'supplier_center') {
				if(!empty($_SESSION['supplier_quicklink'])) {
					$left_menu = array();
					foreach ($_SESSION['supplier_quicklink'] as $value) {
						$value = str_replace("store", "supplier", $value);
						$left_menu[] = $_SESSION['supplier_function_list'][$value];
					}
				}
			} else {
				$left_menu = $_SESSION['supplier_menu'][$current_menu['model']]['child'];
			}
			
			Tpl::output('left_menu', $left_menu);
			Tpl::output('supplier_quicklink', $_SESSION['supplier_quicklink']);
	 ...
	 * @param $state 1成功 0失败
	 */
	protected function recordSupplierLog($content = '', $state = 1){
		$supplier_info = array();
		$supplier_info['log_content'] = $content;
		$supplier_info['log_time'] = TIMESTAMP;
		$supplier_info['log_seller_id'] = $_SESSION['supplier_id'];
		$supplier_info['log_seller_name'] = $_SESSION['supplier_name'];
		$supplier_info['log_store_id'] = $_SESSION['store_id'];
		$supplier_info['log_seller_ip'] = getIp();
		$supplier_info['log_url'] = $_GET['act'].'&'.$_GET['op'];
		$supplier_info['log_state'] = $state;
		$model_supplier_log = Model('seller_log');
		$model_supplier_log->addsellerLog($supplier_info);
	}
	...
	protected function recordStoreCost($cost_price, $cost_remark) {
		...
		$param = array();
		$param['cost_store_id'] = $_SESSION['store_id'];
		$param['cost_seller_id'] = $_SESSION['supplier_id'];
		$param['cost_price'] = $cost_price;
		$param['cost_remark'] = $cost_remark;
		$param['cost_state'] = 0;
		$param['cost_time'] = TIMESTAMP;
		...
		$param['param'] = array(
				'price' => $cost_price,
				'seller_name' => $_SESSION['supplier_name'],
				'remark' => $cost_remark
		);
	...
	protected function getSupplierMenuList($is_admin, $limits) {
		$seller_menu = array();
		if (intval($is_admin) !== 1) {
			$menu_list = $this->_getMenuList();
			foreach ($menu_list as $key => $value) {
				...
				if(count($menu_list[$key]['child']) > 0) {
					$supplier_menu[$key] = $menu_list[$key];
				}
			}
		} else {
			$supplier_menu = $this->_getMenuList();
		}
		$supplier_function_list = $this->_getSupplierFunctionList($supplier_menu);
		return array('supplier_menu' => $supplier_menu, 'supplier_function_list' => $supplier_function_list);
	}

	private function _getCurrentMenu($supplier_function_list) {
		$current_menu = $supplier_function_list[$_GET['act']];
		if(empty($current_menu)) {
			$current_menu = array(
					'model' => 'index',
					'model_name' => '首页'
	...
	private function _getMenuList() {
		$menu_list = array(
				'goods' => array('name' => '产品库', 'child' => array(
						array('name' => '产品库商品发布', 'act'=>'supplier_goods_add', 'op'=>'index'),
						array('name' => '淘宝商品导入', 'act'=>'supplier_goods_csv', 'op'=>'index'),
						array('name' => '产品库中的商品', 'act'=>'supplier_goods_online', 'op'=>'index'),
						array('name' => '仓库中的商品', 'act'=>'supplier_goods_offline', 'op'=>'index'),
						array('name' => '预约/到货通知', 'act' => 'supplier_appoint', 'op' => 'index'),
						array('name' => '经营类目', 'act'=>'supplier_info', 'op'=>'bind_class'),
						array('name' => '关联版式', 'act'=>'supplier_plate', 'op'=>'index'),
						array('name' => '商品规格', 'act' => 'supplier_spec', 'op' => 'index'),
						array('name' => '图片空间', 'act'=>'supplier_album', 'op'=>'album_cate'),
				)),
				'order' => array('name' => '订单物流', 'child' => array(
						array('name' => '实物交易订单', 'act'=>'supplier_order', 'op'=>'index'),
						array('name' => '虚拟兑码订单', 'act'=>'supplier_vr_order', 'op'=>'index'),
						array('name' => '发货', 'act'=>'supplier_deliver', 'op'=>'index'),
						array('name' => '发货设置', 'act'=>'supplier_deliver_set', 'op'=>'daddress_list'),
						array('name' => '运单模板', 'act'=>'supplier_waybill', 'op'=>'waybill_manage'),
						array('name' => '评价管理', 'act'=>'supplier_evaluate', 'op'=>'list'),
						array('name' => '物流工具', 'act'=>'supplier_transport', 'op'=>'index'),
						array('name' => '来单提醒', 'act'=>'order_call', 'op'=>'index'),
						array('name' => '实体店铺', 'act'=>'supplier_map', 'op'=>'index'),
				)),
				'consult' => array('name' => '售后服务', 'child' => array(
						array('name' => '咨询管理', 'act'=>'supplier_consult', 'op'=>'consult_list'),
						array('name' => '投诉管理', 'act'=>'supplier_complain', 'op'=>'list'),
						array('name' => '消费者保障服务', 'act'=>'supplier_contract', 'op'=>'index'),
						array('name' => '退款记录', 'act'=>'supplier_refund', 'op'=>'index'),
						array('name' => '退货记录', 'act'=>'supplier_return', 'op'=>'index'),
				)),
				'statistics' => array('name' => '统计结算', 'child' => array(
						array('name' => '店铺概况', 'act'=>'supplier_statistics_general', 'op'=>'general'),
						array('name' => '商品分析', 'act'=>'supplier_statistics_goods', 'op'=>'goodslist'),
						array('name' => '运营报告', 'act'=>'supplier_statistics_sale', 'op'=>'sale'),
						array('name' => '行业分析', 'act'=>'supplier_statistics_industry', 'op'=>'hot'),
						array('name' => '结算记录', 'act'=>'supplier_bill_log', 'op'=>'index'),
				)),
				'message' => array('name' => '客服消息', 'child' => array(
						array('name' => '系统消息', 'act'=>'supplier_msg', 'op'=>'index'),
						array('name' => '平台客服', 'act'=>'supplier_ask', 'op'=>'index'),
				)),
				'account' => array('name' => '账号', 'child' => array(
						array('name' => '账号列表', 'act'=>'supplier_account', 'op'=>'account_list'),
						array('name' => '账号组', 'act'=>'supplier_account_group', 'op'=>'group_list'),
						array('name' => '账号日志', 'act'=>'supplier_log', 'op'=>'log_list'),
						array('name' => '提现', 'act'=>'supplier_predeposit', 'op'=>'pd_log'),
				)),
				'distribution' => array('name' => '分销管理', 'child' => array(
						array('name' => '员工分销', 'act'=>'supplier_distribution', 'op'=>'index'),
						array('name' => '分销商管理', 'act'=>'supplier_distribution', 'op'=>'fx'),
				))
		);
		return $menu_list;
	}

	private function _getSupplierFunctionList($menu_list) {
		$format_menu = array();
		foreach ($menu_list as $key => $menu_value) {
			foreach ($menu_value['child'] as $submenu_value) {
				$format_menu[$submenu_value['act']] = array(
	 ...
	 */
	private function checkStoreMsg() {//判断cookie是否存在
		$cookie_name = 'storemsgnewnum'.$_SESSION['supplier_id'];
		if (cookie($cookie_name) != null && intval(cookie($cookie_name)) >=0){
			$countnum = intval(cookie($cookie_name));
		}else {
			$where = array();
			$where['store_id'] = $_SESSION['store_id'];
			$where['sm_readids'] = array('exp', 'sm_readids NOT LIKE \'%,'.$_SESSION['supplier_id'].',%\' OR sm_readids IS NULL');
			if ($_SESSION['supplier_smt_limits'] !== false) {
				$where['smt_code'] = array('in', $_SESSION['seller_smt_limits']);
			}
			$countnum = Model('store_msg')->getStoreMsgCount($where);
			setIMCookie($cookie_name,intval($countnum),2*3600);//保存2小时
...
class BaseBranchControl extends BaseSellerControl {
	public function __construct(){
	    parent::__construct() ;
		if ($this->store_info['branch_op']==1){
			...
			showDialog('你所在的店铺等级没有开设分店的权限，请咨询平台管理员！',urlShop('seller_center', 'index'),'succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
		}
	}
}
/********************************** 店铺推广基类 **********************************************/

class BaseExtensionControl extends BaseSellerControl {
	public function __construct(){
	    parent::__construct() ;
		//店铺推广功能是否开通
		if ($this->store_info['extension_op'] !=1){
			showMessage('系统未开通店铺推广功能，如有疑问请与平台联系！', urlShop('seller_center','index'), '', 'error');
		}	
		...
		if ($this->store_info['promotion_op']==1){
		...
		if ($this->store_info['saleman_op']==1){
...
class BasePointShopControl extends Control {
    ...
    public function __construct(){
        ...
        Tpl::setDir('pointprod');
        Tpl::setLayout('pointprod_layout');
    ...
    /**
     * 获得云币中心会员信息包括会员名、ID、会员头像、会员等级、经验值、等级进度、云币、已领代金券、已兑换礼品、礼品购物车
     */
...
class BaseFleaControl extends Control {
    ...
    public function __construct(){
        ...
        Language::read('common,home_layout,home_flea_index');

        Tpl::setDir('flea');

        Tpl::setLayout('flea_layout');
...
class BaseActivityControl extends Control {
