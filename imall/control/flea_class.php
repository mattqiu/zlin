		...
		if($listgoods){
			foreach($listgoods as $replace_key => $replace_val){
				...
				if($listgoods[$replace_key]['member_info']['member_avatar']){
					...
					$listgoods[$replace_key]['member_info']['avatar']	= SKINS_PATH.'/images/default_user_portrait.gif';
				}
				if($replace_val['goods_image']){
					...
					$listgoods[$replace_key]['image_url']	= SHOP_SKINS_URL.'/images/member/default_image.png';
				}
				$exge='/<[^>]*>|\s+/';
