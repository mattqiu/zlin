...
<style type="text/css">
body {
_behavior: url(<?php echo SHOP_SKINS_URL;
?>/css/csshover.htc);
}
</style>
<div class="imcc-container wrapper" >
  <div class="left">
    ...
    <div class="imcc-module imcc-module-style02">
      <div class="title">
        <h3>分类筛选</h3>
      </div>
      <div class="content">
    ...
    <?php }?>
    <!-- S 推荐展位 -->
    <div imtype="booth_goods" class="imcc-module" style="display:none;"> </div>
    <!-- E 推荐展位 -->
    <div class="imcc-module"><?php echo loadadv(37,'html');?></div>
    <!-- 最近浏览 -->
    <div class="imcc-module imcc-module-style03">
      ...
      <div class="content">
        ...
        <a href="<?php echo SHOP_SITE_URL;?>/index.php?act=member_goodsbrowse&op=list" class="imcc-sidebar-all-viewed">全部浏览历史</a></div>
    </div>
  ...
  <div class="right">
    ...
    <div class="imcc-module-filterbox imcc-module-style01">
      <div class="title border-l-r">
        <h3>
      ...
      <div class="content border-l-r">
        <div class="imcc-module-filter">
          <?php if((isset($output['checked_brand']) && is_array($output['checked_brand'])) || (isset($output['checked_attr']) && is_array($output['checked_attr']))){?>
          <dl im_type="ul_filter">
            <dt><?php echo $lang['goods_class_index_selected'].$lang['im_colon'];?></dt>
            <dd class="list">
              <?php if(isset($output['checked_brand']) && is_array($output['checked_brand'])){?>
              <?php foreach ($output['checked_brand'] as $key=>$val){?>
              <span class="selected" imtype="span_filter"><?php echo $lang['goods_class_index_brand'];?>:<em><?php echo $val['brand_name']?></em><i data-uri="<?php echo removeParam(array('b_id' => $key));?>">X</i></span>
              <?php }?>
              <?php }?>
              <?php if(isset($output['checked_attr']) && is_array($output['checked_attr'])){?>
              <?php foreach ($output['checked_attr'] as $val){?>
              <span class="selected" imtype="span_filter"><?php echo $val['attr_name'].':<em>'.$val['attr_value_name'].'</em>'?><i data-uri="<?php echo removeParam(array('a_id' => $val['attr_value_id']));?>">X</i></span>
              <?php }?>
              <?php }?>
            </dd>
          </dl>
          ...
          <dl <?php if($dl>3){?>class="dl_hide"<?php }?>>
            <dt><?php echo $lang['goods_class_index_brand'].$lang['im_colon'];?></dt>
            <dd class="list">
              <ul>
                <?php $i = 0;foreach ($output['brand_array'] as $k=>$v){$i++;?>
                <li <?php if ($i>10){?>style="display:none" im_type="none"<?php }?>><a href="<?php $b_id = (($_GET['b_id'] != '' && intval($_GET['b_id']) != 0)?$_GET['b_id'].'_'.$k:$k); echo replaceParam(array('b_id' => $b_id));?>"><?php echo $v['brand_name'];?></a></li>
                <?php }?>
              </ul>
            </dd>
            <?php if (count($output['brand_array']) > 10){?>
            <dd class="all"><span im_type="show"><i class="fa fa-angle-down"></i><?php echo $lang['goods_class_index_more'];?></span></dd>
            <?php }?>
          </dl>
          <?php $dl++;}?>
          <?php }?>
          ...
          <?php $j = 0; $hnum = 0; foreach ($output['attr_array'] as $key=>$val){$j++;?>
          <?php if(!isset($output['checked_attr'][$key]) && !empty($val['value']) && is_array($val['value'])){?>
          <dl <?php if($j>3){$hnum++; if($hnum < 5){ $hideattr .= $val['name'].',';}?>style="display:none" im_type="hide"<?php }else{?>style="display:block"<?php }?>>
            <dt><?php echo $val['name'].$lang['im_colon'];?></dt>
            <dd class="list">
              <ul>
                <?php $i = 0;foreach ($val['value'] as $k=>$v){$i++;?>
                <li <?php if ($i>10){?>style="display:none" im_type="none"<?php }?>><a href="<?php $a_id = (($_GET['a_id'] != '' && $_GET['a_id'] != 0)?$_GET['a_id'].'_'.$k:$k); echo replaceParam(array('a_id' => $a_id));?>"><?php echo $v['attr_value_name'];?></a></li>
                <?php }?>
              </ul>
            </dd>
      ...
      <?php if ($hnum>0){?>
      <div id="J_selectorMore" class="s-more">
        <span data-more="<?php echo trim($hideattr,',');?>等" class="sm-wrap" im_type="sm-wrap">更多选项（<?php echo trim($hideattr,',');?>等)<i></i></span>
      </div>
      <?php }else{?>
      <div class="n-more"></div>
      <?php }?>
    </div>    
    <?php }?>
    <?php }?>    
    ...
    <div class="shop_con_list" id="main-nav-holder">
      <nav class="sort-bar" id="main-nav">
        ...
        <div class="imcc-sortbar-owner">商品类型： 
          ...
          <span><a href="<?php echo replaceParam(array('type' => '1'));?>" <?php if ($_GET['type'] == 1) {?>class="selected"<?php }?>><i></i>平台自营</a></span> 
          ...
          <span><a href="<?php if ($_GET['gift'] == 1) { echo dropParam(array('gift'));} else { echo replaceParam(array('gift' => '1'));}?>" <?php if ($_GET['gift'] == 1) {?>class="selected"<?php }?>><i></i>赠品</a></span>
        </div>
        <div class="imcc-sortbar-location">商品地域：
          <div class="select-layer">
            ...
            <i class="direction"></i>
            <ul class="options">
              <?php require(BASE_TPL_PATH.'/goods/goods_class_area.php');?>
            </ul>
          </div>
        </div>
      </nav>
      <!-- 商品列表循环  -->
      <div>
        <?php require_once (BASE_TPL_PATH.'/goods/goods.squares.php');?>
      </div>
