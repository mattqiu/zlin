...
<link href="<?php echo SHOP_SKINS_URL;?>/css/store_list.css" rel="stylesheet" type="text/css">
...
<div class="content">
  <div class="imcss-store-class">
    ...
    <dl>
      <dt>店铺类目<?php echo $lang['im_colon'];?></dt>
      <dd>
          ...
          <div class="sort-bar"><!-- 排序方式S -->
            <ul class="array">
              ...
              <li <?php if(!$_GET['key']){?>class="selected"<?php }?>>
                <a href="javascript:void(0)" class="nobg" onClick="javascript:ss_dropParam('key','order');" title="<?php echo $lang['goods_class_index_default_sort'];?>">默认</a>
              </li>
              <!-- 销量 -->
              <li <?php if($_GET['key'] == 'store_sales'){?>class="selected"<?php }?>>
                <a href="javascript:void(0)" <?php if($_GET['key'] == 'store_sales'){?>class="<?php echo $_GET['order'];?>"<?php }?> onClick="javascript:ss_replaceParam('store_sales','<?php echo ($_GET['order'] == 'desc' && $_GET['key'] == 'store_sales')?'asc':'desc' ?>');" title="<?php echo ($_GET['order'] == 'desc' && $_GET['key'] == 'store_sales')?$lang['store_class_index_sold_asc']:$lang['store_class_index_sold_desc']; ?>">销量</a>
              </li>
              <!-- 信用 -->
              <li <?php if($_GET['key'] == 'store_credit'){?>class="selected"<?php }?>>
                <a href="javascript:void(0)" <?php if($_GET['key'] == 'store_credit'){?>class="<?php echo $_GET['order'];?>"<?php }?> onClick="javascript:ss_replaceParam('store_credit','<?php  echo ($_GET['order'] == 'desc' && $_GET['key'] == 'store_credit')?'asc':'desc' ?>');" title="<?php  echo ($_GET['order'] == 'desc' && $_GET['key'] == 'store_credit')?$lang['store_class_index_credit_asc']:$lang['store_class_index_credit_desc']; ?>">信用</a>
              </li>
            </ul>
            <!-- 排序方式E -->
            <?php if (!C('fullindexer.open')){?>
            <div class="sidebox">
              <h5 class="title">店铺名称<?php echo $lang['im_colon'];?></h5>
              <div class="selectbox">
                <input class="text" type="text" name="keyword" value="<?php echo $_GET['keyword'];?>" style=" width:150px;"/>
              </div>
            </div>
            <div class="sidebox">
              <h5><label for="area_info">店铺所在地<?php echo $lang['im_colon'];?></label></h5>
              <div class="selectbox">
  ...
  <ul class="imcss-store-list">
    ...
    <li class="item">
      <dl class="shop-info">
        ...
        <dd class="shopkeeper"><?php echo $lang['store_class_index_owner'].$lang['im_colon'];?><?php echo $store['member_name'];?><a target="_blank" class="message" href="index.php?act=member_message&op=sendmsg&member_id=<?php echo $store['member_id'];?>"></a><span>
              ...
              <dt class="goods-pic">
                <span class="thumb size160"> 
                  ...
                  <a href="index.php?act=goods&goods_id=<?php echo $v['goods_id'];?>" target="_blank"> 
                    <img  onload="javascript:DrawImage(this,160,160);" alt="<?php echo $v['goods_name'];?>" src="<?php echo thumb($v,'small');?>">
                  </a>
                </span>
              </dt>
