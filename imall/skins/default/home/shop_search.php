...
<link href="<?php echo SHOP_SKINS_URL;?>/css/shop_search.css" rel="stylesheet" type="text/css">
...
<script type="text/javascript" src="<?php echo SHOP_RESOURCE_SITE_URL.'/js/shop_search.js';?>" charset="utf-8"></script>
<script type="text/javascript" src="<?php echo RESOURCE_SITE_URL.'/js/class_area_array.js';?>" charset="utf-8"></script>
...
<div class="content">
<div class="imcss-store-class">
  <?php if(!empty($output['class_list']) && is_array($output['class_list'])){?>
  <dl>
    <dt><?php echo $lang['store_class_index_store_class'].$lang['im_colon'];?></dt>
    <dd>
      <?php foreach($output['class_list'] as $k=>$v){?>
	      <?php if ($_GET['cate_id'] == $v['sc_parent_id']){?>
	      <a href="<?php echo urlShop('shop_search','index',array('cate_id'=>$k));?>"><?php echo $v['sc_name'];?></a>
...
<div class="sort-bar">
  <div class="shop_con_list" id="main-nav-holder">
    <nav class="imcss-gl-sort-bar" id="main-nav">
      <form id="searchStore" method="GET" action="index.php">
        ...
        <div class="sort-bar"><!-- 排序方式S -->
          <ul class="array">
          <!-- 默认 -->
            <li <?php if(!$_GET['key']){?>class="selected"<?php }?>><a href="javascript:void(0)" class="nobg" onClick="javascript:ss_dropParam('key','order');" title="<?php echo $lang['goods_class_index_default_sort'];?>"><?php echo $lang['store_class_index_default'];?></a></li>
            <!-- 销量 -->
          ...
          <div class="sidebox w150">
            <h5 class="title"><?php echo $lang['store_class_index_store_name'].$lang['im_colon'];?></h5>
            <div class="selectbox">
              <input class="text" type="text" name="keyword" value="<?php echo $_GET['keyword'];?>" style=" width:80px;"/>
            </div>
          </div>
          <div class="sidebox w150">
            <h5><?php echo $lang['store_class_index_owner'].$lang['im_colon'];?></h5>
            <div class="selectbox">
          ...
          <div class="sidebox width5" style=" background-image: none">
            <div class="selectbox">
              <input class="btn" type="submit" value="<?php echo $lang['im_search'];?>" />
            </div>
          </div>
          <?php }?>
          <div class="sidebox" style="background-image: none; float:right;">
            <div class="selectbox select-layer" style="margin:0;">
              <div class="holder"><em im_type="area_name"><?php echo $lang['store_class_index_location'];?><!-- 所在地 --></em></div>
              <div class="selected"><a im_type="area_name"><?php echo $lang['store_class_index_location'];?><!-- 所在地 --></a></div>
              <i class="direction"></i>
              <ul class="options">
                <?php require(BASE_TPL_PATH.'/goods/goods_class_area_'.(strtolower(CHARSET)=='utf-8'?'utf-8':'gbk').'.html');?>
              </ul>
            </div>
          </div>
        </div>
...
<?php foreach($output['store_list'] as $skey => $store){?>
    <li class="item">
      <dl class="shop-info">
        ...
        <dd class="shop-pic"><a href="<?php echo urlShop('show_store','', array('store_id'=>$store['store_id']),$store['store_domain']);?>" title="" target="_blank"><span class="size72"><img src="<?php echo $store['store_logo'];?>" class="size72" /></span></a></dd>
        <dd class="main-runs" title="<?php echo $store['store_zy']?>"><?php echo $lang['store_class_index_store_zy'].$lang['im_colon'];?><?php echo $store['store_zy']?></dd>
        <dd class="shopkeeper"><?php echo $lang['store_class_index_owner'].$lang['im_colon'];?><?php echo $store['member_name'];?><a target="_blank" class="message" href="index.php?act=home&op=sendmsg&member_id=<?php echo $store['member_id'];?>"></a><span>
      ...
      <dl class="w200">
        <dd><?php echo ($tmp = $store['goods_count']) ? $lang['store_class_index_goods_amount'].$tmp.$lang['piece'] : $lang['im_common_goods_null'];?></dd>
        <dd><?php echo ($tmp = $store['num_sales_jq']) ? $lang['store_class_index_deal'].$tmp.$lang['store_class_index_jian'] : $lang['im_common_sell_null'];?></dd>
        <?php if (!empty($store['search_list_goods'])){?>
        <dd class="more-on" attr='morep' im_type='<?php echo $skey;?>'><span><?php echo $lang['store_class_index_goods_hiden'];?></span><i></i></dd>
        <?php }?>
      </dl>
      <dl class="w150">
      	<!-- 店铺信用度 -->
        <dd><?php if (empty($store['store_credit_average'])){ echo $lang['im_common_credit_null']; }else {?>
          <?php echo $lang['store_class_index_credit_value'].$lang['im_colon'];?>
        ...
        <?php if (empty($store['store_credit_percent'])){?>
        	<?php echo $lang['im_common_rate_null'];?>
        <?php }else{?>	
        	<?php echo $lang['store_class_index_praise_rate'].$lang['im_colon'].$store['store_credit_percent'];?>%
        <?php }?>
        </dd>
        <!-- 店铺动态评分 -->
        <dd class="shop-rate" im_type="shop-rate" store_id='<?php echo $store['store_id'];?>'><?php echo $lang['store_class_index_shop_rate'].$lang['im_colon'];?><span><i></i></span>
          <div class="shop-rate-con">
              <div class="arrow"></div>
              <dl class="rate">
                <?php  foreach ($store['store_credit'] as $key=>$value) {?>
                  <dt><?php echo $value['text'].$lang['im_colon'];?></dt>
      ...
      <div class="imcss-shop-goodslist" im_type='goods_<?php echo $skey;?>'>
        <div class="arrow"></div>
        <ul class="jcarousel-skin-tango" im_type="jcarousel" >
...
$(function(){    
	...
	//选择地区后的地区显示
	$('[im_type="area_name"]').html(im_class_a[<?php echo intval($_GET['area_id']);?>]);
	<?php }?>
});
</script> 
<script type="text/javascript">
...
$(function(){
	//图片轮换
    $('[im_type="jcarousel"]').jcarousel({visible: 4});
    $('[attr="morep"]').click(function(){
    	var id = $(this).attr('im_type');
