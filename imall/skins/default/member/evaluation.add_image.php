...
<div class="imm-flow-layout">
  <div class="imm-flow-container">
    <div class="title"><a href="<?php echo urlShop('member_evaluate', 'list');?>" class="imm-btn-mini fr"><i class="fa fa-reply"></i>返&nbsp;回</a>
      <h3>对商品进行晒单</h3>
    </div>
    <div class="alert">
      <h4>图片上传要求：</h4>
          ...
          <ul>
            ...
            <li>
              <div class="upload-thumb">
                <div imtype="image_item" style="display:none;"> <img src="">
                  <input type="hidden" imtype="input_image" name="evaluate_image[]" value="" >
                  <a href="javascript:;" imtype="del" class="del" title="移除">X</a> </div>
              </div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(".input-file").fileupload({
            ...
            add: function(e, data) {
                ...
                if(data.result.state) {
                    ...
                    $item.find('[imtype="input_image"]').val(data.result.file_name);
                    var file_id = $item.find('[imtype="del"]').attr('data-file-id');
                    album_pic_del(file_id);
                    $item.find('[imtype="del"]').attr('data-file-id', data.result.file_id);
                    $item.find('[imtype="image_item"]').show();
                } else {
        ...
        $('[imtype="del"]').on('click', function() {
            album_pic_del($(this).attr('data-file-id'));
            $item = $(this).parent();
            $item.find('[imtype="input_image"]').val('');
            $item.hide();
        });
