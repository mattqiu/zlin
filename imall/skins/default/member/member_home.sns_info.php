<?php defined('InIMall') or exit('Access Invalid!');?>
  ...
  <div id="friendsShare" class="normal">
    <div class="outline">
      ...
      <?php if (!empty($output['follow_list']) && is_array($output['follow_list'])) { ?>
       <div class="imm-friends-share">
            <ul id="friendsShareList" class="jcarousel-skin-tango">
            <?php foreach($output['follow_list'] as $k => $v){ ?>
              <li>
                <div class="imm-friend-avatar"><a href="index.php?act=member_snshome&op=trace&mid=<?php echo $v['member_id'];?>" target="_blank"><img  src="<?php if ($v['member_avatar']!='') { echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.DS.$v['member_avatar']; } else { echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('default_user_portrait'); } ?>"></a></div>
                <dl>
                  <dt class="imm-friend-name"><a href="index.php?act=member_snshome&op=trace&mid=<?php echo $v['member_id'];?>" target="_blank"><?php echo $v['friend_tomname']; ?></a></dt>
     ...
     <?php } else { ?>
      <dl class="null-tip">
        ...
        <dd>
          ...
          <p><a target="_blank" href="<?php echo SHOP_SITE_URL?>/index.php?act=member_snsfriend&op=follow" class="imm-btn-mini" >查看我的全部好友</a></p>
        </dd>
      </dl>
    <?php } ?>  
    </div>
  ...
  <div id="circle" class="normal">
    ...
    <?php if(!empty($output['circle_list'])){?>
      <div class="imm-circle">
            <ul id="circleList" class="jcarousel-skin-tango">
              <?php foreach($output['circle_list'] as $val){?>
              <li>
                <div class="imm-circle-pic"><a href="<?php echo CIRCLE_SITE_URL;?>/index.php?act=group&c_id=<?php echo $val['circle_id'];?>" target="_blank"><img alt="" src="<?php echo circleLogo($val['circle_id']);?>"></a></div>
                <dl>
      ...
      <dl class="null-tip">
        ...
        <dd>
          ...
          <p><a target="_blank" href="<?php echo CIRCLE_SITE_URL;?>/index.php?act=index&op=add_group" class="imm-btn-mini">创建圈子</a></p>
        </dd>
      </dl>
      <?php } ?>
    </div>
  ...
  <div id="browseMark" class="normal">
    <div class="outline">
      ...
      <div class="imm-browse-mark">
            <ul id="browseMarkList" class="jcarousel-skin-tango">
            <?php foreach($output['viewed_goods'] as $goods_id => $goods_info) { ?>
              <li>
                <div class="imm-goods-pic"><a href="<?php echo $goods_info['url'];?>" target="_blank"><img alt="" src="<?php echo $goods_info['goods_image'];?>"></a></div>
                <dl>
                  <dt class="imm-goods-name"><a href="<?php echo $goods_info['url'];?>" title="<?php echo $goods_info['goods_name'];?>" target="_blank"><?php echo $goods_info['goods_name'];?></a></dt>
                  <dd class="imm-goods-price"><em>￥<?php echo $goods_info['goods_promotion_price'];?></em></dd>
                </dl>
              </li>
            <?php } ?>
            </ul>
       ...
       <?php } else { ?>
          <dl class="null-tip">
            ...
            <dd>
              <h4>您的商品浏览记录为空</h4>
              <h5>赶紧去商城看看促销活动吧</h5>
              <p><a target="_blank" href="<?php echo SHOP_SITE_URL;?>" class="imm-btn-mini">浏览商品</a></p>
            </dd>
          </dl>
       <?php } ?>
    </div>
