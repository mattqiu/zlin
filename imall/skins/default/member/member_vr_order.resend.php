...
<div class="eject_con" style="overflow: visible;">
  <form id="add_form" action="index.php?act=member_vr_order&op=resend" method="post"  onsubmit="ajaxpost('add_form','','','onerror')">
    ...
    <div class="bottom">
        ...
        <a class="imm-btn ml5" href="javascript:DialogManager.close('vr_code_resend');">取消</a>
    </div>
  </form>
</div>
	...
	$("#add_form").validate({
		...
		messages: {
			buyer_phone : {
				...
				digits : "<i class='fa fa-exclamation-circle'></i>请正确填写手机号码",
				minlength : "<i class='fa fa-exclamation-circle'></i>请正确填写手机号码"
			}
		}
	});
});
