...
<div class="dialog-share">
  <form method="post" action="index.php?act=member_snsindex&op=sharestore" id="sharestore_form">
    ...
    <!-- 验证码 -->
    <div id="ss_seccode" class="seccode"><?php echo $lang['im_checkcode'].$lang['im_colon']; ?>
    ...
    <!-- 防止点击Enter键提交 -->
    <div class="bottom">
      <input name="<?php echo $lang['im_snsshare'];?>" type="button" class="button" value="<?php echo $lang['im_snsshare'];?>" id="weibobtn_store" />
	...
	$("#weibobtn_store").click(function(){
		if($("#sharestore_form").valid()){
			...
			if(cookienum >= max_recordnum && $("#ss_seccode").css('display') == 'none'){
				//显示验证码
				$("#ss_seccode").show();
				var imhash = $("#ss_seccode").find("[name='imhash']").val();
				$("#ss_seccode").find("[name='codeimage']").attr('src','index.php?act=seccode&op=makecode&imhash='+imhash+'&t=' + Math.random());
...
<script>
    ...
    window._bd_share_config={
		"common":{
			"bdSnsKey":{},
			"bdText":"<?php echo $output['store_info']['store_name']; ?>",
			"bdDesc":"<?php echo $output['store_info']['store_name']; ?>",
			"bdUrl":"<?php echo $output['store_info']['store_url'];?>",
			"bdPic":"<?php echo getStoreLogo($output['store_info']['store_avatar']);?>",
			"bdPopTitle":"分享店铺到",	 //'请参考自定义pop窗口标题'
			"bdMini":"2",
			"bdMiniList":false,
			"bdStyle":"1",
			"bdSize":"32",
