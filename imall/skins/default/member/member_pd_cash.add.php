...
<div class="wrap">
  ...
  <div class="alert alert-success">
    <h4>温馨提示：</h4>
    <ul>
      <li>1. 使用银行账户提现：需缴纳提现金额0.15%的服务费（服务费下线2元/笔、服务费上限25元/笔）</li>
      <li>2. 使用支付宝账户提现：需缴纳提现金额0.5%的服务费（服务费下线1元/笔、服务费上限25元/笔）</li>
      <li>3. 注：所有提现操作会在两个工作日内完成！</li>
    </ul>
  </div>
...
元</em> （当前可用金额：<strong class="orange"><?php echo floatval($output['member_info']['available_predeposit']); ?></strong>&nbsp;&nbsp;元）<span></span>
          <p class="hint mt5"></p>
        </dd>
      </dl>
      <dl>
...
请填写详细的开户银行分行名称，虚拟账户如支付宝、财付通填写“支付宝”、“财付通”即可。</p>
      ...
      <dl>
        ...
        <p class="hint">
              ...
              <strong class="red">还未设置支付密码</strong><a href="<?php echo SHOP_SITE_URL;?>/index.php?act=member_security&op=auth&type=modify_paypwd" class="imm-btn-mini imm-btn-acidblue vm ml10" target="_blank">马上设置</a>
              <?php } ?>
        </p>
          </dd>
      </dl>
      <dl class="bottom"><dt>&nbsp;</dt>
          <dd><label class="submit-border"><input type="submit"  class="submit" value="确认提现" /></label><a class="imm-btn ml10" href="javascript:history.go(-1);">取消并返回</a></dd>
      </dl>
    </form>
  </div>
</div>
...
$(function(){
	$('#cash_form').validate({
        ...
        messages : {
        	pdc_amount	  : {
            	required  :'<i class="fa fa-exclamation-circle"></i>请正确输入提现金额',
            	number    :'<i class="fa fa-exclamation-circle"></i>请正确输入提现金额',
            	min    	  :'<i class="fa fa-exclamation-circle"></i>请正确输入提现金额',
            	max       :'<i class="fa fa-exclamation-circle"></i>请正确输入提现金额'
            },
            pdc_bank_name :{
            	required   :'<i class="fa fa-exclamation-circle"></i>请输入收款银行'
            },
            pdc_bank_no : {
            	required   :'<i class="fa fa-exclamation-circle"></i>请输入收款账号'
            },
            pdc_bank_user : {
	        	required  : '<i class="fa fa-exclamation-circle"></i>请输入开户人姓名'
	        },
	        password : {
		        required : '<i class="fa fa-exclamation-circle"></i>请输入支付密码'
		    }
        }
    });
});
