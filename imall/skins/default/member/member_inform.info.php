...
<div class="imm-flow-layout">
  <div id="immInformFlow" class="imm-flow-container">
    <div class="title">
    ...
    <div class="imm-default-form">
      ...
      <dl>
        <dt><?php echo $lang['inform_goods_name'].$lang['im_colon'];?></dt>
        <dd>
          <div class="imm-inform-item">
            <div class="imm-goods-thumb-mini"><a href="<?php echo urlShop('goods', 'index',array('goods_id'=>$output['goods_info']['goods_id']));?>" target="_blank"><img src="<?php echo thumb($output['goods_info'],60); ?>" onMouseOver="toolTip('<img src=<?php echo thumb($output['goods_info'],240); ?>>')" onMouseOut="toolTip()" /></a></div>
      ...
      <dl>
        <dt><?php echo $lang['inform_type'].$lang['im_colon'];?></dt>
        <dd> <?php echo $output['subject_info']['inform_subject_content'];?> </dd>
      </dl>
      <dl>
        <dt><?php echo $lang['inform_subject'].$lang['im_colon'];?></dt>
        <dd><?php echo $output['subject_info']['inform_subject_type_name'];?></dd>
      </dl>
      <dl>
        <dt><?php echo $lang['inform_content'].$lang['im_colon'];?></dt>
        <dd>
          <?php echo $output['inform_info']['inform_content'];?>
        </dd>
      </dl>
      <dl class="noborder">
        <dt><?php echo $lang['inform_pic'].$lang['im_colon'];?></dt>
        <dd>
        <ul class="imm-evidence-pic">
        <?php if (!empty($output['inform_info']['inform_pic1'])) {?>
        <li><a href="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_PATH.'/inform/'.$output['inform_info']['inform_pic1'];?>" imtype="nyroModal" rel="gal"><img class="show_image" src="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_PATH.'/inform/'.$output['inform_info']['inform_pic1'];?>"></a></li>
        <?php }?>
        <?php if (!empty($output['inform_info']['inform_pic2'])) {?>
        <li><a href="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_PATH.'/inform/'.$output['inform_info']['inform_pic2'];?>" imtype="nyroModal" rel="gal"><img class="show_image" src="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_PATH.'/inform/'.$output['inform_info']['inform_pic2'];?>"></a></li>
        <?php }?>
        <?php if (!empty($output['inform_info']['inform_pic3'])) {?>
        <li><a href="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_PATH.'/inform/'.$output['inform_info']['inform_pic3'];?>" imtype="nyroModal" rel="gal"><img class="show_image" src="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_PATH.'/inform/'.$output['inform_info']['inform_pic3'];?>"></a></li>
        <?php }?>
        </ul>
        </dd>
      </dl>
      <div class="bottom"><a href="javascript:history.go(-1);" class="imm-btn"><i class="fa fa-reply"></i>返回列表</a></div>
    </div>
  </div>
    ...
    $.ajax({
        ...
        success:function(type_list){
            ...
            if(type_list.length >= 1) {
                $("#inform_subject").addOption('','<?php echo $lang['im_please_choose'];?>');
