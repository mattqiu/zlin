...
<div class="wrap">
  ...
  <div class="imm-default-form">
    <form method="post" id="profile_more" action="index.php?act=member_information&op=more">
      ...
      <div imtype="list" class="user-tag-optional">
        <?php if(!empty($output['mtag_list']) && is_array($output['mtag_list'])){?>
		<?php foreach ($output['mtag_list'] as $val){?>
        <span imtype="able" data-param='{"id":"<?php echo $val['mtag_id'];?>"}'><?php echo $val['mtag_name'];?></span>
        <?php }?>
        <?php }?>
      </div>
      <h4 class="w90 mt20 mb10 tip" title="<?php echo $lang['home_member_more_my_tag_title'];?>"><?php echo $lang['home_member_more_my_tag'];?></h4>
      <div imtype="choose" class="user-tag-selected">
        <?php if(!empty($output['mtm_list']) && is_array($output['mtm_list'])){?>
        <?php foreach ($output['mtm_list'] as $val){?>
        <span imtype="able" data-param='{"id":"<?php echo $val['mtag_id'];?>"}'><?php echo $val['mtag_name'];?><a href="javascript:void(0)" imtype="delTag">
        <input type="hidden" name="mid[]" value="<?php echo $val['mtag_id'];?>" />
        X</a></span>
        <?php }?>
        <?php }?>
        <span imtype="ep" class="ep">&nbsp;</span> </div>
      <div class="bottom">
...
//注册表单验证
$(function(){
	var $list = $('div[imtype="list"]');
	var $choose = $('div[imtype="choose"]');
	var $ep = $('span[imtype="ep"]');
	$('span[imtype="able"]', $list).draggable({ 
		cancel: "a.ui-icon",
        revert: "invalid",
	...
	$('span[imtype="able"]', $choose).draggable({ 
		cancel: "a.ui-icon",
        revert: "invalid",
        containment: "document",
       	helper: "clone",
    ...
    function chooseTeg($item){
    	$item.fadeOut('fast',function(){
    		...
    		$item.append('<a href="javascript:void(0)" imtype="delTag"><input type="hidden" name="mid[]" value="'+data_param.id+'" />X</a>')
	...
	$('a[imtype="delTag"]').live('click', function(){
		recycleIeg($(this).parent());
	});

	$('div[imtype="list"]').find('span').live('click', function(){
		chooseTeg($(this));
	});
