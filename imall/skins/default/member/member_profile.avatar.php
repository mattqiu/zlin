...
<div class="wrap">
  ...
  <form action="index.php?act=member_information&op=upload" enctype="multipart/form-data" id="form_avaster" method="post">
    ...
    <div class="imm-default-form">
      <dl>
        <dt><?php echo $lang['home_member_avatar_thumb'].$lang['im_colon'];?></dt>
        <dd>
          <div class="user-avatar"><span><img src="<?php echo getMemberAvatar($output['member_avatar']).'?'.microtime(); ?>" alt="" im_type="avatar" /></span></div>
          <p class="hint mt5"><?php echo $lang['im_member_avatar_hint'];?></p>
        </dd>
      </dl>
      <dl>
        <dt><?php echo $lang['home_member_change_avatar'].$lang['im_colon'];?></dt>
        <dd>
          <div class="imm-upload-btn"> <a href="javascript:void(0);"><span>
  ...
  <form action="index.php?act=member_information&op=cut" id="form_cut" method="post">
    ...
    <div class="pic-cut-120">
      <div class="work-title"><?php echo $lang['im_comm_workarea'];?></div>
      <div class="work-layer">
        <p><img id="imcropbox" src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_AVATAR.DS.$output['newfile'].'?'.microtime(); ?>"/></p>
      </div>
      <div class="thumb-title"><?php echo $lang['im_comm_cut_view'];?></div>
      <div class="thumb-layer">
        <p><img id="preview" src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_AVATAR.DS.$output['newfile'].'?'.microtime();?>"/></p>
      </div>
      <div class="cut-help">
        <h4><?php echo $lang['im_comm_op_help'];?></h4>
        <p><?php echo $lang['im_comm_op_help_tip'];?></p>
      </div>
      <div class="cut-btn">
        <input type="button" id="imsubmit" class="submit" value="<?php echo $lang['im_submit'];?>" />
      </div>
    </div>
  </form>
  <?php }?>
