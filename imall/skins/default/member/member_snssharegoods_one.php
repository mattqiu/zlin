...
/*百度分享部分*/
.bdsharebuttonbox { display: block; padding: 10px 20px;}
/*分享设定部分*/
...
.dialog-share .share-widget .s-app i { background: url("<?php echo SHOP_SKINS_URL;?>/images/member/shareicon/shareicons.png") no-repeat 0 0; display: block; width: 24px; height: 24px; cursor: pointer;}
...
.dialog-share .share-widget .s-app .tip .arrow { background: url("<?php echo SHOP_SKINS_URL;?>/images/member/shareicon/shareicons.png") no-repeat -36px 0; display: block; width: 11px; height: 6px; position: absolute; z-index: 2; top: -6px; left: 20px;}
...
.share-privacy .privacytab .arrow { background: url("<?php echo SHOP_SKINS_URL;?>/images/member/shareicon/shareicons.png") no-repeat -24px 0; display: block; width: 11px; height: 6px; margin-right: -6px; position: absolute; z-index: 2; top: -6px; right: 50%;}
...
.share-privacy .privacytab ul li .selected { background: url("<?php echo SHOP_SKINS_URL;?>/images/member/shareicon/shareicons.png") no-repeat -32px -20px;}
...
.dialog-share .share-content .avatar i { background: url("<?php echo SHOP_SKINS_URL;?>/images/member/shareicon/shareicons.png") no-repeat -30px -50px; width: 10px; height: 10px; position: absolute; z-index: 1; top: 10px; right: -15px;}
...
<div class="dialog-share">  
  <form method="post" action="index.php?act=member_snsindex&op=sharegoods" id="sharegoods_form">
    ...
    <div class="share-set">
      <div class="share-widget"><?php echo $lang['sharebind_alsoshareto'];?>
        <ul class="s-app">
          ...
          <li im_type="appitem_<?php echo $k;?>" title="<?php echo $v['name'];?>" class="<?php echo $v['isbind']?'checked':'disable';?>"> <i class="i-<?php echo $k;?>" im_type="bindbtn" data-param='{"apikey":"<?php echo $k;?>","apiname":"<?php echo $v['name'];?>"}' attr_isbind="<?php echo $v['isbind']?'1':'0';?>"></i>
      ...
      <div im_type="formprivacydiv" class="share-privacy"><a href="javascript:void(0);" im_type="formprivacybtn"><i class="fa fa-users"></i>可见范围</a>
        <div class="privacytab" im_type="formprivacytab" style="display:none;"> <span class="arrow"></span>
          <ul>
            <li im_type="formprivacyoption" data-param='{"v":"0","hiddenid":"gprivacy"}'><span class="selected"></span><?php echo $lang['sns_weiboprivacy_all'];?></li>
            <li im_type="formprivacyoption" data-param='{"v":"1","hiddenid":"gprivacy"}'><span></span><?php echo $lang['sns_weiboprivacy_friend'];?></li>
            <li im_type="formprivacyoption" data-param='{"v":"2","hiddenid":"gprivacy"}'><span></span><?php echo $lang['sns_weiboprivacy_self'];?></li>
          </ul>
        </div>
      </div>
    </div>
    ...
    <!-- 验证码 -->
    <div id="sg_seccode" class="seccode"><?php echo $lang['im_checkcode'].$lang['im_colon']; ?>
    ...
    <!-- 防止点击Enter键提交 -->
    <div class="bottom">
      <input name="<?php echo $lang['im_snsshare'];?>" type="button" class="button" value="<?php echo $lang['im_snsshare'];?>" id="weibobtn_goods" />
...
<div class="bindtooltip">
  <dl>
    <dt><img src="<?php echo SHOP_SKINS_URL;?>/images/member/shareicon/shareicon_@apikey.png"></dt>
    <dd>
      <p><?php echo $lang['sharebind_list_popup_tip1'];?><strong class="ml5 mr5">@apiname</strong><?php echo $lang['sharebind_list_popup_tip2'];?></p><p class="hint"><?php echo $lang['sharebind_list_popup_tip3'];?>@apiname    <?php echo $lang['sharebind_list_popup_tip4'];?></p>
    </dd>
  </dl>
  <div class="bottom">
    <a href="javascript:void(0);" im_type="finishbtn" data-param='{"apikey":"@apikey"}' class="mini-btn"><?php echo $lang['sharebind_list_finishbind'];?></a><span><?php echo $lang['sharebind_list_unfinishedbind'];?>
    <a target="_blank" href="<?php echo SHOP_SITE_URL;?>/api.php?act=sharebind&type=@apikey"><?php echo $lang['sharebind_list_againbind'];?></a></span>
  </div>
</div>
</textarea>
...
$(function(){
	...
	$("[im_type='bindbtn']").bind('click',function(){
	    ...
	    if(isbind == '1'){//已经绑定
		    if($("#checkapp_"+data_str.apikey).val() == '1'){
			    $("[im_type='appitem_"+data_str.apikey+"']").removeClass('checked');
		    	$("[im_type='appitem_"+data_str.apikey+"']").addClass('disable');
            	$("#checkapp_"+data_str.apikey).val('0');
			}else{
				$("[im_type='appitem_"+data_str.apikey+"']").removeClass('disable');
            	$("[im_type='appitem_"+data_str.apikey+"']").addClass('checked');
	...
	$("[im_type='finishbtn']").live('click',function(){
		var data_str = $(this).attr('data-param');
	    eval( "data_str = "+data_str);
		//验证是否绑定成功
        ...
        $.getJSON(url, {'k':data_str.apikey}, function(data){
            ...
            if (data.done)
            {
            	$("[im_type='appitem_"+data_str.apikey+"']").addClass('check');
            	$("[im_type='appitem_"+data_str.apikey+"']").removeClass('disable');
            	$('#checkapp_'+data_str.apikey).val('1');
            	$("[im_type='appitem_"+data_str.apikey+"']").find('i').attr('attr_isbind','1');
            }
            else
            {
...
<div class="bdsharebuttonbox" data-tag="share_1">  
  ...
  <a title="分享到天涯社区" class="bds_ty" href="#" data-cmd="ty"></a>
  <a title="分享到百度空间" class="bds_hi" href="#" data-cmd="hi"></a>
  <a title="分享到百度贴吧" class="bds_tieba" href="#" data-cmd="tieba"></a>
  <a title="分享到飞信" class="bds_fx" href="#" data-cmd="fx"></a>  
  <a title="分享到蘑菇街" class="bds_mogujie" href="#" data-cmd="mogujie"></a>  
  <a class="bds_more" href="#" data-cmd="more" id="bdshare"></a> 
</div>
<script>
    function getbdMore(){
        document.getElementById('bdshare').click();
    }
    window._bd_share_config={
		"common":{
			"bdSnsKey":{},
			"bdText":"<?php echo $output['goods_info']['goods_name']; ?>",
			"bdDesc":"<?php echo str_replace("\n", "<br>", $output['goods_info']['goods_jingle']);?>",
			"bdUrl":"<?php echo $output['goods_info']['goods_url']; ?>",
			"bdPic":"<?php echo thumb($output['goods_info'], 240); ?>",
			"bdPopTitle":"分享商品到",	 //'请参考自定义pop窗口标题'
			"bdMini":"2",
			"bdMiniList":false,
			"bdStyle":"1",
			"bdSize":"32",
			//'onBeforeClick': '';,	 //在用户点击分享按钮时执行代码，更改配置。cmd为分享目标id，config为当前设置，返回值为更新后的设置。
			//'onAfterClick': '';,	 //在用户点击分享按钮后执行代码，cmd为分享目标id。可用于统计等。
