      ...
      <div class="imm-default-form">
        <form id="post_form1" enctype="multipart/form-data" method="post" action="index.php?act=member_refund&op=add_refund&order_id=<?php echo $output['order']['order_id']; ?>&goods_id=<?php echo $output['goods']['rec_id']; ?>">
          ...
          <dl>
            ...
            <dd>
              <input type="text" class="text w50" name="refund_amount" value="<?php echo $output['goods']['goods_pay_price']; ?>" /><em class="add-on">元</em>（最多 <strong class="green" title="可退金额由系统根据订单商品实际成交额和已退款金额自动计算得出"><?php echo $output['goods']['goods_pay_price']; ?></strong> 元） <span class="error"></span>
          ...
          <div class="bottom">
            ...
            <a href="javascript:history.go(-1);" class="imm-btn ml10">取消并返回</a> </div>
        </form>
      </div>
    </div>
    <div id="saleRefundReturn" show_id="2" style="display: none;">
      ...
      <div class=" imm-default-form">
        ...
        <form id="post_form2" method="post" enctype="multipart/form-data" action="index.php?act=member_refund&op=add_refund&order_id=<?php echo $output['order']['order_id']; ?>&goods_id=<?php echo $output['goods']['rec_id']; ?>">
          ...
          <dl>
            <dt><i class="required">*</i><?php echo $lang['refund_order_refund'].$lang['im_colon'];?></dt>
            <dd>
              <input type="text" class="text w50" name="refund_amount" value="<?php echo $output['goods']['goods_pay_price']; ?>" />
              <em class="add-on">元</em> （最多 <strong class="green" title="可退金额由系统根据订单商品实际成交额和已退款金额自动计算得出。"><?php echo $output['goods']['goods_pay_price']; ?></strong> 元） <span class="error"></span>
              <p class="hint"><?php echo '退款金额不能超过可退金额。';?></p>
            </dd>
          </dl>
          <dl>
            <dt><i class="required">*</i><?php echo $lang['return_order_return'].$lang['im_colon'];?></dt>
            <dd>
...
$(function(){
    $('#post_form1').validate({
        ...
        messages : {
            reason_id  : {
                required  : '<i class="fa fa-exclamation-circle"></i><?php echo '请选择退款原因';?>'
            },
            refund_amount  : {
                required  : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['refund_pay_refund'];?> <?php echo $output['goods']['goods_pay_price']; ?>',
                number   : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['refund_pay_refund'];?> <?php echo $output['goods']['goods_pay_price']; ?>',
                min   : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['refund_pay_min'];?> 0.01',
	            max   : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['refund_pay_refund'];?> <?php echo $output['goods']['goods_pay_price']; ?>'
            },
            buyer_message  : {
                required   : '<i class="fa fa-exclamation-circle"></i>请填写退款说明'
            },
            refund_pic1: {
                accept : '<i class="fa fa-exclamation-circle"></i>图片必须是jpg/jpeg/gif/png格式'
            },
            refund_pic2: {
                accept : '<i class="fa fa-exclamation-circle"></i>图片必须是jpg/jpeg/gif/png格式'
            },
    ...
    $('#post_form2').validate({
        ...
        messages : {
            reason_id  : {
                required  : '<i class="fa fa-exclamation-circle"></i><?php echo '请选择退货退款原因';?>'
            },
            refund_amount  : {
                required  : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['refund_pay_refund'];?> <?php echo $output['goods']['goods_pay_price']; ?>',
                number   : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['refund_pay_refund'];?> <?php echo $output['goods']['goods_pay_price']; ?>',
            ...
            goods_num  : {
                required  : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['return_add_return'];?> <?php echo $output['goods']['goods_num']; ?>',
                digits   : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['return_add_return'];?> <?php echo $output['goods']['goods_num']; ?>',
                min   : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['return_number_min'];?> 1',
	            max   : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['return_number_max'];?> <?php echo $output['goods']['goods_num']; ?>'
            },
            buyer_message  : {
                required   : '<i class="fa fa-exclamation-circle"></i>请填写退货退款说明'
            },
            refund_pic1: {
                accept : '<i class="fa fa-exclamation-circle"></i>图片必须是jpg/jpeg/gif/png格式'
            },
