...
<div id="lazymodule">
  ...
  <ul class="fd-list">
    ...
    <li im_type="tracerow_<?php echo $v['trace_id']; ?>">
      <div class="fd-aside"><span class="thumb size60"><i></i> <a href="index.php?act=member_snshome&mid=<?php echo $v['trace_memberid'];?>" target="_blank"> <img src="<?php if ($v['trace_memberavatar']!='') { echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.DS.$v['trace_memberavatar']; } else {  echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('default_user_portrait'); } ?>" data-param="{'id':<?php echo $v['trace_memberid'];?>}" imtype="mcard" onload="javascript:DrawImage(this,60,60);"> </a> </span> 
        <!--<p class="arrow"></p>--> 
      </div>
      <dl class="fd-wrap">
        <dt>
          <h3><a href="index.php?act=member_snshome&mid=<?php echo $v['trace_memberid'];?>" target="_blank" data-param="{'id':<?php echo $v['trace_memberid'];?>}" imtype="mcard"><?php echo $v['trace_membername'];?></a><?php echo $lang['im_colon'];?></h3>
          <h5><?php echo parsesmiles($v['trace_title']);?></h5>
          <?php if ($_SESSION['member_id'] == $v['trace_memberid']){?>
          <span class="fd-handle">
          <p class="hover-arrow"><i></i><a href="javascript:void(0);" class="down-sub" im_type="fd_del" data-param='{"txtid":"<?php echo $v['trace_id'];?>"}'><?php echo $lang['im_delete'];?></a> </p>
          </span>
          <?php }?>
        </dt>
        <dd>
