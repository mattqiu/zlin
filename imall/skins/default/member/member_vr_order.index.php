  ...
  <table class="imm-default-table order">
    ...
    <tbody>
      ...
      <tr>
        ...
        &nbsp;<?php if ($order_info['order_from'] == 2){?><i class="fa fa-mobile"></i><?php }?></span><span>下单时间：<?php echo date("Y-m-d H:i",$order_info['add_time']);?></span><span><a href="<?php echo urlShop('show_store','index',array('store_id'=>$order_info['store_id']));?>" title="<?php echo $order_info['store_name'];?>"><?php echo $order_info['store_name'];?></a></span>
        ...
        <!-- QQ -->
          ...
          <a href="javascript:void(0)" class="share-goods" im_type="sharegoods" data-param='{"gid":"<?php echo $order_info['goods_id'];?>"}'><i class="fa fa-share-square-o"></i><?php echo $lang['member_order_snsshare'];?></a>
          <?php } ?>
          </th>
      </tr>
      <tr>
        <td class="bdl"></td>
        <td class="w70"><div class="imm-goods-thumb"><a href="<?php echo urlShop('goods','index',array('goods_id'=>$order_info['goods_id']));?>" target="_blank" onMouseOver="toolTip('<img src=<?php echo thumb($order_info, 240);?>>')" onMouseOut="toolTip()"/><img src="<?php echo thumb($order_info, 60);?>"/></a></div></td>
        ...
        <td> <?php if($order_info['if_refund']){ ?>
          ...
          <?php } ?></td>
        <td class="bdl"><strong><?php echo $order_info['order_amount'];?></strong><p title="<?php echo $lang['member_order_pay_method'].$lang['im_colon'];?><?php echo $order_info['payment_name']; ?>"><?php echo $order_info['payment_name']; ?></p></td>
        ...
        <td class="bdl bdr">
          ...
          <p><a class="imm-btn imm-btn-orange" href="index.php?act=buy_virtual&op=pay&order_id=<?php echo $order_info['order_id']; ?>"><i class="fa fa-shield"></i>订单支付</a></p>
          <?php } ?>

          <!--取消订单-->
          <?php if ($order_info['if_cancel']) {?>
          <p class="mt5"><a href="javascript:void(0)" class="imm-btn imm-btn-red" im_type="dialog" dialog_width="480" dialog_title="取消订单" dialog_id="buyer_order_cancel_order" uri="index.php?act=member_vr_order&op=change_state&state_type=order_cancel&order_id=<?php echo $order_info['order_id'];?>"  id="order<?php echo $order_info['order_id'];?>_action_cancel"><i class="fa fa-ban"></i>取消订单</a></p>
		  <?php } ?>

          <!-- 评价 -->
          <?php if ($order_info['if_evaluation']) { ?>
          <p><a class="imm-btn imm-btn-acidblue" href="index.php?act=member_evaluate&op=add_vr&order_id=<?php echo $order_info['order_id']; ?>"><i class="fa fa-thumbs-o-up"></i><?php echo $lang['member_order_want_evaluate'];?></a></p>
          <?php } ?>
