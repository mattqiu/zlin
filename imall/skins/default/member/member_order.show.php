...
<div class="imm-oredr-show">
  <div class="imm-order-info">
    <div class="imm-order-details">
      <div class="title"><?php echo $lang['member_show_order_info'];?></div>
      <div class="content">
        <dl>
          <dt><?php echo $lang['member_show_order_receiver'].$lang['im_colon'];?></dt>
        ...
        <dl>
          <dt><?php echo $lang['member_show_order_buyer_message'].$lang['im_colon'];?></dt>
          <dd><?php echo $output['order_info']['extend_order_common']['order_message']; ?></dd>
        </dl>
        <dl class="line">
          <dt><?php echo $lang['member_change_order_no'].$lang['im_colon'];?></dt>
          <dd><?php echo $output['order_info']['order_sn']; ?><a href="javascript:void(0);">更多<i class="fa fa-angle-down"></i>
            <div class="more"><span class="arrow"></span>
              <ul>
                <?php if($output['order_info']['payment_name']) { ?>
                <li><?php echo $lang['member_order_pay_method'].$lang['im_colon'];?><span><?php echo $output['order_info']['payment_name']; ?>
                  ...
                  (<?php echo '付款单号'.$lang['im_colon'];?><?php echo $output['order_info']['pay_sn']; ?>)
                  <?php } ?>
                  </span> </li>
                <?php } ?>
                <li><?php echo $lang['member_order_time'].$lang['im_colon'];?><span><?php echo date("Y-m-d H:i:s",$output['order_info']['add_time']); ?></span></li>
                <?php if(intval($output['order_info']['payment_time'])) { ?>
                <li><?php echo $lang['member_show_order_pay_time'].$lang['im_colon'];?><span><?php echo date("Y-m-d H:i:s",$output['order_info']['payment_time']); ?></span></li>
                <?php } ?>
                <?php if($output['order_info']['extend_order_common']['shipping_time']) { ?>
                <li><?php echo $lang['member_show_order_send_time'].$lang['im_colon'];?><span><?php echo date("Y-m-d H:i:s",$output['order_info']['extend_order_common']['shipping_time']); ?></span></li>
                <?php } ?>
                <?php if(intval($output['order_info']['finnshed_time'])) { ?>
                <li><?php echo $lang['member_show_order_finish_time'].$lang['im_colon'];?><span><?php echo date("Y-m-d H:i:s",$output['order_info']['finnshed_time']); ?></span></li>
                <?php } ?>
              </ul>
            </div>
            </a></dd>
        ...
        <dl>
          <dt><?php echo $lang['member_show_order_seller_info'].$lang['im_colon'];?></dt>
          <dd><?php echo $output['order_info']['extend_store']['store_name']; ?><a href="javascript:void(0);">更多<i class="fa fa-angle-down"></i>
            <div class="more"><span class="arrow"></span>
              <ul>
                <li><?php echo $lang['member_address_location'].$lang['im_colon'];?><span><?php echo $output['order_info']['extend_store']['area_info'].'&nbsp;'.$output['order_info']['extend_store']['store_address']; ?></span></li>
    ...
    <div class="imm-order-condition">
      ...
      <ul>
        <li>1. 您尚未对该订单进行支付，请<a href="index.php?act=buy&op=pay&pay_sn=<?php echo $output['order_info']['pay_sn'];?>" class="imm-btn-mini imm-btn-orange"><i></i>支付订单</a>以确保商家及时发货。</li>
        <li>2. 如果您不想购买此订单的商品，请选择<a href="#order-step" class="imm-btn-mini">取消订单</a>操作。</li>
    ...
    <div class="imm-order-condition">
      ...
      <ul>
        ...
        <li>3. 如果您不想购买此订单的商品，请选择<a href="#order-step" class="imm-btn-mini">取消订单</a>操作。</li>
        <?php } else { ?>
        <li>1. 您已使用“<?php echo orderPaymentName($output['order_info']['payment_code']);?>”方式成功对订单进行支付，支付单号 “<?php echo $output['order_info']['pay_sn'];?>”。</li>
        <li>2. 订单已提交商家进行备货发货准备。</li>
        <li>3. 如果您想取消购买，请与商家沟通后对订单进行<a class="imm-btn-mini" href="#order-step">申请退款</a>操作。</li>
        <?php } ?>
      </ul>
    </div>
    <?php } ?>
    ...
    <div class="imm-order-condition">
      ...
      <ul>
        ...
        <li>2. 如果您已收到货，且对商品满意，您可以<a href="#order-step" class="imm-btn-mini imm-btn-green">确认收货</a>完成交易。</li>
        <li>3. 系统将于
    ...
    <div class="imm-order-condition">
      ...
      <ul>
        ...
        <li>2. 将感兴趣的<a href="index.php?act=member_favorites&op=fglist" class="imm-btn-mini">收藏商品</a>加入购物车进行购买。</li>
        <li>3. 看一看<a href="<?php echo urlShop('show_store','index',array('store_id'=>$output['order_info']['store_id']), $output['order_info']['extend_store']['store_domain']);?>" class="imm-btn-mini">该店铺</a>中有什么新商品上架。</li>
      </ul>
    </div>
    <?php } else { ?>
    <div class="imm-order-condition">
      ...
      <ul>
        ...
        <li>3. 交易已完成，你可以对购买的商品及商家的服务进行<a href="#order-step" class="imm-btn-mini imm-btn-acidblue">评价</a>及晒单。</li>
        <?php } ?>
      </ul>
    </div>
    <?php } ?>
    <?php } ?>
    <div class="mall-msg">有疑问可咨询<a href="javascript:void(0);" onclick="ajax_form('mall_consult', '平台客服', '<?php echo urlShop('member_mallconsult', 'add_mallconsult', array('inajax' => 1));?>', 640);"><i class="fa fa-comments"></i>平台客服</a></div>
  </div>
  <?php if ($output['order_info']['order_state'] != ORDER_STATE_CANCEL) { ?>
  <div id="order-step" class="imm-order-step">
  ...
  <?php } ?>
  <div class="imm-order-contnet">
    <table class="imm-default-table order">
      <thead>
        <tr>
          <th class="w10"></th>
      ...
      <tbody>
        ...
        <tr>
          <th colspan="7" style="border-right: none;"> <div class="order-deliver"><span>物流公司： <a target="_blank" href="<?php echo $output['order_info']['express_info']['e_url'];?>"><?php echo $output['order_info']['express_info']['e_name'];?></a></span><span> <?php echo $lang['member_show_order_shipping_no'].$lang['im_colon'];?> <?php echo $output['order_info']['shipping_code'];?> </span><span><a href="javascript:void(0);" id="show_shipping">物流跟踪<i class="fa fa-angle-down"></i>
              <div class="more"><span class="arrow"></span>
                <ul id="shipping_ul">
                  <li>加载中...</li>
                </ul>
          ...
          <th colspan="3" style=" border-left: none;"><?php if(!empty($output['daddress_info'])) { ?>
            <dl class="daddress-info">
              <dt>发&nbsp;&nbsp;货&nbsp;&nbsp;人：</dt>
              <dd><?php echo $output['daddress_info']['seller_name']; ?><a href="javascript:void(0);">更多<i class="fa fa-angle-down"></i>
                <div class="more"><span class="arrow"></span>
                  <ul>
        ...
        <tr class="bd-line">
          <td>&nbsp;</td>
          <td class="w70"><div class="imm-goods-thumb"><a target="_blank" href="<?php echo $goods['goods_url']; ?>"><img src="<?php echo $goods['image_60_url']; ?>" onMouseOver="toolTip('<img src=<?php echo $goods['image_240_url']; ?>>')" onMouseOut="toolTip()" /></a></div></td>
          ...
          <td rowspan="<?php echo $output['order_info']['goods_count'];?>"><?php if ($output['order_info']['if_lock']) { ?>
            ...
            <p><a href="javascript:void(0)" im_type="sharegoods" data-param='{"gid":"<?php echo $output['order_info']['extend_order_goods'][0]['goods_id'];?>"}'>分享商品</a></p>
            <?php } ?></td>
          <?php } ?>
          <!-- E 合并TD --> 
        </tr>
        ...
        <tr class="bd-line">
          <td>&nbsp;</td>
          <td colspan="7" class="tl"><div class="imm-goods-gift">赠品：
              <ul>
      ...
      <tfoot>
        ...
        <tr>
          <th colspan="20"><dl class="imm-store-sales">
        ...
        <tr>
          <td colspan="20"><dl class="freight">
              <dd>
                ...
                <?php echo $lang['member_show_order_tp_fee'].$lang['im_colon'];?><span><?php echo $lang['currency'];?><?php echo $output['order_info']['shipping_fee']; ?></span>
            ...
            <dl class="sum">
              <dt><?php echo $lang['member_order_sum'].$lang['im_colon'];?></dt>
