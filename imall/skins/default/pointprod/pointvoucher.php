...
<link href="<?php echo SHOP_SKINS_URL;?>/css/home_point.css" rel="stylesheet" type="text/css">
<link href="<?php echo SHOP_SKINS_URL;?>/css/home_login.css" rel="stylesheet" type="text/css">
<div class="imcp-container">
  <?php if ($_SESSION['is_login'] == '1'){ ?>
  <div class="imcp-member-top">
    <?php include_once BASE_TPL_PATH.'/pointprod/pointshop.minfo.php'; ?>
  </div>
  <?php } ?>
  <div class="imcp-main-layout">
    <div class="imcp-category">
      <dl>
        <dt>选择分类：</dt>
        <dd>
          <ul>
            <input type="hidden" id="sc_id" value="<?php echo intval($_GET['sc_id'])>0?intval($_GET['sc_id']):'';?>"/>
            <li><a class="<?php echo intval($_GET['sc_id']) <= 0?'selected':'';?>" href="javascript:void(0);" im_type="search_cate" data-param='{"sc_id":""}'>所有分类</a></li>
            <?php foreach ($output['store_class'] as $k=>$v){?>
            <li><a class="<?php echo intval($_GET['sc_id']) == $v['sc_id']?'selected':'';?>" href="javascript:void(0);" im_type="search_cate" data-param='{"sc_id":"<?php echo $v['sc_id'];?>"}'><?php echo $v['sc_name'];?></a></li>
            <?php } ?>
          </ul>
        </dd>
      </dl>
      ...
      <dl class="searchbox">
        ...
        <dd>
          <ul>
            ...
            <?php } else { ?>
            <li im_type="search_orderby" data-param='{"orderval":"default"}'>默认排序</li>
            <?php }?>
            <!-- 默认排序e -->

            <!-- 兑换量s -->
            <?php if ($_GET['orderby'] == 'exchangenumdesc'){//降序选中 ?>
            <li class="selected" im_type="search_orderby" data-param='{"orderval":"exchangenumasc"}'>兑换量<em class="desc"></em></li>
            <?php } elseif ($_GET['orderby'] == 'exchangenumasc') {//升序选中 ?>
            <li class="selected" im_type="search_orderby" data-param='{"orderval":"exchangenumdesc"}'>兑换量<em class="asc"></em></li>
            <?php } else {//未选中?>
            <li im_type="search_orderby" data-param='{"orderval":"exchangenumdesc"}'>兑换量<em class="desc"></em></li>
            <?php } ?>
            <!-- 兑换量e -->

            <!-- 积分值s -->
            <?php if ($_GET['orderby'] == 'pointsdesc'){//降序选中 ?>
            <li class="selected" im_type="search_orderby" data-param='{"orderval":"pointsasc"}'>积分值<em class="desc"></em></li>
            <?php } elseif ($_GET['orderby'] == 'pointsasc') {//升序选中 ?>
            <li class="selected" im_type="search_orderby" data-param='{"orderval":"pointsdesc"}'>积分值<em class="asc"></em></li>
            <?php } else {//未选中?>
            <li im_type="search_orderby" data-param='{"orderval":"pointsdesc"}'>积分值<em class="desc"></em></li>
            <?php } ?>
            <!-- 积分值e -->
            <li>&nbsp;</li>
            <!-- 面额s -->
            ...
            <li>所需积分：
              ...
              <a href="javascript:searchvoucher();" class="imcp-btn">搜索</a> </li>
            <!-- 所需积分e -->
            <?php if($_SESSION['is_login'] == '1'){ ?>
            <li>
    ...
    <ul class="imcp-voucher-list">
      <?php foreach ($output['voucherlist'] as $k=>$v){?>
      <li>
        <div class="imcp-voucher">
          <div class="cut"></div>
          ...
          <div class="button"><a target="_blank" href="###" im_type="exchangebtn" data-param='{"vid":"<?php echo $v['voucher_t_id'];?>"}' class="imcp-btn imcp-btn-red">立即兑换</a></div>
        </div>
      </li>
      <?php }?>
    </ul>
...
<script>
$(function () {
	$("[im_type='search_orderby']").click(function(){
		var data_str = $(this).attr('data-param');
	    eval( "data_str = "+data_str);
	    $("#orderby").val(data_str.orderval);
	    searchvoucher();
	});
	$("[im_type='search_cate']").click(function(){
		var data_str = $(this).attr('data-param');
	    eval( "data_str = "+data_str);
