...
<div class="wrapper">
  <div class="imcs-goods-layout expanded" >
    <div class="imcs-goods-main">
      <div class="imcs-goods-title-bar">
      ...
      <div class="imcs-goods-info-content bd" id="imGoodsRate">
        <div class="top" style="overflow: hidden;">
          ...
          <div class="imcs-cosult-askbtn"><a href="#askQuestion" class="imcs-btn imcs-btn-red">我要提问</a></div>
        </div>
      ...
      <!-- S 咨询表单部分 -->
      <div class="imcs-goods-title-bar" id="askQuestion">
        <h4>发表咨询</h4>
      </div>
        ...
        <div class="imcs-consult-form">
          ...
          <dl>
            ...
            <dd>
              ...
              <label>
                <input type="radio" <?php if ($checked) {?>checked="checked"<?php }?> imtype="ctype<?php echo $val['ct_id'];?>" name="consult_type_id" class="radio" value="<?php echo $val['ct_id'];?>" />
                <?php echo $val['ct_name'];?></label>
              <?php $checked = false;}?>
            </dd>
          </dl>
          <?php $checked = true;foreach ($output['consult_type'] as $val) {?>
          <div class="imcs-consult-type-intro" <?php if (!$checked) {?>style="display:none;"<?php }?> imtype="ctype<?php echo $val['ct_id'];?>"> <?php echo $val['ct_introduce'];?> </div>
          <?php $checked = false;}?>
          <?php }?>
          <div>
            <?php if($_SESSION['member_id']){ ?>
            <label><strong><?php echo $lang['goods_index_member_name'].$lang['im_colon'];?></strong><?php echo $_SESSION['member_name'];?></label>
          ...
          <dl>
            ...
            <dd>
              ...
              <div class="code">
                <div class="arrow"></div>
                <div class="code-img"><a href="javascript:void(0)" onclick="javascript:document.getElementById('codeimage').src='index.php?act=seccode&op=makecode&imhash=<?php echo getIMhash();?>&t=' + Math.random();"><img src="index.php?act=seccode&op=makecode&imhash=<?php echo getIMhash();?>" name="codeimage" border="0" id="codeimage" onclick="this.src='index.php?act=seccode&op=makecode&imhash=<?php echo getIMhash();?>&t=' + Math.random()"/></a></div>
                <a href="JavaScript:void(0);" id="hide" class="close" title="关闭"><i></i></a> <a href="JavaScript:void(0);" onclick="javascript:document.getElementById('codeimage').src='index.php?act=seccode&op=makecode&imhash=<?php echo getIMhash();?>&t=' + Math.random();" class="change" title="<?php echo $lang['goods_index_change_checkcode'];?>"><i></i></a> </div>
              <?php } ?>
              <a href="JavaScript:void(0);" imtype="consult_submit" title="<?php echo $lang['goods_index_publish_consult'];?>" class="imcs-btn imcs-btn-red"><?php echo $lang['goods_index_publish_consult'];?></a></dd>
            <dd imtype="error_msg"></dd>
          </dl>
        </div>
      </form>
      <!-- E 咨询表单部分 -->
      ...
      <div class="imcs-sidebar-container">
        ...
        <div class="content">
          <dl class="imcs-comment-goods">
            ...
            <dd class="goods-price"><?php echo $lang['goods_index_goods_price'].$lang['im_colon'];?><em class="saleP"><?php echo $lang['currency'].$output['goods']['goods_price'];?></em></dd>
            <dd class="goods-raty"><?php echo $lang['goods_index_evaluation'].$lang['im_colon'];?> <span class="raty" data-score="<?php echo $output['goods']['evaluation_good_star'];?>"></span> </dd>
          </dl>
        </div>
        <!--S 店铺信息-->
...
$(function(){
    ...
    <?php if($output['consult_able']) { ?>
    $('a[imtype="consult_submit"]').click(function(){
    	$('#message').submit();
    });

    $('#message').validate({
    	errorPlacement: function(error, element){
    		$('dd[imtype="error_msg"]').append(error);
        },
            		...
            		remote   : {
                        ...
                        complete: function(data) {
                            if(data.responseText == 'false') {
                            	document.getElementById('codeimage').src='index.php?act=seccode&op=makecode&imhash=<?php echo getIMhash();?>&t=' + Math.random();
                            }
                        }
                    }
            	}
    ...
    $('input[type="radio"]').click(function(){
        $('div[imtype^="ctype"]').hide();
        $('div[imtype="' + $(this).attr('imtype') + '"]').show();
    });
