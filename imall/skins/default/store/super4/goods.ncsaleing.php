  ...
  <div class="imcs-cosult-list">
    <dl class="asker">
	  ...
	  <?php if($v['member_id']== '0') echo $lang['im_guest']; else if($v['isanonymous'] == 1){?>
        ...
        <?php }else{?>
        <a href="index.php?act=member_snshome&mid=<?php echo $v['member_id'];?>" target="_blank" data-param="{'id':<?php echo $v['member_id'];?>}" imtype="mcard"><?php echo str_cut($v['cmember_name'],8);?></a>
        <?php }?>        
      ...
      <dl class="ask-con"><dt><?php echo $lang['goods_index_consult_content'];?><?php echo $lang['im_colon'];?></dt>
...
<input name="imhash" type="hidden" value="<?php echo getIMhash();?>" />
  ...
  <div class="imcs-consult-form">
    <div class="asker-info">
      ...
      <label><strong><?php echo $lang['goods_index_member_name'].$lang['im_colon'];?></strong><?php echo $_SESSION['member_name'];?><input type="hidden" name="email" id="gbTextfield" value="<?php echo $_SESSION['member_email']; ?>" /></label>
      ...
      <label for="captcha"><strong><?php echo $lang['goods_index_checkcode'];?></strong>
        ...
        <img src="index.php?act=seccode&op=makecode&imhash=<?php echo getIMhash();?>" name="codeimage" border="0" id="codeimage" onclick="this.src='index.php?act=seccode&op=makecode&imhash=<?php echo $output['imhash'];?>&t=' + Math.random()"/><span><?php echo $lang['goods_index_change_checkcode'];?></span></label>
      <?php } ?>
    </div>
    ...
    <div class="bottom"><a href="JavaScript:void(0);" im_type="consult_submit" title="<?php echo $lang['goods_index_publish_consult'];?>" class="imcs-btn imcs-btn-red"><?php echo $lang['goods_index_publish_consult'];?></a><span id="consultcharcount"></span></div>
  </div>
</form>
<script type="text/javascript">
$(function(){
	$('a[im_type="consult_submit"]').click(function(){
	    if($("#message").valid()){
