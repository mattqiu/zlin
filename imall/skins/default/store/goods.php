    ...
    <div class="ncs-goods-summary">
      ...
      <div class="ncs-meta">
        ...
        <dl>
          ...
          <dd class="cost-price"><strong><?php echo $lang['currency'].imPriceFormat($output['goods']['goods_marketprice']);?></strong></dd>
        </dl>
        <!-- E 商品参考价格 --> 
        <!-- S 商品发布价格 -->
        <dl>
          ...
          <dd class="price">
            ...
            <strong><i><?php echo $lang['currency'];?></i><?php echo imPriceFormat($output['goods']['promotion_price']);?></strong><em>(原售价<?php echo $lang['nc_colon'];?><?php echo $lang['currency'].imPriceFormat($output['goods']['goods_price']);?>)</em>
            <?php } else {?>
            <?php if ($output['member_info']['member_grade'] == 0 ) {?>
		        <strong><?php echo $lang['currency'].imPriceFormat($output['goods']['goods_price']);?></strong>
            <?php } else {?>
            	<!-- S 店铺的云币状态  -->
            	<?php if ($output['store_info']['store_points_way'] == 0 ) {?>
            	<strong><?php echo $lang['currency'].imPriceFormat($output['goods']['vip_price']);?></strong>
            		<em>+<?php echo $output['goods']['vip_points'];?>云币</em>
            	<?php } else {?>
            		<strong><?php echo $lang['currency'].imPriceFormat($output['goods']['goods_price']);?></strong>
            		<em>赠送：<?php echo $output['goods']['vip_points'];?>云币</em>
            	<?php }?>
    		<?php }?>
            <?php }?>
          </dd>
        ...
        <dl class="rate">
          ...
          <p>
	  <img src="<?php echo GetGoodsQRCode($output['goods']['store_id'],$output['goods']['goods_id']);?>"  title="用微信客户端扫描二维码直达商品详情内">
	  <!-- <img src="<?php echo goodsQRCode($output['goods']);?>"  title="用商城手机客户端扫描二维码直达商品详情内容"> -->
	  </p>
          <span class="ncs-goods-code-note"><i></i>微信扫购有惊喜</span> </div>
      </div>
        <!-- E 商品发布价格 -->
        <?php } else {?>
        <!-- S 预定商品 -->
        ...
        <dl>
          ...
          <dd class="price"><strong><?php echo $lang['currency'].imPriceFormat((floatval($output['goods']['book_down_payment']) + floatval($output['goods']['book_final_payment'])));?></strong></dd>
        </dl></div>
        <div class="ncs-book-down">
          <div class="rule-price"><strong>定金</strong><?php echo $lang['currency'].imPriceFormat($output['goods']['book_down_payment']);?></div>
          <div class="rule-symbol">+</div>
          <div class="rule-price"><strong>尾款</strong><?php echo $lang['currency'].imPriceFormat($output['goods']['book_final_payment']);?> </div>
      ...
      <?php if (isset($output['goods']['promotion_type']) || !empty($output['goods']['jjg_explain']) || !empty($output['mansong_info'])) {?>
        <dl>
          ...
          <dd class="promotion-info">
            ...
            <span class="sale-rule w400">直降<em><?php echo $lang['currency'].imPriceFormat($output['goods']['down_price']);?></em>
            ...
            <div class="ncs-mansong"> <span class="sale-name">满即送</span> <span class="sale-rule">
              ...
              <em><?php echo $lang['currency'].imPriceFormat($rule['price']);?></em>
              <?php if(!empty($rule['discount'])) { ?>
              ，<?php echo $lang['nc_reduce'];?><em><?php echo $lang['currency'].imPriceFormat($rule['discount']);?></em>
              <?php } ?>
              ...
              <div class="sale-rule-content" style="display: none;" nctype="rule-content">
                ...
                <div class="content">
                  ...
                  <ul class="mjs-info">
                    ...
                    <li> <span class="sale-rule"><?php echo $lang['nc_man'];?><em><?php echo $lang['currency'].imPriceFormat($rule['price']);?></em>
                      <?php if(!empty($rule['discount'])) { ?>
                      ， <?php echo $lang['nc_reduce'];?><em><?php echo $lang['currency'].imPriceFormat($rule['discount']);?></em>
                      <?php } ?>
                      <?php if(!empty($rule['goods_id'])) { ?>
...
$shownLevelSkus[$sku] = true; ?>
      ...
      <div class="ncs-logistics"><!-- S 物流与运费新布局展示 InIMall网店运维 -->
        ...
        <dl id="ncs-freight" class="ncs-freight">
          ...
          <dd class="ncs-freight_box">
            ...
            <div id="ncs-freight-prompt"> <strong><?php echo $output['goods']['goods_storage'] > 0 ? '有货' : '无货'?></strong>
              ...
              <?php echo $output['goods']['goods_freight'] > 0 ? '运费：'.imPriceFormat($output['goods']['goods_freight']).' 元' : '免运费' ?>
              <?php } ?>
            </div>
          </dd>
        </dl>
      ...
      <div class="ncs-key"> 
        ...
        <dl>
          ...
          <dd>
            <ul>
              <li class="sp-txt">
	      <a href="javascript:void(0)" class="hovered">
	      	<?php if($output['goods']['presell_days'] > 0) {?>
	  	下单<?php echo $output['goods']['presell_days'];?>天后
          	<?php } else {?>
          		<?php echo date('Y-m-d', $output['goods']['presell_deliverdate']);?>&nbsp;日发货<i></i>
          	<?php }?>
	      </a>
	      </li>
            </ul>
  ...
  <div class="ncs-goods-main" id="main-nav-holder">
    ...
    <div class="ncg-salelog">
      ...
      <div class="ncs-goods-info-content bd" id="ncGoodsTraded">
        <div class="top">
          <div class="price"><?php echo $lang['goods_index_goods_price'];?><strong><?php echo imPriceFormat($output['goods']['goods_price']);?></strong><?php echo $lang['goods_index_yuan'];?><span><?php echo $lang['goods_index_price_note'];?></span></div>
        </div>
        <!-- 成交记录内容部分 -->
          ...
          <ol>
            ...
            <li>
              <dl>
                ...
                <dd class="price pngFix"><?php echo imPriceFormat($g['goods_promotion_price']); ?></dd>
              </dl>
            </li>
            <?php } ?>
          </ol>
