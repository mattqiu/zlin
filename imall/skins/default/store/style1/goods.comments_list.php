    ...
    <div class="imcs-goods-main" id="main-nav-holder">
      <div class="imcs-comment">
        ...
        <div class="imcs-goods-info-content bd" id="imGoodsRate">
          ...
          <div id="goodseval" class="imcs-commend-main">
            <?php if(!empty($output['goodsevallist']) && is_array($output['goodsevallist'])){?>
            <?php foreach($output['goodsevallist'] as $k=>$v){?>
            <div id="t" class="imcs-commend-floor">
              <div class="user-avatar"><a <?php if($v['geval_isanonymous'] != 1){?>href="index.php?act=member_snshome&mid=<?php echo $v['geval_frommemberid'];?>" target="_blank" data-param="{'id':<?php echo $v['geval_frommemberid'];?>}" imtype="mcard"<?php }?>><img src="<?php echo getMemberAvatarForID($v['geval_frommemberid']);?>" ></a></div>
              <dl class="detail">
                <dt> <span class="user-name">
                  <?php if($v['geval_isanonymous'] == 1){?>
                  <?php echo str_cut($v['geval_frommembername'],2).'***';?>
                  <?php }else{?>
                  <a href="index.php?act=member_snshome&mid=<?php echo $v['geval_frommemberid'];?>" target="_blank" data-param="{'id':<?php echo $v['geval_frommemberid'];?>}" imtype="mcard"><?php echo $v['geval_frommembername'];?></a>
                  <?php }?>
                  </span>
                ...
                <dd class="explain"><?php echo $lang['im_credit_explain'];?>：<span><?php echo $v['geval_explain'];?></span></dd>
                <?php }?>
                ...
                <li><a imtype="nyroModal"  href="<?php echo snsThumb($value, 1024);?>">
            ...
            <?php }else{?>
            <div class="imcs-norecord"><?php echo $lang['no_record'];?></div>
            <?php }?>
          </div>
        </div>
      </div>
      ...
      <div class="imcs-sidebar-container">
        ...
        <div class="content">
          <dl class="imcs-comment-goods">
            ...
            <dd class="goods-raty"><?php echo $lang['goods_index_evaluation'].$lang['im_colon'];?> <span class="raty" data-score="<?php echo $output['goods_evaluate_info']['star_average'];?>"></span> </dd>
          </dl>
        </div>
        <!--S 店铺信息-->
