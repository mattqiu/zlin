...
<div class="imcs-goods-title-nav" imtype="gbc_nav">
  <ul>
...
<div class="imcs-goods-info-content" imtype="gbc_content">
  <?php if (!empty($output['bundling_array']) && !empty($output['b_goods_array'])) {?>
  <!--S 组合销售 -->
  <div class="imcs-bundling-container">
    <div class="F-center">
      ...
      <div class="imcs-bundling-price">
        <dl>
          <dt><?php echo $val['name'];?></dt>
          <dd class="tcj">原 价：<span><?php echo $lang['currency'].imPriceFormat($val['cost_price']);?></span></dd>
          <dd class="tcj">套装价：<span><?php echo $lang['currency'].$val['price'];?></span></dd>
          <dd class="js"><?php echo $lang['bundling_save'];?><span><?php echo $lang['currency'].imPriceFormat(floatval($val['cost_price'])-floatval($val['price']));?></span></dd>
          <?php if ($val['freight'] > 0) {?>
          <dd class="">运&emsp;费：<span><?php echo $lang['currency'].$val['freight'];?></span></dd>
          <?php }?>
          <dd class="mt5"><a href="javascript:void(0);"  imtype="addblcart_submit" bl_id="<?php echo $val['id']?>" class="imcs-btn imcs-btn-red"><i class="fa fa-th-large"></i><?php echo $lang['bundling_buy'];?></a></dd>
        </dl>
      </div>
      <?php }?>
      <?php }?>
  ...
  <!--E 组合销售 --><script>
            $(function(){
            	$('#imcs-bundling').show();
            	$('.imcs-bundling-container').F_slider({len:<?php echo $i;?>});
  ...
  <?php }?>
  <?php if (!empty($output['gcombo_list'])) {?>
  <div class="imcs-combo-container" <?php if ($current) {?>style="display:none;"<?php }?>>
    <div class="default-goods">
      ...
      <dl>
        ...
        <dd class="goods-price">会员价<?php echo $lang['im_colon'].$lang['currency'].$output['goods_info']['goods_promotion_price'];?></dd>
      </dl>
    </div>
    <div class="combo-goods-list" imtype="combo_list">
      <ul>
        ...
        <li <?php if ($j == 0) {?>class="combo-goods-first"<?php $j++;}?>>
          ...
          <dl>
            ...
            <dd>
              <input type="checkbox" class="checkbox" imtype="comb"  name="<?php echo $combo['goods_id'];?>" data-param="{price:<?php echo $combo['goods_promotion_price'];?>,marketprice:<?php echo $combo['goods_marketprice'];?>}">
    ...
    <dl>
    <dt>推荐购买组合</dt>
    <dd>商城总价：<?php echo $lang['currency'];?><em imtype="gbcc_p"><?php echo $output['goods_info']['goods_promotion_price'];?></em></dd>
    <dd>市场总价：<?php echo $lang['currency'];?><em imtype="gbcc_mp"><?php echo $output['goods_info']['goods_marketprice'];?></em></dd>
    <dd class="mt5"><a class="imcs-btn imcs-btn-red" imtype="addblcart_submit_comb" data-param="<?php echo $comb_ids.$output['goods_info']['goods_id'];?>" href="javascript:void(0);"><i class="fa fa-th-large"></i><?php echo $lang['bundling_buy'];?></a></dd>
    </dl>
  </div>
  ...
  $(function(){
      ...
      $('div[imtype="combo_list"]').find('input[type="checkbox"]').click(function(){
          var gbcc_p = g_p;
          var gbcc_mp = mg_p;
          $('div[imtype="combo_list"]').find('input[type="checkbox"]:checked').each(function(){
              eval( 'data_str =' + $(this).attr('data-param'));
              gbcc_p += data_str.price;
              gbcc_mp += data_str.marketprice;
          });
          $('em[imtype="gbcc_p"]').html(number_format(gbcc_p,2));
          $('em[imtype="gbcc_mp"]').html(number_format(gbcc_mp,2));
      });
      $('a[imtype="addblcart_submit_comb"]').click(function(){
...
function addcombcart(goods_ids)
{
	...
	<?php if ($_SESSION['is_login'] !== '1'){?>
	   login_dialog();
    <?php } else {?>
    $('input[imtype="comb"]').each(function(){
...
<script type="text/javascript">
$(function(){
    $('div[imtype="gbc_nav"]').find('li').click(function(){
        $('div[imtype="gbc_nav"]').find('li').removeClass('current');
        $(this).addClass('current');
        $('div[imtype="gbc_content"]').children().hide().eq($(this).index()).show();
    });
});
</script>
<?php }?>
