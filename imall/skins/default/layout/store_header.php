...
<div class="header-wrap">
  <header class="public-head-layout wrapper">
    <h1 class="site-logo">
      <?php if (!$output['show_own_copyright']){?>
      ...
      <?php }else{?>
      <a href="<?php echo urlShop('show_store','', array('store_id'=>$output['store_info']['store_id']),$output['store_info']['store_domain']);?>" name="top">
        <img src="<?php echo getStoreLogo($output['store_info']['store_label'],'store_logo');?>" />
      </a>
      <?php }?>
    </h1>    
    <div class="head-app">
    ...
    <div class="search-bar">
      <div class="searchBox channel-selectBox-<?php echo $output['channel_style'];?>">
        <div class="selectBox">
          <span class="selectitem" id="selectBoxInput">搜本店</span>
          <div  class="selectList channel-selectList-<?php echo $output['channel_style'];?>">
            <ul>
              <li><a href="javascript:void(0);" <?php if($_GET['act']=='store_search'){?> cur='true'<?php }?> title="请输入您要搜索的商品关键字" act="search">搜本店</a></li>
              <li><a href="javascript:void(0);" <?php if($_GET['act']=='search'){?> cur='true'<?php }?> title="请输入您要搜索的商品关键字" act="store_list">搜全站</a></li>
            </ul>
          </div>
        </div>      
        ...
        <dl class="my-mall">
          ...
          <dd>
            <div class="sub-title">
              ...
              <a href="<?php echo urlShop('member', 'home');?>" class="arrow">我的用户中心<i></i></a>
            </div>
  ...
  <!-- 不启用店铺装修 -->
  <div class="banner">
    <a href="<?php echo urlShop('show_store', 'index', array('store_id'=>$output['store_info']['store_id']));?>" class="img">
    ...
    <?php }else{?>
    <div class="imcs-default-banner"></div>
    <?php }?>
    </a>
  </div>
  <?php } ?>
