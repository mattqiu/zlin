...
<link href="<?php echo SHOP_SKINS_URL?>/css/supplier_center.css" rel="stylesheet" type="text/css">
...
<header class="ncsc-head-layout w">
  <div class="wrapper">
    <div class="ncsc-admin">
      <div class="ncsc-admin-info">
        <div class="admin-avatar"><img src="<?php echo getMemberAvatarForID($_SESSION['member_id']);?>" width="32" class="pngFix" alt=""/></div>
        <div class="fl">
        <div class="admin-permission">当前用户</div>
        <div class="admin-name"><?php echo $_SESSION['supplier_name'];?></div>
      	</div>
      </div>
      ...
      <a href="<?php echo urlShop('supplier_logout', 'logout');;?>" title="安全退出"><i class="icon-signout"></i></a>
      </div>
    </div>
        ...
        <div class="sitemap-menu">
          ...
          <div id="quicklink_list" class="content">
            ...
            <dl <?php echo ($key == 'promotion' ? 'class="double"' : '');?>>
              ...
              <dd <?php if(!empty($output['supplier_quicklink'])) {echo in_array($submenu_value['act'], $output['supplier_quicklink'])?'class="selected"':'';}?>><i nctype="btn_add_quicklink" data-quicklink-act="<?php echo $submenu_value['act'];?>" class="icon-check" title="添加为常用功能菜单"></i><a href="index.php?act=<?php echo $submenu_value['act'];?>&op=<?php echo $submenu_value['op'];?>"> <?php echo $submenu_value['name'];?> </a></dd>
              <?php } ?>
              <?php } ?>
            </dl>
            <?php } ?>
    ...
    <nav class="imsc-nav">
      <dl class="<?php echo $output['current_menu']['model'] == 'index'?'current':'';?>">
        <dt><a href="index.php?act=supplier_center&op=index">首页</a></dt>
        <dd class="arrow"></dd>
      </dl>
      <?php if(!empty($output['menu']) && is_array($output['menu'])) {?>
      <?php foreach($output['menu'] as $key => $menu_value) {?>
...
<?php if(!$output['supplier_layout_no_menu']) { ?>
<div class="ncsc-layout wrapper">
  <div id="layoutLeft" class="ncsc-layout-left">
    <div id="sidebar" class="sidebar">
      ...
      <div class="column-menu">
        <ul id="supplier_center_left_menu">
          <?php if(!empty($output['left_menu']) && is_array($output['left_menu'])) {?>
          <?php foreach($output['left_menu'] as $submenu_value) {?>
          <li <?php echo $_GET['act'] == 'supplier_center'?"id='quicklink_".$submenu_value['act']."'":'';?>class="<?php echo $submenu_value['act'] == $_GET['act']?'current':'';?>"> <a href="index.php?act=<?php echo $submenu_value['act'];?>&op=<?php echo $submenu_value['op'];?>"> <?php echo $submenu_value['name'];?> </a> </li>
          <?php } ?>
          <?php } else { ?>
          <?php if ($_GET['act'] == 'supplier_center') { ?>
  ...
  <div id="layoutRight" class="ncsc-layout-right">
    <div class="ncsc-path"><i class="icon-desktop"></i>供货商管理中心<i class="icon-angle-right"></i><?php echo $output['current_menu']['model_name'];?><i class="icon-angle-right"></i><?php echo $output['current_menu']['name'];?></div>
...
$(document).ready(function(){
    ...
    $('[nctype="btn_add_quicklink"]').on('click', function() {
        ...
        if($quicklink_item.hasClass('selected')) {
            $.post("<?php echo urlShop('supplier_center', 'quicklink_del');?>", { item: item }, function(data) {
            ...
            if(count >= 8) {
                ...
                $.post("<?php echo urlShop('supplier_center', 'quicklink_add');?>", { item: item }, function(data) {
                    ...
                    <?php if ($_GET['act'] == 'supplier_center') { ?>
                        ...
                        $(menu_item).appendTo('#supplier_center_left_menu').hide().fadeIn();
                    <?php } ?>
                }, "json");
            }
        }
...
<div id="tbox">
  <div class="btn" id="msg"><a href="<?php echo urlShop('supplier_msg', 'index');?>"><i class="msg"><?php if ($output['store_msg_num'] > 0) { ?><em><?php echo $output['store_msg_num'];?></em><?php } ?></i>站内消息</a></div>
