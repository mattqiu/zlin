...
<div id="apply_store_info" class="apply-store-info">
  ...
  <div id="apply_company_info" class="apply-company-info">  
    <form id="form_store_info" action="index.php?act=store_joinin&op=step4_p" method="post" >
        ...
        <tr>
          ...
          <tr>
            ...
            <td><select name="sg_id" id="sg_id">
                ...
                <?php $sg_price = (empty($v['sg_price']) || $v['sg_price']==0)?'免费':$v['sg_price'].'元/年';?>
                <?php $explain = '商品数：'.$goods_limit.' 模板数：'.$v['sg_template_number'].' 收费标准：'.$sg_price.' 附加功能：'.$v['function_str'];?>
                <option value="<?php echo $v['sg_id'];?>" data-explain="<?php echo $v['sg_name'];?>"><?php echo $v['sg_name'].'('.$explain.')';?></option>
                <?php } ?>
                <?php } ?>
              </select>
          ...
          <tr>
            <th><i>*</i>申请入驻时长：</th>
            <td>
              <select name="store_joinin_times" id="store_joinin_times">
                <option value="1" selected="selected"> 1 年</option>
          ...
          <tr>
            ...
            <td><select name="sc_id" id="sc_id">
                ...
                <?php $sc_bail = (empty($v['sc_bail']) || $v['sc_bail']==0)?'免收':$v['sc_bail'].'元';?>
                <option value="<?php echo $v['sc_id'];?>" data-explain="<?php echo $v['sc_name'];?>"><?php echo $v['sc_name'].'(保证金：'.$sc_bail.')';?></option>
                <?php } ?>
                <?php } ?>
              </select>
          ...
          <tr>
            <th><i>*</i>经营类目：</th>
            <td>
              <a href="###" id="btn_select_category" class="btn">+选择添加类目</a>
...
$(document).ready(function(){
    ...
    $('#btn_add_category').on('click', function() {
        ...
        if(validation) {
            ...
            tr_category += '<td><a imtype="btn_drop_category" href="javascript:;">删除</a></td>';
    ...
    $('#table_category').on('click', '[imtype="btn_drop_category"]', function() {
    ...
    $('#sg_id').on('change', function() {
        if($(this).val() > 0) {
            $('#sg_name').val($(this).find('option:selected').attr('data-explain'));
        } else {
            $('#sg_name').val('');
        }
    });
    ...
    $('#sc_id').on('change', function() {
        if($(this).val() > 0) {
            $('#sc_name').val($(this).find('option:selected').attr('data-explain'));
        } else {
            $('#sc_name').val('');
        }
    });
