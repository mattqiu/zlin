...
<?php if ($output['is_owner']){ ?>
    ...
    <div class="release-content"><span class="arrow"></span>
      <form id="weiboform" method="post" action="index.php?act=member_snsindex&op=addtrace">
        <textarea name="content" id="content_weibo" im_type="contenttxt" class="textarea"resize="none"></textarea>
        <span class="error"></span>
        <div class="smile"><em></em><a href="javascript:void(0)" im_type="smiliesbtn" data-param='{"txtid":"weibo"}'>表情</a></div>
        <div id="weibocharcount" class="weibocharcount"></div>
        <div id="weiboseccode" class="weiboseccode">
          <label for="captcha" class="ml5 fl"><strong><?php echo $lang['im_checkcode'].$lang['im_colon'];?></strong></label>
          <input name="captcha" class="w40 fl text" type="text" id="captcha" size="4" maxlength="4"/>
          <a href="javascript:void(0)" class="ml5 fl"><img src="" title="<?php echo $lang['wrong_checkcode_change'];?>" name="codeimage" border="0" id="codeimage" onclick="this.src='index.php?act=seccode&op=makecode&imhash=<?php echo $output['imhash'];?>&t=' + Math.random()" /></a>
          <input type="hidden" name="imhash" value="<?php echo $output['imhash'];?>"/>
        </div>
        <div class="handle">
          <div im_type="formprivacydiv" class="privacy-module"><span class="privacybtn" style="width:55px;" im_type="formprivacybtn"><i></i>所有人</span>
            <div class="privacytab" im_type="formprivacytab" style="display:none;">
              <ul class="menu-bd">
                <li im_type="formprivacyoption" data-param='{"v":"0"}'><span class="selected">所有人可见</span></li>
                <li im_type="formprivacyoption" data-param='{"v":"1"}'><span>仅好友可见</span></li>
                <li im_type="formprivacyoption" data-param='{"v":"2"}'><span>仅自己可见</span></li>
              </ul>
            </div>
          </div>
        ...
        <!-- 防止点击Enter键提交 -->
        <input name="<?php echo $lang['im_snsshare'];?>" type="button" class="button" value="<?php echo $lang['im_snsshare'];?>" id="weibobtn" />
      </form>
      <!-- 表情弹出层 -->
	...
	$("#weibobtn").bind('click',function(){			
		if($("#weiboform").valid()){
			...
			if(cookienum >= max_recordnum && $("#weiboseccode").css('display') == 'none'){
				//显示验证码
				$("#weiboseccode").show();
				$("#weiboseccode").find("#codeimage").attr('src','index.php?act=seccode&op=makecode&imhash=<?php echo $output['imhash'];?>&t=' + Math.random());
