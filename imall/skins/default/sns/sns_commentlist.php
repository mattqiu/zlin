...
<div class="comment-widget">
  <div class="comment-edit">
    <form id="commentform_<?php echo $output['tid'];?>" method="post" action="index.php?act=member_snsindex&op=addcomment">
      ...
      <div class="comment-add">
        ...
        <!-- 防止点击Enter键提交 -->
        <div class="act"> <span class="skin-blue"><span class="btn"><a href="javascript:void(0);" im_type="commentbtn" data-param='{"txtid":"<?php echo $output['tid'];?>"}'><?php echo $lang['sns_comment'];?></a></span></span> <span id="commentcharcount<?php echo $output['tid'];?>" style="float:right;"></span> <a class="face" im_type="smiliesbtn" data-param='{"txtid":"comment<?php echo $output['tid'];?>"}' href="javascript:void(0);" ><?php echo $lang['sns_smiles'];?></a> </div>
      </div>
    </form>
  </div>
  <div class="clear"></div>
  ...
  <ul class="comment-list">
    ...
    <li im_type="commentrow_<?php echo $v['comment_id']; ?>"> <a target="_blank" href="index.php?act=member_snshome&mid=<?php echo $v['comment_memberid'];?>"><img width="30" height="30" class="clogo" src="<?php if ($v['comment_memberavatar']!='') { echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.DS.$v['comment_memberavatar']; } else {  echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('default_user_portrait'); } ?>" onload="javascript:DrawImage(this,30,30);"></a>
      <div class="detail">
        <p class="words"><a target="_blank" href="index.php?act=member_snshome&mid=<?php echo $v['comment_memberid'];?>" class="name"><?php echo $v['comment_membername'];?></a><?php echo $lang['im_colon']; ?><?php echo parsesmiles($v['comment_content']);?></p>
        <p class="other">
          <?php if ($_SESSION['member_id'] == $v['comment_memberid']){?>
          <span class="opt"><a href="javascript:void(0);" im_type="comment_del" data-param='{"cid":"<?php echo $v['comment_id'];?>"}'><?php echo $lang['im_delete'];?></a></span>
          <?php }?>
...
$(function(){
	//删除评论
	$("[im_type='comment_del']").live('click',function(){
		var obj = $(this);
	...
	//评论提交
	$("[im_type='commentbtn']").die().live('click',function(){
		...
		if($("#commentform_"+data.txtid).valid()){			
			...
			if(cookienum >= <?php echo $output['max_recordnum'];?> && $("#commentseccode"+data.txtid).css('display')=="none"){
				...
				var imhash = $("#commentseccode"+data.txtid).find("[name='imhash']").val();
				$("#commentseccode"+data.txtid).find("[name='codeimage']").attr('src','index.php?act=seccode&op=makecode&imhash='+imhash+'&t=' + Math.random());
