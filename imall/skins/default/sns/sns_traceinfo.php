...
<ul class="fd-list">
  ...
  <li im_type="tracerow_<?php echo $output['traceinfo']['trace_id']; ?>">
    <dl class="fd-wrap">
      <dt><a href="index.php?act=member_snshome&mid=<?php echo $output['traceinfo']['trace_memberid'];?>" target="_blank"> <img src="<?php if ($output['traceinfo']['trace_memberavatar']!='') { echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.DS.$output['traceinfo']['trace_memberavatar']; } else {  echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('default_user_portrait'); } ?>"> </a>
        <h3><a href="index.php?act=member_snshome&mid=<?php echo $output['traceinfo']['trace_memberid'];?>" target="_blank"><?php echo $output['traceinfo']['trace_membername'];?><?php echo $lang['im_colon'];?></a><?php echo parsesmiles($output['traceinfo']['trace_title']);?></h3>
        <?php if ($_SESSION['member_id'] == $output['traceinfo']['trace_memberid']){?>
        <div class="del-btn fr" title="<?php echo $lang['im_delete'];?>"><a href="javascript:void(0);" im_type="fd_del" data-param='{"txtid":"<?php echo $output['traceinfo']['trace_id'];?>"}'><i></i>&nbsp;</a></div>
        <?php }?>
      </dt>
      <dd>
      ...
      <dd>
        ...
        <span class="fr"><a href="javascript:void(0);" im_type="fd_forwardbtn" data-param='{"txtid":"<?php echo $output['traceinfo']['trace_id'];?>"}'><?php echo $lang['sns_forward'];?></a>&nbsp;|&nbsp;<a href="javascript:void(0);" im_type="fd_commentbtn" data-param='{"txtid":"<?php echo $output['traceinfo']['trace_id'];?>","mid":"<?php echo $output['traceinfo']['trace_memberid'];?>"}'><?php echo $lang['sns_comment'];?><?php echo $output['traceinfo']['trace_commentcount']>0?"({$output['traceinfo']['trace_commentcount']})":'';?></a></span>
      </dd>
      <!-- 评论模块start -->
          ...
          <div class="forward-edit">
            <form id="forwardform_<?php echo $output['traceinfo']['trace_id'];?>" method="post" action="index.php?act=member_snsindex&op=addforward">
              ...
              <div class="forward-add">
                ...
                <!-- 防止点击Enter键提交 -->
                <div class="act"> <span class="skin-blue"><span class="btn"><a href="javascript:void(0);" im_type="forwardbtn" data-param='{"txtid":"<?php echo $output['traceinfo']['trace_id'];?>"}'><?php echo $lang['sns_forward'];?></a></span></span> <span id="forwardcharcount<?php echo $output['traceinfo']['trace_id'];?>" style="float:right;"></span> <a class="face" im_type="smiliesbtn" data-param='{"txtid":"forward<?php echo $output['traceinfo']['trace_id'];?>"}' href="javascript:void(0);" ><?php echo $lang['sns_smiles'];?></a> </div>
              </div>
            </form>
          </div>
