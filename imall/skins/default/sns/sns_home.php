...
<div class="left-content">
  ...
  <div class="tabmenu">
    ...
    </ul><span class="more"><a href="index.php?act=member_snshome&op=shareglist<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><?php echo $lang['im_more'];?></a></span>
  </div>
  <ul class="sns-home-share">
    ...
    <li id="recordone_<?php echo $v['share_id'];?>"><a href="index.php?act=member_snshome&op=goodsinfo&mid=<?php echo $v['share_memberid'];?>&id=<?php echo $v['share_id'];?>" title="<?php echo $v['snsgoods_goodsname']?>" class="pic" style=" background-image:url(<?php echo cthumb($v['snsgoods_goodsimage'],240,$v['snsgoods_storeid']);?>)"> </a>
      ...
      <div class="ops"> <span class="ops-like" id="likestat_<?php echo $v['share_goodsid'];?>"> <a href="javascript:void(0);" im_type="likebtn" data-param='{"gid":"<?php echo $v['share_goodsid'];?>"}' class="<?php echo $v['snsgoods_havelike']==1?'noaction':''; ?>"><i class="<?php echo $v['snsgoods_havelike']==1?'noaction':''; ?> pngFix"></i><?php echo $lang['sns_like'];?></a> <em im_type="likecount_<?php echo $v['share_goodsid'];?>"><?php echo $v['snsgoods_likenum'];?></em> </span> <span class="ops-comment"><a href="index.php?act=member_snshome&op=goodsinfo&mid=<?php echo $v['share_memberid'];?>&id=<?php echo $v['share_id'];?>" title="<?php echo $lang['sns_comment'];?>"><i class="pngFix"></i></a><em><?php echo $v['share_commentcount'];?></em> </span> </div>
    </li>
    <?php }?>
  </ul>
  <?php }?>
  ...
  <div class="tabmenu">
    ...
    </ul><span class="more"><a href="index.php?act=sns_album<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><?php echo $lang['im_more'];?></a></span>
  </div>
  <ul class="sns-home-album" >
    ...
    <li><a href="index.php?act=sns_album&op=album_pic_info&id=<?php echo $v['ap_id'];?>&class_id=<?php echo $v['ac_id']?>&mid=<?php echo $output['master_id'];?><?php if(!empty($_GET['sort'])){?>&sort=<?php echo $_GET['sort']; }?>" title="<?php echo $v['ap_name']?>" style=" background-image:url(<?php echo UPLOAD_SITE_URL.DS.ATTACH_MALBUM.DS.$output['master_id'].DS.str_ireplace('.', '_240.', $v['ap_cover']);?>)"> </a>
      <p><?php echo $lang['sns_upload_time'].$lang['im_colon'].date("Y-m-d",$v['upload_time'])?> </p>
    </li>
    <?php }?>
  </ul>
  <?php }?>
  ...
  <div class="tabmenu">
    ...
    <span class="more"><a href="index.php?act=member_snshome&op=storelist<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><?php echo $lang['im_more'];?></a></span>
  </div>
      ...
      <li id="recordone_<?php echo $v['share_id']; ?>">
        ...
        <div class="shop-content">
          ...
          <div class="detail">
            ...
            <ul im_type="mycarousel" class="jcarousel-skin-tango">
  ...
  <div class="tabmenu">
    ...
    </ul><span class="more"><a href="index.php?act=member_snshome&op=trace<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><?php echo $lang['im_more'];?></a></span>
  </div>
  <ul class="fd-list" id="friendtrace">
    <?php foreach ($output['tracelist'] as $k=>$v){?>
    <li im_type="tracerow_<?php echo $v['trace_id']; ?>">
      <dl class="fd-wrap">
        <dt><img src="<?php echo getMemberAvatarForID($v['trace_memberid']);?>"  data-param="{'id':<?php echo $v['trace_memberid'];?>}" imtype="mcard">
          <h3><a href="index.php?act=member_snshome&mid=<?php echo $v['trace_memberid'];?>" target="_blank" data-param="{'id':<?php echo $v['trace_memberid'];?>}" imtype="mcard"><?php echo $v['trace_membername'];?><?php echo $lang['im_colon'];?></a><?php echo parsesmiles($v['trace_title']);?></h3>
        </dt>
        <dd>
        ...
        <dd>
          ...
          <span class="fr"><a href="javascript:void(0);" im_type="fd_forwardbtn" data-param='{"txtid":"<?php echo $v['trace_id'];?>"}'><?php echo $lang['sns_forward']; ?></a>&nbsp;|&nbsp;<a href="javascript:void(0);" im_type="fd_commentbtn" data-param='{"txtid":"<?php echo $v['trace_id'];?>","mid":"<?php echo $v['trace_memberid'];?>"}'><?php echo $lang['sns_comment']; ?><?php echo $v['trace_commentcount']>0?"({$v['trace_commentcount']})":'';?></a></span>
        </dd>
        <!-- 评论模块start -->
            ...
            <div class="forward-edit">
              <form id="forwardform_<?php echo $v['trace_id'];?>" method="post" action="index.php?act=member_snsindex&op=addforward&type=<?php echo $output['type'];?>&irefresh=1">
                ...
                <div class="forward-add">
                  ...
                  <!-- 防止点击Enter键提交 -->
                  <div class="act"> <span class="skin-blue"><span class="btn"><a href="javascript:void(0);" im_type="forwardbtn" data-param='{"txtid":"<?php echo $v['trace_id'];?>"}'><?php echo $lang['sns_forward'];?></a></span></span> <span id="forwardcharcount<?php echo $v['trace_id'];?>" style="float:right;"></span> <a class="face" im_type="smiliesbtn" data-param='{"txtid":"forward<?php echo $v['trace_id'];?>"}' href="javascript:void(0);" ><?php echo $lang['sns_smiles'];?></a> </div>
                </div>
              </form>
            </div>
...
$(function(){
	//图片轮换
    $('[im_type="mycarousel"]').jcarousel({visible: 4});
  	//删除分享的店铺
	$("[im_type='storedelbtn']").live('click',function(){
		var data_str = $(this).attr('data-param');
        eval( "data_str = "+data_str);
        showDialog('<?php echo $lang['im_common_op_confirm'];?>','confirm', '', function(){
