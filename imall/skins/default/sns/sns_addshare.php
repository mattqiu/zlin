...
<div class="goods-pic"><span class="thumb size120"><i></i><img src="<?php echo cthumb($output['sharegoods_info']['snsgoods_goodsimage'], 240,$output['sharegoods_info']['snsgoods_storeid']);?>"/></span></div>
  <ul id="goods_images">
    <?php for($i=0;$i<5;$i++){?>
    <li im_type="handle_pic" data-param="{'apid':'<?php echo isset($output['pic_list'][$i])?$output['pic_list'][$i]['ap_id']:0;?>'}" >
      ...
      <div class="bg" im_type="handler" style="display:none;"><span class="delete" title="<?php echo $lang['im_delete'];?>" imtype="drop_image"><?php echo $lang['im_delete'];?></span></div>
      <div class="upload-btn"><a href="javascript:void(0);"><span style="width: 66px; height: 28px; position: absolute; left: 0; top: 0; z-index: 999; cursor:pointer; ">
        ...
        <input id="submit_button" style="display:none" type="button" value="<?php echo $lang['im_submit'];?>" onClick="submit_form($(this))">
        </a></div>
    </li>
    <?php }?>
  </ul>
...
$(function(){
	...
	$('span[imtype="drop_image"]').click(function(){
		...
		$.getJSON('index.php?act=member_snshome&op=del_sharepic', {apid:data_str.apid}, function(data){
			if(data.type == 'false'){
				...
				p_li.find('*[im_type="handler"]').hide().css('z-index', '0');
				img_handle();
			}
		});
	});
...
function ajaxFileUpload(id){
	...
	var loading_img = SHOP_SKINS_URL+'/images/loading.gif';
