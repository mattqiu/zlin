...
<div class="side-message">
  ...
  <div class="guest-form">
    <form id="send_form" action='index.php?act=member_message&op=savemsg&type=sns_board' method="post">
      ...
      <div class="action"> <a class="face" href="javascript:void(0);" data-param='{"txtid":"msg"}' im_type="smiliesbtn"><?php echo $lang['sns_smiles'];?></a><a href="javascript:void(0);" imtype="commentbtn" class="btn"><?php echo $lang['sns_speak'];?></a><span class="charcount"><em id="commentcharcount"></em>/140</span></div>
    </form>
  </div>
  <?php }?>
  <div imtype="message_list" class="message-list">
    <?php if(!empty($output['message_list'])){?>
    <?php foreach($output['message_list'] as $val){?>
    <dl imtype="dl<?php echo $val['message_id'];?>">
      <dt><a href="index.php?act=member_snshome&mid=<?php echo $val['from_member_id'];?>" data-param="{'id':<?php echo $val['from_member_id'];?>}" imtype="mcard"><?php echo $val['from_member_name'].$lang['im_colon'];?></a><span><?php echo parsesmiles($val['message_body']);?></span></dt>
      <dd data-param="{'msgid':'<?php echo $val['message_id'];?>','fname':'<?php echo $val['from_member_name'];?>'}"><span class="time"><?php echo $val['message_time'];?></span><span class="handle">
        <?php if($output['relation'] == 3){?>
        <a href="javascript:void(0);" imtype="reply_msg"><?php echo $lang['sns_reply'];?></a>|
        <?php }?>
        <?php if($output['relation'] == 3 || $val['from_member_id'] == $_SESSION['member_id']){?>
        <a  onclick="ajax_get_confirm('<?php echo $lang['im_common_op_confirm'];?>', 'index.php?act=member_message&op=dropcommonmsg&drop_type=sns_msg&message_id=<?php echo $val['message_id'];?>');" href="javascript:void(0);"><?php echo $lang['im_delete'];?></a>
        <?php }?>
        </span> </dd>
      ...
      <dd>
        ...
        <dl class="re-content">
          <dt><a href="index.php?act=member_snshome&mid=<?php echo $v['from_member_id'];?>"><?php echo $v['from_member_name'];?><?php echo $lang['sns_reply'].$lang['im_colon'];?></a><span><?php echo parsesmiles($v['message_body']);?></span></dt>
...
<script >
$(function(){
	$("[im_type='visitmodule']").bind('click',function(){
		var data_str = $(this).attr('data-param');
	    eval( "data_str = "+data_str);
	    $("[im_type='visitmodule']").removeClass('active');
	    $("[im_type='visitmodule']").addClass('normal');
	    $(this).removeClass('normal');
	    $(this).addClass('active');
	    $("[im_type='visitlist']").hide();
	    $("#"+data_str.name).show();
	});

	// 回复提交
	$("[imtype='commentbtn']").live('click',function(){
		if($("#send_form").valid()){
	...
	// 回复
	$('a[imtype="reply_msg"]').live('click', function(){
		...
		if(!p_dd.next().is('dd[nctyoe="replyform"]')){
			$('<dd nctyoe="replyform" class="re-msg"></dd>')
				...
				.append('<a class="face" im_type="smiliesbtn" data-param=\'{"txtid":"msg'+data.msgid+'"}\' href="javascript:void(0);"><?php echo $lang['sns_smiles'];?></a>')
				.append('<a im_type="commentbtn'+data.msgid+'" class="btn" href="javascript:void(0);"><?php echo $lang['im_submit'];?></a>')
			...
			// 回复提交
			$("[im_type='commentbtn"+data.msgid+"']").live('click',function(){
...
function leaveMsgSuccess(data){
	$('<dl></dl>')
		.append('<dt><a href="index.php?act=member_snshome&mid='+data.from_member_id+'">'+data.from_member_name+'<?php echo $lang['im_colon'];?></a><span>'+data.msg_content+'</span></dt>')
		.append('<dd data-param="{\'msgid\':\''+data.msg_id+'\',\'fname\':\''+data.from_member_name+'\'}"><span class="time"><?php echo $lang['sns_just'];?></span><span class="handle"><a href="javascript:void(0);"onclick="ajax_get_confirm(\'<?php echo $lang['im_common_op_confirm'];?>\', \'index.php?act=member_message&op=dropcommonmsg&drop_type=sns_msg&message_id='+data.msg_id+'\');"><?php echo $lang['im_delete'];?></a></span></dd>')
		.prependTo('div[imtype="message_list"]');
	$('div[imtype="message_list"]').children('div').hide();
}

function replyMsgSuccess(data){
	$('dd[nctyoe="replyform"]').remove();
	var to = $('dl[imtype="dl'+data.message_parent_id+'"]').children('dd:last');
	$('<dl class="re-content"></dl>')
		.append('<dt><a href="index.php?act=member_snshome&mid='+data.from_member_id+'">'+data.from_member_name+'<?php echo $lang['sns_reply'].$lang['im_colon'];?></a><span>'+data.msg_content+'</span></dt>')
