...
<ul>
  ...
  <li class="inv_item <?php echo $k == 0 ? 'imcr-selected-item' : null; ?>">
    <input content="<?php echo $val['content'];?>" id="inv_<?php echo $val['inv_id']; ?>" im_type="inv" type="radio" name="inv" value="<?php echo $val['inv_id']; ?>" <?php echo $k == 0 ? 'checked' : null; ?>/>
    <label for="inv_<?php echo $val['inv_id']; ?>">&nbsp;&nbsp;<?php echo $val['content']; ?></label>
    &emsp;&emsp;&emsp;<a href="javascript:void(0);" onclick="delInv(<?php echo $val['inv_id']?>);" class="del">[ <?php echo $lang['im_delete'];?> ]</a> </li>
  <?php } ?>
  <li class="inv_item">
    <?php if (count($output['inv_list']) < 10) {?>
    <input value="0" im_type="inv" id="add_inv" type="radio" name="inv">
  ...
  <div id="add_inv_box" style="display:none">
    <form method="POST" id="inv_form" action="index.php">
      ...
      <div class="imcr-form-default">
        <dl>
          <dt>发票类型<?php echo $lang['im_colon'];?></dt>
          <dd>
            <label>
      ...
      <div id="invoice_panel" class="imcr-form-default">
        <dl>
          <dt>发票抬头<?php echo $lang['im_colon'];?></dt>
          <dd>
        ...
        <dl>
          <dt>发票内容<?php echo $lang['im_colon'];?></dt>
          <dd>
      ...
      <div id="vat_invoice_panel" class="imcr-form-default" style="display:none">
        <dl>
          <dt><i class="required">*</i>单位名称<?php echo $lang['im_colon'];?></dt>
          <dd>
            <input type="text" class="text w200" maxlength="50" name="inv_company" value="">
          </dd>
        </dl>
        <dl>
          <dt><i class="required">*</i>纳税人识别号<?php echo $lang['im_colon'];?></dt>
          <dd>
            <input type="text" class="text w200" maxlength="50" name="inv_code" value="">
          </dd>
        </dl>
        <dl>
          <dt><i class="required">*</i>注册地址<?php echo $lang['im_colon'];?></dt>
          <dd>
            <input type="text" class="text w200" maxlength="50" name="inv_reg_addr" value="">
          </dd>
        </dl>
        <dl>
          <dt><i class="required">*</i>注册电话<?php echo $lang['im_colon'];?></dt>
          <dd>
            <input type="text" class="text w200" maxlength="50" name="inv_reg_phone" value="">
          </dd>
        </dl>
        <dl>
          <dt><i class="required">*</i>开户银行<?php echo $lang['im_colon'];?></dt>
          <dd>
            <input type="text" class="text w200" maxlength="50" name="inv_reg_bname" value="">
          </dd>
        </dl>
        <dl>
          <dt><i class="required">*</i>银行账户<?php echo $lang['im_colon'];?></dt>
          <dd>
        ...
        <dl>
          <dt><i class="required">*</i>发票内容<?php echo $lang['im_colon'];?></dt>
          <dd>明细</dd>
        </dl>
        <dl>
          <dt><i class="required">*</i>收票人姓名<?php echo $lang['im_colon'];?></dt>
          <dd>
            <input type="text" class="text w200" maxlength="50" name="inv_rec_name" value="">
          </dd>
        </dl>
        <dl>
          <dt><i class="required">*</i>收票人手机号<?php echo $lang['im_colon'];?></dt>
          <dd>
            <input type="text" class="text w200" maxlength="50" name="inv_rec_mobphone" value="">
          </dd>
        </dl>
        <dl>
          <dt><i class="required">*</i>收票人省份<?php echo $lang['im_colon'];?></dt>
          <dd id="region">
            <select>
            </select>
        ...
        <dl>
          <dt><i class="required">*</i>送票地址<?php echo $lang['im_colon'];?></dt>
          <dd>
...
<div class="hr16"> <a id="hide_invoice_list" class="imcr-btn imcr-btn-red" href="javascript:void(0);"><?php echo $lang['cart_step1_invoice_submit'];?></a> <a id="cancel_invoice" class="imcr-btn ml10" href="javascript:void(0);">不需要发票</a></div>
<script>
...
$(function(){
    ...
    //使用新的发票信息
    $('input[im_type="inv"]').on('click',function(){
       	regionInit("region");
        if ($(this).val() == '0') {
            $('.inv_item').removeClass('imcr-selected-item');
            $('#add_inv_box').show();
        } else {
            $('.inv_item').removeClass('imcr-selected-item');
            $(this).parent().addClass('imcr-selected-item');
            $('#add_inv_box').hide();
        }
    });
    <?php if (empty($output['inv_list'])) {?>
    //$('input[im_type="inv"]').click();
    <?php } ?>
    //保存发票信息
    ...
    $('#inv_form').validate({
        ...
        messages : {
            inv_company : {
                required : '<i class="fa fa-exclamation-circle"></i>单位名称不能为空'
            },
            inv_code : {
                required : '<i class="fa fa-exclamation-circle"></i>纳税人识别号不能为空'
            },
            inv_reg_addr : {
                required : '<i class="fa fa-exclamation-circle"></i>注册地址不能为空'
            },
			inv_reg_phone : {
				required : '<i class="fa fa-exclamation-circle"></i>注册电话不能为空'
			},
            inv_reg_bname : {
                required : '<i class="fa fa-exclamation-circle"></i>开户银行不能为空'
            },
            inv_reg_baccount : {
                required : '<i class="fa fa-exclamation-circle"></i>银行账户不能为空'
            },
            inv_rec_name : {
                required : '<i class="fa fa-exclamation-circle"></i>收票人姓名不能为空'
            },
            inv_rec_mobphone : {
                required : '<i class="fa fa-exclamation-circle"></i>收票人手机号不能为空'
            },
            area_id : {
                required : '<i class="fa fa-exclamation-circle"></i>请选择地区',
                min : '<i class="fa fa-exclamation-circle"></i>请选择地区',
                checkarea:'<i class="fa fa-exclamation-circle"></i>请选择地区'
            },
            inv_goto_addr : {
                required : '<i class="fa fa-exclamation-circle"></i>送票地址不能为空'
            }
        }
    });
});
