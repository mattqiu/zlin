...
<div class="imcr-main">
  ...
  <form action="index.php?act=payment&op=pd_order" method="POST" id="buy_form">
    ...
    <div class="imcr-receipt-info">
      ...
      <ul class="imcr-payment-list">
        <?php foreach($output['payment_list'] as $val) { ?>
        <li payment_code="<?php echo $val['payment_code']; ?>">
          <label for="pay_<?php echo $val['payment_code']; ?>">
          <i></i>
          <div class="logo" for="pay_<?php echo $val['payment_id']; ?>"> <img src="<?php echo SHOP_SKINS_URL?>/images/payment/<?php echo $val['payment_code']; ?>_logo.gif" /> </div>
          <div class="predeposit" im_type="predeposit" style="display:none">
            <?php if ($val['payment_code'] == 'predeposit') {?>
                <?php if ($output['available_predeposit']) {?>
                <p>当前积分余额<br/>￥<?php echo $output['available_predeposit'];?><br/>不足以支付该订单<br/><a href="<?php echo SHOP_SITE_URL.'/index.php?act=predeposit';?>">马上充值</a></p>
                <?php } else {?>
                <input type="password" class="text w120" name="password" maxlength="40" id="password" value="">
                <p>使用站内积分进行支付时，需输入您的登录密码进行安全验证。</p>
                <?php } ?>
            <?php } ?>
          </div>
          </label>
    ...
    <div class="imcr-bottom tc mb50"><a href="javascript:void(0);" id="next_button" class="imcr-btn imcr-btn-green"><i class="fa fa-shield"></i>确认提交支付</a></div>
  </form>
</div>
<script type="text/javascript">
$(function(){
    $('.imcr-payment-list > li').on('click',function(){
    	$('.imcr-payment-list > li').removeClass('using');
        $(this).addClass('using');
