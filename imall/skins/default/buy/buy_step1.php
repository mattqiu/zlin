...
function calcOrder() {
    var allTotal = 0;
	var allqcbTotal = 0;
    $('em[im_type="eachStoreTotal"]').each(function(){
		...
		if ($('#eachStoreGoodQcbPayTotal_'+store_id).length > 0) {			
			eachQcbTotal += parseFloat($('#eachStoreGoodQcbPayTotal_'+store_id).html());
	    }	
        if ($('#eachStoreFreight_'+store_id).length > 0) {
        	eachTotal += parseFloat($('#eachStoreFreight_'+store_id).html());
			if (eachQcbTotal>0){
			    eachQcbTotal += parseFloat($('#eachStoreFreight_'+store_id).html());
			}
	    }        
        if ($('#eachStoreManSong_'+store_id).length > 0) {
        	eachTotal += parseFloat($('#eachStoreManSong_'+store_id).html());
			if (eachQcbTotal>0){
			    eachQcbTotal += parseFloat($('#eachStoreManSong_'+store_id).html());
			}
	    }
        if ($('#eachStoreVoucher_'+store_id).length > 0) {
        	eachTotal += parseFloat($('#eachStoreVoucher_'+store_id).html());
			if (eachQcbTotal>0){
			    eachQcbTotal += parseFloat($('#eachStoreVoucher_'+store_id).html());
			}
        }
        $(this).html(number_format(eachTotal,2));
		if (eachQcbTotal>0){
			$('#eachStoreQcbPayTotal_'+store_id).html(number_format(eachQcbTotal,2));
	    }
        allTotal += eachTotal;
		allqcbTotal += eachQcbTotal;
    });
    $('#orderTotal').html(number_format(allTotal,2));
	if (allqcbTotal>0){
		$('#orderQcbTotal').html(number_format(allqcbTotal,2));
	}
}
$(function(){
    $.ajaxSetup({
    ...
    $('select[imtype="voucher"]').on('change',function(){
        if ($(this).val() == '') {
    ...
    <?php if (!empty($output['available_pd_amount']) || !empty($output['available_rcb_amount']) || !empty($output['available_qc_amount']) || !empty($output['available_points'])) { ?>
    function showPaySubmit() {
        if ($('input[name="pd_pay"]').attr('checked') || $('input[name="rcb_pay"]').attr('checked') || $('input[name="qcb_pay"]').attr('checked') || $('input[name="jf_pay"]').attr('checked')) {
        	$('#pay-password').val('');
	...
	<?php if (!empty($output['available_qc_amount'])) { ?>
    $('input[name="qcb_pay"]').on('change',function(){
    	showPaySubmit();
    	if ($(this).attr('checked') && !$('input[name="rcb_pay"]').attr('checked') && !$('input[name="pd_pay"]').attr('checked')) {
    		
        	if (<?php echo $output['available_qc_amount']?> >= parseFloat($('#orderQcbTotal').html()) && parseFloat($('#orderQcbTotal').html())>=parseFloat($('#orderTotal').html())) {            	
				$('input[name="rcb_pay"]').attr('checked',false).attr('disabled',true);
				$('input[name="pd_pay"]').attr('checked',false).attr('disabled',true);
				$('input[name="js_pay"]').attr('disabled',false);
        	}
    	} else {
    ...
    $('input[name="js_pay"]').on('change',function(){
    	showPaySubmit();
    	if ($(this).attr('checked') && !$('input[name="points_pay"]').attr('checked')) {
        	if (<?php echo $output['available_points']?> >= parseFloat($('#orderQcbTotal').html())) {            	
				$('input[name="js_pay"]').attr('checked',false).attr('disabled',true);
				$('input[name="pd_pay"]').attr('disabled',false);
        	}
    	} else {
			$('input[name="js_pay"]').attr('disabled',false);
    ...
    $('input[name="rcb_pay"]').on('change',function(){
    	showPaySubmit();
    	if ($(this).attr('checked') && !$('input[name="pd_pay"]').attr('checked') && !$('input[name="qcb_pay"]').attr('checked')) {
        	if (<?php echo $output['available_rcb_amount']?> >= parseFloat($('#orderTotal').html())) {
				$('input[name="qcb_pay"]').attr('checked',false).attr('disabled',true);
            	$('input[name="pd_pay"]').attr('checked',false).attr('disabled',true);
        	}
    	} else {
			$('input[name="qcb_pay"]').attr('disabled',false);
    		$('input[name="pd_pay"]').attr('disabled',false);
    	}
    });
    <?php } ?>
    ...
    $('input[name="pd_pay"]').on('change',function(){
    	showPaySubmit();
    	if ($(this).attr('checked') && !$('input[name="rcb_pay"]').attr('checked') && !$('input[name="qcb_pay"]').attr('checked')) {
...
function disableOtherEdit(showText){
	$('a[im_type="buy_edit"]').each(function(){
...
function ableOtherEdit(){
	$('a[im_type="buy_edit"]').show().next('font').remove();
	ableSubmitOrder();

}
function ableSubmitOrder(){
	$('#submitOrder').on('click',function(){submitNext()}).css('cursor','').addClass('imcr-btn-acidblue');
}
function disableSubmitOrder(){
	$('#submitOrder').unbind('click').css('cursor','not-allowed').removeClass('imcr-btn-acidblue');
}
</script> 