...
<div class="imcr-receipt-info" id="paymentCon">
  <div class="imcr-receipt-info-title">
    <h3>支付方式</h3>
    <?php if (!$output['deny_edit_payment']) {?>
    <a href="javascript:void(0)" im_type="buy_edit" id="edit_payment">[修改]</a>
    <?php }?>
  </div>
  <div class="imcr-candidate-items">
    <ul>
      <li>在线支付</li>
    </ul>
  </div>
  <div id="payment_list" class="imcr-candidate-items" style="display:none">
    <ul>
      <li>
      ...
      <li>
        ...
        <a id="show_goods_list" style="display: none" class="imcr-payment-showgoods" href="javascript:void(0);">
          <i class="fa fa-truck"></i>货到付款 (<span data-cod-nums="offline"><?php echo count($output['pay_goods_list']['offline']);?></span>种商品) + <i class="fa fa-credit-card"></i>在线支付 (<span data-cod-nums="online"><?php echo count($output['pay_goods_list']['online']);?></span>种商品)
        </a>
      </li>
    </ul>
    <div class="hr16"> <a href="javascript:void(0);" class="imcr-btn imcr-btn-red" id="hide_payment_list">保存支付方式</a></div>
  </div>
  <div id="imcr-payment-showgoods-list" class="imcr-payment-showgoods-list">
    <dl>
...
<div id="confirm_offpay_goods_list" style="display: none;">
  <dl class="imcr-offpay-list" data-hideshow="offline">
  ...
  <dl class="imcr-offpay-list" data-hideshow="online">
  ...
  <div class="tc mt10 mb10"><a href="javascript:void(0);" class="imcr-btn imcr-btn-orange" id="close_confirm_button">确认支付方式</a></div>
</div>
...
var setCodGoodsShow = function() {
    ...
    $('#hide_payment_list').on('click',function(){
        ...
        if (payment_type == 'offline'){
            ...
            if (hybrid) {
                ...
                $('#edit_payment').parent().after('<div class="imcr-candidate-items"><ul><li>您选择货到付款 + 在线支付完成此订单<br/><a href="javsacript:void(0);" id="show_goods_list" class="imcr-payment-showgoods">'+content+'</a></li></ul></div>');
                $('#show_goods_list').hover(function(){showPayGoodsList(this)},function(){$('#imcr-payment-showgoods-list').fadeOut()});
            } else {
                $('#edit_payment').parent().after('<div class="imcr-candidate-items"><ul><li>'+content+'</li></ul></div>');
    ...
    function showPayGoodsList(item){
		...
		$("#imcr-payment-showgoods-list").css({'left' : pos_x, 'top' : pos_y,'position' : 'absolute','display' : 'block'});
        $('#imcr-payment-showgoods-list').addClass('imcr-payment-showgoods-list').fadeIn();
    }
