 ...
 <div class="imcr-title">
  ...
  <input type="hidden" name="quantity" value="<?php echo $output['goods_info']['quantity'];?>">
    <div class="imcr-receipt-info">
      ...
      <div id="invoice_list" class="imcr-candidate-items">
        ...
        <p><i class="fa fa-info-circle"></i>您本次购买的商品不需要收货地址，请正确输入接收手机号码，确保及时获得“电子兑换码”。可使用您已经绑定的手机或重新输入其它手机号码。</p>
      </div>
    </div><div class="imcr-receipt-info"><div class="imcr-receipt-info-title">
    ...
    <table class="imcr-table-style" im_type="table_cart">
      ...
      <tbody>
        ...
        <tr class="shop-list">
          <td class="w10"></td>
          <td class="w60"><a href="<?php echo urlShop('goods','index',array('goods_id'=>$output['goods_info']['goods_id']));?>" target="_blank" class="imcr-goods-thumb"><img src="<?php echo thumb($output['goods_info'],60);?>" alt="<?php echo $output['goods_info']['goods_name']; ?>" /></a></td>
          <td class="tl"><dl class="imcr-goods-info">
		...
		<tr>
			...
			<td class="tl" colspan="2">买家留言：
				<textarea name="buyer_msg" class="imcr-msg-textarea" maxlength="150" placeholder="选填：对本次交易的说明（建议填写已经和商家达成一致的说明）" title="选填：对本次交易的说明（建议填写已经和商家达成一致的说明）"></textarea></td>
     ...
     <tr id="pd_panel">
        <td class="pd-account" colspan="20"><div class="imcr-pd-account">
        ...
        <?php if ($output['member_info']['available_predeposit'] > 0) { ?>
            <div class="mt5 mb5"><label><input type="checkbox" class="vm mr5" value="1" name="pd_pay">使用积分（可用金额：<em><?php echo $output['member_info']['available_predeposit'];?></em>元）</label></div>
        <?php } ?>
      <?php if ($output['member_info']['available_predeposit'] > 0 && $output['member_info']['available_rc_balance'] > 0) { ?>
     ...
     <?php if (!empty($output['store_voucher_list']) && is_array($output['store_voucher_list'])) {?>
      <tr>
        <td class="tr" colspan="20"><div class="imcr-store-account">
            <dl class="voucher">
              <dt>
                <select imtype="voucher" name="voucher">
    ...
    <!-- E voucher list -->
      ...
      <tfoot>
        <tr>
          <td colspan="20"><div class="imcr-all-account">商品总价<em id="orderTotal"><?php echo $output['goods_info']['goods_total']; ?></em>元</div></td>
        </tr>
      </tfoot>
    </table>

</div></form>
  <div class="imcr-bottom"><a id="submitOrder" href="javascript:void(0)" class="imcr-btn imcr-btn-acidblue fr">提交订单</a></div>
...
$(document).ready(function(){
    ...
    $('#submitOrder').on('click',function(){
    	...
    	if (($('input[name="pd_pay"]').attr('checked') || $('input[name="rcb_pay"]').attr('checked')) && $('#password_callback').val() != '1') {
    		showDialog('使用充值卡/积分支付，需输入支付密码并使用  ', 'error','','','','','','','','',2);
    		return;
    	}
    	SUBMIT_FORM = false;
    	$('#form_buy').submit();
