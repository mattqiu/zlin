  ...
  <form action="<?php echo urlShop('show_live_groupbuy','livegroupbuystep2');?>" method="POST" id="apply_form" name="apply_form">
    ...
    <table class="imcr-table-style" im_type="table_cart">
      ...
      <tbody>
        ...
        <tr>
          <td class="w10"></td>
          <td class="w60"><a class="imcr-goods-thumb" target="_blank" href="<?php echo urlShop('show_live_groupbuy','groupbuy_detail',array('groupbuy_id'=>$output['live_groupbuy']['groupbuy_id']));?>"> <img alt="<?php echo $output['live_groupbuy']['groupbuy_name'];?>" src="<?php echo lgthumb($output['live_groupbuy']['groupbuy_pic'],'small');?>"> </a></td>
          ...
          <td><span id="deal-buy-total"><?php echo imPriceFormat($output['q_number']*$output['live_groupbuy']['groupbuy_price']);?></span></td>
        </tr>
          ...
          <td class="pd-account" colspan="20"><div class="imcr-pd-account">
              <div class="mt5 mb5">
                <label>
                  ...
                  使用积分支付（当前可用余额：<em>￥<?php echo $output['member']['available_predeposit'];?></em>）</label>
              </div>
      ...
      <tfoot>
        <tr>
          <td colspan="20"><div class="imcr-all-account">订单总额：￥<em id="payment"><?php echo imPriceFormat($output['q_number']*$output['live_groupbuy']['groupbuy_price']);?></em>元</div></td>
        </tr>
      </tfoot>
    </table>
    ...
    <div class="imcr-bottom"> <a href="javascript:void(0)" id='submitOrder' class="imcr-btn imcr-btn-acidblue fr"><?php echo $lang['cart_index_submit_order'];?></a> </div>
  </form>
</div>
...
$(function(){
	...
	function submitNext(){
		if ($('input[name="pd_pay"]').attr('checked') && $('#password_callback').val() != '1') {
			showDialog('使用积分支付，需输入支付密码并使用  ', 'error','','','','','','','','',2);
			return;
		}
		$('#apply_form').submit();
	}
