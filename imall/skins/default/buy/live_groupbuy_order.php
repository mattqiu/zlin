    ...
    <table class="imcr-table-style" im_type="table_cart">
      ...
      <tbody>
        ...
        <tr class="shop-list">
          <td class="w10"></td>
          <td class="w60">
			<a class="imcr-goods-thumb" target="_blank" href="<?php echo urlShop('show_live_groupbuy','groupbuy_detail',array('groupbuy_id'=>$output['live_groupbuy']['groupbuy_id']));?>"> <img alt="<?php echo $output['live_groupbuy']['groupbuy_name'];?>" src="<?php echo lgthumb($output['live_groupbuy']['groupbuy_pic'],'small');?>"> </a>
		  </td>
          <td class="tl">
      ...
      <tfoot>
        <tr>
          <td colspan="20"><div class="imcr-all-account">总额（购买数量不能超过<?php echo $output['live_groupbuy']['buyer_limit'];?>券）￥<em id="payment"><?php echo $output['live_groupbuy']['groupbuy_price'];?></em>元</div></td>
        </tr>
      </tfoot>
    </table>
    <input type="hidden" name="groupbuy_id" value="<?php echo $output['live_groupbuy']['groupbuy_id'];?>">
    <input type="hidden" name="form_submit" value="ok" />
    <div class="imcr-bottom"><a id="groupbuy_submit" href="javascript:void(0)" class="imcr-btn imcr-btn-acidblue fr"><i class="fa fa-pencil"></i><?php echo $lang['cart_index_input_next'].$lang['cart_index_ensure_info'];?></a></div>
  </form>
</div>
<script type="text/javascript">
	$(function(){
		$('#add').click(function(){
			...
			if(isNaN(num)){
				alert('<?php echo $lang['im_groupbuy_order_fillthenumbers'];?>');
				return false;
			}
		...
		$('#reduce').click(function(){
			...
			var total_num = eval(num-1);
			if(total_num<1){
				alert('<?php echo $lang['im_groupbuy_order_more_than_zero'];?>');
				return false;				
			}
		...
		$('#groupbuy_submit').click(function(){
			...
			if(is_login!=1){
				alert('<?php echo $lang['im_groupbuy_order_loginfirst'];?>');
				return false;
			}
			...
			if(quantity>limit){
				alert('<?php echo $lang['im_groupbuy_order_cannotover'];?>');
				return false;
			}
