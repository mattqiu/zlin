...
<ul>
  ...
  <li class="receive_add address_item <?php echo $k == 0 ? 'imcr-selected-item' : null; ?>">
    <input address="<?php echo intval($val['dlyp_id']) ? '[自提服务站] ' : '';?><?php echo $val['area_info'].'&nbsp;'.$val['address']; ?>" true_name="<?php echo $val['true_name'];?>" id="addr_<?php echo $val['address_id']; ?>" im_type="addr" type="radio" class="radio" city_id="<?php echo $val['city_id']?>" area_id=<?php echo $val['area_id'];?> name="addr" value="<?php echo $val['address_id']; ?>" phone="<?php echo $val['mob_phone'] ? $val['mob_phone'] : $val['tel_phone'];?>" <?php echo $val['is_default'] == '1' ? 'checked' : null; ?> />
    <label for="addr_<?php echo $val['address_id']; ?>"><span class="true-name"><?php echo $val['true_name'];?></span><span class="address"><?php echo intval($val['dlyp_id']) ? '[自提服务站] ' : '';?><?php echo $val['area_info']; ?>&nbsp;<?php echo $val['address']; ?></span><span class="phone"><i class="fa fa-mobile"></i><?php echo $val['mob_phone'] ? $val['mob_phone'] : $val['tel_phone'];?></span></label>
    <a href="javascript:void(0);" onclick="delAddr(<?php echo $val['address_id']?>);" class="del">[ 删除 ]</a> 
  </li>
  <?php } ?>
  <li class="receive_add addr_item">
    <input value="0" im_type="addr" id="add_addr" type="radio" name="addr">
...
<div class="hr16"> <a id="hide_addr_list" class="imcr-btn imcr-btn-red" href="javascript:void(0);"><?php echo $lang['cart_step1_addnewaddress_submit'];?></a></div>
...
$(function(){
    ...
    $('input[im_type="addr"]').on('click',function(){
        if ($(this).val() == '0') {
            $('.address_item').removeClass('imcr-selected-item');
            $('#add_addr_box').load(SITEURL+'/index.php?act=buy&op=add_addr');
        } else {
            $('.address_item').removeClass('imcr-selected-item');
    ...
    $('#hide_addr_list').on('click',function(){
        if ($('input[im_type="addr"]:checked').val() == '0'){
            submitAddAddr();
        } else {
            if ($('input[im_type="addr"]:checked').size() == 0) {
                return false;
            }
