...
<table class="ncsc-default-table">
  <thead>
    <tr class="sortbar-array">
      ...
      <th>商品名称</th>
      <th>价格</th>
      <th class="align-center"><a title="点击进行排序" im_type="orderitem" data-param='{"orderby":"ordergoodsnum"}' class="<?php echo (!$output['orderby'] || $output['orderby']=='ordergoodsnum desc')?'selected desc':''; echo $output['orderby']=='ordergoodsnum asc'?'selected asc':''; ?>">近30天下单商品数<i></i></a></th>
      <th class="align-center"><a title="点击进行排序" im_type="orderitem" data-param='{"orderby":"ordergamount"}' class="<?php echo $output['orderby']=='ordergamount desc'?'selected desc':''; echo $output['orderby']=='ordergamount asc'?'selected asc':''; ?>">近30天下单金额<i></i></a></th>
  ...
  <tbody>
    ...
    <tr class="bd-line">
      ...
      <td><?php echo imPriceFormat($v['goods_price']);?></td>
      <td><?php echo $v['ordergoodsnum'];?></td>
      <td><?php echo $lang['currency'].$v['ordergamount'];?></td>
      <td><a href="javascript:void(0);" im_type='showdata' data-param='{"gid":"<?php echo $v['goods_id'];?>"}'>走势图</a></td>
    </tr>
    <?php }?>
    <?php } else { ?>
    <tr>
...
$(function(){
    ...
    <?php }?>
    $("[im_type='showdata']").click(function(){
    	var data_str = $(this).attr('data-param');
		eval('data_str = '+data_str);
		getStatdata(data_str.gid);
    });
    //排序
    $("[im_type='orderitem']").click(function(){
    	var data_str = $(this).attr('data-param');
	    eval( "data_str = "+data_str);
...
function getStatdata(gid){
	$('#goodsinfo_div').load('index.php?act=supplier_statistics_goods&op=goodsinfo&gid='+gid);
}
</script>