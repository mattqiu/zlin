...
<?php if ($notOwnShop = !checkPlatformStore()) { ?>
  <a href="javascript:void(0);" class="ncbtn ncbtn-mint" im_type="dialog" dialog_title="申请新的经营类目" dialog_id="my_goods_brand_apply" dialog_width="480" uri="index.php?act=supplier_info&op=bind_class_add">申请新的经营类目</a>
<?php } ?>
  </div>
...
<?php if ($notOwnShop) { ?>
      ...
      <?php if ($val['state'] == '0') {?>
      <span><a href="javascript:void(0)" class="btn-grapefruit" onclick="ajax_get_confirm('<?php echo $lang['im_ensure_del'];?>', 'index.php?act=supplier_info&op=bind_class_del&bid=<?php echo $val['bid']; ?>');"><i class="icon-trash"></i><p><?php echo $lang['im_del'];?></p></a></span>
     <?php } ?>
      </td>
<?php } ?>
    </tr>
