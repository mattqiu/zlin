...
<div class="ncsc-form-default">
  <form method="post" action="index.php?act=supplier_info&op=bind_class_save" target="_parent" name="supplier_bind_class_form" id="supplier_bind_class_form">
    <input type="hidden" name="form_submit" value="ok" />
    <input id="goods_class" name="goods_class" type="hidden" value="">
    <dl>
      <dt>选择分类<?php echo $lang['im_colon'];?></dt>
      <dd id="gcategory">
...
<label id="error_message" style="display: none" class="error" for="sn_title"><i class="icon-exclamation-sign"></i>请选择分类</label>
    ...
    <div class="bottom">
      <label class="submit-border"><input type="button" id="btn_add_bind_class" class="submit" value="<?php echo $lang['im_submit'];?>" /></label>
    </div>
  </form>
</div>
...
$(document).ready(function(){
    ...
    $('#btn_add_bind_class').on('click', function() {
        ...
        showDialog('您所选的分类为：' + category_name + ' ， 确认申请吗？', 'confirm', '', function(){
        	ajaxpost('supplier_bind_class_form', '', '', 'onerror')
        });
    });
});
</script> 
