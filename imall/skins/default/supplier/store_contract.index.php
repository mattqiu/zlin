                        ...
                        <div style="height: 35px;">
                            <em style="float:left; margin-bottom: 12px;font-size: 16px;font-weight: 700;"><?php echo $v['cti_name']; ?></em>
                                ...
                                <?php if($v['ct_auditstate_key'] == 'auditpass' && $v['ct_closestate_key'] == 'open' && $v['ct_joinstate_key'] == 'applying'){ //申请通过，待付款?>
                                    <span style="float:right;">审核通过，<a href="index.php?act=supplier_contract&op=applypay&itemid=<?php echo $v['cti_id']; ?>">点击支付保证金</a></span>
                                <?php } ?>
                                <?php if($v['ct_auditstate_key'] == 'auditfailure' && $v['ct_closestate_key'] == 'open'){//申请未通过?>
                                    <span style="float:right;"><?php echo $v['ct_auditstate_text']; ?>，<a href="index.php?act=supplier_contract&op=contractlog&itemid=<?php echo $v['cti_id']; ?>">点击查看详情</a></span>
                                <?php } ?>
                                ...
                                <?php if($v['ct_auditstate_key'] == 'costfailure' && $v['ct_closestate_key'] == 'open'){ //保证金审核失败?>
                                    <span style="float:right;"><?php echo $v['ct_auditstate_text']; ?>，<a href="index.php?act=supplier_contract&op=applypay&itemid=<?php echo $v['cti_id']; ?>">重新提交</a></span>
                                <?php } ?>
                                ...
                                <?php if($v['ct_state_sign'] == 'added' && $v['ct_quitstate_key'] == 'applyfailure' && $v['ct_closestate_key'] == 'open'){ //显示退出申请审核失败 ?>
                                    <span style="float:right;"><?php echo $v['ct_quitstate_text']; ?>，<a href="index.php?act=supplier_contract&op=contractlog&itemid=<?php echo $v['cti_id']; ?>">点击查看详情</a></span>
                                <?php } ?>
                        </div>
                        <div style="text-align: left;">
                            <?php if($v['ct_state_sign'] == 'notapply' || !$v['ct_state_sign']){//未申请服务 ?>
                            <a title="加入" class="ncbtn ncbtn-mint" im_type="applybtn" data-param='{"itemid":"<?php echo $v['cti_id']; ?>"}'>加入</a>
                            <?php } ?>
                            ...
                            <a href="index.php?act=supplier_contract&op=contractlog&itemid=<?php echo $v['cti_id']; ?>" style="margin-left: 30px;">查看服务详情</a>
                            <?php } ?>
                            <?php if($v['ct_state_sign'] == 'added' && in_array($v['ct_quitstate_key'],array('notapply','applyfailure'))){ //已加入服务，并且未申请退出 ?>
                            |&nbsp;<a im_type="quitbtn" href="javascript:void(0);" data-param='{"itemid":"<?php echo $v['cti_id']; ?>"}'>退出</a>
                            <?php } ?>
                        </div>
                    </div>
                </div>
...
$(document).ready(function(){
    $("[im_type='applybtn']").click(function(){
        var data_str = $(this).attr('data-param');
        if(data_str){
            eval( "data_str = "+data_str);
            var itemid = parseInt(data_str.itemid);
            ajaxget('index.php?act=supplier_contract&op=ctiapply&itemid='+itemid);
        }
    });
    $("[im_type='quitbtn']").click(function(){
        var data_str = $(this).attr('data-param');
        if(data_str){
            eval( "data_str = "+data_str);
            var itemid = parseInt(data_str.itemid);
            ajaxget('index.php?act=supplier_contract&op=ctiquit&itemid='+itemid);
        }
    });
});
</script> 
