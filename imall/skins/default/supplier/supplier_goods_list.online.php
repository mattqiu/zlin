...
<div class="tabmenu">
  <?php include template('layout/submenu');?>
    <a title="批量生成商品二维码" class="ncbtn ncbtn-aqua" href="index.php?act=supplier_goods_online&amp;op=maker_qrcode" style="right:100px" onclick="return confirm('您确定要执行批量生成二维码吗？');">批量生成商品二维码</a>
  <a href="<?php echo urlShop('supplier_goods_add');?>" class="ncbtn ncbtn-mint" title="<?php echo $lang['store_goods_index_add_goods'];?>"> <?php echo $lang['store_goods_index_add_goods'];?></a> </div>
<form method="get" action="index.php">
  <table class="search-form">
    <input type="hidden" name="act" value="supplier_goods_online" />
...
<table class="ncsc-default-table">
  <thead>
    ...
    <tr>
      ...
      <td colspan="20"><label for="all" ><?php echo $lang['im_select_all'];?></label>
        <a href="javascript:void(0);" class="ncbtn-mini" im_type="batchbutton" uri="<?php echo urlShop('supplier_goods_online', 'drop_goods');?>" name="commonid" confirm="<?php echo $lang['im_ensure_del'];?>"><i class="icon-trash"></i><?php echo $lang['im_del'];?></a> 
        <a href="javascript:void(0);" class="ncbtn-mini" im_type="batchbutton" uri="<?php echo urlShop('supplier_goods_online', 'goods_unshow');?>" name="commonid"><i class="icon-level-down"></i><?php echo $lang['store_goods_index_unshow'];?></a> 
        <a href="javascript:void(0);" class="ncbtn-mini" nctype="batch" data-param="{url:'<?php echo urlShop('supplier_goods_online', 'edit_jingle');?>', sign:'jingle'}"><i></i>设置广告词</a> 
        <a href="javascript:void(0);" class="ncbtn-mini" nctype="batch" data-param="{url:'<?php echo urlShop('supplier_goods_online', 'edit_plate');?>', sign:'plate'}"><i></i>设置关联版式</a>
      </td>
    </tr>
    <?php } ?>
  </thead>
    ...
    <tr>
      ...
      <td class="tl"><dl class="goods-name">
          ...
          <dd class="serve"> <span class="<?php if ($val['goods_commend'] == 1) { echo 'open';}?>" title="店铺推荐商品"><i class="commend">荐</i></span> <span class="<?php if ($val['mobile_body'] != '') { echo 'open';}?>" title="手机端商品详情"><i class="icon-tablet"></i></span> <span class="" title="商品页面二维码"><i class="icon-qrcode"></i>
            ...
            <?php if ($val['is_fcode'] ==1) {?>
            <span><a class="ncbtn-mini ncbtn-grapefruit" href="<?php echo urlShop('supplier_goods_online', 'download_f_code_excel', array('commonid' => $val['goods_commonid']));?>">下载F码</a></span>
            <?php }?>
          </dd>
        </dl></td>
      <td><span><?php echo $lang['currency'].imPriceFormat($val['goods_price']); ?></span></td>
      ...
      <td class="nscs-table-handle"><?php if ($val['goods_lock'] == 0) {?>
        <span><a href="<?php echo urlShop('supplier_goods_online', 'edit_goods', array('commonid' => $val['goods_commonid']));?>" class="btn-bluejeans"><i class="icon-edit"></i>
        <p><?php echo $lang['im_edit'];?></p>
        </a></span> <span><a href="javascript:void(0);" onclick="ajax_get_confirm('<?php echo $lang['im_ensure_del'];?>', '<?php echo urlShop('supplier_goods_online', 'drop_goods', array('commonid' => $val['goods_commonid']));?>');" class="btn-grapefruit"><i class="icon-trash"></i>
        <p><?php echo $lang['im_del'];?></p>
        </a></span>
        <?php } else {?>
        <span class="tip" title="该商品参加团购活动期间不能进行编辑及删除等操作,但可以编辑赠品"><a href="<?php if ($val['is_virtual'] ==1 ) {echo 'javascript:void(0);';} else {echo urlShop('supplier_goods_online', 'add_gift', array('commonid' => $val['goods_commonid']));}?>" class="btn-darkgray"><i class="icon-lock"></i>
        <p>锁定</p>
        </a></span>
        <?php }?></td>
    </tr>
  ...
  <tfoot>
    ...
    <tr>
      ...
      <th colspan="10"><label for="all2"><?php echo $lang['im_select_all'];?></label>
        <a href="javascript:void(0);" im_type="batchbutton" uri="<?php echo urlShop('supplier_goods_online', 'drop_goods');?>" name="commonid" confirm="<?php echo $lang['im_ensure_del'];?>" class="ncbtn-mini"><i class="icon-trash"></i><?php echo $lang['im_del'];?></a> 
        <a href="javascript:void(0);" im_type="batchbutton" uri="<?php echo urlShop('supplier_goods_online', 'goods_unshow');?>" name="commonid" class="ncbtn-mini"><i class="icon-level-down"></i><?php echo $lang['store_goods_index_unshow'];?></a> 
        <a href="javascript:void(0);" class="ncbtn-mini" nctype="batch" data-param="{url:'<?php echo urlShop('supplier_goods_online', 'edit_jingle');?>', sign:'jingle'}"><i></i>设置广告词</a> 
        <a href="javascript:void(0);" class="ncbtn-mini" nctype="batch" data-param="{url:'<?php echo urlShop('supplier_goods_online', 'edit_plate');?>', sign:'plate'}"><i></i>设置关联版式</a> 
      </th>
    </tr>
...
<script src="<?php echo SHOP_RESOURCE_SITE_URL;?>/js/supplier_goods_list.js"></script> 
<script>
$(function(){
    //Ajax提示
    $('.tip').poshytip({
