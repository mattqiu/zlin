      ...
      <tr>
        ...
        <td><?php if(is_array($output['pic_list']) && !empty($output['pic_list'])){?>
          ...
          <dd id="batchClass" style=" display:none;">
            ...
            <?php }else{?>
            <span><?php echo $lang['album_plist_move_album_only_one'];?><a href="JavaScript:void(0);" uri="index.php?act=supplier_album&op=album_add" im_type="dialog" dialog_title="<?php echo $lang['album_class_add'];?>"><?php echo $lang['album_class_add'];?></a><?php echo $lang['album_plist_move_album_only_two'];?></span>
            <?php }?>
          </dd>
          </dl>
          <?php }?></td>
        ...
        <td class="w100"><?php if(is_array($output['pic_list']) && !empty($output['pic_list'])){?>
          <form name="select_sort" id="select_sort">
            <input type="hidden" name="act" value="supplier_album" />
    ...
    <div class="ncsc-picture-list">
      ...
      <ul>
        ...
        <li>
          <dl>
            <dt>
              <div class="picture"><a href="index.php?act=supplier_album&op=album_pic_info&id=<?php echo $v['apic_id'];?>&class_id=<?php echo $v['aclass_id']?><?php if(!empty($_GET['sort'])){?>&sort=<?php echo $_GET['sort']; }?>&curpage=<?php echo ceil((15*($curpage-1)+$ii)/9);?>&start_index=<?php echo ($tmp = (15*($curpage-1)+$ii)%9) ? $tmp-1 :6;?>"> <img id="img_<?php echo $v['apic_id'];?>" src="<?php echo thumb($v, 240);?>"></a></div>
...
$(function() {
    ...
    $('input[nctype="replace_image"]').each(function(){
        $(this).fileupload({
            dataType: 'json',
            url: '<?php echo SHOP_SITE_URL;?>/index.php?act=supplier_album&op=replace_image_upload&id=' + $(this).attr('id'),
    ...
    $('#fileupload').fileupload({
        dataType: 'json',
        url: '<?php echo SHOP_SITE_URL;?>/index.php?act=supplier_album&op=image_upload',
        add: function (e,data) {
...
function _save(obj){
		$.post('index.php?act=supplier_album&op=change_pic_name', {id:obj.id,name:obj.value}, function(data) {
			if(data == 'false'){
...
function submit_form(type){
	...
	$('#checkboxform').attr('action','<?php echo SHOP_SITE_URL?>/index.php?act=supplier_album&op=album_pic_'+type);
	ajaxpost('checkboxform', '', '', 'onerror');
}
// 相册封面
function cover(id){
	if($('#aclass_cover').attr('src') != $('#img_'+id).attr('src')){
		ajaxget('index.php?act=supplier_album&op=change_album_cover&id='+id);
	}else{
