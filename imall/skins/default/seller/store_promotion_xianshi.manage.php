...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        //添加限时折扣商品弹出窗口 
        $('#div_goods_search_result').on('click', '[imtype="btn_add_xianshi_goods"]', function() {
            ...
            $('#dialog_add_xianshi_goods').im_show_dialog({width: 450, title: '添加商品'});
        ...
        //编辑限时活动商品
        $('#xianshi_goods_list').on('click', '[imtype="btn_edit_xianshi_goods"]', function() {
            $edit_item = $(this).parents('tr.bd-line');
            var xianshi_goods_id = $(this).attr('data-xianshi-goods-id');
            var xianshi_price = $edit_item.find('[imtype="xianshi_price"]').text();
            ...
            $('#dialog_edit_xianshi_goods').im_show_dialog({width: 450, title: '修改价格'});
        });
        ...
        $('#btn_edit_xianshi_goods_submit').on('click', function() {
            ...
            if(!isNaN(xianshi_price) && xianshi_price > 0 && xianshi_price < goods_price) {
                $.post('<?php echo urlShop('store_promotion_xianshi', 'xianshi_goods_price_edit');?>',
                    ...
                    function(data) {
                        if(data.result) {
                            $edit_item.find('[imtype="xianshi_price"]').text(data.xianshi_price);
                            $edit_item.find('[imtype="xianshi_discount"]').text(data.xianshi_discount);
        ...
        //删除限时活动商品
        $('#xianshi_goods_list').on('click', '[imtype="btn_del_xianshi_goods"]', function() {
            var $this = $(this);
            if(confirm('确认删除？')) {
                var xianshi_goods_id = $(this).attr('data-xianshi-goods-id');
...
<table class="imsc-default-table">
  <tbody>
    <tr>
      <td class="w90 tr"><strong><?php echo $lang['xianshi_name'].$lang['im_colon'];?></strong></td>
...
<div class="bottom">
  <label class="submit-border"><input type="submit" class="submit" id="submit_back" value="<?php echo $lang['im_back'].$lang['xianshi_index'];?>" onclick="window.location='index.php?act=store_promotion_xianshi&op=xianshi_list'"></label>
</div>
<div id="dialog_edit_xianshi_goods" class="eject_con" style="display:none;">
    ...
    <dl><dt>折扣价格：</dt><dd><input id="dialog_edit_xianshi_price" type="text" class="text w70"><em class="add-on">元</em>
    <p id="dialog_edit_xianshi_goods_error" style="display:none;"><label for="dialog_edit_xianshi_goods_error" class="error"><i class='fa fa-exclamation-circle'></i>折扣价格不能为空，且必须小于商品价格</label></p>
    </dl>    
    <div class="eject_con">
...
<tr class="bd-line">
    ...
    <td><?php echo $lang['currency']; ?><span imtype="xianshi_price"><%=xianshi_price%></span></td>
    <td><span imtype="xianshi_discount"><%=xianshi_discount%></span></td>
    <td class="nscs-table-handle">
    <?php if($output['xianshi_info']['editable']) { ?>
    <span><a imtype="btn_edit_xianshi_goods" class="btn-blue" data-xianshi-goods-id="<%=xianshi_goods_id%>" data-goods-price="<%=goods_price%>" href="javascript:void(0);"><i class="fa fa-pencil-square-o"></i><p><?php echo $lang['im_edit'];?></p></a></span>
        <span><a imtype="btn_del_xianshi_goods" class="btn-red" data-xianshi-goods-id="<%=xianshi_goods_id%>" href="javascript:void(0);"><i class="fa fa-trash-o"></i><p><?php echo $lang['im_del'];?></p></a></span>
    <?php } ?>
    </td>
</tr>
</script> 
