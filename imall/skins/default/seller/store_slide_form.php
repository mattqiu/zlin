...
<div class="imsc-form-default"> <div class="alert">
      <ul>
  ...
  <form action="index.php?act=store_setting&op=store_slide" id="store_slide_form" method="post" onsubmit="ajaxpost('store_slide_form', '', '', 'onerror');return false;">
    ...
    <!-- 图片上传部分 -->
    <ul class="imsc-store-slider" id="goods_images">
      <?php for($i=0;$i<5;$i++){?>
      <li im_type="handle_pic" id="thumbnail_<?php echo $i;?>">
        <div class="picture" imtype="file_<?php echo $i;?>">
          <?php if (empty($output['store_slide'][$i])) {?>
          <i class="fa fa-picture-o"></i>
          <?php } else {?>
          <img imtype="file_<?php echo $i;?>" src="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_SLIDE.DS.$output['store_slide'][$i];?>" />
          <?php }?>
          <input type="hidden" name="image_path[]" imtype="file_<?php echo $i;?>" value="<?php echo $output['store_slide'][$i];?>" /><a href="javascript:void(0)" imtype="del" class="del" title="移除">X</a>
        </div>        
        <div class="url">
        ...
        <div class="imsc-upload-btn"> 
          ...
          <p><i class="fa fa-upload"></i><?php echo $lang['store_slide_image_upload'];?></p>
          </a>
        </div>
      </li>
      <?php } ?>
