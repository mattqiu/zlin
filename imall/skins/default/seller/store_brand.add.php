<div class="eject_con">
  ...
  <form method="post" target="_parent" action="index.php?act=store_brand&op=<?php if ($output['brand_array']['brand_id']!='') echo 'brand_edit'; else echo 'brand_save'; ?>"enctype="multipart/form-data" id="brand_apply_form">
    ...
    <dl>
      <dt><i class="required">*</i><?php echo $lang['store_goods_brand_icon'].$lang['im_colon'];?></dt>
      <dd>
        <div class=""><span class="sign"><img src="<?php echo brandImage($output['brand_array']['brand_pic']);?>" onload="javascript:DrawImage(this,150,50)" im_type="logo1"/></span></div>
        <div class="imsc-upload-btn"> <a href="javascript:void(0);"><span>
          <input type="file" hidefocus="true" size="1" class="input-file" name="brand_pic" id="brand_pic" im_type="logo"/>
          </span>
...
$(function(){
    ...
    $('#brand_apply_form').validate({
        ...
        messages : {
            brand_name : {
                required : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['store_goods_brand_input_name'];?>',
                rangelength: '<i class="fa fa-exclamation-circle"></i><?php echo $lang['store_goods_brand_name_error'];?>'
            },
            brand_initial : {
			...
			,
            brand_pic : {
                required : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['store_goods_brand_icon_null'];?>'
			}
			<?php } ?>
        }
    });
	$('input[im_type="logo"]').change(function(){
		var src = getFullPath($(this)[0]);
		$('img[im_type="logo1"]').attr('src', src);
	});
});
