...
<div class="imsc-form-default">
  <form id="add_form" action="<?php echo urlShop('store_account_group', 'group_save');?>" method="post">
    ...
    <?php } ?>
    <dl>
      <dt><i class="required">*</i>组名称<?php echo $lang['im_colon'];?></dt>
      <dd>
    ...
    <dl id="function_list">
      <dt><i class="required">*</i>权限<?php echo $lang['im_colon'];?></dt>
      <dd>
        ...
        <div class="imsc-account-container">
          <h4>
            <input id="<?php echo $key;?>" class="checkbox" imtype="btn_select_module" type="checkbox" />
    ...
    <dl>
      <dt><i class="required"></i>消息接收权限<?php echo $lang['im_colon'];?></dt>
      <dd>
        <div class="imsc-account-all">
        ...
        <div class="imsc-account-container">
          ...
          <ul class="imsc-account-container-list" style=" width: 99%; padding-left: 1%;">
    ...
    <dl id="function_list">
      <dt><i class="required">*</i>APP权限<?php echo $lang['im_colon'];?></dt>
      <dd>
        <div class="imsc-account-all">
          <input id="app_select_all" name="app_select_all" class="checkbox" type="checkbox" />
          <label for="app_select_all">全选</label>
          <span></span>
          <?php if(!empty($output['app_menu']) && is_array($output['app_menu'])) {?>
          <?php foreach($output['app_menu'] as $key => $value) {?>
        </div>
        <div class="imsc-account-container">
          <h4>
            <input id="<?php echo $key;?>" class="checkbox" imtype="app_select_module" type="checkbox" />
            <label for="<?php echo $key;?>"><?php echo $value['name'];?></label>
          </h4>
          <?php $submenu = $value['child'];?>
          <?php if(!empty($submenu) && is_array($submenu)) {?>
          <ul class="imsc-account-container-list">
            <?php foreach($submenu as $submenu_value) {?>
            <li>
              <input id="<?php echo $submenu_value['power'];?>" class="checkbox" name="app_limits[]" value="<?php echo $submenu_value['power'];?>" <?php if(!empty($output['app_limits'])) {if(in_array($submenu_value['power'], $output['app_limits'])) { echo 'checked'; }}?> type="checkbox" />
              <label for="<?php echo $submenu_value['power'];?>"><?php echo $submenu_value['name'];?></label>
            </li>
            <?php } ?>
          </ul>
          <?php } ?>
    ...
    <div class="bottom">
      <label class="submit-border">
        <input type="submit" class="submit" value="<?php echo $lang['im_submit'];?>设置">
      </label>
    </div>
  </form>
</div>
...
$(document).ready(function(){
    ...
    $('[imtype="btn_select_module"]').on('click', function() {
        if($(this).prop('checked')) {
            $(this).parents('.imsc-account-container').find('input:checkbox').prop('checked', true);
        } else {
            $(this).parents('.imsc-account-container').find('input:checkbox').prop('checked', false);
        }
    });
    ...
    $('#app_select_all').on('click', function() {
        if($(this).prop('checked')) {
            $(this).parents('dl').find('input:checkbox').prop('checked', true);
        } else {
            $(this).parents('dl').find('input:checkbox').prop('checked', false);
    ...
    $('[imtype="app_select_module"]').on('click', function() {
        if($(this).prop('checked')) {
            $(this).parents('.imsc-account-container').find('input:checkbox').prop('checked', true);
        } else {
            $(this).parents('.imsc-account-container').find('input:checkbox').prop('checked', false);
