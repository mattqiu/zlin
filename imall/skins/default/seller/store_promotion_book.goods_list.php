...
<?php if ($output['isOwnShop']) { ?>
  <a class="imsc-btn imsc-btn-acidblue" href="javascript:void(0);" nctype="select_goods"><i class="icon-plus-sign"></i>添加商品</a>
<?php } else { ?>
  <?php if(empty($output['book_quota'])) { ?>
  <a class="imsc-btn imsc-btn-green" href="<?php echo urlShop('store_promotion_book', 'book_quota_add');?>" title="购买套餐"><i class="icon-money"></i>购买套餐</a>
  <?php } else { ?>
  <a class="imsc-btn imsc-btn-acidblue" href="javascript:void(0);" nctype="select_goods" style="right:100px"><i class="icon-plus-sign"></i>添加商品</a>
  <a class="imsc-btn imsc-btn-acidblue" href="<?php echo urlShop('store_promotion_book', 'book_renew');?>"><i class="icon-money"></i>套餐续费</a>
  <?php } ?>
<?php } ?>
</div>
...
<?php } else {?>
  <strong>套餐过期时间<?php echo $lang['im_colon'];?></strong> <strong style=" color:#F00;"><?php echo date('Y-m-d H:i:s',$output['book_quota']['bkq_endtime']);?></strong>
<?php }?>
  <ul>
...
<div nctype="div_goods_select" class="div-goods-select" style="display: none;">
    <table class="search-form">
      <tr><th class="w150"><strong>第一步：搜索店内商品</strong></th><td class="w160"><input nctype="search_goods_name" type="text w150" class="text" name="goods_name" value=""/></td>
        <td class="w70 tc"><label class="submit-border"><input nctype="btn_search_goods" type="button" value="<?php echo $lang['im_search'];?>" class="submit"/></label></td><td class="w10"></td><td><p class="hint">不输入名称直接搜索将显示店内所有出售中的商品</p></td>
      </tr>
    </table>
  ...
  <tbody nctype="choose_goods_list">
    ...
    <tr class="bd-line">
      ...
      <td class="tl">
        <dl>
          <dt>发货时间：
          <?php if($val['presell_deliverdate'] > 0) {?>
          	<?php echo date('Y-m-d', $val['presell_deliverdate']);?>
          <?php } else if ($val['is_presell'] == 1) {?>
          	下单<?php echo $val['presell_days'];?>天后
          <?php }?>
          </dt>
        </dl>
      </td>
      <?php }?>
      <td class="goods-price">￥<?php echo imPriceFormat($val['goods_price']);?></td>
      <td class="nscs-table-handle">
        <span><a class="btn-grapefruit" href='javascript:void(0);' nctype="del_choosed" data-param="{gid:<?php echo $val['goods_id'];?>, type:<?php if ($val['is_book'] == 1) {?>'book'<?php } else if ($val['is_presell'] == 1) {?>'presell'<?php }?>}"><i class="icon-trash"></i><p><?php echo $lang['im_del'];?></p></a></span></td>
    </tr>
    <?php } ?>
    <?php } ?>
  </tbody>
