  ...
  <form id="add_form" action="<?php echo urlShop('store_groupbuy', 'groupbuy_quota_add_save');?>" method="post">
    <dl>
      ...
      <dd>
        ...
        <p class="hint"><?php echo '每月(30天)您需要支付'.$output['setting_config']['groupbuy_price'].$lang['im_yuan'];?> ; </p>
...
$(document).ready(function(){
    ...
    $("#add_form").validate({
    	...
    	submitHandler:function(form){
            ...
            showDialog('确认购买？您总共需要支付'+price+'<?php echo $lang['im_yuan'];?>', 'confirm', '', function(){
        ...
        messages : {
            groupbuy_quota_quantity : {
                required : "<i class='fa fa-exclamation-circle'></i>数量不能为空且必须为数字",
                digits : "<i class='fa fa-exclamation-circle'></i>数量不能为空且必须为数字",
                min : "<i class='fa fa-exclamation-circle'></i>数量不能为空且必须为数字"
            }
        }
    });
});
