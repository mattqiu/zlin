...
<style type="text/css">
#fixedNavBar {filter:progid:DXImageTransform.Microsoft.gradient(enabled='true',startColorstr='#CCFFFFFF', endColorstr='#CCFFFFFF');background:rgba(255,255,255,0.8); width: 90px; margin-left: 600px; border-radius: 4px; position: fixed; z-index: 999; top: 172px; right: 10px;}
    ...
    <div class="imsc-form-goods">
      ...
      <dl>
        ...
        <dd im_type="no_spec">
          ...
          <span></span>
          <p class="hint">
          	<?php echo $lang['store_goods_index_store_price_help'];?>，且不能高于吊牌价。<br>
          	  此价格为商品实际销售价格，如果商品存在规格，该价格显示所有规格中最低价格。
          </p>
        </dd>
      </dl>
      
      <dl>
        <dt><i class="required">*</i>商家供货价<?php echo $lang['im_colon'];?></dt>
        <dd>
      ...
      <dl>
        ...
        <dd>
          <input name="g_marketprice" value="<?php echo $output['goods']['goods_marketprice']; ?>" type="text" class="text w60" /><em class="add-on">元</em> <span></span>
      ...
      <dl>
        ...
        <dd>
          ...
          <p class="hint">
          	此价格为批发价格，将商品供货给平台分销商的成本价。<br/>
          	此价格为默认系统会自动计算，计算公式：商家供货价 + 会员价*平台扣点。若想赚取更多可以相应调整此价格<br/>
          	由于 分销利润 = 实际售价 - 批发价，建议 批发价不要设置过高，这部分利润越高商品在分销市场的优势才更大。
          </p>
        </dd>
      </dl>
      
      <dl style="display:none;">
        <dt im_type="no_spec">商家平台供货成本<?php echo $lang['im_colon'];?></dt>
        <dd im_type="no_spec">
          ...
          <p class="hint">此价格为 商家供货价 + 实际售价*平台扣点。<br>不会在前台销售页面中显示，自动生成，不需要编辑。</p>
        </dd>
      </dl>
      ...
      <dl edit_model="full_model">
        ...
        <dd>
          ...
          <p class="hint">此平台利润是用来分给消费者推广奖励 + 门店租金补贴收益。<br> 根据 会员价 - 批发价 自动生成，不需要编辑。</p>
        </dd>
      </dl>
      ...
      <dl>
        <dt im_type="no_spec">赠送云币<?php echo $lang['im_colon'];?></dt>
        <dd im_type="no_spec">
          <input name="g_points" value="<?php echo $output['goods']['give_points']; ?>" type="text" class="text w60" /><em class="add-on">个</em> <span></span>
          <p class="hint">购买此商品送多少云币</p>
        </dd>
      </dl>
      ...
      <dl imtype="is_market" <?php if ($output['goods']['is_market'] == 0){?>style="display:none;"<?php }?>>
        ...
        <ul class="imsc-form-radio-list">
          ...
          <span></span>
          <p class="hint">
          	该价格区间用于控制分销商的售价，分销商的利润=(批发价-平台供货价)+(分销商售价-(批发价+(分销商售价*平台扣点)))；<br>
      ...
      <dl edit_model="full_model">
        ...
        <dd> 
          ...
          <select name="saleman_cid">
           ...
           <span class="mr30">
          <label>会员返利模板</label>
          <select name="rebate_cid">
            <option value="0" <?php if ($output['goods']['rebate_cid'] == 0) {?>selected="selected"<?php }?>>无返利</option>
            <?php if(!empty($output['commis_list']) && is_array($output['commis_list'])){ ?>
            <?php   foreach($output['commis_list'] as $k => $val){ ?>
            <option value="<?php echo $val['commis_id']?>" <?php if ($output['goods']['rebate_cid'] == $val['commis_id']) {?>selected="selected"<?php }?>><?php echo $val['commis_name'].$val['commis_rate'];?></option>
            <?php }?>
            <?php }?>
          </select>
          </span> 
      ...
      <h3 edit_model="full_model" id="demo6"><?php echo $lang['store_goods_index_goods_other_info']?></h3>
      <dl edit_model="full_model">
...
$(function(){
    ...
    }, '<i class="fa fa-exclamation-circle"></i>平台供货价价不能高于(会员价-会员价*平台扣点)');
    ...
    }, '<i class="fa fa-exclamation-circle"></i>批发价不能低于平台供货价');
...
function goods_stock_set(){
    ...
    $('input[name="g_barcode"]').attr('readonly','readonly').css('background','#E7E7E7 none');
    ...
    if(str == '<tr>'){
        ...
        $('input[name="g_barcode"]').removeAttr('readonly').css('background','');
...
function recursionSpec($len,$sign) {
    if($len < $sign){
        ...
        echo "str +='<td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][marketprice]\" data_type=\"marketprice\" im_type=\"'+spec_bunch+'|marketprice\" value=\"\" /><em class=\"add-on\">元</em></td><td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][price]\" data_type=\"price\" im_type=\"'+spec_bunch+'|price\" value=\"\" /><em class=\"add-on\">元</em></td><td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][tradeprice]\" data_type=\"tradeprice\" im_type=\"'+spec_bunch+'|tradeprice\" value=\"\" /><em class=\"add-on\">元</em></td><td><input class=\"text stock\" type=\"text\" name=\"spec['+spec_bunch+'][stock]\" data_type=\"stock\" im_type=\"'+spec_bunch+'|stock\" value=\"\" /></td><td><input class=\"text stock\" type=\"text\" name=\"spec['+spec_bunch+'][alarm]\" data_type=\"alarm\" im_type=\"'+spec_bunch+'|alarm\" value=\"\" /></td><td><input class=\"text sku\" type=\"text\" name=\"spec['+spec_bunch+'][sku]\" data_type=\"serial\" im_type=\"'+spec_bunch+'|sku\" value=\"\" /></td><td><input class=\"text sku\" type=\"text\" name=\"spec['+spec_bunch+'][barcode]\" data_type=\"barcode\" im_type=\"'+spec_bunch+'|barcode\" value=\"\" /></td></tr>';\n";
        for($i=0; $i< $len; $i++){
            echo "}\n";
        }
    }
