<a href="javascript:void(0);" imtype="meai_cancel" class="imsc-btn mt5"><i class=" fa fa-arrow-circle-up"></i>关闭相册</a>
<div class="goods-gallery add-step2"><a class='sample_demo' id="select_submit" href="index.php?act=store_album&op=pic_list&item=goods" style="display:none;"><?php echo $lang['im_submit'];?></a>
  <div class="nav"><span class="l"><?php echo $lang['store_goods_album_users'];?> >
    ...
    <select name="jumpMenu" id="jumpMenu" style="width:100px;">
      <option value="0" style="width:80px;"><?php echo $lang['im_please_choose'];?></option>
  ...
  <?php }else{?>
  <div class="warning-option"><i class="fa fa-exclamation-triangle"></i><span>相册中暂无图片</span></div>
  <?php }?>
  <div class="pagination"><?php echo $output['show_page']; ?></div>
</div>
<script type="text/javascript">
$(function(){
    $('[imtype="mea_img"]').find('a[class="demo"]').click(function(){
        $('[imtype="mea_img"]').load($(this).attr('href'));
        return false;
    });
    $('#jumpMenu').change(function(){
        $('[imtype="mea_img"]').load('index.php?act=store_album&op=pic_list&item=mobile<?php if ($output['type'] == 'replace') {?>&type=replace<?php }?>&id=' + $(this).val());
    });
});
</script>