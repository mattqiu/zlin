...
.bill_alert {
  ...
  <div class="bill_alert bill-alert-block mt10">
    ...
    <a class="imsc-btn mt5" onclick="ajax_get_confirm('一旦确认将无法恢复，系统将自动进入结算环节,<BR/>确认系统出账单计算无误吗?', 'index.php?act=store_vr_bill&op=confirm_bill&ob_no=<?php echo $_GET['ob_no'];?>');" href="javascript:void(0)">本期结算无误，我要确认</a>
    <?php } elseif ($output['bill_info']['ob_state'] == BILL_STATE_SUCCESS) {?>
    <a class="imsc-btn mt5" target="_blank" href="index.php?act=store_vr_bill&op=bill_print&ob_no=<?php echo $_GET['ob_no'];?>">打印结算单</a>
    <?php } ?>
    </div>
