...
<?php if ($output['isOwnShop']) { ?>
  <a class="imsc-btn imsc-btn-green"  href="index.php?act=store_promotion_bundling&op=bundling_add" ><i class="fa fa-plus-circle"></i><?php echo $lang['bundling_add'];?></a>

<?php } else { ?>

  <?php if (empty($output['bundling_quota'])) { ?>
  <a href="javascript:void(0)" class="imsc-btn imsc-btn-acidblue" onclick="go('index.php?act=store_promotion_bundling&op=bundling_quota_add');" title="<?php echo $lang['bundling_quota_add'];?>"><i class="fa fa-money"></i><?php echo $lang['bundling_quota_add'];?></a>
  ...
  <a class="imsc-btn imsc-btn-green"  href="index.php?act=store_promotion_bundling&op=bundling_add" style="right:100px"><i class="fa fa-plus-circle"></i><?php echo $lang['bundling_add'];?></a>
  <?php }?>
  <a class="imsc-btn imsc-btn imsc-btn-acidblue" href="index.php?act=store_promotion_bundling&op=bundling_renew"><i class="fa fa-money"></i>套餐续费</a>
  <?php } ?>
<?php } ?>
...
<?php } else {?>
  <strong>套餐过期时间<?php echo $lang['im_colon'];?></strong> <strong style=" color:#F00;"><?php echo date('Y-m-d H:i:s',$output['bundling_quota']['bl_quota_endtime']);?></strong>
<?php }?>
  <ul>
...
<table class="imsc-default-table">
  <thead>
    <tr>
      ...
      <th class="w90"><?php echo $lang['im_state'];?></th>
    ...
    <tr>
      ...
      <td colspan="20"><label for="all" ><?php echo $lang['im_select_all'];?></label>
        <a href="javascript:void(0);" class="imsc-btn-mini" im_type="batchbutton" uri="index.php?act=store_promotion_bundling&op=drop_bundling" name="bundling_id" confirm="<?php echo $lang['im_ensure_del'];?>"><i class="fa fa-trash-o"></i><?php echo $lang['im_del'];?></a></td>
    </tr>
  </thead>
  ...
  <tbody>
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle"><span><a href="index.php?act=store_promotion_bundling&op=bundling_add&bundling_id=<?php echo $val['bl_id'];?>" class="btn-blue"><i class="fa fa-cog"></i>
        <p><?php echo $lang['bundling_edit'];?></p>
        </a></span> <span><a class="btn-red" href='javascript:void(0);' onclick="ajax_get_confirm('<?php echo $lang['im_ensure_del'];?>', '<?php echo urlShop('store_promotion_bundling', 'drop_bundling', array('bundling_id' => $val['bl_id']));?>');"><i class="fa fa-trash-o"></i>
  ...
  <tfoot>
    <tr>
      ...
      <th colspan="20"><label for="all" ><?php echo $lang['im_select_all'];?></label>
        <a href="javascript:void(0);" class="imsc-btn-mini" im_type="batchbutton" uri="index.php?act=store_promotion_bundling&op=drop_bundling" name="bundling_id" confirm="<?php echo $lang['im_ensure_del'];?>"><i class="fa fa-trash-o"></i><?php echo $lang['im_del'];?></a></th>
    </tr>
    <tr>
