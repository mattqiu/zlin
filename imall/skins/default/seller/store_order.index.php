...
<table class="imsc-default-table order">
  ...
  <tbody>
    ...
    <tr>
      <th colspan="20">
        <span class="ml10"><?php echo $lang['store_order_order_sn'].$lang['im_colon'];?><em><?php echo $order['order_sn']; ?></em>
        <?php if ($order['order_from'] == 2){?>
        <i class="fa fa-mobile"></i>
        <?php }?>
        </span> 
        <span><?php echo $lang['store_order_add_time'].$lang['im_colon'];?><em class="goods-time"><?php echo date("Y-m-d H:i:s",$order['add_time']); ?></em></span> 
        <span class="fr mr5"> <a href="index.php?act=store_order_print&order_id=<?php echo $order_id;?>" class="imsc-btn-mini" target="_blank" title="打印发货单"/><i class="fa fa-print"></i>打印发货单</a></span>
      </th>
    </tr>
    <?php $i = 0;?>
    ...
    <tr>
      ...
      <td class="w70">
        <div class="imsc-goods-thumb">
          <a href="<?php echo $goods['goods_url'];?>" target="_blank"><img src="<?php echo $goods['image_60_url'];?>" onMouseOver="toolTip('<img src=<?php echo $goods['image_240_url'];?>>')" onMouseOut="toolTip()"/></a>
        </div>
      </td>
      <td class="tl">
        <dl class="goods-name">
      ...
      <td class="bdl" rowspan="<?php echo $order['goods_count'];?>"><div class="buyer"><?php echo $order['buyer_name'];?>
          <p member_id="<?php echo $order['buyer_id'];?>">
            ...
            <a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=<?php echo $order['extend_member']['member_qq'];?>&site=qq&menu=yes" title="QQ: <?php echo $order['extend_member']['member_qq'];?>">
              <img border="0" src="http://wpa.qq.com/pa?p=2:<?php echo $order['extend_member']['member_qq'];?>:52" style=" vertical-align: middle;"/>
            </a>
            <?php }?>
            <?php if(!empty($order['extend_member']['member_ww'])){?>
            <a target="_blank" href="http://amos.im.alisoft.com/msg.aw?v=2&uid=<?php echo $order['extend_member']['member_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>" >
              <img border="0" src="http://amos.im.alisoft.com/online.aw?v=2&uid=<?php echo $order['extend_member']['member_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>" alt="Wang Wang" style=" vertical-align: middle;" />
            </a>
            <?php }?>
          </p>
          <div class="buyer-info"> <em></em>
            <div class="con">
              ...
              <dl>
                <dt><?php echo $lang['store_order_receiver'].$lang['im_colon'];?></dt>
                <dd><?php echo $order['extend_order_common']['reciver_name'];?></dd>
              </dl>
              <dl>
                <dt><?php echo $lang['store_order_phone'].$lang['im_colon'];?></dt>
      ...
      <td class="bdl" rowspan="<?php echo $order['goods_count'];?>"><p class="imsc-order-amount"><?php echo $order['order_amount']; ?></p>
        ...
        <p class="goods-pay" title="<?php echo $lang['store_order_pay_method'].$lang['im_colon'];?><?php echo $order['payment_name']; ?>"><?php echo $order['payment_name']; ?></p></td>
	...
	<!-- 分销选入店铺 start zhang -->
        <!-- 发货 -->
        <?php if ($order['if_send'] && empty($order['up_id'])){  echo $order['if_send'];?>
        ...
        <p><a class="imsc-btn imsc-btn-green mt10" href="index.php?act=store_deliver&op=send&order_id=<?php echo $order['order_id']; ?>"/><i class="fa fa-truck"></i><?php echo $lang['store_order_send'];?></a></p>
        <?php } ?>
    ...
    <tr>
      <td class="bdl"></td>
      <td colspan="4" class="tl"><div class="imsc-goods-gift">赠品：
