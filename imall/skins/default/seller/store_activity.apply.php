...
<div class="tabmenu">
  ...
  <div class="text-intro"> <?php echo $lang['store_activity_theme'].$lang['im_colon'];//'活动主题';?> <?php echo $output['activity_info']['activity_title'];?></div>
</div>
<form method="GET">
  ...
  <div class="div-goods-select">
    ...
    <form method="POST" id="apply_form" onsubmit="ajaxpost('apply_form','','','onerror');" action="index.php?act=store_activity&op=activity_apply_save">
      ...
      <?php }else{?>
      <div class="warning-option"><i class="fa fa-exclamation-triangle"></i><span><?php echo $lang['no_record'];//您尚未发布任何商品?></span></div>
      <?php }?>
      <?php if(!empty($output['goods_list']) and is_array($output['goods_list'])){?>
