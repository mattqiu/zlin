<?php defined('InIMall') or exit('Access Invalid!');?>
  ...
  <div class="imsc-form-default">
    <form method="post" action="index.php?act=store_deliver_set&op=print_set" enctype="multipart/form-data" id="my_store_form">
      ...
      <dl class="setup">
        <dt><?php echo $lang['store_printsetup_desc'].$lang['im_colon'];?></dt>
      ...
      <dl class="setup">
        <dt><?php echo $lang['store_printsetup_stampimg'].$lang['im_colon'];?></dt>
        <dd>
          <input type="hidden" name="store_stamp_old" value="<?php echo $output['store_info']['store_stamp'];?>" />
          <div class="imsc-upload-thumb watermark-pic">
          	<p>
          	<img src="<?php if(!empty($output['store_info']['store_stamp'])){echo UPLOAD_SITE_URL.'/'.ATTACH_STORE.'/'.$output['store_info']['store_stamp'];}?>" im_type="store_stamp" /></p> </div>
          <p>
            <input name="store_stamp" type="file"  hidefocus="true" im_type="change_store_stamp"/>
          </p>
...
$(function(){
	$('input[im_type="change_store_stamp"]').change(function(){
		var src = getFullPath($(this)[0]);
		$('img[im_type="store_stamp"]').attr('src', src);
	});
	$('#my_store_form').validate({
        ...
        messages : {
        	store_printdesc: {
        		required: '<i class="fa fa-exclamation-circle"></i><?php echo $lang['store_printsetup_desc_error'];?>',
