  ...
  <table class="search-form">
    <tbody>
      <tr>
        ...
        <td class="tc w70"><a href="index.php?act=store_promotion_bundling&op=bundling_add_goods" imtype="search_a" class="imcs-btn"><i class="fa fa-search"></i><?php echo $lang['im_search'];?></a></td>
        <td class="w10"></td>
      </tr>
    </tbody>
  </table>
	...
	if(typeof(T) != 'undefined'){
		T.each(function(){
			if(typeof(A[$(this).attr('imtype')]) != 'undefined'){
				$(this).children(':last').html('<a href="JavaScript:void(0);" onclick="bundling_operate_delete($(\'#bundling_tr_'+$(this).attr('imtype')+'\'), '+$(this).attr('imtype')+')" class="imsc-btn-mini imsc-btn-orange"><i class="fa fa-ban"></i><?php echo $lang['bundling_goods_add_bundling_exit'];?></a>');
			}
		});
	}
});
...
function bundling_goods_add(o){
    ...
    $('<tr id="bundling_tr_' + _data.gid + '"></tr>')
        .append('<input type="hidden" imtype="goods_id" name="goods[g_' + _data.gid + '][gid]" value="' + _data.gid + '">')
        ...
        .append('<td class="nscs-table-handle w90"><span><a href="javascript:void(0);" onclick="bundling_operate_delete($(\'#bundling_tr_' + _data.gid + '\'), ' + _data.gid + ')" class="btn-orange"><i class="fa fa-ban"></i><p><?php echo $lang['bundling_goods_remove'];?></p></a></span></td>')
        .fadeIn().appendTo('tbody[imtype="bundling_data"]');

    $('li[imtype="' + _data.gid + '"]').children(':last').html('<a href="JavaScript:void(0);" class="imsc-btn-mini imsc-btn-orange" onclick="bundling_operate_delete($(\'#bundling_tr_' + _data.gid + '\'), ' + _data.gid + ')"><i class="fa fa-ban"></i><?php echo $lang['bundling_goods_add_bundling_exit'];?></a>');
