...
<?php if ($output['isOwnShop']) { ?>
    <a class="imsc-btn imsc-btn-red mr10" style="right:100px" href="<?php echo urlShop('pointvoucher', 'index', array('store_id' => $_SESSION['store_id']));?>" target="_blank"><i class="fa fa-eye"></i>查看活动列表</a>
    <a class="imsc-btn imsc-btn-green" href="<?php echo urlShop('store_voucher', 'templateadd');?>"><i class="fa fa-plus-circle"></i><?php echo $lang['voucher_templateadd']; ?></a>
<?php } else { ?>
    <?php if(!empty($output['current_quota'])) { ?>
    <a class="imsc-btn imsc-btn-red mr10" style="right:200px" href="<?php echo urlShop('pointvoucher', 'index', array('store_id' => $_SESSION['store_id']));?>" target="_blank"><i class="fa fa-eye"></i>查看活动列表</a>
    <a class="imsc-btn imsc-btn-green" style="right:100px" href="<?php echo urlShop('store_voucher', 'templateadd');?>"><i class="fa fa-plus-circle"></i><?php echo $lang['voucher_templateadd']; ?></a>
    <a class="imsc-btn imsc-btn-acidblue" href="<?php echo urlShop('store_voucher', 'quotaadd');?>" title=""><i class="fa fa-money"></i>套餐续费</a>
    <?php } else { ?>
    <a class="imsc-btn imsc-btn-acidblue" href="<?php echo urlShop('store_voucher', 'quotaadd');?>" title=""><i class="fa fa-money"></i>购买套餐</a>
    <?php } ?>
...
<?php } else { ?>
  <div class="alert alert-block mt10 mb10">
      <?php if(!empty($output['current_quota'])) { ?>
      <strong>套餐过期时间<?php echo $lang['im_colon'];?></strong><strong style="color:#F00;"><?php echo date('Y-m-d H:i:s', $output['current_quota']['quota_endtime']);?></strong>
      <?php } else { ?>
...
<i class="fa fa-calendar"></i>
        ...
        <th><?php echo $lang['im_status'];?></th>
        <td class="w120"><select class="w80" name="select_state">
            <option value="0" <?php if (!$_GET['select_state'] == '0'){ echo 'selected=true';}?>><?php echo $lang['im_please_choose'];?></option>
  ...
  <table class="imsc-default-table">
    <thead>
      <tr>
        ...
        <th class="w60"><?php echo $lang['im_status'];?></th>
    ...
    <tbody>
      ...
      <tr class="bd-line">
        ...
        <td class="nscs-table-handle">
        	<?php if($val['voucher_t_state']==$output['templatestate_arr']['usable'][0] && !$val['voucher_t_giveout']) {//代金券模板有效并且没有领取时可以编辑?>
        		<span>
        		  <a class="btn-blue" href="index.php?act=store_voucher&op=templateedit&tid=<?php echo $val['voucher_t_id'];?>">
        		      <i class="fa fa-pencil-square-o"></i><p>编辑</p>
        		  </a>
        	   </span>
        	<?php }else {//代金券模板失效或者有领取时可以查看?>
        		<span>
        		  <a class="btn-blue" href="index.php?act=store_voucher&op=templateinfo&tid=<?php echo $val['voucher_t_id'];?>"><i class="fa fa-th-list"></i>
        		      <p>详细</p>
        		  </a>
        	   </span>
        	<?php }?>
        	...
        	<span>
        	   <a class="btn-red" href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['im_ensure_del'];?>','index.php?act=store_voucher&op=templatedel&tid=<?php echo $val['voucher_t_id'];?>');">
        	       <i class="fa fa-trash-o"></i><p>删除</p>
        	   </a>
        	</span>
        	<?php }?>
        </td>
