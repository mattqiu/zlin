...
<div class="imsc-gallery">
  <ul>
    <?php foreach($output['image_list'] as $key => $value) {?>
    <li>
      <div class="pic-thumb"><a imtype="nyroModal" href="<?php echo $value['image_url'];?>"><img src="<?php echo $value['image_url'];?>" alt="<?php echo $value['image_name'];?>"></a></div>
      <div class="pic-name"><?php echo $value['image_origin_name'];?></div>
      <div class="pic-handle"><span><?php echo $value['upload_time_format'];?></span><a imtype="btn_del_image" href="javascript:void(0);" data-image-id="<?php echo $value['image_id'];?>" class="imsc-btn-mini"><i class="fa fa-trash-o"></i>删除</a></div>
    </li>
    <?php } ?>
  </ul>
</div>
...
<script type="text/javascript">
    $(document).ready(function(){
        $('[imtype="btn_del_image"]').on('click', function() {
            if(confirm('确认删除？')) {
                $this = $(this);
