...
<table class="imsc-table-style">
  ...
  <tr>
    ...
    <td>
      <a href="javascript:void(0);" imtype="goods_stubbs" class="imsc-btn imsc-btn-green"><i class="fa fa-share-alt"></i>确定调拔</a>
    </td>
  </tr>
</table>
...
$(function(){
    ...
    $('i[imtype="ajaxGoodsList"]').toggle(
        function(){
            ...
            if (_div.html() == '') {
                $.getJSON('index.php?act=seller_branch_stubbs&op=get_goods_list_ajax' , {commonid : _commonid}, function(date){
                    if (date != 'false') {
                        var _ul = $('<ul class="imsc-goods-sku-list"></ul>');
                        $.each(date, function(i, o){
                            $('<li name="goods_' +_commonid+'" id="goods_' + o.goods_id +'" data=""><div class="goods-thumb" title="商家货号：' + o.goods_serial + '"><a href="' + o.url + '" target="_blank"><image src="' + o.goods_image + '" ></a></div>' + o.goods_spec + '<div class="goods-price">价格：<em title="￥' + o.goods_price + '">￥' + o.goods_price + '</em></div><div class="goods-storage" ' + o.alarm + '>库存：<em title="' + o.goods_storage + '" ' + o.alarm + '>' + o.goods_storage + '</em></div><div class="goods-storage">调拔价：<input type="text" class="text w40" id="price_item_'+o.goods_id+'" name="stubbs_price" value="' + o.goods_tradeprice + '" orig="'+o.goods_tradeprice+'" onkeyup="change_price('+o.goods_id+', this);"/> </div><div class="goods-storage">调拔数：<input type="text" class="text w40" id="nums_item_'+o.goods_id+'" name="stubbs_num" value="0" orig="'+o.goods_storage+'" onkeyup="change_nums('+o.goods_id+', this);"/></div></li>').appendTo(_ul);
                        });
                        _ul.appendTo(_div);
                        _parenttr.next().show();
    ...
    $('a[imtype="goods_stubbs"]').click(function(){		
		...
		ajax_get_confirm('真的要调拔商品吗?','<?php echo urlShop('seller_branch_stubbs', 'stubbs_save');?>&data=' + _items +'&stores=' + _stores);
    });
});
/**
  * 更改购物车数量
  ...
  * @param input
*/
function change_price(id, input){
    var goods_id = id;
	var goods_price = input.value;
	var goods_num = $('#nums_item_' + id).val();
	if (goods_num==0  || goods_num==''){
		$('#goods_'+goods_id).attr('data','');
	}else{
	    $('#goods_'+goods_id).attr('data',goods_id+'|'+goods_price+'|'+goods_num);
	}
}
  
function change_nums(id, input){
    var goods_id = id;
	var goods_price = $('#price_item_' + id).val();
