...
<div class="imsc-form-default">
  <form method="post" action="index.php?act=store_setting&op=store_mobile" enctype="multipart/form-data" id="my_store_form">
    ...
    <dl>
      <dt>云币可用比例<?php echo $lang['nc_colon']; ?></dt>
      <dd class="mb-sliders">
        <input type="text" class="w50 text"  name="jf_ratio" value="<?php echo $output['jf_ratio']; ?>" />%
        <p class="hint">云币可用的比例，是根据消费的金额而定</p>
      </dd>
    </dl>
    <dl>
      <dt>最多可用云币数<?php echo $lang['nc_colon']; ?></dt>
      <dd class="mb-sliders">
        <input type="text" class="w100 text"  name="jf_limit" value="<?php echo $output['jf_limit']; ?>" />
        <p class="hint">订单最高可使用的云币数</p>
      </dd>
    </dl>
    ...
    <dl>
      ...
      <dd class="mb-sliders">
        <?php if ($output['mb_slide']) { ?>
        <div class="img-wrapper"> <img alt="" src="<?php echo $output['mb_slide']; ?>" /> </div>
        <?php } else { ?>
        <p>暂无图片</p>
        <?php } ?>
        <p>
          <input type="file" name="mb_slide" />
        </p>
        <p class="hint">手机店铺页面头部背景图片，默认为白色纯色背景，推荐图片大小640x400</p>
      </dd>
    </dl>
    <dl>
      <dt>轮播<?php echo $lang['im_colon']; ?></dt>
      <dd>
...
$(function() {
    ...
    var LOADING_IMAGE = '<?php echo SHOP_SKINS_URL; ?>/images/loading.gif';
