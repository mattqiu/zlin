...
<div class="tabmenu">
  ...
  <a id="open_uploader" href="JavaScript:void(0);" class="imsc-btn imsc-btn-acidblue"><i class="fa fa-cloud-upload"></i><?php echo $lang['album_class_list_img_upload'];?></a>
    ...
    <form method="post" action="" id="fileupload" enctype="multipart/form-data">
      ...
      <div class="upload-con-div">选择文件：
        <div class="imsc-upload-btn"> <a href="javascript:void(0);"><span>
          ...
          <p><i class="fa fa-upload"></i><?php echo $lang['album_class_list_img_upload'];?></p>
          </a> </div>
      </div>
      <div imtype="file_msg"></div>
      <div class="upload-pmgressbar" imtype="file_loading"></div>
...
<div id="pictureFolder" class="imsc-picture-folder">
  <dl class="imsc-album-intro">
    ...
    <tbody>
      <tr>
        ...
        <td><?php if(is_array($output['pic_list']) && !empty($output['pic_list'])){?>
          <a href="JavaScript:void(0);" class="imsc-btn-mini" onClick="checkAll()"><i class="fa fa-check-square-o"></i><?php echo $lang['album_plist_check_all'];?></a><a href="JavaScript:void(0);" class="imsc-btn-mini" onClick="uncheckAll()"><i class="fa fa-check-square"></i><?php echo $lang['album_plist_cancel'];?></a><a href="JavaScript:void(0);" class="imsc-btn-mini" onClick="switchAll()"><i class="fa fa-square-o"></i><?php echo $lang['album_plist_inverse'];?></a><a href="JavaScript:void(0);" class="imsc-btn-mini" onClick="submit_form('del')"><i class="fa fa-trash-o"></i><?php echo $lang['album_class_delete'];?></a><a href="JavaScript:void(0);" class="imsc-btn-mini" id="img_move"><i class="fa fa-arrows"></i><?php echo $lang['album_plist_move_album'];?></a>

          <a href="JavaScript:void(0);" class="imsc-btn-mini" onClick="submit_form('watermark')"><i class=" fa fa-clipboard"></i><?php echo $lang['album_plist_add_watermark'];?></a>

          <dd id="batchClass" style=" display:none;">
            <?php if(!empty($output['class_list'])){?>
            <span><?php echo $lang['album_plist_move_album_change'].$lang['im_colon'];?></span>
            ...
            <?php }else{?>
            <span><?php echo $lang['album_plist_move_album_only_one'];?><a href="JavaScript:void(0);" uri="index.php?act=store_album&op=album_add" im_type="dialog" dialog_title="<?php echo $lang['album_class_add'];?>"><?php echo $lang['album_class_add'];?></a><?php echo $lang['album_plist_move_album_only_two'];?></span>
            <?php }?>
          </dd>
          </dl>
          <?php }?></td>
        ...
        <li>
          <dl>
            <dt>
              ...
              <span onDblClick="_focus($(this).prev());" title="<?php echo $lang['album_plist_double_click_edit'];?>"><i class="fa fa-pencil"></i></span></dt>
            <dd class="date">
              <p><?php echo $lang['album_plist_upload_time'].$lang['im_colon'].date("Y-m-d",$v['upload_time'])?></p>
              <p><?php echo $lang['album_plist_original_size'].$lang['im_colon'].$v['apic_spec']?></p>
            </dd>
            <dd class="buttons">
              <div class="upload-btn"><a href="javascript:void(0);"> <span>
                <input type="file" name="file_<?php echo $v['apic_id'];?>" id="file_<?php echo $v['apic_id'];?>" class="input-file" size="1" hidefocus="true" imtype="replace_image" />
                </span>
                <div class="upload-button"><i class="fa fa-upload"></i><?php echo $lang['album_plist_replace_upload'];?></div>
              ...
              <a href="JavaScript:void(0);" im_type="dialog" dialog_title="<?php echo $lang['album_plist_move_album'];?>" uri="index.php?act=store_album&op=album_pic_move&cid=<?php echo $output['class_info']['aclass_id']?>&id=<?php echo $v['apic_id']?>"><i class="fa fa-arrows"></i><?php echo $lang['album_plist_move_album'];?></a> <a href="JavaScript:void(0);" onclick="cover(<?php echo $v['apic_id'];?>)"><i class="fa fa-picture-o"></i><?php echo $lang['album_plist_set_to_cover'];?></a> <a href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['album_plist_delete_confirm_message'];?>','index.php?act=store_album&op=album_pic_del&id=<?php echo $v['apic_id'];?>');"><i class="fa fa-trash-o"></i><?php echo $lang['album_plist_delete_img'];?></a> </dd>
          </dl>
        </li>
        <?php }?>
      </ul>
...
$(function() {
    //鼠标触及区域li改变class
    $(".imsc-picture-list ul li").hover(function() {
        $(this).addClass("hover");
    }, function() {
        $(this).removeClass("hover");
    });
    //凸显鼠标触及区域、其余区域半透明显示
    $(".imsc-picture-list > ul > li").jfade({
        start_opacity:"1",
        high_opacity:"1",
        low_opacity:".5",
        timing:"200"
    ...
    // 替换图片
    $('input[imtype="replace_image"]').each(function(){
        $(this).fileupload({
            dataType: 'json',
    ...
    $('#fileupload').fileupload({
        ...
        add: function (e,data) {
        	$.each(data.files, function (index, file) {
                $('<div imtype=' + file.name.replace(/\./g, '_') + '><p>'+ file.name +'</p><p class="loading"></p></div>').appendTo('div[imtype="file_loading"]');
            });
        	data.submit();
        },
        done: function (e,data) {
            var param = data.result;
            $this = $('div[imtype="' + param.origin_file_name.replace(/\./g, '_') + '"]');
            $this.fadeOut(3000, function(){
                $(this).remove();
                if ($('div[imtype="file_loading"]').html() == '') {
                    setTimeout("window.location.reload()", 1000);
                }
            });
            if(param.state == 'true'){
                upload_num++;
                $('div[imtype="file_msg"]').html('<i class="fa fa-check-circle">'+'</i>'+'<?php echo $lang['album_upload_complete_one'];?>'+upload_num+'<?php echo $lang['album_upload_complete_two'];?>');
...
function _save(obj){
		$.post('index.php?act=store_album&op=change_pic_name', {id:obj.id,name:obj.value}, function(data) {
			if(data == 'false'){
				showError('<?php echo $lang['im_common_op_fail'];?>');
			}else{
				showDialog('<?php echo $lang['im_common_op_succ']?>','succ');
			}
		});
