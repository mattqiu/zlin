...
<table class="imsc-default-table">
    ...
    <td class="nscs-table-handle">
        <span>
            <?php if($value['bind']) { ?>
            <a href="<?php echo urlShop('store_waybill', 'waybill_setting', array('store_waybill_id' => $value['store_waybill_id']));?>" class="btn-blue"><i class="fa fa-wrench"></i><p>设置</p></a></span><span><a href="javascript:;" imtype="btn_set_default" data-store-waybill-id="<?php echo $value['store_waybill_id'];?>" class="btn-green"><i class="fa fa-check-circle"></i><p>默认</p></a></span><span><a href="javascript:;" imtype="btn_unbind" data-store-waybill-id="<?php echo $value['store_waybill_id'];?>" class="btn-red"><i class="fa fa-chain-broken"></i><p>解绑</p></a></span>
        <?php } else { ?>
                    <span><a href="<?php echo urlShop('store_waybill', 'waybill_bind', array('express_id' => $value['id']));?>" class="btn-blue"><i class="fa fa-check-circle-o"></i><p>选择模板</p></a>
                    <?php } ?>
                </span>
            </td>
        </tr>
        ...
        <?php }else{?>
        <tr>
            <td colspan="20" class="norecord"><div class="warning-option"><i class="fa fa-exclamation-triangle"></i>
...
<script type="text/javascript">
    $(document).ready(function(){
        $('[imtype="btn_unbind"]').on('click', function() {
            if(confirm('确认解绑？')) {
        ...
        $('[imtype="btn_set_default"]').on('click', function() {
            $('#store_waybill_id').val($(this).attr('data-store-waybill-id'));
