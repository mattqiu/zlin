...
<div class="eject_con">
      ...
      <tr>
        ...
        <td class="goods-time">
          <div class="goods-storage">
            <input type="text" class="text w40" id="nums_item_<?php echo $val['goods_id'];?>" p-good-id="<?php echo $val['goods_id'];?>" b-good-id="<?php echo $val['b_id'];?>" data-nums="<?php echo $val['nums']; ?>" name="returned_num" value="<?php echo $val['nums']; ?>" onkeyup="change_nums(<?php echo $val['goods_id'];?>, this);"/>
          </div>		
		</td>
      </tr>
      <?php } ?>
  ...
  <div class="bottom">
    <a href="javascript:void(0);" class="imsc-btn imsc-btn-green mt10 mb10" name="returned_verify" imtype="verify_returned" op_id="30"><i class="fa fa-home"></i>同意(已收货)</a>
    <a href="javascript:void(0);" class="imsc-btn imsc-btn-acidblue mt10 mb10" name="returned_verify" imtype="verify_returned" op_id="20"><i class="fa fa-truck"></i>同意(未收货)</a>
    <a href="javascript:void(0);" class="imsc-btn imsc-btn-black mt10 mb10" name="returned_verify" imtype="verify_returned" op_id="10"><i class="fa fa-thumbs-down"></i>拒绝</a>
    <a href="javascript:void(0);" class="imsc-btn imsc-btn-orange mt10 mb10" name="returned_verify" imtype="verify_returned" op_id="0"><i class="fa fa-reply-all"></i>关闭</a>
  </div>
</div>
<script>
$(function(){
    $('a[imtype="verify_returned"]').click(function(){
		...
		ajax_get_confirm('确定'+msg+'分店退货吗?','<?php echo urlShop('seller_branch_stubbs', 'returned_save',array('bp_id'=>$output['apply_info']['bp_id']));?>&data=' + _items +'&op_id=' + _op_id);
    });
});
