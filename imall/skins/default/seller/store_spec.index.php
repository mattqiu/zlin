  ...
  <tr>
    ...
    <td>
        ...
        <?php if (!empty($output['gc_list'])) {?>
          <select imtype="gc" data-param="{deep:1}">
        ...
        <?php }?></span>
        <span imtype="gc2"></span>
        <span imtype="gc3"></span>
    </td>
    <td>&nbsp;</td>
  </tr>
</table>
<div imtype="class_spec" class="imsc-goods-spec">
  <div imtype="spec_ul" class="spec-tabmenu"></div>
  <div imtype="spec_iframe" class="spec-iframe">
...
$(function() {
    ...
    $('select[imtype="gc"]').change(function(){
        $(this).parents('td:first').nextAll().html('');
        $('div[imtype="spec_ul"]').html('');
        $('div[imtype="spec_iframe"]').html('');
        getClassSpec($(this));
    });
});
...
function getClassSpec($this) {
    ...
    $.getJSON('index.php?act=store_spec&op=ajax_class&id=' + id + '&deep=' + deep, function(data){
    	$('div[imtype="spec_iframe"]').empty();
        $('div[imtype="spec_ul"]').empty();
        if (data) {
            if (data.type == 'class') {
...
function nextClass(data, deep) {
    $('span[imtype="gc' + deep + '"]').html('').append('<select data-param="{deep:' + deep + '}"></select>')
        .find('select').change(function(){
            getClassSpec($(this));
        }).append('<option><?php echo $lang['im_please_choose'];?></option>');
    $.each(data, function(i, n){
        if (n != null) {
            $('span[imtype="gc' + deep + '"] > select').append('<option value="' + n.gc_id + '">' + n.gc_name + '</option>');
        }
    });
    // 清理分类
    clearClassHtml(parseInt(deep)+1);
...
function specList(data, deep, gcid) {
    if (typeof(data) != 'undefined' && data != '') {
        var $_ul = $('<ul></ul>');
        $.each(data, function(i, n){
            $_ul.append('<li><a href="javascript:void(0);" imtype="editSpec" data-param="{spid:'+ n.sp_id +',gcid:' + gcid + '}">编辑' + n.sp_name + '规格</a></li>');
        });
        ...
        $('div[imtype="spec_ul"]').append($_ul);
    } else {
        $('div[imtype="spec_ul"]').append('<div class="warning-option"><i class="fa fa-exclamation-triangle"></i><span>该分类不能添加规格</span></div>');
    }
    // 清理分类
    clearClassHtml(deep);
}
...
function clearClassHtml(deep) {
    switch (deep) {
        case 2:
            $('span[imtype="gc2"]').empty();
        case 3:
            $('span[imtype="gc3"]').empty();
            break;
    }
}
...
function editSpecValue(o) {
    ...
    $('div[imtype="spec_iframe"]').append($_iframe);
}
