...
<div class="imsc-oredr-show">
  <div class="imsc-order-info">
    <div class="imsc-order-details">
      <div class="title"><?php echo $lang['store_show_order_info'];?></div>
      <div class="content">
        <dl>
          <dt><?php echo $lang['store_show_order_receiver'].$lang['im_colon'];?></dt>
        ...
        <dl>
          <dt><?php echo $lang['store_show_order_buyer_message'].$lang['im_colon'];?></dt>
          <dd><?php echo $output['order_info']['extend_order_common']['order_message']; ?></dd>
        </dl>
        <dl class="line">
          <dt><?php echo $lang['store_order_order_sn'].$lang['im_colon'];?></dt>
          <dd><?php echo $output['order_info']['order_sn']; ?><a href="javascript:void(0);">更多<i class="fa fa-angle-down"></i>
            <div class="more"><span class="arrow"></span>
              <ul>
                ...
                <li><?php echo $lang['store_order_pay_method'].$lang['im_colon'];?><span><?php echo $output['order_info']['payment_name']; ?>
                ...
                <?php } ?>
                <li><?php echo $lang['store_order_add_time'].$lang['im_colon'];?><span><?php echo date("Y-m-d H:i:s",$output['order_info']['add_time']); ?></span></li>
                <?php if(intval($output['order_info']['payment_time'])) { ?>
                <li><?php echo $lang['store_show_order_pay_time'].$lang['im_colon'];?><span><?php echo date("Y-m-d H:i:s",$output['order_info']['payment_time']); ?></span></li>
                <?php } ?>
                <?php if($output['order_info']['extend_order_common']['shipping_time']) { ?>
                <li><?php echo $lang['store_show_order_send_time'].$lang['im_colon'];?><span><?php echo date("Y-m-d H:i:s",$output['order_info']['extend_order_common']['shipping_time']); ?></span></li>
                <?php } ?>
                <?php if(intval($output['order_info']['finnshed_time'])) { ?>
                <li><?php echo $lang['store_show_order_finish_time'].$lang['im_colon'];?><span><?php echo date("Y-m-d H:i:s",$output['order_info']['finnshed_time']); ?></span></li>
                <?php } ?>
              </ul>
            </div>
            </a></dd>
    ...
    <div class="imsc-order-condition">
      <dl>
    ...
    <div class="imsc-order-condition">
      ...
      <ul>
        ...
        <li>2. 可以在<a href="index.php?act=store_evaluate&op=list" class="imsc-btn-mini">评价管理</a>查看详细内容。</li>
      </ul>
    </div>
    <?php } else { ?>
    <div class="imsc-order-condition">
  ...
  <?php if ($output['order_info']['order_state'] != ORDER_STATE_CANCEL) { ?>
  <div id="order-step" class="imsc-order-step">
    ...
    <table class="imsc-default-table order">
      ...
      <tbody>
        ...
        <tr class="bd-line">
          ...
          <?php echo $goods['goods_pay_price']; ?> * <?php echo $goods['commis_rate']; ?>% = <b><?php echo imPriceFormat($goods['goods_pay_price']*$goods['commis_rate']/100); ?></b>
          <?php } ?>
          </td>
          ...
          <td class="bdl bdr" rowspan="<?php echo $output['order_info']['goods_count'];?>"><?php echo $output['order_info']['state_desc']; ?>
            ...
            <p><a href="javascript:void(0)" class="imsc-btn" uri="index.php?act=store_order&op=change_state&state_type=modify_price&order_sn=<?php echo $output['order_info']['order_sn']; ?>&order_id=<?php echo $output['order_info']['order_id']; ?>" dialog_width="480" dialog_title="<?php echo $lang['store_order_modify_price'];?>" im_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $output['order_info']['order_id']; ?>_action_adjust_fee" />修改运费</a></p>
            <?php }?>

            <!-- 取消订单 -->
            <?php if ($output['order_info']['if_cancel']) { ?>
            <p><a href="javascript:void(0)" style="color:#F30; text-decoration:underline;" im_type="dialog" uri="index.php?act=store_order&op=change_state&state_type=order_cancel&order_sn=<?php echo $output['order_info']['order_sn']; ?>&order_id=<?php echo $output['order_info']['order_id']; ?>" dialog_title="<?php echo $lang['store_order_cancel_order'];?>" dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $output['order_info']['order_id']; ?>_action_cancel" /><?php echo $lang['store_order_cancel_order'];?></a></p>
            <?php } ?>

            <!-- 发货 -->
            <?php if ($output['order_info']['if_send']&& empty($output['order_info']['up_id'])) {?>
            <p><a class="imsc-btn" href="index.php?act=store_deliver&op=send&order_id=<?php echo $output['order_info']['order_id']; ?>"/><i class="fa fa-truck"></i><?php echo $lang['store_order_send'];?></a></p>
            <?php } ?></td>
          <?php } ?>
          <!-- E 合并TD -->
        </tr>
        ...
        <tr class="bd-line">
          <td>&nbsp;</td>
          <td colspan="6" class="tl"><div class="imsc-goods-gift">赠品：
      ...
      <tfoot>
        ...
        <tr>
          <td colspan="20"><dl class="imsc-store-sales">
              <dt>其它信息<?php echo $lang['im_colon'];?></dt>
        ...
        <tr>
          <td colspan="20"><dl class="freight">
            ...
            <dl class="sum">
              <dt><?php echo $lang['store_order_sum'].$lang['im_colon'];?></dt>
