...
<ul class="add-goods-step">
  ...
  <li><i class="icon fa fa-pencil-square-o"></i>
    <h6>STEP.2</h6>
    <h2>填写商品详情</h2>
    <i class="arrow fa fa-angle-right"></i> </li>
...
<form method="post" id="goods_image" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_image'); } else { echo urlShop('store_goods_add', 'save_image');}?>">
  ...
  <div class="imsc-form-goods-pic">
    <div class="container">
      <?php foreach ($output['value_array'] as $value) {?>
      <div class="imsc-goodspic-list">
        <div class="title">
          <h3>颜色：<?php if (isset($output['value'][$value['sp_value_id']])) { echo $output['value'][$value['sp_value_id']];} else {echo $value['sp_value_name'];}?></h3>
        </div>
        <ul imtype="ul<?php echo $value['sp_value_id'];?>">
          <?php for ($i = 0; $i < 5; $i++) {?>
          <li class="imsc-goodspic-upload">
            <div class="upload-thumb"><img src="<?php echo cthumb($output['img'][$value['sp_value_id']][$i]['goods_image'], 240);?>" imtype="file_<?php echo $value['sp_value_id'] . $i;?>">
              <input type="hidden" name="img[<?php echo $value['sp_value_id'];?>][<?php echo $i;?>][name]" value="<?php echo $output['img'][$value['sp_value_id']][$i]['goods_image'];?>" imtype="file_<?php echo $value['sp_value_id'] . $i;?>">
            </div>
            <div class="show-default<?php if ($output['img'][$value['sp_value_id']][$i]['is_default'] == 1) {echo ' selected';}?>" imtype="file_<?php echo $value['sp_value_id'] . $i;?>">
              <p><i class="fa fa-check-circle-o"></i>默认主图
              ...
              </p><a href="javascript:void(0)" imtype="del" class="del" title="移除">X</a>
            </div>
            ...
            <div class="imsc-upload-btn"><a href="javascript:void(0);"><span><input type="file" hidefocus="true" size="1" class="input-file" name="file_<?php echo $value['sp_value_id'] . $i;?>" id="file_<?php echo $value['sp_value_id'] . $i;?>"></span><p><i class="fa fa-upload"></i>上传</p>
              </a></div>
            
          </li>
          <?php }?>
        </ul>
        <div class="imsc-select-album">
          <a class="imsc-btn" href="index.php?act=store_album&op=pic_list&item=goods_image&color_id=<?php echo $value['sp_value_id'];?>" imtype="select-<?php echo $value['sp_value_id'];?>"><i class="fa fa-picture-o"></i>从图片空间选择</a>
          <a href="javascript:void(0);" imtype="close_album" class="imsc-btn ml5" style="display: none;"><i class=" fa fa-arrow-circle-up"></i>关闭相册</a>
        </div>
        <div imtype="album-<?php echo $value['sp_value_id'];?>"></div>
      </div>
      <?php }?>
    </div>
...
$(function(){
    ...
    /* ajax打开图片空间 */
    <?php foreach ($output['value_array'] as $value) {?>
    $('a[imtype="select-<?php echo $value['sp_value_id'];?>"]').ajaxContent({
        event:'click', //mouseover
        loaderType:"img",
        loadingMsg:SHOP_SKINS_URL+"/images/loading.gif",
        target:'div[imtype="album-<?php echo $value['sp_value_id'];?>"]'
    }).click(function(){
        $(this).hide();
        $(this).next().show();
    });
