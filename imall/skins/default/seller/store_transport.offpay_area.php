...
.area-list {display:inline-block;width:200px;padding:5px 0px;}
  ...
  <form id="area_form" method="post">
    ...
    <table class="imsc-default-table order" id="table_area_box">
      <tbody>
...
<?php foreach ($areas['children'][$provinceId] as $cityId) { ?>
            <div class="area-list">
              <label>
                <input type="checkbox" im_province="<?php echo $provinceId; ?>"<?php if ($output['city_checked_array'][$cityId]) echo ' checked="checked"';?> value="<?php echo $cityId; ?>" name="city[]" />
              ...
              <a city_id="<?php echo $cityId; ?>" im_title="<?php echo $areas['name'][$cityId]; ?>" province_id="<?php echo $provinceId; ?>" im_type="edit" href="javascript:void(0);" title="选择下级地区"><i class="fa fa-pencil"></i></a>
            </div>
<?php } ?>
          </td>
        </tr>
<?php } ?>
      ...
      <tfoot>
        <tr class="tfoot">
          ...
          <a href="javascript:void(0);" class="imsc-btn imsc-btn-green" id="submitBtn"><span>提交保存</span></a>
          </div>
          </td>
        </tr>
      </tfoot>
...
var im_a = $.parseJSON('<?php echo $output['area_array_json']; ?>');
...
$(function(){
    ...
    $('input[name="province[]"]').on('click',function(){
        if ($(this).attr('checked') == 'checked'){
            $('input[im_province="' + $(this).val() + '"]').each(function(){
                $(this).attr('checked','checked');
                if (typeof im_a[$(this).val()] == 'object') {
                    county_array = im_a[$(this).val()];
                } else {
    ...
    //点击编辑事件
    $('a[im_type="edit"]').on('click',function(){
            if (typeof CUR_COUNTY[$(this).attr('city_id')] == 'object'){
                cur_county = CUR_COUNTY[$(this).attr('city_id')];
            }else{
                cur_county = new Array();
            }
            var province_array = im_a[$(this).attr('city_id')];
            if (typeof im_a[$(this).attr('city_id')] == 'object'){
                county_array = im_a[$(this).attr('city_id')];
            }else{
                county_array = new Array();
            }
            ...
            county_html += '</td></tr><tr><td style="padding:5px;"><a id="county_submit" class="imsc-btn imsc-btn-green" href="javascript:void(0);"><span>确认</span></a> <span style="color:#f30;">确认后，还需要点击页面底部的“提交保存”按钮完成保存操作</span></td></tr></tbody></table>';
            html_form('select_county', '选择 '+ $(this).attr('im_title') +' 子地区', county_html, 500,1);
    });
    ...
    $('input[name="city[]"]').on('click',function(){
        if ($(this).attr('checked')) {
            ...
            count = county_array.length;
            if ($('input[im_province="'+$(this).attr('im_province')+'"]').size() == $('input[im_province="'+$(this).attr('im_province')+'"]:checked').size()) {
                $('input[value="'+$(this).attr('im_province')+'"]').attr('checked',true);
            } else {
                $('input[value="'+$(this).attr('im_province')+'"]').attr('checked',false);
            }
        } else {
            CUR_COUNTY[$(this).val()] = undefined;
            count = 0;
    ...
    $('body').on('click','#county_submit',function(){
        ...
        if ($('input[im_province="'+cur_td.attr('province_id')+'"]').size() == $('input[im_province="'+cur_td.attr('province_id')+'"]:checked').size()) {
