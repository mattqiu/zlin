...
<?php if ($output['isOwnShop']) { ?>
  <a class="imsc-btn imsc-btn-green" href="<?php echo urlShop('store_promotion_mansong', 'mansong_add');?>"><i class="fa fa-plus-circle"></i><?php echo $lang['mansong_add'];?></a>

<?php } else { ?>

  <?php if(!empty($output['current_mansong_quota'])) { ?>
  <a class="imsc-btn imsc-btn-green" style="right:100px" href="<?php echo urlShop('store_promotion_mansong', 'mansong_add');?>"><i class="fa fa-plus-circle"></i><?php echo $lang['mansong_add'];?></a> <a class="imsc-btn imsc-btn-acidblue" href="<?php echo urlShop('store_promotion_mansong', 'mansong_quota_add');?>" title=""><i class="fa fa-money"></i>套餐续费</a>
  <?php } else { ?>
  <a class="imsc-btn imsc-btn-acidblue" href="<?php echo urlShop('store_promotion_mansong', 'mansong_quota_add');?>" title=""><i class="fa fa-money"></i>购买套餐</a>
  <?php } ?>
<?php } ?>
...
<div class="alert alert-block mt10">
  <?php if(!empty($output['current_mansong_quota'])) { ?>
  <strong>套餐过期时间<?php echo $lang['im_colon'];?></strong><strong style="color: #F00;"><?php echo date('Y-m-d H:i:s', $output['current_mansong_quota']['end_time']);?></strong>
  </li>
  <?php } else { ?>
...
<table class="imsc-default-table">
  ...
  <tbody>
    ...
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle"><span><a href="index.php?act=store_promotion_mansong&op=mansong_detail&mansong_id=<?php echo $val['mansong_id'];?>" class="btn-blue"><i class="fa fa-th-list"></i>
        <p><?php echo $lang['im_detail'];?></p>
        </a></span> <span><a imtype="btn_mansong_del" data-mansong-id="<?php echo $val['mansong_id'];?>" href="javascript:return void(0)" class="btn-red"><i class="fa fa-trash-o"></i>
...
<script type="text/javascript">
    $(document).ready(function(){
        $('[imtype="btn_mansong_del"]').on('click', function() {
            if(confirm('<?php echo $lang['im_ensure_cancel'];?>')) {
