...
<div class="imsc-form-default">
  <form id="add_form" action="index.php?act=store_groupbuy&op=groupbuy_save" method="post" enctype="multipart/form-data">
    <dl>
      <dt><i class="required">*</i><?php echo $lang['group_name'].$lang['im_colon'];?></dt>
      <dd>
...
<?php } ?>
    ...
    <dl>
      <dt><i class="required">*</i><?php echo $lang['groupbuy_goods'].$lang['im_colon'];?></dt>
      <dd>
      <div imtype="groupbuy_goods_info" class="selected-group-goods " style="display:none;">
      <div class="goods-thumb"><img id="groupbuy_goods_image" src=""/></div>
          <div class="goods-name">
          <a imtype="groupbuy_goods_href" id="groupbuy_goods_name" href="" target="_blank"></a>
          </div>
          <div class="goods-price">会员价：￥<span imtype="groupbuy_goods_price"></span></div>
      </div>
    ...
    <dl imtype="groupbuy_goods_info" style="display:none;">
      <dt><?php echo $lang['groupbuy_index_store_price'].$lang['im_colon'];?></dt>
      <dd> <?php echo $lang['currency'];?><span imtype="groupbuy_goods_price"></span><input id="input_groupbuy_goods_price" type="hidden"></dd>
    </dl>
    <dl>
      <dt><i class="required">*</i><?php echo $lang['groupbuy_price'].$lang['im_colon'];?></dt>
      <dd>
    ...
    <dl>
      ...
      <div class="imsc-upload-thumb groupbuy-pic">
          ...
          <img imtype="img_groupbuy_image" style="display:none;" src=""/></p>
      </div>
    ...
    <dl>
      <dt><?php echo $lang['groupbuy_class'].$lang['im_colon'];?></dt>
      <dd>
    ...
    <dl>
      <dt><?php echo $lang['virtual_quantity'].$lang['im_colon'];?></dt>
      <dd>
    ...
    <dl>
      <dt><?php echo $lang['sale_quantity'].$lang['im_colon'];?></dt>
      <dd>
    ...
    <dl>
      <dt><?php echo $lang['group_intro'].$lang['im_colon'];?></dt>
      <dd>
        <?php showEditor('groupbuy_intro','','740px','360px','','false',false);?>
        <p class="hr8"><a class="des_demo imsc-btn" href="index.php?act=store_album&op=pic_list&item=groupbuy"><i class="fa fa-picture-o"></i><?php echo $lang['store_goods_album_insert_users_photo'];?></a></p>
...
$(document).ready(function(){
    ...
    //选择商品
    $('#div_goods_search_result').on('click', '[imtype="btn_add_groupbuy_goods"]', function() {
        ...
        $.get('<?php echo urlShop('store_groupbuy', 'groupbuy_goods_info');?>', {goods_commonid: goods_commonid}, function(data) {
            if(data.result) {
                ...
                $('[imtype="groupbuy_goods_price"]').text(data.goods_price);
                $('#input_groupbuy_goods_price').val(data.goods_price);
                $('[imtype="groupbuy_goods_href"]').attr('href', data.goods_href);
                $('[imtype="groupbuy_goods_info"]').show();
                $('#div_search_goods').hide();
            } else {
...
<?php } ?>
        ...
        messages : {
            groupbuy_name: {
                required : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['group_name_error'];?>'
            },
            start_time : {
                required : '<i class="fa fa-exclamation-circle"></i>抢购开始时间不能为空',
                greaterThanDate : '<i class="fa fa-exclamation-circle"></i><?php echo sprintf('抢购开始时间必须大于{0}',date('Y-m-d H:i',$output['current_groupbuy_quota']['start_time']));?>'
            },
            end_time : {
                required : '<i class="fa fa-exclamation-circle"></i>抢购结束时间不能为空',
<?php if (!$output['isOwnShop']) { ?>
                lessThanDate : '<i class="fa fa-exclamation-circle"></i><?php echo sprintf('抢购结束时间必须小于{0}',date('Y-m-d H:i',$output['current_groupbuy_quota']['end_time']));?>',
<?php } ?>
            ...
            groupbuy_price: {
                ...
                lessThanGoodsPrice: '<i class="fa fa-exclamation-circle"></i>抢购价格必须小于商品价格',
