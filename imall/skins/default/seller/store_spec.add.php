...
<form id="spec_form" method="post" action="<?php echo SHOP_SITE_URL;?>/index.php?act=store_spec&op=save_spec">
  ...
  <div class="imsc-spec-info" style=" width: 960px;"><span>规格名称：<?php echo $output['sp_info']['sp_name'];?></span><span>所属分类：<?php echo $output['gc_info']['gc_name'];?></span><a href="javascript:void(0);" imtype="add_sv" class="imsc-btn"><i class="fa fa-th-list"></i>添加规格值</a></div>
  <table class="imsc-default-table" style="width: 960px;">
    <thead>
      <tr>
        <th class="w80">排序</th>
      ...
      <table class="imsc-default-table">
        <tbody imtype="spec_tbody">
          ...
          <tr class="bd-line">
            ...
            <?php if ($output['sp_info']['sp_id'] == DEFAULT_SPEC_COLOR_ID) {?>
            <td class="w300 tl"><input type="text" class="text w100" name="sv[old][<?php echo $value['sp_value_id'];?>][color]" value="<?php echo $value['sp_value_color'];?>" imtype="spec_color" /></td>
            <?php }?>
            <td class="w110 nscs-table-handle"><span><a href="javascript:void(0);" class="btn-red" imtype="spec_del" data-param="{id:<?php echo $value['sp_value_id'];?>}"><i class="fa fa-trash-o"></i>
              <p>删除</p>
              </a></span></td>
          </tr>
          <?php }?>
          ...
          <tr class="bd-line">
            ...
            <?php if ($output['sp_info']['sp_id'] == DEFAULT_SPEC_COLOR_ID) {?>
            <td class="w300 tl"><input type="text" class="text w100" name="sv[new][1][color]" imtype="spec_color" /></td>
            <?php }?>
            <td class="w110 nscs-table-handle"><span><a href="javascript:void(0);" class="btn-orange" imtype="tr_remove"><i class="fa fa-minus-circle"></i>
              <p>移除</p>
              </a></span></td>
          </tr>
          <?php }?>
        ...
        <tfoot>
          <tr>
            <td colspan="20" class="bottom"><a href="javascript:void(0);" imtype="submit-btn" class="submit mt20">提交保存规格值</a></td></tr><tr>
...
$(function(){
    // 提交表单
    $('a[imtype="submit-btn"]').click(function(){
        var _submit = true;
        $('#spec_form').find('input[name$="[name]"]').each(function(){
            if ($(this).val() == '') {
                _submit = false;
                if (!$(this).hasClass('error')) {
                    $(this).addClass('error').parents('td:first').append('<label class="error"><i class="fa fa-exclamation-circle"></i>请填写内容</label>');
                }
            }
        });
        if (_submit) {
    ...
    // 颜色选择器
    $('input[imtype="spec_color"]').colorpicker({showOn:'both'}).removeAttr('imtype');

    // 移除
    $('a[imtype="tr_remove"]').click(function(){
        $(this).parents('tr:first').remove();
    });

    // 删除
    $('a[imtype="spec_del"]').click(function(){
        var $this = $(this);
    ...
    // 添加规格值
    var i = 2;  // 自增，防止冲突
    $('a[imtype="add_sv"]').click(function(){
        ...
        <?php if ($output['sp_info']['sp_id'] == DEFAULT_SPEC_COLOR_ID) {?>
        $('<td class="w300 tl"><input type="text" class="text w100" name="sv[new]['+ i +'][color]" imtype="spec_color" /></td>').appendTo(_tr);  // 颜色色块
        <?php }?>
        
        $('<td class="w110 nscs-table-handle"><span><a href="javascript:void(0);" class="btn-orange"><i class="fa fa-minus-circle"></i><p>移除</p></a</span></td>').find('a').click(function(){
            $(this).parents('tr:first').remove();
        }).end().appendTo(_tr);   // 操作
        _tr.appendTo('tbody[imtype="spec_tbody"]');
        // 颜色选择器
        $('tbody[imtype="spec_tbody"]').find('input[imtype="spec_color"]').colorpicker({showOn:'both'}).removeAttr('imtype');
        i++;
    });
});
