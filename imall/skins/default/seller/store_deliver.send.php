...
.sticky .tabmenu { padding: 0;  position: relative; }
        ...
        <span class="fr mr5"> <a href="<?php echo urlShop('store_deliver', 'waybill_print', array('order_id' => $output['order_info']['order_id']));?>" class="imsc-btn-mini" target="_blank" title="打印运单"/><i class="fa fa-print"></i>打印运单</a></span>
<div class="wrap">
  ...
  <form name="deliver_form" method="POST" id="deliver_form" action="index.php?act=store_deliver&op=send&order_id=<?php echo $_GET['order_id'];?>" onsubmit="ajaxpost('deliver_form', '', '', 'onerror');return false;">
    ...
    <table class="imsc-default-table order deliver">
      <tbody>
        ...
        <tr>
          <th colspan="20"><a href="index.php?act=store_order_print&order_id=<?php echo $output['order_info']['order_id'];?>" target="_blank" class="fr" title="<?php echo $lang['store_show_order_printorder'];?>"/><i class="print-order"></i></a><span class="fr mr30"></span><span class="ml10"><?php echo $lang['store_order_order_sn'].$lang['im_colon'];?><?php echo $output['order_info']['order_sn']; ?></span><span class="ml20"><?php echo $lang['store_order_add_time'].$lang['im_colon'];?><em class="goods-time"><?php echo date("Y-m-d H:i:s",$output['order_info']['add_time']); ?></em></span>
        </tr>
        ...
        <tr>
          ...
          <td class="bdl bdr order-info w500" rowspan="<?php echo count($output['order_info']['extend_order_goods']);?>"><dl>
              <dt><?php echo $lang['store_deliver_shipping_amount'].$lang['im_colon'];?></dt>
              <dd>
            ...
            <dl>
              <dt><?php echo $lang['store_deliver_forget'].$lang['im_colon'];?></dt>
              <dd>
        ...
        <?php }?>
        <tr>
          <td colspan="20" class="tl bdl bdr" style="padding:8px" id="address"><strong class="fl"><?php echo $lang['store_deliver_buyer_adress'].$lang['im_colon'];?></strong><span id="buyer_address_span"><?php echo $output['order_info']['extend_order_common']['reciver_name'];?>&nbsp;<?php echo $output['order_info']['extend_order_common']['reciver_info']['phone'];?>&nbsp;<?php echo $output['order_info']['extend_order_common']['reciver_info']['address'];?></span><?php echo $output['order_info']['extend_order_common']['reciver_info']['dlyp'] ? '[自提服务站]' : '';?>
          <a href="javascript:void(0)" im_type="dialog" dialog_title="<?php echo $lang['store_deliver_buyer_adress'];?>" dialog_id="edit_buyer_address" uri="index.php?act=store_deliver&op=buyer_address_edit&order_id=<?php echo $output['order_info']['order_id'];?>" dialog_width="550" class="imsc-btn-mini fr"><i class="fa fa-pencil-square-o"></i><?php echo $lang['im_edit'];?></a></td>
        </tr>
        <?php } else { ?>
        <tr>
    ...
    <div class="deliver-sell-info"><strong class="fl"><?php echo $lang['store_deliver_my_daddress'].$lang['im_colon'];?></strong>
      <a href="javascript:void(0);" onclick="ajax_form('modfiy_daddress', '<?php echo $lang['store_deliver_select_daddress'];?>', 'index.php?act=store_deliver&op=send_address_select&order_id=<?php echo $output['order_info']['order_id'];?>', 640,0);" class="imsc-btn-mini fr"><i class="fa fa-pencil-square-o"></i><?php echo $lang['im_edit'];?></a> <span id="seller_address_span">
    ...
    <table class="imsc-default-table order" id="texpress1">
      <tbody>
        <tr>
          ...
          <td class="bdr w90 tc"><?php echo $lang['im_common_button_operate'];?></td>
        </tr>
        ...
        <tr>
          ...
          <td class="bdl"><input name="shipping_code" type="text" class="text w200 tip-r" title="<?php echo $lang['store_deliver_shipping_code_tips'];?>" maxlength="20" im_type='eb' im_value="<?php echo $v;?>" /></td>
          <td class="bdl gray" im_value="<?php echo $v;?>"></td>
          <td class="bdl bdr tc"><a im_type='eb' im_value="<?php echo $v;?>" href="javascript:void(0);" class="imsc-btn"><?php echo $lang['im_common_button_confirm'];?></a></td>
        </tr>
        <?php }?>
        <?php }?>
      </tbody>
    </table>
    <table class="imsc-default-table order" id="texpress2" style="display:none">
      <tbody>
        <tr>
          <td colspan="2"></td>
        </tr>
        <tr>
          ...
          <td class="bdr tl w400">&emsp;<a im_type='eb' im_value="e1000" href="javascript:void(0);" class="imsc-btn"><?php echo $lang['im_common_button_confirm'];?></a></td>
        </tr>
        <tr>
          <td colspan="2"></td>
        </tr>
...
$(function(){
	...
	$('a[im_type="eb"]').on('click',function(){
		if ($('input[im_value="'+$(this).attr('im_value')+'"]').val() == ''){
			showDialog('<?php echo $lang['store_deliver_shipping_code_pl'];?>', 'error','','','','','','','','',2);return false;
		}
		$('input[im_type="eb"]').attr('disabled',true);
		$('input[im_value="'+$(this).attr('im_value')+'"]').attr('disabled',false);
		$('#shipping_express_id').val($(this).attr('im_value'));
		$('#deliver_form').submit();
	});
    ...
    <?php if ($output['order_info']['shipping_code'] != ''){?>
    	$('input[im_value="<?php echo $output['order_info']['extend_order_common']['shipping_express_id'];?>"]').val('<?php echo $output['order_info']['shipping_code'];?>');
    	$('td[im_value="<?php echo $output['order_info']['extend_order_common']['shipping_express_id'];?>"]').html('<?php echo $output['order_info']['extend_order_common']['deliver_explain'];?>');
    <?php } ?>
