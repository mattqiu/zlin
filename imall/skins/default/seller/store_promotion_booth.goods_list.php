...
<?php if ($output['isOwnShop']) { ?>
  <a class="imsc-btn imsc-btn-green" href="javascript:void(0);" imtype="select_goods"><i class="fa fa-plus-circle"></i>添加商品</a>

<?php } else { ?>

  <?php if(empty($output['booth_quota'])) { ?>
  <a class="imsc-btn imsc-btn-acidblue" href="<?php echo urlShop('store_promotion_booth', 'booth_quota_add');?>" title="购买套餐"><i class="fa fa-money"></i>购买套餐</a>
  <?php } else { ?>
  <?php if ($output['booth_quota']['booth_state'] == 1) {?>
  <a class="imsc-btn imsc-btn-green" href="javascript:void(0);" imtype="select_goods" style="right:100px"><i class="fa fa-plus-circle"></i>添加商品</a>
  <?php } ?>
  <a class="imsc-btn imsc-btn imsc-btn-acidblue" href="<?php echo urlShop('store_promotion_booth', 'booth_renew');?>"><i class="fa fa-money"></i>套餐续费</a>
  <?php } ?>
<?php } ?>
...
<?php } else {?>
  <strong>套餐过期时间<?php echo $lang['im_colon'];?></strong> <strong style=" color:#F00;"><?php echo date('Y-m-d H:i:s',$output['booth_quota']['booth_quota_endtime']);?></strong>
<?php }?>
  <ul>
...
<div nvtype="div_goods_select" class="div-goods-select" style="display: none;">
    <table class="search-form">
      <tr><th class="w150"><strong>第一步：搜索店内商品</strong></th><td class="w160"><input imtype="search_goods_name" type="text w150" class="text" name="goods_name" value=""/></td>
        <td class="w70 tc"><label class="submit-border"><input imtype="btn_search_goods" type="button" value="<?php echo $lang['im_search'];?>" class="submit"/></label></td><td class="w10"></td><td><p class="hint">不输入名称直接搜索将显示店内所有出售中的商品</p></td>
      </tr>
    </table>
  <div imtype="div_goods_search_result" class="search-result"></div>
  <a imtype="btn_hide_goods_select" class="close" href="javascript:void(0);">X</a> </div>
<table class="imsc-default-table">
  ...
  <tbody imtype="choose_goods_list">
    <tr imtype="tr_no_promotion" style="display:none;">
    ...
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle">
        <span><a class="btn-red" href='javascript:void(0);' imtype="del_choosed" data-param="{gid:<?php echo $val['goods_id'];?>}"><i class="fa fa-trash-o"></i><p><?php echo $lang['im_del'];?></p></a></span></td>
    </tr>
    <?php }?>
    <?php } ?>
  </tbody>
...
$(function(){
    ...
    // 搜索商品
    $('input[imtype="btn_search_goods"]').click(function(){
        _url = '<?php echo urlShop('store_promotion_booth', 'booth_select_goods');?>';
        $('div[imtype="div_goods_search_result"]').html('').load(_url, {'goods_name':$('input[imtype="search_goods_name"]').val()});
    });
    $('div[nvtype="div_goods_select"]').on('click', '.demo', function(){
        $('div[imtype="div_goods_search_result"]').load($(this).attr('href'));
        return false;
    });

    // 选择商品
    $('div[nvtype="div_goods_select"]').on('click', 'a[imtype="a_choose_goods"]', function(){
        ...
        $.getJSON(_url, {gid : data_str.gid}, function(data){
            if (data.result == 'true') {
                ...
                $('<tr class="bd-line"></tr>')
                    ...
                    .append('<td class="nscs-table-handle"><span><a class="btn-red" href="javascript:void(0);" data-param="{gid:'+ data.goods_id +'}" imtype="del_choosed"><i class="fa fa-trash-o"></i><p>删除</p></a></span></td>')
                    .appendTo('tbody[imtype="choose_goods_list"]');
                // 验证是否已经选择商品
                choosed_goods();
            } else {
                showError(data.msg);
    ...
    // 删除商品
    $('tbody[imtype="choose_goods_list"]').on('click','a[imtype="del_choosed"]', function(){
        $this = $(this);
...
function choosed_goods() {
    if ($('tbody[imtype="choose_goods_list"]').children('tr').length == 1) {
        $('tr[imtype="tr_no_promotion"]').show();
    } else {
        $('tr[imtype="tr_no_promotion"]').hide();
    }
}
</script>