    ...
    <tbody>
        ...
        <tr>
            ...
            <td rowspan="2" class="nscs-table-handle vt">
                <span><a imtype="btn_show_explain_dialog" data-geval-id="<?php echo $v['geval_id'];?>" data-geval-content="<?php echo $v['geval_content'];?>" href="javascript:;" class="btn-acidblue">
        ...
        <tr class="bd-line" >
            <td class="tl" colspan="20">
                <div <?php echo empty($v['geval_explain'])?'style="display:none;"':''?>>
                    <strong>解释内容：</strong>
                    <span imtype="explain"><?php echo $v['geval_explain'];?></span>
                </div>
            </td>
        </tr>
        <?php }?>
...
$(document).ready(function(){
    ...
    $('[imtype="btn_show_explain_dialog"]').on('click', function() {
        $item = $(this).parents('tr').next('tr').find('[imtype="explain"]');
        ...
        $('#geval_explain').val('');
        $('#dialog_explain').im_show_dialog({title:'解释评价'});
        });
    ...
    $('#explain_form').validate({
        ...
        messages : {
        	geval_explain : {
                required : '<i class="fa fa-exclamation-circle"></i>解释内容不能为空'
            }
        }
    });
});
