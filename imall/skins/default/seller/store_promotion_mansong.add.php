...
<div class="imsc-form-default">
  <div class="alert alert-block">
    <h4><?php echo $lang['im_explain'].$lang['im_colon'];?></h4>
    <ul>
  ...
  <form id="add_form" action="index.php?act=store_promotion_mansong&op=mansong_save" method="post">
    ...
    <dl>
      <dt><i class="required">*</i><?php echo $lang['mansong_name'].$lang['im_colon'];?></dt>
      <dd>
    ...
    <dl>
      <dt><i class="required">*</i><?php echo $lang['start_time'].$lang['im_colon'];?></dt>
      <dd>
    ...
    <dl>
      <dt><i class="required">*</i><?php echo $lang['end_time'].$lang['im_colon'];?></dt>
      <dd>
      <input id="end_time" name="end_time" type="text" class="text w130"/><em class="add-on"><i class="fa fa-calendar"></i></em>
        <span class="error-message"></span>
        <p class="hint">
...
<?php } ?>
    ...
    <dl>
      <dt><i class="required">*</i>满即送规则<?php echo $lang['im_colon'];?></dt>
      <dd>
        <input type="hidden" id="mansong_rule_count" name="rule_count">
        <ul id="mansong_rule_list" class="imsc-mansong-rule-list">
        </ul>
        <a href="javascript:void(0);" id="btn_add_rule" class="imsc-btn imsc-btn-acidblue"><i class="fa fa-plus-circle"></i>添加规则</a>
        <div id="div_add_rule" style="display:none;">
        <div class="imsc-mansong-error"><span id="mansong_price_error" style="display:none;"><i class="fa fa-exclamation-circle"></i>规则金额不能为空且必须为数字</span><span id="mansong_discount_error" style="display:none;"><i class="fa fa-exclamation-circle"></i>满减金额必须小于规则金额</span></div>
        <div class="imsc-mansong-rule">
        <span>单笔订单满&nbsp;<input id="mansong_price" type="text" class="text w50"><em class="add-on">元</em>，</span>
        <span>立减现金&nbsp;<input id="mansong_discount" type="text" class="text w50"><em class="add-on">元</em>，</span>
        <span>送礼品&nbsp;<a href="javascript:void(0);" id="btn_show_search_goods" class="imsc-btn"><i class="fa fa-gift"></i>选择礼品</a></span> <div id="mansong_goods_item" class="gift"></div>
        ...
        <div id="div_search_goods" class="div-goods-select mt10" style="display: none;">
                    <table class="search-form">
                        <tr>
                            ...
                            <td class="w70 tc">
                                <a href="javascript:void(0);" id="btn_search_goods" class="imsc-btn"/><i class="fa fa-search"></i><?php echo $lang['im_search'];?></a>
                            </td>
                            <td class="w10"></td>
                            <td>
            ...
            <div class="mt10">
            <a href="javascript:void(0);" id="btn_save_rule" class="imsc-btn imsc-btn-acidblue"><i class="fa fa-check-circle-o"></i>确定规则设置</a>
            <a href="javascript:void(0);" id="btn_cancel_add_rule" class="imsc-btn imsc-btn-orange"><i class="fa fa-ban"></i>取消</a></div>
        </div>
        <span class="error-message"></span>
    ...
    <dl>
      <dt><?php echo $lang['text_remark'].$lang['im_colon'];?></dt>
      <dd>
...
<a imtype="btn_del_mansong_rule" href="javascript:void(0);" class="imsc-btn-mini imsc-btn-red"><i class="fa fa-trash-o"></i>删除</a>
</li>
</script>
<script id="mansong_goods_template" type="text/html">
    <div imtype="mansong_goods" class="selected-mansong-goods">
    <a href="<%=goods_url%>" title="<%=goods_name%>" class="goods-thumb" target="_blank">
        <img src="<%=goods_image_url%>"/>
    </a>
    <input imtype="mansong_goods_id" type="hidden" value="<%=goods_id%>">
    </div><a imtype="btn_del_mansong_goods" href="javascript:void(0);" class="imsc-btn-mini imsc-btn-red"><i class="fa fa-trash-o"></i>删除已选择的礼品</a>
</script>
...
<?php } ?>
        ...
        messages : {
            mansong_name : {
                required : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['mansong_name_error'];?>'
            },
            start_time : {
                required : '<i class="fa fa-exclamation-circle"></i><?php echo sprintf($lang['mansong_add_start_time_explain'],date('Y-m-d H:i',$output['start_time']));?>',
                greaterThanDate : '<i class="fa fa-exclamation-circle"></i><?php echo sprintf($lang['mansong_add_start_time_explain'],date('Y-m-d H:i',$output['start_time']));?>'
            },
            end_time : {
                required : '<i class="fa fa-exclamation-circle"></i><?php echo sprintf($lang['mansong_add_end_time_explain'],date('Y-m-d H:i',$output['end_time']));?>',
<?php if (!$output['isOwnShop']) { ?>
                lessThanDate : '<i class="fa fa-exclamation-circle"></i><?php echo sprintf($lang['mansong_add_end_time_explain'],date('Y-m-d H:i',$output['end_time']));?>',
<?php } ?>
                greaterThanStartDate : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['greater_than_start_time'];?>'
            },
            rule_count: {
    ...
    $('#btn_save_rule').on('click', function() {
        ...
        mansong.goods = $('#mansong_goods_item').find('[imtype="mansong_goods"]').html();
        mansong.goods_id = Number($('#mansong_goods_item').find('[imtype="mansong_goods_id"]').val());
    ...
    // 删除已添加的规则
    $('#mansong_rule_list').on('click', '[imtype="btn_del_mansong_rule"]', function() {
        $(this).parents('[imtype="mansong_rule_item"]').remove();
        close_div_add_rule();
    });
    ...
    function close_div_add_rule() {
        var rule_count = $('#mansong_rule_list').find('[imtype="mansong_rule_item"]').length;
        if( rule_count >= 3) {
            $('#btn_add_rule').hide();
        } else {
            $('#btn_add_rule').show();
    ...
    // 选择商品
    $('#div_goods_search_result').on('click', '[imtype="btn_add_mansong_goods"]', function() {
        var goods = {};
    ...
    // 删除以选的商品
    $('#mansong_goods_item').on('click', '[imtype="btn_del_mansong_goods"]', function() {
        $('#mansong_goods_item').html('');
    });
