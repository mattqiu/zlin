...
<?php if ($output['isOwnShop']) { ?>
  <a class="imsc-btn imsc-btn-green" href="<?php echo urlShop('store_promotion_xianshi', 'xianshi_add');?>"><i class="fa fa-plus-circle"></i><?php echo $lang['xianshi_add'];?></a>

<?php } else { ?>

  <?php if(!empty($output['current_xianshi_quota'])) { ?>
  <a class="imsc-btn imsc-btn-green" style="right:100px" href="<?php echo urlShop('store_promotion_xianshi', 'xianshi_add');?>"><i class="fa fa-plus-circle"></i><?php echo $lang['xianshi_add'];?></a> <a class="imsc-btn imsc-btn-acidblue" href="<?php echo urlShop('store_promotion_xianshi', 'xianshi_quota_add');?>" title=""><i class="fa fa-money"></i>套餐续费</a>
  <?php } else { ?>
  <a class="imsc-btn imsc-btn-acidblue" href="<?php echo urlShop('store_promotion_xianshi', 'xianshi_quota_add');?>" title=""><i class="fa fa-money"></i><?php echo $lang['xianshi_quota_add'];?></a>
  <?php } ?>
...
<div class="alert alert-block mt10">
  <?php if(!empty($output['current_xianshi_quota'])) { ?>
  <strong>套餐过期时间<?php echo $lang['im_colon'];?></strong><strong style="color:#F00;"><?php echo date('Y-m-d H:i:s', $output['current_xianshi_quota']['end_time']);?></strong>
  <?php } else { ?>
  ...
  <tbody id="xianshi_list">
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle tr">
          ...
          <span>
              <a href="javascript:;" imtype="btn_del_xianshi" data-xianshi-id=<?php echo $val['xianshi_id'];?> class="btn-red">
...
<script type="text/javascript">
    $(document).ready(function(){
        $('[imtype="btn_del_xianshi"]').on('click', function() {
