...
<style type="text/css">
.imsc-form-default dl dt { width: 16%;}
.imsc-form-default dl dd { width: 82%;}
</style>
...
<div class="imsc-form-default">
  <form method="post" action="<?php if (empty($output['plate_info'])) { echo urlShop('store_plate', 'plate_add');} else { echo urlShop('store_plate', 'plate_edit');}?>" id="plate_form">
    ...
    <input type="hidden" name="p_id" value="<?php echo $output['plate_info']['plate_id'];?>"/>
        ...
        <dl>
          <dt><i class="required">*</i>版式名称<?php echo $lang['im_colon'];?></dt>
          <dd>
        ...
        <dl>
          <dt><i class="required">*</i>版式位置<?php echo $lang['im_colon'];?></dt>
          <dd id="gcategory">
        ...
        <dl>
          <dt><i class="required">*</i>版式内容<?php echo $lang['im_colon'];?></dt>
          <dd>
            <?php showEditor('p_content',$output['plate_info']['plate_content'],'100%','480px','visibility:hidden;',"false",$output['editor_multimedia']);?>
            <p class="hr8">
              <a class="imsc-btn" imtype="show_desc" href="index.php?act=store_album&op=pic_list&item=des"><i class="fa fa-picture-o"></i>插入相册图片</a>
              <a href="javascript:void(0);" imtype="del_desc" class="imsc-btn ml5" style="display: none;"><i class=" fa fa-arrow-circle-up"></i>关闭相册</a>
            </p>
            <p id="des_demo"></p>
          </dd>
        </dl>
...
$(function(){
    $('#plate_form').validate({
        ...
        messages : {
            p_name : {
                required : '<i class="fa fa-exclamation-circle"></i>请填写版式名称',
                maxlength: '<i class="fa fa-exclamation-circle"></i>版式名称不能超过10个字符'
            },
            p_content : {
                required : '<i class="fa fa-exclamation-circle"></i>请填写版式内容'
            }
        }
    });

    // 版式内容使用
    $('a[imtype="show_desc"]').ajaxContent({
