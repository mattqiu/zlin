...
<div class="clear">&nbsp;</div>
<div id="footer">
	<p><a href="<?php echo SiteUrl;?>"><?php echo $lang['im_index'];?></a>
...
<div id="think_page_trace" class="trace">
  <fieldset id="querybox">
    <legend><?php echo $lang['im_debug_trace_title'];?></legend>
    <div>
...
var searchTxt = '<?php echo $lang['im_searchdefault']; ?>';
	       	...
	       	if(result.goods_all_num >0){
	            ...
	            for (i = 0; i < data.length; i++)
	            {
	            	...
	            	html+="<td class='name'><a href='<?php echo SiteUrl.'/';?>index.php?act=goods&goods_id="+data[i]['goodsid']+"' title='"+data[i]['gname']+"' target='_top'>"+data[i]['gname']+"</a></td>";
		          	html+="<td class='price'><p><?php echo $lang['currency'];?>"+data[i]['price']+"<?php echo $lang['im_sign_multiply']; ?>"+data[i]['num']+"</p><p><a href='javascript:void(0)' onClick='drop_topcart_item("+data[i]['storeid']+","+data[i]['specid']+");' style='color: #999;'><?php echo $lang['im_delete'];?></a></p></td>";
		          	html+="</tr>";
		        }
	         	html+="<tr><td colspan='3' class='no-border'><span class='all'><?php echo $lang['im_goods_num_one'];?><strong class='goods_num'>"+result.goods_all_num+"</strong><?php echo $lang['im_goods_num_two'].$lang['im_colon'];?><strong id='cart_amount'><?php echo $lang['currency'];?>"+result.goods_all_price+"</strong></span><span class='button' ><a href='<?php echo SiteUrl.'/';?>index.php?act=cart' target='_top' title='<?php echo $lang['im_accounts_goods'];?>' style='color: #FFF;' ><?php echo $lang['im_accounts_goods'];?></a></span></td></tr>";
	      }else{
	      	html="<div class='no-order'><span><?php echo $lang['im_cart_no_goods'];?></span><a href='<?php echo SiteUrl.'/';?>index.php?act=cart' class='button' target='_top' title='<?php echo $lang['im_check_cart'];?>' style=' color: #FFF;' ><?php echo $lang['im_check_cart'];?></a></div>";
	        }
