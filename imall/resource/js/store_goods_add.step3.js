$(function(){
    // 商品图片ajax上传
    $('.imsc-upload-btn').find('input[type="file"]').unbind().bind('change', function(){
        var id = $(this).attr('id');
        ajaxFileUpload(id);
    });
    //凸显鼠标触及区域、其余区域半透明显示
    ...
    // 关闭相册
    $('a[imtype="close_album"]').click(function(){
        $(this).hide();
        $(this).prev().show();
        $(this).parent().next().html('');
    });
    // 绑定点击事件
    $('div[imtype^="file"]').each(function(){
    ...
    $.ajaxFileUpload({
        ...
        success : function (data, status) {
                    if (typeof(data.error) != 'undefined') {
                        alert(data.error);
                        $('img[imtype="' + id + '"]').attr('src',DEFAULT_GOODS_IMAGE);
                    } else {
                        $('input[imtype="' + id + '"]').val(data.name);
                        $('img[imtype="' + id + '"]').attr('src', data.thumb_name);
                        selectDefaultImage($('div[imtype="' + id + '"]'));      // 选择默认主图
                    }
...
function selectDefaultImage($this) {
    ...
    // 删除
    $this.parents('li:first').find('a[imtype="del"]').click(function(){
...
function insert_img(name, src, color_id) {
    var $_thumb = $('ul[imtype="ul'+ color_id +'"]').find('.upload-thumb');
