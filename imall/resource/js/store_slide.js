...
$(function(){
    ...
    $('a[imtype="del"]').unbind().click(
        function() {
            ...
            obj.find('img:first').attr('src', SHOP_SKINS_URL + "/images/loading.gif");
            $.getJSON('index.php?act=store_setting&op=dorp_img', {file_id : file_id, img_src : img_src}, function(data) {
                obj.find('img:first').replaceWith('<i class="fa fa-picture-o"></i>');
...
{
    $('div[imtype="'+id+'"]').find('i').remove().end().find('img').remove()
            .end().prepend('<img imtype="'+id+'" scr="'+SHOP_SKINS_URL+'/images/loading.gif">');
	...
	(
		{
			...
			success: function (data, status)
			{
				...
				{
					alert(data.error);
					$('img[imtype="'+id+'"]').attr('src',UPLOAD_SITE_URL+'/'+ATTACH_COMMON+'/default_goods_image.gif');
				}else{
					$('input[imtype="'+id+'"]').val(data.file_name).attr('file_id',data.file_id);
