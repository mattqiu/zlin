(function($) {
	...
	$.fn.jfocusV = function(settings) {//首页焦点广告图切换
		var defaults = {
			time: 5000,
			num : 6
		};
		...
		return this.each(function(){
			...
			var len = Math.ceil($this.find("ul li").length/settings.num);
			var index = 0;
			var picTimer;
			var btn = "<div class='pagination'>";
			for (var i = 0; i < len; i++) {
			...
			$this.find("ul").css("height", sHeight * (len));
			$this.hover(function() {
				clearInterval(picTimer);
			},
			function() {
...
$(function(){
	...
	//首页Tab标签卡滑门切换
    $(".top-navbar > .middle-layout > .tabs-nav > li > h3").bind('mouseover', (function(e) {
    	if (e.target == this) {			
    		var tabs = $(this).parent().parent().children("li");
    		var panels = $(this).parent().parent().parent().parent().parent().children(".panel_box");
    		var index = $.inArray(this, $(this).parent().parent().find("h3"));
    ...
    window.onscroll = function(){
        ...
        $('.nav-Sidebar-layout').each(function(index){
            var that = $(this);
            that.index = index;
        ...
        if(_arr.length){
            $('.nav_Sidebar a').eq(_arr[_arr.length-1]).css({'backgroundImage':'url('+SHOP_SITE_URL+'/skins/default/images/home-nav-icon-hover.png)'}).addClass('current').siblings().css({'backgroundImage':'url('+SHOP_SITE_URL+'/skins/default/images/home-nav-icon.png)'}).removeClass('current');
            _arr = [];
        }
    }
    $('.nav_Sidebar a').each(function(index){
        $(this).click(function(){
            $('html,body').animate({scrollTop: $('.nav-Sidebar-layout').eq(index).offset().top - 20 + 'px'}, 500);
        }).mouseover(function(){
            if($(this).hasClass('current')){
                return;
            }else{
                $(this).css({'backgroundImage':'url('+SHOP_SITE_URL+'/skins/default/images/home-nav-icon-hover2.png)'});
            }
        }).mouseout(function(){
            if($(this).hasClass('current')){
                return;
            }else{
                $(this).css({'backgroundImage':'url('+SHOP_SITE_URL+'/skins/default/images/home-nav-icon.png)'});
            }
        });
    });
