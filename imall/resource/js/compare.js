	...
	if (gid > 0){
       	$.ajax({
            ...
            success: function(data){
                if(data.done == true) {
                	$("[im_type='compare_"+gid+"']").addClass('selected');
                	loadCompare(true);
	...
	$.ajax({
        ...
        success: function(data){  
            if(data.done == true) {
            	...
            	if(type == 'mini'){
            		if(gid == 'all'){
            			$("[im_type^='compare_']").removeClass('selected');
            		} else {
            			$("[im_type='compare_"+gid+"']").removeClass('selected');
            		}
            	}
            	//加载对比信息
            	if (type == 'mini'){
...
function initCompare(){
	//绑定对比按钮事件
	$("[im_type^='compare_']").bind('click',function(){
	...
	$.getJSON('index.php?act=compare&op=checkcompare', function(data){
        if(data){
			$.each(data, function(i,val){
				$("[im_type='compare_"+val+"']").addClass('selected');
			});
        }
    });
}