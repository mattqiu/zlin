$(function(){
    ...
    $('#goods_image').fileupload({
        dataType: 'json',
        url: SITEURL + '/index.php?act=supplier_goods_add&op=image_upload&upload_type=uploadedfile',
        formData: {name:'goods_image'},
        add: function (e,data) {
        	$('img[nctype="goods_image"]').attr('src', SHOP_SKINS_URL + '/images/loading.gif');
            data.submit();
        },
    ...
    $('#add_album').fileupload({
        dataType: 'json',
        url: SITEURL+'/index.php?act=supplier_goods_add&op=image_upload',
    ...
    // 商品所在地
    $("#region").im_region({show_deep:2,tip_type:1});
    ...
    // 计算三级分佣比例
    $('input[name="fx_commission_two"],input[name="fx_commission_three"]').change(function(){
        threeCommissionCalculator();
    });
    ...
    $('a[nctype="mb_add_img"]').click(function(){
        $('a[nctype="meat_cancel"]').click();
        $('div[nctype="mea_img"]').show().load('index.php?act=supplier_album&op=pic_list&item=mobile');
    });
    // 关闭 图片选择
    ...
    $('div[nctype="mobile_pannel"]').on('click', 'a[nctype="mp_rpl"]', function(){
        $('a[nctype="meat_cancel"]').click();
        $('div[nctype="mea_img"]').show().load('index.php?act=supplier_album&op=pic_list&item=mobile&type=replace');
    });
    // 插图图片
...
function commission(){
    // 计算最高分拥比例
    var _commission = 0;var _commission_sign = false;
    $('input[data_type="fx_commission"]').each(function(){
        if($(this).val() != '' && $(this)){
            if(!_commission_sign){
                _commission = parseInt($(this).val());
                _commission_sign = true;
            }else{
                _commission = (parseInt($(this).val())  < _commission) ? _commission : parseInt($(this).val());
            }
        }
    });
    $('input[name="fx_commission"]').val(_commission);
}
// 计算最低三级分拥比例
function minCommission(){
    // 计算最低三级分拥比例
    var _commission = 0;var _commission_sign = false;
    $('input[data_type="fx_commission_three"]').each(function(){
        if($(this).val() != '' && $(this)){
            if(!_commission_sign){
                _commission = parseInt($(this).val());
                _commission_sign = true;
    ...
    $('input[name="fx_commission_three"]').val(_commission);
    var sign = 0;
    $($('input[data_type="fx_commission_three"]')).each(function(){
        if($(this).val() == _commission){
            sign = $(this).attr('nc_type').split('|');
        }
    });
    var one = $("input[nc_type='"+sign[0]+"|fx_commission_one']").val();
    var two = $("input[nc_type='"+sign[0]+"|fx_commission_two']").val();
    $('input[name="fx_commission_two"]').val(two);
    $('input[name="fx_commission_one"]').val(one);
}
...
// 计算三级分拥比例
function threeCommissionCalculator() {
    var fx_commission_one = parseInt($("input[name='fx_commission_one']").val());
    var fx_commission_two = parseInt($("input[name='fx_commission_two']").val());
    var fx_three = parseInt(100 - fx_commission_one - fx_commission_two);
    $("input[name='fx_commission_three']").val(fx_three);
}

// 多规格商品计算三级分拥比例
function fxCommission() {
    $("input[data_type='fx_commission_three']").each(function(){
        var sign = $(this).attr('nc_type').split("|");//分割标识
        //查找对应标识的一二级比例
        var fx_commission_one = parseInt($("input[nc_type='"+sign[0]+"|fx_commission_one']").val());
        var fx_commission_two = parseInt($("input[nc_type='"+sign[0]+"|fx_commission_two']").val());
        if(!isNaN(fx_commission_one) && !isNaN(fx_commission_two)){
            var value = 100 - fx_commission_one - fx_commission_two;
            $(this).val(value);
        }
