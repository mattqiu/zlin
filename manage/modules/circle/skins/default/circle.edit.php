...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?act=circle_manage&op=circle_list" title="返回圈子列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
        <h3><?php echo $lang['im_circle_manage'];?> - <?php echo $output['circle_info']['circle_status'] == 2 ? '审核' : $lang['im_edit'];?>圈子“<?php echo $output['circle_info']['circle_name'];?>”</h3>
        <h5><?php echo $lang['im_circle_manage_subhead'];?></h5>
      </div>
    </div>
  </div>
  <form id="circle_form" method="post">
    ...
    <div class="imap-form-default">
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <div class="onoff">
            <label for="c_recommend1" class="cb-enable <?php if($output['circle_info']['is_recommend'] == 1) echo 'selected';?>" ><?php echo $lang['im_yes'];?></label>
            <label for="c_recommend0" class="cb-disable <?php if($output['circle_info']['is_recommend'] == 0) echo 'selected';?>" ><?php echo $lang['im_no'];?></label>
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <div class="onoff">
            <label for="is_hot1" class="cb-enable <?php if($output['circle_info']['is_hot'] == 1) echo 'selected';?>" ><?php echo $lang['im_yes'];?></label>
            <label for="is_hot0" class="cb-disable <?php if($output['circle_info']['is_hot'] == 0) echo 'selected';?>" ><?php echo $lang['im_no'];?></label>
		...
		(
			{
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['im_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?act=common&op=pic_cut&type=circle&x=120&y=120&resize=1&ratio=1&filename=<?php echo UPLOAD_SITE_URL.'/'.ATTACH_CIRCLE;?>/group/<?php echo $_GET['c_id'];?>.jpg&url='+data.url,690);
					}else{
						alert(data.msg);
					}
