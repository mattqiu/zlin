        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if($circleid){
                    ...
                    $this->log(L('im_add_circle').'['.$circleid.']');
                    showMessage(L('im_common_op_succ'));
                }else{
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if($rs){
                    ...
                    $this->log(L('im_edit_circle').'['.intval($_POST['c_id']).']');
                    showMessage(L('im_common_op_succ'), 'index.php?act=circle_manage&op=circle_list');
                }else{
                    showMessage(L('im_common_op_fail'));
                }
            }
...
Tpl::showpage('circle.choose_master', 'null_layout');
    ...
    public function circle_delOp(){
        ...
        $this->log(L('im_del_circle').'['.$id.']');
        exit(json_encode(array('state'=>true,'msg'=>L('im_common_op_succ'))));
    }
    /**
     * 会员名称检测
     */
    ...
    public function ajaxOp(){
        switch ($_GET['branch']){
            case 'status':
                $this->log(L('im_circle_pass_cerify').'['.intval($_GET['id']).']');
            case 'recommend':
                $update = array(
