...
Tpl::showpage('microshop_store.add');
    ...
    public function store_add_saveOp()
    {
        ...
        if($result) {
            ...
            showMessage(Language::get('im_common_op_fail'),'','','error');
        }
    }
    ...
    public function store_sort_updateOp() {
        ...
        if ($new_sort > 255){
            ...
            if($result) {
                echo json_encode(array('result'=>TRUE,'message'=>'im_common_op_succ'));
                die;
            } else {
                echo json_encode(array('result'=>FALSE,'message'=>Language::get('im_common_op_fail')));
                die;
            }
        }
    }
    ...
    private function show_menu_store($menu_key) {
        $menu_array = array(
                'store_manage'=>array('menu_type'=>'link','menu_name'=>Language::get('im_manage'),'menu_url'=>'index.php?act=store&op=store_manage'),
                'store_add'=>array('menu_type'=>'link','menu_name'=>Language::get('im_new'),'menu_url'=>'index.php?act=store&op=store_add'),
        );
