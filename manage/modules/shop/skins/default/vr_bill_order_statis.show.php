  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base im-row">
        <li><a href="index.php?act=vr_bill">结算管理</a></li>
        <li><a class="current" href="JavaScript:void(0);">账单列表</a></li>
      </ul>
    </div>
  </div>
  <!-- 操作说明 -->
        ...
        <div id="searchCon" class="content">
          <div class="layout-box">
            <dl>
              <dt>账单编号</dt>
              <dd>
                <input type="text" value="" name="ob_no" id="ob_no" class="s-input-txt">
              </dd>
            </dl>
            <dl>
              <dt>商家名称</dt>
...
$(function(){
    ...
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=vr_bill&op=get_bill_xml&os_month=<?php echo $_GET['os_month'];?>'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?act=vr_bill&op=get_bill_xml&os_month=<?php echo $_GET['os_month'];?>',
        colModel : [
            {display: '操作', name : 'operation', width : 60, sortable : false, align: 'center', className: 'handle-s'},
            {display: '账单编号', name : 'ob_no', width : 70, sortable : true, align: 'center'}, 
        ...
        searchitems : [
       {display: '账单编号', name : 'ob_no', isdefault: true},
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString()+'&op=export_bill&os_month=<?php echo $_GET['os_month'];?>&ob_no='+id;
}
</script> 
