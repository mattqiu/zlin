    ...
    <div class="imap-form-default">
      ...
      <dl class="row">
        ...
        <dd class="opt">
          ...
          <span class="err"></span>
          <p class="notic">请输入面额，面额不可超过10000</p>
        </dd>
      </dl>
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <input class="input-txt" type="text" name="buy_limit" />
          <span class="err"></span>
          <p class="notic">限制每个会员充值的次数，0表示不限制</p>
        </dd>
      </dl>
      <dl class="row">
        <dt class="tit">
          <label><em>*</em>卡的类型</label>
        </dt>
        <dd class="opt">
          <label class="mr15">
          <input class="radio cardgrant" type="radio" name="card_grant" value="0" checked="checked"/> 领取</label>
          <label class="mr15">
          <input class="radio cardgrant" type="radio" name="card_grant" value="1" /> 换购</label>
        </dd>
      </dl>
      <dl class="row cardgrant-target"  style="display:none;">
        <dt class="tit">
          <label><em>*</em>充值卡金额</label>
        </dt>
        <dd class="opt">
          <input class="input-txt" type="text" name="rcb_amount" />
          <span class="err"></span>
          <p class="notic">如果在换购模式下，请输入充值卡金额，如果为0，则表示只送云币就直接绑定用户</p>
        </dd>
      </dl>
      
      <dl class="row  cardgrant-target"  style="display:none;">
        <dt class="tit">
          <label><em>*</em>赠送云币</label>
        </dt>
        <dd class="opt">
          <input class="input-txt" type="text" name="card_points" />
          <span class="err"></span>
          <p class="notic">请输入购买此卡赠送多少云币</p>
        </dd>
      </dl>
      ...
      <dl class="row cardgrant-target"  style="display:none;">
        ...
        <dd class="opt">
          <input class="input-txt" type="text" name="give_exppoints" />
          <span class="err"></span>
          <p class="notic">请输入购买此卡赠送多少经验值,经验值是和会员的等级有关</p>
        </dd>
      </dl>
      <dl class="row cardgrant-target"  style="display:none;">
        <dt class="tit">
          <label><em>*</em>换购积分</label>
        </dt>
        <dd class="opt">
          <input class="input-txt" type="text" name="pd_amount" />
          <span class="err"></span>
          <p class="notic">请输入积分（积分），如果为0。就直接绑定用户</p>
        </dd>
      </dl>
      ...
      <dl class="row cardgrant-target"  style="display:none;">
        ...
        <dd class="opt">
          <input class="input-txt" type="text" name="commis_amount" />
          <span class="err"></span>
          <p class="notic">请输入上级返佣的直接金额，系统将被邀请人第一次充值后的钱返佣给邀请人</p>
        </dd>
      </dl>
      <dl class="row cardgrant-target"  style="display:none;">
        <dt class="tit">
        ...
        <dd class="opt">
          <input class="input-txt" type="text" name="commis_points" />
          <span class="err"></span>
          <p class="notic">请输入返给邀请人的云币，系统将被邀请人充值成功后的云币返给邀请人</p>
        </dd>
      </dl>
...
$('.cardgrant').click(function() {
    var i = parseInt(this.value);
    if(i==1){
    	$('.cardgrant-target').show();
    }else{
    	$('.cardgrant-target').hide();
    }
});
...
$("#form_add").validate({
    ...
    messages : {
        denomination : {
            ...
            max: '<i class="fa fa-exclamation-circle"></i>面额不能大于100000'
        },
        batchflag : {
