...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?act=goods_class&op=goods_class&gc_id=<?php echo $output['class_array']['gc_parent_id']?>" title="返回商品分类列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
        <h3><?php echo $lang['goods_class_index_class'];?> - <?php echo $lang['im_edit'];?>“<?php echo $output['class_array']['gc_name'];?>”</h3>
  ...
  <form id="goods_class_form" name="goodsClassForm" enctype="multipart/form-data" method="post">
    ...
    <div class="imap-form-default">
      ...
      <dl class="row">
        <dt class="tit"><?php echo $lang['goods_class_add_if_recommend'];?></dt>
        <dd class="opt">
          <div class="onoff">
            <label for="gc_recommend1" class="cb-enable <?php if($output['class_array']['gc_recommend'] == 1){ ?> selected<?php } ?>" ><?php echo $lang['im_yes'];?></label>
            <label for="gc_recommend0" class="cb-disable <?php if($output['class_array']['gc_recommend'] == 0){ ?> selected<?php } ?>" ><?php echo $lang['im_no'];?></label>
            <input id="gc_recommend1" name="gc_recommend" <?php if($output['class_array']['gc_recommend'] == 1){ ?>checked="checked"<?php } ?> value="1" type="radio">
            <input id="gc_recommend0" name="gc_recommend" <?php if($output['class_array']['gc_recommend'] == 0){ ?>checked="checked"<?php } ?> value="0" type="radio">
          </div>
          <p class="notic"><?php echo $lang['goods_class_add_if_recommend_tip'];?></p>
        </dd>
      </dl>
      <?php if ($output['class_array']['gc_parent_id'] == 0) {?>
      <dl class="row">
        <dt class="tit">
          <label for="pic"><?php echo '分类图标';?></label>
        </dt>
        <dd class="opt">
          <div class="input-file-show">
            <span class="show">
              <a class="nyroModal" rel="gal" href="<?php echo $output['class_array']['pic'];?>"> 
                <i class="fa fa-picture-o" onMouseOver="toolTip('<img src=<?php echo $output['class_array']['pic'];?>>')" onMouseOut="toolTip()"></i> 
              </a>
            </span>
            <span class="type-file-box">
              <input class="type-file-file" id="pic" name="pic" type="file" size="30" im_type="change_pic" hidefocus="true" title="点击前方预览图可查看大图，点击按钮选择文件并提交表单后上传生效">
              <input type="text" name="textfield" id="textfield1" class="type-file-text" />
              <input type="button" name="button" id="button1" value="选择上传..." class="type-file-button" />
            </span>
          </div>
          <p class="notic"><?php echo '只有第一级分类可以上传图片，建议用16px * 16px，超出后自动隐藏';?></p>
        </dd>
      </dl>
      <?php } ?>
      <dl class="row">
      ...
      <dl class="row">
        ...
        <dd class="opt">
          ...
          <p class="notic mb10"><?php echo $lang['goods_class_add_type_desc_one'];?><a onclick="window.parent.openItem('shop|type')" href="JavaScript:void(0);" class="imap-btn mr5 ml5"><?php echo $lang['im_type_manage'];?></a><?php echo $lang['goods_class_add_type_desc_two'];?></p>
      ...
      <dl class="row">
        <dt class="tit">
          <label for="gc_sort"><?php echo $lang['im_sort'];?></label>
        </dt>
        <dd class="opt">
