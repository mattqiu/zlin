...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?act=type&op=type" title="返回列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
        <h3><?php echo $lang['im_type_manage'];?> - <?php echo $lang['im_edit'];?>“<?php echo $output['type_info']['type_name'];?>”</h3>
        <h5><?php echo $lang['im_type_manage_subhead'];?></h5>
      </div>
    </div>
  </div>
  ...
  <form id="type_form" method="post">
    ...
    <div class="imap-form-default">
      <dl class="row">
        <dt class="tit">
          <label for="t_name"><em>*</em><?php echo $lang['type_index_type_name'];?></label>
        </dt>
        <dd class="opt">
          <input type="text" class="input-txt" name="t_name" id="t_name" value="<?php echo $output['type_info']['type_name'];?>" />
          <span class="err"></span>
          <p class="notic"></p>
      </dl>
      <dl class="row">
        ...
        <dd class="opt">
          <div id="gcategory">
            ...
            <input class="edit_gcategory" type="button" value="<?php echo $lang['im_edit'];?>">
            <?php }?>
      ...
      <dl class="row">
        <dt class="tit">
          <label for="t_sort"><em>*</em><?php echo $lang['im_sort'];?></label>
        </dt>
        <dd class="opt">
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <div id="speccategory">快捷定位
            ...
            分类下对应的规格<a class="imap-btn" imtype="spec_hide" href="javascript:void(0);"><?php echo $lang['type_common_checked_hide'];?></a></div>
          <div id="spec_div" class="scrollbar-box">
            <div class="imap-type-spec-list">
              <input type="hidden" value="" name="spec[form_submit]" im_type="submit_value" />
              ...
              <dl>
                ...
                <dd>
                  ...
                  <label>
                    <input class="checkitem" im_type="change_default_spec_value" type="checkbox" <?php if(in_array($v['sp_id'], $output['spec_related'])){ echo 'checked="checked"';}?> value="<?php echo $v['sp_id'];?>" name="spec_id[]">
              ...
              <?php }?>
              <?php }else{?>
              <div><?php echo $lang['type_add_spec_null_one'];?><a href="JavaScript:void(0);" onclick="window.parent.openItem('shop|spec')"><?php echo $lang['im_spec_manage'];?></a><?php echo $lang['type_add_spec_null_two']?></div>
              <?php }?>
            </div>
          </div>
        </dd>
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <div id="brandcategory">快捷定位
            ...
            分类下对应的品牌 <a class="imap-btn" imtype="brand_hide" href="javascript:void(0);"><?php echo $lang['type_common_checked_hide'];?></a></div>
          <div id="brand_div" class="scrollbar-box">
            <div class="imap-type-spec-list">
              <?php if(is_array($output['brand_list']) && !empty($output['brand_list'])) {?>
              <input type="hidden" value="" name="brand[form_submit]" im_type="submit_value" />
              ...
              <?php }?>
              <?php }else{?>
              <div><?php echo $lang['type_add_brand_null_one'];?><a href="JavaScript:void(0);" onclick="window.parent.openItem('shop|brand')"><?php echo $lang['im_brand_manage'];?></a><?php echo $lang['type_add_brand_null_two']?></div>
              <?php }?>
            </div>
          </div>
        </dd>
      ...
      <dl class="row">
        <dt class="tit"><?php echo $lang['type_add_attr_add'].$lang['im_colon'];?></dt>
        <dd class="opt">
          <ul class="imap-ajax-add" id="ul_attr">
            <?php if (is_array($output['attr_list']) && !empty($output['attr_list'])){?>
            <?php foreach ($output['attr_list'] as $aval){?>
            <li>
              <input type="hidden" value="" name="at_value[<?php echo $aval['attr_id'];?>][form_submit]" im_type="submit_value" />
              <input type="hidden" value="<?php echo $aval['attr_id'];?>" name="at_value[<?php echo $aval['attr_id'];?>][a_id]" im_type='ajax_attr_id' />
              <label>删
                <input type="checkbox" name="a_del[<?php echo $aval['attr_id'];?>]" value="<?php echo $aval['attr_id'];?>" />
              </label>
              <label title="<?php echo $lang['im_sort'];?>">
              ...
              <label> <?php echo $lang['im_display'];?>
              ...
              <label><a href="javascript:void(0);" onclick="ajax_form('attr_deit', '编辑属性', '<?php echo urlAdminShop('type', 'attr_edit', array('attr_id' => $aval['attr_id']));?>', 640)" class="imap-btn"><?php echo $lang['im_edit'];?></a></label>
            </li>
            <?php }?>
            <?php }?>
            <a id="add_type" class="imap-btn" href="JavaScript:void(0);"><i class="fa fa-plus"></i><?php echo $lang['type_add_attr_add_one'];?></a>
          </ul>
        </dd>
      </dl>
      <dl class="row">
        ...
        <dd class="opt">
          <ul class="imap-ajax-add" id="ul_custom">
            ...
            <?php }?>
            <?php }?>
            <a id="add_custom" class="imap-btn" style=" display: block; clear: both; width: 100px;" href="JavaScript:void(0);"><i class="fa fa-plus"></i>添加自定义属性</a>
          </ul>
          <p class="notic">自定义属性用于为商家自行添加某些属性值的预留选项，平台只需建立并设定属性名称即可，属性值由商家自行添加。注意：自定义属性不能作为商品检索项使用。</p>
        </dd>
      </dl>
      <div class="bot"><a id="submitBtn" class="imap-btn-big imap-btn-green" href="JavaScript:void(0);"><?php echo $lang['im_submit'];?></a></div>
    </div>
  </form>
</div>
...
$(function(){
    ...
    var ul_attr = '<li>' +
        ...
        '<label title="<?php echo $lang['im_sort'];?>"><input type="text" class="w30" name="at_value[key][sort]" value="0" /></label>' +
        ...
        '<label><?php echo $lang['im_display'];?>&nbsp;<input type="checkbox" checked="checked" name="at_value[key][show]" value="1" /></label>' +
        '<label><a onclick="remove_attr($(this));" class="imap-btn imap-btn-red" href="JavaScript:void(0);"><?php echo $lang['type_add_remove'];?></a></label>' +
        '</li>';
    ...
    var ul_custom = '<li style="width: 47%;">' +
        ...
        '<label><a onclick="remove_custom($(this));" class="imap-btn imap-btn-red" href="JavaScript:void(0);"><?php echo $lang['type_add_remove'];?></a></label>' +
        '</li>';
    ...
    $('input[im_type="change_default_spec_value"]').click(function(){
        $(this).parents('div:first').find("input[im_type='submit_value']").val('ok');
    });

    $('input[class="change_default_submit_value"]').change(function(){
        $(this).parents('li:first').find("input[im_type='submit_value']").val('ok');
    });
    ...
    // 规格隐藏未选项
    $('a[imtype="spec_hide"]').live('click',function(){
        checked_hide('spec');
    });
    // 规格全部显示
    $('a[imtype="spec_show"]').live('click',function(){
        checked_show('spec');
    });
    // 品牌隐藏未选项
...
function checked_show(str) {
    ...
    $('a[imtype="'+str+'_show"]').attr('imtype',str+'_hide').html('<?php echo $lang['type_common_checked_hide'];?>');
...
function checked_hide(str) {
    ...
    $('a[imtype="'+str+'_hide"]').attr('imtype',str+'_show').html('<?php echo $lang['type_common_checked_show'];?>');
    $('#'+str+'_div').perfectScrollbar('destroy').perfectScrollbar();
}

function spec_check(){
    var id='';
    $('input[im_type="change_default_spec_value"]:checked').each(function(){
        if(!isNaN($(this).val())){
            id += $(this).val();
        }
    });
