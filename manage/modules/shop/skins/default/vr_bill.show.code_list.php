...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?act=vr_bill&op=get_bill_info_code_xml&query_type=<?php echo $_GET['query_type'];?>&ob_no=<?php echo $_GET['ob_no'];?>',
        colModel : [
        ...
        sortname: "rec_id",
        sortorder: "desc",
        title: '<?php echo $_GET['ob_no'].'期';?>账单-兑换码列表'
    });
});
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = 'index.php?act=vr_bill&ob_no=<?php echo $_GET['ob_no'];?>&query_type=<?php echo $_GET['query_type'];?>&op=export_order&rec_id='+id;
}
</script>
