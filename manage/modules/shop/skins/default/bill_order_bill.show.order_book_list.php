...
$(function(){
    $("#flexigrid").flexigrid({
        ...
        sortname: "book_id",
        sortorder: "desc",
        title: '<?php echo $_GET['ob_no'].'期';?>账单-未退定金的预定订单列表'
    });
});
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString() +'&ob_no=<?php echo $_GET['ob_no'];?>&op=export_book&order_id='+id;
}
</script>
