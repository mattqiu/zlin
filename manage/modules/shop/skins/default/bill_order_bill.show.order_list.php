...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=bill&op=get_bill_info_xml&query_type=<?php echo $_GET['query_type'];?>&ob_no=<?php echo $_GET['ob_no'];?>&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=bill&op=get_bill_info_xml&query_type=<?php echo $_GET['query_type'];?>&ob_no=<?php echo $_GET['ob_no'];?>'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?act=bill&op=get_bill_info_xml&query_type=<?php echo $_GET['query_type'];?>&ob_no=<?php echo $_GET['ob_no'];?>',
        colModel : [
       ...
       {display: '买家账号', name : 'buyer_name'}
        ...
        sortname: "order_id",
        sortorder: "desc",
        title: '<?php echo $_GET['ob_no'].'期';?>账单-订单列表'
    });
});
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString() +'&ob_no=<?php echo $_GET['ob_no'];?>&op=export_order&order_id='+id;
}
</script>
