      ...
      <tbody>
        ...
        <tr class="<?php echo $val['class_parent_id'] == '0' ? '' : 'two'; ?> <?php echo 'parent' . $val['class_parent_id']; ?>" data-id="<?php echo $val['class_id']; ?>">
          <td class="sign"><?php if ($val['have_child'] == '1') { ?>
            <img class="node_parent" state="close" node_id="<?php echo 'parent' . $val['class_id']; ?>" src="<?php echo ADMIN_SKINS_URL; ?>/images/tv-expandable.gif">
            <?php } ?></td>
          <td class="handle"><a href="javascript:;" onclick="submit_delete(<?php echo $val['class_id']; ?>)" class="btn red"><i class="fa fa-trash-o"></i>删除</a>
            ...
            <a href="index.php?act=groupbuy&op=class_add&parent_id=<?php echo $val['class_id']; ?>" class="btn green"><i class="fa fa-plus"></i><?php echo $lang['im_add_sub_class']; ?></a>
            <?php } ?></td>
          <td class="sort"><span im_type="class_sort" column_id="<?php echo $val['class_id']; ?>" title="可编辑" class="editable"><?php echo $val['sort'];?></span></td>
          <td class="name"><?php if ($val['class_parent_id'] != '0') { ?>
            <img fieldid="<?php echo $val['class_id'];?>" status="close" im_type="flex" src="<?php echo ADMIN_SKINS_URL;?>/images/tv-item1.gif">
            <?php } ?>
            <span im_type="class_name" column_id="<?php echo $val['class_id']; ?>" title="可编辑" class="editable"><?php echo $val['class_name']; ?></span></td>
          <td></td>
        </tr>
        <?php } ?>
        <?php } else { ?>
...
$(function() {
    ...
    // 行内ajax编辑
    $('span[im_type="class_sort"]').inline_edit({act: 'groupbuy', op: 'class_sort_update'});
    $('span[im_type="class_name"]').inline_edit({act: 'groupbuy', op: 'class_name_update'});
