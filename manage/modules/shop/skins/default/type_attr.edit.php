<?php defined('InIMall') or exit('Access Invalid!');?>
  ...
  <form id="attr_form" method="post" class="imap-form-dialog" action="<?php echo urlAdminShop('type', 'attr_edit');?>">
    ...
    <div class="imap-form-default">
      ...
      <dl class="row">
        <dt class="tit">
          <label for="attr_sort"><em>*</em><?php echo $lang['im_sort'];?></label>
        </dt>
        <dd class="opt">
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <div class="onoff">
            <label for="attr_show1" class="cb-enable <?php if($output['attr_info']['attr_show'] == '1'){?>selected<?php }?>"><?php echo $lang['im_yes'];?></label>
            <label for="attr_show0" class="cb-disable <?php if($output['attr_info']['attr_show'] == '0'){?>selected<?php }?>"><?php echo $lang['im_no'];?></label>
    ...
    <div class="imap-form-all">
      ...
      <dl class="row">
        <dd class="opt">
          <ul class="imap-ajax-add" id="attr_model">
            <?php if(is_array($output['attr_value_list']) && !empty($output['attr_value_list'])) {?>
            <?php foreach($output['attr_value_list'] as $val) {?>
            <li>
              <input type="hidden" im_type="submit_value" name='attr_value[<?php echo $val['attr_value_id'];?>][form_submit]' value='' />
              <label>删除：
                <input type="checkbox" name="attr_del[<?php echo $val['attr_value_id'];?>]" value="<?php echo $val['attr_value_id'];?>" />
              </label>
              <label class="ml20"><?php echo $lang['im_sort'];?>：
                <input type="text" im_type="change_default_submit_value" name="attr_value[<?php echo $val['attr_value_id'];?>][sort]" value="<?php echo $val['attr_value_sort'];?>" class="w30"/>
              </label>
              <label class="ml20">可选值：
                <input type="text" im_type="change_default_submit_value" name="attr_value[<?php echo $val['attr_value_id'];?>][name]" value="<?php echo $val['attr_value_name'];?>"  class="w150"/>
              </label>
              <label></label>
            </li>
            <?php }?>
            ...
            <?php }?>
            <a class="imap-btn" id="add_attr" href="JavaScript:void(0);"><i class="fa fa-plus"></i>添加一个属性可选值</a>
          </ul>
        </dd>
      </dl>
      <div class="bot"><a id="attrSubmitBtn" class="imap-btn-big imap-btn-green" href="JavaScript:void(0);"><?php echo $lang['im_submit'];?></a></div>
    </div>
  </form>
...
$(function(){
//自动加载滚动条
    $('.imap-form-dialog').perfectScrollbar();
	
    var attr_i=0;
	var attr_model = '<li>'+
		'<label style="width: 64px;"></label><label class="ml20"><?php echo $lang['im_sort'];?>：<input type="text" name="attr_value[key][sort]" value="0" class="w30"/></label>'+
		'<label class="ml20">可选值：<input type="text" name="attr_value[key][name]" value="" class="w150"/></label>'+
		'<label class="ml10"><a onclick="remove_tr($(this));" href="JavaScript:void(0);" class="imap-btn imap-btn-red"><?php echo $lang['im_del'];?></a></label>'+
	'</li>';
