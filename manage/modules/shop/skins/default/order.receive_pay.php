  ...
  <form method="post" name="form1" id="form1" action="index.php?act=<?php echo $_GET['act'];?>&op=change_state&state_type=receive_pay&order_id=<?php echo intval($_GET['order_id']);?>">
    ...
    <div class="imap-form-default">
      ...
      <dl class="row">
        ...
        <dd class="opt"><?php echo imPriceFormat($output['order_info']['order_amount']);?>
        </dd>
      </dl>
      ...
      <dl class="row">
        ...
        <?php foreach ($output['order_info']['book_list'] as $book_info) { ?>
            <?php echo $book_info['book_step'];?>，
                                ...
                                支付充值卡：<?php echo imPriceFormat($book_info['book_rcb_amount']);?>，
                                支付积分：<?php echo imPriceFormat($book_info['book_pd_amount']);?>，
      ...
      <div class="bot"><a href="JavaScript:void(0);" id="ncsubmit" class="imap-btn-big imap-btn-green"><?php echo $lang['im_submit'];?></a> </div>
    </div>
  </form>
</div>
