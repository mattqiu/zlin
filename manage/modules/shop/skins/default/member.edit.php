...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?act=member&op=member" title="返回列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
        <h3><?php echo $lang['member_index_manage']?> - <?php echo $lang['im_edit']?>会员“<?php echo $output['member_array']['member_name'];?>”</h3>
  ...
  <form id="user_form" enctype="multipart/form-data" method="post">
    ...
    <div class="imap-form-default">
      <dl class="row">
        ...
        <dd class="opt">
          <input type="text" class="input-txt" name="member_name" value="<?php echo $output['member_array']['member_name'];?>" />
      ...
      <dl class="row">
        <dt class="tit">
          <label for="member_email"><em>*</em>手机号</label>
        </dt>
        <dd class="opt">
          <input type="text" value="<?php echo $output['member_array']['member_mobile'];?>" id="member_mobile" name="member_mobile" class="input-txt">
          <span class="err"></span>
          <p class="notic">请输入常用的手机号，将用来找回密码、接受订单通知等。</p>
        </dd>
      </dl>
      <dl class="row">
        <dt class="tit">
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <div class="onoff">
            <label for="member_state_1" class="cb-enable <?php if($output['member_array']['member_state'] == '1'){ ?>selected<?php } ?>" ><span><?php echo $lang['member_edit_allow'];?></span></label>
            <label for="member_state_2" class="cb-disable <?php if($output['member_array']['member_state'] == '0'){ ?>selected<?php } ?>" ><span><?php echo $lang['member_edit_deny'];?></span></label>
            <input id="member_state_1" name="member_state" <?php if($output['member_array']['member_state'] == '1'){ ?>checked="checked"<?php } ?>  value="1" type="radio">
            <input id="member_state_2" name="member_state" <?php if($output['member_array']['member_state'] == '0'){ ?>checked="checked"<?php } ?> value="0" type="radio">
          </div>
          <p class="notic"></p>
        </dd>
      </dl>
      ...
      <dl class="row">
        ...
        <dd class="opt"><strong class="red"><?php echo $output['member_array']['member_points']; ?></strong>&nbsp;云币 </dd>
      </dl>
      <dl class="row">
        <dt class="tit">
          <label>经验值</label>
		...
		(
			{
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['im_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?act=common&op=pic_cut&type=member&x=120&y=120&resize=1&ratio=1&filename=<?php echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR;?>/avatar_<?php echo $_GET['member_id'];?>.jpg&url='+data.url,690);
					}else{
						alert(data.msg);
					}
