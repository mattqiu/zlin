...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?act=brand&op=brand" title="返回品牌列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
        <h3><?php echo $lang['brand_index_brand'];?> - <?php echo $lang['im_edit'];?>品牌“<?php echo $output['brand_array']['brand_name']?>”</h3>
  ...
  <form id="brand_form" method="post" name="form1" enctype="multipart/form-data">
    ...
    <div class="imap-form-default">
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <div class="onoff">
            <label for="brand_recommend1" class="cb-enable <?php if($output['brand_array']['brand_recommend'] == '1'){ ?>selected<?php } ?>" title="<?php echo $lang['im_yes'];?>"><?php echo $lang['im_yes'];?></label>
            <label for="brand_recommend0" class="cb-disable <?php if($output['brand_array']['brand_recommend'] == '0'){ ?>selected<?php } ?>" title="<?php echo $lang['im_no'];?>"><?php echo $lang['im_no'];?></label>
		...
		(
			{
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['im_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?act=common&op=pic_cut&type=brand&x=150&y=50&resize=1&ratio=3&url='+data.url,690);
					}else{
						alert(data.msg);
					}
