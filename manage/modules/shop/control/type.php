...
class typeControl extends SystemControl {
    ...
    public function type_addOp(){
        ...
        if (chksubmit()){
            ...
            $obj_validate->validateparam = array(
                array("input"=>$_POST["t_name"], "require"=>"true", "message"=>$lang['type_add_name_no_null']),
            ...
            $this->log(L('im_add,type_index_type_name').'['.$_POST['t_name'].']',1);
        ...
        if(is_array($brand_list) && !empty($brand_list)){
            foreach($brand_list as $k=>$val){
                ...
                $b_list[$val['class_id']]['name'] = $val['brand_class']==''?L('im_default'):$val['brand_class'];
            }
        }
        ksort($b_list);
        //规格列表
        ...
        if(is_array($spec_list) && !empty($spec_list)){
            foreach($spec_list as $k=>$val){
                ...
                $s_list[$val['class_id']]['name'] = $val['class_name']==''?L('im_default'):$val['class_name'];
            }
        }
        ksort($s_list);
        // 一级分类列表
    ...
    public function type_editOp(){
        ...
        if (chksubmit()){
            ...
            if ($return){
                ...
                $this->log(L('im_edit,type_index_type_name').'['.$_POST['t_name'].']',1);
                showMessage($lang['im_common_save_succ'],$url);
            } else {
                $this->log(L('im_edit,type_index_type_name').'['.$_POST['t_name'].']',0);
                showMessage($lang['im_common_save_fail']);
            }
        }
    ...
    public function type_delOp(){
        ...
        if(is_array($type_list) && !empty($type_list)){
            ...
            $this->log(L('im_delete,type_index_type_name').'[ID:'.$id.']',1);
            exit(json_encode(array('state'=>true,'msg'=>L('type_index_del_succ'))));
        }else{
            $this->log(L('im_delete,type_index_type_name').'[ID:'.$id.']',0);
            exit(json_encode(array('state'=>false,'msg'=>L('param_error'))));
        }
    }
}
