...
class domainControl extends SystemControl{
    ...
    public function store_domain_settingOp(){
        ...
        if (chksubmit()){
            ...
            if ($result === true){
                $this->log(L('im_edit,im_domain_manage'),1);
    ...
    public function store_domain_editOp(){
        ...
        if (chksubmit()){
            ...
            $this->log(L('im_edit,im_domain_manage').'['.$store_domain.']',1);
            showMessage(Language::get('im_common_save_succ'),'index.php?act=domain&op=store_domain_list');//保存成功
        }
