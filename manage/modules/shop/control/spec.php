...
class specControl extends SystemControl {
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if($return) {
                    ...
                    $this->log(L('im_add,spec_index_spec_name').'['.$_POST['s_name'].']',1);
                    showMessage($lang['im_common_save_succ'], $url);
                }else {
                    $this->log(L('im_add,spec_index_spec_name').'['.$_POST['s_name'].']',0);
        ...
        if (chksubmit()) {
            ...
            if ($error != '') {
                ...
                if ($return) {
                    ...
                    $this->log(L('im_edit,spec_index_spec_name').'['.$_POST['s_name'].']',1);
                    showMessage($lang['im_common_save_succ'], $url);
                } else {
                    $this->log(L('im_edit,spec_index_spec_name').'['.$_POST['s_name'].']',0);
                    showMessage($lang['im_common_save_fail']);
                }
            }
        }
    ...
    public function spec_delOp(){
        ...
        if(is_array($spec_list) && !empty($spec_list)){
            ...
            if(!$return){
                exit(json_encode(array('state'=>false,'msg'=>L('im_common_save_fail'))));
            }
            ...
            $this->log(L('im_delete,spec_index_spec_name').'[ID:'.$id.']',1);
            exit(json_encode(array('state'=>true,'msg'=>L('im_common_del_succ'))));
        }else{
            $this->log(L('im_delete,spec_index_spec_name').'[ID:'.$id.']',0);
            exit(json_encode(array('state'=>false,'msg'=>L('param_error'))));
        }
    }
}
