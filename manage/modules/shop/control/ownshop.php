...
class ownshopControl extends SystemControl
{
    ...
    public function addOp()
    {
        if (chksubmit())
        {
			...
			$member_info = array();
			$member_info['member_name']      = $memberName;
			$member_info['member_passwd']    = $memberPasswd;
			$member_info['password_confirm'] = $memberPasswd;
				
            $member_info = $model_member->register($member_info);
			if(!empty($member_info['error'])) {
				showMessage($member_info['error']);
			}
    ...
    public function edit_save_operatingOP() {
        if (chksubmit()) {
            ...
            if ($result) {
                showMessage(L('im_common_op_succ'), 'index.php?act=ownshop&op=index');
            } else {
    ...
    public function get_xmlOp() {		
        ...
        foreach ($store_list as $value) {			
            ...
            if ($value['bind_all_gc'] != 1 || empty($stores_with_goods[$value['store_id']])) {
                $operation .= "<span class='btn'><em><i class='fa fa-cog'></i>" . L('im_set') . " <i class='arrow'></i></em><ul>";
