...
class orderControl extends SystemControl{
    ...
    public function get_xmlOp(){
        ...
        foreach ($order_list as $order_id => $order_info) {
            ...
            $list['order_sn'] = $order_info['order_sn'].str_replace(array(1,2), array(null,' [预定]'), $order_info['order_type']);
            $list['order_from'] = str_replace(array(1,2,3), array('PC端','移动端','小程序'),$order_info['order_from']);
			...
			if ($order_info['shipping_fee']) {
			    $list['order_amount'] .= '(含运费'.imPriceFormat($order_info['shipping_fee']).')';
			}
            ...
            $list['rcb_amount'] = imPriceFormat($order_info['rcb_amount']);
            $list['pd_amount'] = imPriceFormat($order_info['pd_amount']);
            $list['shipping_code'] = $order_info['shipping_code'];
            $list['refund_amount'] = imPriceFormat($order_info['refund_amount']);
    ...
    private function _order_receive_pay($order_info, $post) {
        ...
        if (!chksubmit()) {			
			...
			$payment_list = array();
            //显示pc端支付接口列表			
            $pc_payment_list = Model('payment')->getPaymentOpenList();				
            //去掉积分和货到付款
            foreach ($pc_payment_list as $key => $value){
                if ($value['payment_code'] == 'predeposit' || $value['payment_code'] == 'offline') {
                    //unset($payment_list[$key]);
                }else{
					$payment_list[$value['payment_code']] = array('payment_code'=>$value['payment_code'],'payment_name'=>$value['payment_name']);
				}
            }
			//显示手机端支付接口列表
	        $mb_payment_list = Model('mb_payment')->getMbPaymentOpenList();
	        //去掉积分和货到付款
	        foreach ($mb_payment_list as $key => $value){
	            if ($value['payment_code'] == 'predeposit' || $value['payment_code'] == 'offline') {
	                //unset($mb_payment_list[$key]);
	            }else{
					$payment_list[$value['payment_code']] = array('payment_code'=>$value['payment_code'],'payment_name'=>$value['payment_name']);
				}
	        }
			//$payment_list = array_merge($payment_list,$mb_payment_list);
        ...
        //同步异步通知时,预定支付尾款时需要用到已经支付的状态
        $order_list = $model_order->getOrderList(array('pay_sn'=>$order_info['pay_sn'],'order_state'=>array('in',array(ORDER_STATE_NEW,ORDER_STATE_CANCEL,ORDER_STATE_PAY))));
        //取订单其它扩展信息
    ...
    private function createExcel($data = array()){
        ...
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'积分支付(元)');
