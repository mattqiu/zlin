...
class vr_billControl extends SystemControl{
    ...
    private $links = array(
        array('url'=>'act=vr_bill&op=index','lang'=>'im_manage'),
    );
    ...
    public function get_bill_xmlOp(){
        ...
        foreach ($bill_list as $bill_info) {
            ...
            if (in_array($bill_info['ob_state'],array(2,3))) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?act=vr_bill&op=show_bill&ob_no={$bill_info['ob_no']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a class=\"btn green\" href=\"index.php?act=vr_bill&op=show_bill&ob_no={$bill_info['ob_no']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
    ...
    public function get_bill_info_code_xmlOp(){
        ...
        foreach ($code_list as $code_info) {
            ...
            $list['pay_price'] = imPriceFormat($code_info['pay_price']);
            $list['commis_rate'] = imPriceFormat($code_info['pay_price']*$code_info['commis_rate']/100);
    ...
    public function export_orderOp(){
        ...
        $csv->filename = $_GET['ob_no'].'-'.$file_name;
        $csv->export($export_data);
    }
