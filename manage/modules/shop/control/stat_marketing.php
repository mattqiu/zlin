    ...
    public function get_grouplist_xmlOp(){
        ...
        if (!empty($grouplist) && is_array($grouplist)){
            ...
            foreach ($grouplist as $k => $v){
                ...
                $out_array['goods_price'] = imPriceFormat($v['goods_price']);
                $out_array['groupbuy_price'] = imPriceFormat($v['groupbuy_price']);
                $out_array['goodsamount'] = imPriceFormat($v['goodsamount']);
                $statlist[] = $out_array;
            }
        }
        $data = array();
        ...
        $data['list'] = $statlist;
        echo Tpl::flexigroupbuyXML($data);exit();
    }
    /**
     * 抢购商品统计
     */
