...
class mall_consultControl extends SystemControl{
    ...
    public function type_listOp() {
        ...
        if (chksubmit()) {
            ...
            if ($result) {
                ...
                showMessage(L('im_common_del_succ'));
            } else {
                $this->log('删除平台客服咨询类型 ID:'.implode(',', $mctid_array), 0);
                showMessage(L('im_common_del_fail'));
            }
        }
    ...
    public function type_addOp() {
        if (chksubmit()) {
            ...
            if ($result){
                $this->log('新增咨询类型',1);
                showMessage(L('im_common_save_succ'), urlAdminShop('mall_consult', 'type_list'));
            }else {
                $this->log('新增咨询类型',0);
    ...
    public function type_editOp() {
        ...
        if (chksubmit()) {
            ...
            if ($result){
                ...
                showMessage(L('im_common_op_succ'), urlAdminShop('mall_consult', 'type_list'));
            }else {
    ...
    public function onlineOp() {
		...
		if (chksubmit()){
			if (!empty($_POST['mall_im'])){
				$member_info = $model_member->getMemberInfo(array('member_name'=>$_POST['mall_im']),'member_id');
				if (empty($member_info)){
					showMessage('无效的会员帐号','','','error');
				}
				$_POST['mall_im'] = $member_info['member_id'];
			}
			$consult_array = array();
			$consult_array['qq'] 	= $_POST['mall_qq'];
			$consult_array['ww'] 	= $_POST['mall_ww'];
			$consult_array['im'] 	= $_POST['mall_im'];
			
			$data = array('mall_consult'=>serialize($consult_array));
		...
		$consult_setting = $model_setting->getRowSetting('mall_consult');
		$consult_setting = unserialize($consult_setting['value']);
		if (!empty($consult_setting['im'])){
			$member_info = $model_member->getMemberInfoByID($consult_setting['im'],'member_name');
			if (!empty($member_info)){
				$consult_setting['im'] = $member_info['member_name'];
			}else{
				$consult_setting['im'] = '';
			}
		}
		Tpl::output('consult_setting',$consult_setting);
		
        Tpl::setDirquna('shop');
		Tpl::showpage('mall_consult.online');
    }
}
