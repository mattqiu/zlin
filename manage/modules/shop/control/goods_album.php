...
class goods_albumControl extends SystemControl{
    ...
    public function get_xmlOp() {
        ...
        foreach ($album_list as $value) {
            ...
            $param['aclass_cover'] = "<a href='javascript:void(0);' class='pic-thumb-tip' onMouseOut='toolTip()' onMouseOver='toolTip(\"<img src=".($value['aclass_cover'] != '' ? cthumb($value['aclass_cover'], 60, $value['store_id']) : ADMIN_SITE_URL.'/skins/'.TPL_NAME.'/images/member/default_image.png').">\")'><i class='fa fa-picture-o'></i></a>";
    ...
    public function aclass_delOp(){
        ...
        $this->log(L('im_delete,g_album_one').'[ID:'.intval($_GET['aclass_id']).']',1);
        exit(json_encode(array('state'=>true,'msg'=>L('im_common_del_succ'))));
    }
    ...
    public function del_album_picOp(){
        ...
        $this->log(L('im_delete,g_album_pic_one').'[ID:'.$apic_id.']',1);
        exit('1');
    }
    ...
    public function del_more_picOp(){
        ...
        $this->log(L('im_delete,g_album_pic_one').'[ID:'.implode(',',$_POST['delbox']).']',1);
        redirect();
    }
