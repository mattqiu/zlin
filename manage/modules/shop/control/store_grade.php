...
class store_gradeControl extends SystemControl{
    ...
    public function store_gradeOp(){
        ...
        if (chksubmit()){
            if (!empty($_POST['check_sg_id'])){
                ...
                dkcache('store_grade');
                $this->log(L('im_del,store_grade').'[ID:'.implode(',',$_POST['check_sg_id']).']',1);
                showMessage($lang['im_common_del_succ']);
            }else {
                showMessage($lang['im_common_del_fail']);
            }
        }
    ...
    public function store_grade_addOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
				...
				$insert_array['sg_confirm'] = intval($_POST['sg_confirm']);
				...
				$insert_array['branch_op'] = trim($_POST['branch_op']);
				$insert_array['branch_limit'] = trim($_POST['branch_limit']);
				$insert_array['extension_op'] = trim($_POST['extension_op']);
				$insert_array['promotion_limit'] = trim($_POST['promotion_limit']);
				$insert_array['saleman_limit'] = trim($_POST['saleman_limit']);
				$insert_array['store_type'] = intval($_POST['store_type']);

                $result = $model_grade->add($insert_array);
                if ($result){
                    dkcache('store_grade');
                    $this->log(L('im_add,store_grade').'['.$_POST['sg_name'].']',1);
                    showMessage($lang['im_common_save_succ'],'index.php?act=store_grade&op=store_grade');
                }else {
    ...
    public function store_grade_editOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
				...
				$update_array['sg_confirm'] = intval($_POST['sg_confirm']);
                $update_array['sg_description'] = trim($_POST['sg_description']);
                $update_array['sg_sort'] = trim($_POST['sg_sort']);
				$update_array['branch_op'] = trim($_POST['branch_op']);
				$update_array['branch_limit'] = trim($_POST['branch_limit']);
				$update_array['extension_op'] = trim($_POST['extension_op']);
				$update_array['promotion_limit'] = trim($_POST['promotion_limit']);
				$update_array['saleman_limit'] = trim($_POST['saleman_limit']);
				$update_array['store_type'] = intval($_POST['store_type']);

                $result = $model_grade->where(array('sg_id'=>intval($_POST['sg_id'])))->update($update_array);

                if ($result){
					Model('store')->where(array('grade_id'=>$update_array['sg_id']))->update(array('branch_op'=>$update_array['branch_op']));
                    dkcache('store_grade');
                    $this->log(L('im_edit,store_grade').'['.$_POST['sg_name'].']',1);
                    showMessage($lang['im_common_save_succ']);
                }else {
                    showMessage($lang['im_common_save_fail']);
                }
            }
    ...
    public function store_grade_delOp(){
        ...
        if (intval($_GET['sg_id']) > 0){
            ...
            dkcache('store_grade');
            $this->log(L('im_del,store_grade').'[ID:'.intval($_GET['sg_id']).']',1);
            showMessage($lang['im_common_del_succ'],'index.php?act=store_grade&op=store_grade');
        }else {
            showMessage($lang['im_common_del_fail'],'index.php?act=store_grade&op=store_grade');
        }
    }
    ...
    public function store_grade_templatesOp(){
        ...
        if (chksubmit()){
            ...
            if ($result){
                $this->log(L('im_edit,store_grade_tpl'),1);
                showMessage($lang['im_common_save_succ'],'index.php?act=store_grade&op=store_grade');
            }else {
                showMessage($lang['im_common_save_fail']);
            }
