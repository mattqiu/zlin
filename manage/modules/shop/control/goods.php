...
class goodsControl extends SystemControl{
    ...
    public function get_xmlOp() {
        ...
        $param = array('goods_commonid', 'goods_name', 'goods_price', 'goods_state', 'goods_verify', 'goods_image', 'goods_jingle', 'gc_id'
                ...
                , 'goods_freight', 'is_virtual', 'virtual_indate', 'virtual_invalid_refund', 'is_fcode', 'is_appoint', 'appoint_satedate'
        ...
        foreach ($goods_list as $value) {
            ...
            $param['goods_price'] = imPriceFormat($value['goods_price']);
            ...
            $param['goods_marketprice'] = imPriceFormat($value['goods_marketprice']);
            $param['goods_costprice'] = imPriceFormat($value['goods_costprice']);
            $param['goods_freight'] = $value['goods_freight'] == 0 ? '免运费' : imPriceFormat($value['goods_freight']);
            ...
            $param['is_appoint'] = $value['is_appoint'] ==  '1' ? '<span class="yes"><i class="fa fa-check-circle"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
            $param['appoint_satedate'] = $value['is_appoint'] == '1' && $value['appoint_satedate'] > 0 ? date('Y-m-d', $value['appoint_satedate']) : '--';
    ...
    public function goods_lockupOp() {
        if (chksubmit()) {
            ...
            showDialog(L('im_common_op_succ'), '', 'succ', '$("#flexigrid").flexReload();CUR_DIALOG.close()');
        }
...
//         echo json_encode($goods_list);
    ...
    public function goods_setOp() {
        ...
        if (chksubmit()){
            ...
            if ($result === true){
                $this->log(L('im_edit,im_goods_set'),1);
                showMessage(L('im_common_save_succ'));
            }else {
                $this->log(L('im_edit,im_goods_set'),0);
    ...
    private function createCsv($goods_list) {
        ...
        @header("Content-Disposition: attachment; filename=member_class.csv");
        $data = array();
        foreach ($goods_list as $value) {
            $param = array();
            $param['goods_commonid'] = $value['goods_commonid'];
            ...
            $param['is_appoint'] = $value['is_appoint'] ==  '1' ? '是' : '否';
            $param['appoint_satedate'] = $value['is_appoint'] == '1' && $value['appoint_satedate'] > 0 ? date('Y-m-d', $value['appoint_satedate']) : '--';
