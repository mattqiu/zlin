...
class stat_tradeControl extends SystemControl{
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['op']) || in_array($_REQUEST['op'],array('income','get_income_xml','sale_trend','get_sale_xml','get_plat_sale'))){
    ...
    public function incomeOp(){
        ...
        if (!empty($_GET['id']) && is_array($_GET['id'])){
            $condition['ob_store_id'] = array('in',$_GET['id']);
        }
    ...
    public function get_plat_incomeOp(){
		...
		echo '<dl class="row"><dd class="opt"><ul class="im-row">';
        echo '<li title="收款金额：'. number_format($plat_data[0]['oot'],2).'元"><h4>收款金额</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'.$plat_data[0]['oot'].'"></h2><h6>元</h6></li>';
        echo '<li title="退款金额：'. number_format($plat_data[0]['oort'],2).'元"><h4>退款金额</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'.$plat_data[0]['oort'].'"></h2><h6>元</h6></li>';
        echo '<li title="实收金额：'. (number_format($plat_data[0]['oot']-$plat_data[0]['oort'],2)).'元"><h4>实收金额</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'. ($plat_data[0]['oot']-$plat_data[0]['oort']).'"></h2><h6>元</h6></li>';
        echo '<li title="佣金总额：'. number_format($plat_data[0]['oct'],2).'元"><h4>佣金总额</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'.$plat_data[0]['oct'].'"></h2><h6>元</h6></li>';
        echo '<li title="店铺费用：'. number_format($plat_data[0]['osct'],2).'元"><h4>店铺费用</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'.$plat_data[0]['osct'].'"></h2><h6>元</h6></li>';
        echo '<li title="总收入：'. number_format($plat_data[0]['ort'],2).'元"><h4>总收入</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'.$plat_data[0]['ort'].'"></h2><h6>元</h6></li>';
		echo '</ul></dd><dl>';
        exit();
    }
    ...
    public function get_income_xmlOp(){
        ...
        $order_type = array('member_name','ob_order_totals','ob_commis_totals','ob_order_return_totals','ob_commis_return_totals','ob_store_cost_totals','ob_result_totals');
        ...
        //店铺数据
        $list = $model->getBillList($condition, 'ob', $page, $orderby);
        $statlist = array();
    ...
    public function get_plat_predepositOp(){
		...
		echo '<div class="title"><h3>积分情况一览</h3></div>';
		echo '<dl class="row"><dd class="opt"><ul class="im-row">';
