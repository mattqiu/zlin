...
class settingControl extends SystemControl{
    private $links = array(
        ...
        array('url'=>'act=setting&op=search','lang'=>'im_admin_search_set'),
    );
    ...
    public function seoOp(){
        ...
        if (chksubmit()){
            ...
            $result = $model_setting->updateSetting($update_array);
            if ($result === true){
                $this->log(L('im_edit,im_seo_set'),1);
                showMessage(L('im_common_save_succ'));
            }else {
                $this->log(L('im_edit,im_seo_set'),0);
                showMessage(L('im_common_save_fail'));
            }
        }
        $list_setting = $model_setting->getListSetting();
    ...
    public function searchOp(){
        if (chksubmit()){
            ...
            */
            ...
            $result = $model_setting->updateSetting(array(
                    'hot_search'=>str_replace($comma,',',$_POST['hot_search']),'adv_search'=>str_replace($comma,',',$_POST['adv_search'])));
            if ($result){
        ...
        Tpl::output('list_setting',$list_setting);
        //输出子菜单
        Tpl::output('top_link',$this->sublink($this->links,'search'));
        Tpl::setDirquna('shop');
