                ...
                if ($i <= $datanum){
                    ...
                    switch ($_GET['type']){
                       case 'ordernum':
                           ...
                           break;
                       case 'orderamount':
                           $tmp['orderamount'] = imPriceFormat($store_arr[$k]['orderamount']);
                           break;
                    }
    ...
    public function get_storesales_xmlOp(){
        ...
        if (!empty($list) && is_array($list)){
            foreach ($list as $k => $v){
                ...
                $out_array['operation'] = "<a class='btn green' href='javascript:void(0);' im_type='showtrends' data-param='{\"storeid\":\"{$v['store_id']}\"}'><i class='fa fa-line-chart'></i>走势图</a>";
                $out_array['orderamount'] = imPriceFormat($v['orderamount']);
                $statlist[$v['store_id']] = $out_array;
            }
        }
