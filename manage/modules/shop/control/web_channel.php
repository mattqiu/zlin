...
class web_channelControl extends SystemControl{
    ...
    public function add_channelOp() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('im_common_save_succ'),'index.php?act=web_channel&op=web_channel');
            } else {
    ...
    public function add_floorOp() {
        ...
        if (chksubmit()) {
			...
			$web_array['web_layout'] = empty($_POST["web_layout"])?'floor':$_POST["web_layout"];
            ...
            if ($state) {
                ...
                showMessage(Language::get('im_common_save_succ'),'index.php?act=web_channel&op=floor_list');
            } else {
                showMessage(Language::get('im_common_save_fail'));
            }
        }
    ...
    public function edit_channel_flOp() {
        ...
        $floor_list = $model_web_config->getWebList(array('web_id'=>"$web_id"));
		$web_layout = empty($floor_list[0]['web_layout'])?'floor':$floor_list[0]['web_layout'];
		
        Tpl::output('floor',$floor_list[0]);
        Tpl::setDirquna('shop');
        Tpl::showpage('web_channel_fl.edit.'.$web_layout);
    }
    ...
    public function html_updateOp() {
        ...
        if(!empty($web_array) && is_array($web_array)) {
            ...
            showMessage(Language::get('im_common_op_succ'),'index.php?act=web_channel&op=floor_list');
        } else {
