...
EOB;
    ...
    public function add_cardOp()
    {
        ...
        $card_grant = $_POST['card_grant'];
        $card_points = $_POST['card_points'];
        $pd_amount =  $_POST['pd_amount'];
        $commis_amount = $_POST['commis_amount'];
        $give_exppoints = $_POST['give_exppoints'];
        $buy_limit =  $_POST['buy_limit']; //次数
        $commis_points = $_POST['commis_points']; //赠送邀请人的云币
        $snKeys = array();
        ...
        if ($totalKeys < 1 || $totalKeys > 9999) {
            showMessage('只能在一次操作中增加1~99999个充值卡号', '', 'html', 'error');
            exit;
        }
        ...
        foreach (array_keys($snKeys) as $sn) {
            $snToInsert[] = array(
            	...
            	'give_exppoints' => $give_exppoints,
            	'pd_amount' => $pd_amount,
            	'commis_amount' => $commis_amount,
            	'buy_limit' =>  $buy_limit, //次数
            	'commis_points' => $commis_points //赠送邀请人的云币
            );
        }
