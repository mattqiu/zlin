...
EOB;
    ...
    public function newOp(){
        ...
        if($result){
            $this->log(L('im_add,activity_index').'['.$_POST['activity_title'].']',null);
            showMessage(Language::get('im_common_op_succ'),'index.php?act=activity&op=activity');
        }else{
    ...
    public function delOp()
    {
        ...
        if($activity_detail->del($id_new)){
            if($activity->del($id_new)){
                $this->log(L('im_del,activity_index').'[ID:'.$id.']',null);
                $this->jsonOutput();
            }
        }
    ...
    public function editOp(){
        ...
        if($result){
            ...
            $this->log(L('im_edit,activity_index').'[ID:'.$_POST['activity_id'].']',null);
            showMessage(Language::get('im_common_save_succ'),'index.php?act=activity&op=activity');
        }else{
            if($_FILES['activity_banner']['name']!=''){
                @unlink(BASE_UPLOAD_PATH.DS.ATTACH_ACTIVITY.DS.$upload->file_name);
            }
    ...
    public function dealOp()
    {
        ...
        if($activity_detail->updates(array('activity_detail_state'=>intval($_GET['state'])),$id)){
            $this->log(L('im_edit,activity_index').'[ID:'.$id.']',null);
    ...
    public function del_detailOp()
    {
        ...
        if($activity_detail->delList($condition_arr)){
            $this->log(L('im_del,activity_index_content').'[ID:'.$id.']',null);
