...
class voucherControl extends SystemControl{
    ...
    public function settingOp(){
        ...
        if (chksubmit()){
            ...
            if ($result){
                $this->log(L('admin_voucher_setting,im_voucher_price_manage'));
    ...
    public function priceaddOp(){
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($rs){
                    $this->log(L('im_add,admin_voucher_priceadd').'['.$_POST['voucher_price'].']');
                    showMessage(Language::get('im_common_save_succ'),'index.php?act=voucher&op=pricelist');
                }else {
                    showMessage(Language::get('im_common_save_fail'),'index.php?act=voucher&op=priceadd');
                }
            }
        }else {
    ...
    public function priceeditOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($rs){
                    $this->log(L('im_edit,admin_voucher_priceadd').'['.$_POST['voucher_price'].']');
                    showMessage(Language::get('im_common_save_succ'),'index.php?act=voucher&op=pricelist');
                }else {
                    showMessage(Language::get('im_common_save_fail'),'index.php?act=voucher&op=pricelist');
                }
            }
        }else {
    ...
    public function pricedropOp(){
        ...
        if ($rs){
            $this->log(L('im_del,admin_voucher_priceadd').'[ID:'.$voucher_price_id.']');
            showMessage(Language::get('im_common_del_succ'),'index.php?act=voucher&op=pricelist');
        }else{
            showMessage(Language::get('im_common_del_fail'),'index.php?act=voucher&op=pricelist');
        }
    }
    ...
    public function templatelist_xmlOp()
    {
        ...
        $model = Model();
        $list = (array) $model->table('voucher_template')
            ->where($condition)
            ->order('voucher_t_state asc, ' . $sort)
            ->page($_REQUEST['rp'])
            ->select();
        ...
        foreach ($list as $val) {
            ...
            $i['state_text'] = $val['voucher_t_state'] == '1' ? '有效' : '失效';
    ...
    public function templateeditOp(){
        ...
        if(chksubmit()){
            ...
            if($rs){
                $this->log(L('im_edit,im_voucher_price_manage,admin_voucher_styletemplate').'[ID:'.$t_id.']');
                showMessage(Language::get('im_common_save_succ'),'index.php?act=voucher&op=templatelist','succ');
            }else{
                showMessage(Language::get('im_common_save_fail'),'index.php?act=voucher&op=templatelist','error');
            }
        }else{
            //查询店铺分类
