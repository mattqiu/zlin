...
class web_configControl extends SystemControl{
    ...
    public function web_editOp(){
        ...
        if (chksubmit()){
			...
			$web_array['web_layout'] = empty($_POST["web_layout"])?'default':$_POST["web_layout"];
            $web_array['web_sort'] = intval($_POST["web_sort"]);
			$web_array['web_style'] = intval($_POST["web_style"]);
            ...
            showMessage(Language::get('im_common_save_succ'),'index.php?act=web_config&op=web_config');
        }
    ...
    public function code_editOp(){
        ...
        if(is_array($code_list) && !empty($code_list)) {
            ...
            Tpl::output('style_array',$style_array);
            $web_list = $model_web_config->getWebList(array('web_id'=>$web_id));
			$web_layout = empty($web_list[0]['web_layout'])?'default':$web_list[0]['web_layout'];
			
            Tpl::output('web_array',$web_list[0]);
			Tpl::setDirquna('shop');
            Tpl::showpage('web_code.edit.'.$web_layout);
        } else {
            showMessage(Language::get('im_no_record'));
        }
    }
    ...
    public function web_htmlOp(){
        ...
        if(!empty($web_array) && is_array($web_array)) {
            ...
            showMessage(Language::get('im_common_op_succ'),'index.php?act=web_config&op=web_config');
        } else {
     ...
     * 保存背景图片
     */
    public function upload_bg_picOp() {
        $code_id = intval($_POST['code_id']);
        $web_id = intval($_POST['web_id']);
        $model_web_config = Model('web_config');
        $code = $model_web_config->getCodeRow($code_id,$web_id);
        if (!empty($code)) {
            ...
            $state = $model_web_config->updateCode(array('code_id'=> $code_id),array('code_info'=> $code_info));
			Tpl::setDirquna('shop');
            Tpl::showpage('web_upload_bg_pic','null_layout');
        }
    }
	...
	public function brand_editOp() {		
		...
		Tpl::showpage('web_brand.edit');
	}
	
	/**
	 * ajax上传背景图片
	 */
	public function bgimage_uploadOP() {
	    $pic_name = '';
		
		if (!empty($_FILES[$_POST['name']]['name'])) {//上传图片
			$upload = new UploadFile();
			$ext = end(explode('.', $_FILES[$_POST['name']]['name']));
			$file_name = 'web-'.$_POST['imgid'].'-bgimg';			
    		$upload->set('default_dir',ATTACH_EDITOR);			
    		$upload->set('file_name',$file_name.".".$ext);
		...
		$pic_name = UPLOAD_SITE_URL.'/'.ATTACH_EDITOR."/".$upload->file_name.'?'.mt_rand(100,999);//加随机数防止浏览器缓存图片
		
		$data = array ();
        $data ['name']      = $upload->file_name;
		$data ['fullname']  = $pic_name;
