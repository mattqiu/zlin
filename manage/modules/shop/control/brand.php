...
class brandControl extends SystemControl{
	private $links = array(
        array('url'=>'act=brand&op=brand','text'=>'管理'),
        array('url'=>'act=brand&op=brand_apply','text'=>'待审核'),
		array('url'=>'act=brand&op=brand_focus','text'=>'焦点区')
    );
    ...
    public function brandOp(){
		Tpl::output('top_link',$this->sublink($this->links, 'brand'));
    ...
    private function createCsv($brand_list) {
        ...
        //output('brand_list' .$_GET['curpage'] . '-'.date('Y-m-d'), $data, $header);
    }
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $this->log(L('im_add,brand_index_brand').'['.$_POST['brand_name'].']',1);
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $this->log(L('im_edit,brand_index_brand').'['.$_POST['brand_name'].']',1);
                    showMessage($lang['im_common_save_succ'],$url);
                }else {
                    $this->log(L('im_edit,brand_index_brand').'['.$_POST['brand_name'].']',0);
                    showMessage($lang['im_common_save_fail']);
                }
            }
        }
    ...
    public function brand_delOp(){
        ...
        $this->log(L('im_delete,brand_index_brand').'[ID:'.$brand_id.']',1);
    ...
    public function brand_applyOp(){
		Tpl::output('top_link',$this->sublink($this->links, 'brand_apply'));
	...
	public function brand_focusOp() {
		...
		Tpl::output('top_link',$this->sublink($this->links, 'brand_focus'));		
		
		Tpl::setDirquna('shop');
		Tpl::showpage('web_brand_focus.edit');
	}
	/**
	 * 更新html内容
	 */
