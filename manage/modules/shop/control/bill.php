...
class billControl extends SystemControl{
    ...
    private $links = array(
        array('url'=>'act=bill&op=index','lang'=>'im_manage'),
    );
    ...
    public function get_bill_info_xmlOp(){
        ...
        if ($_GET['query_type'] == 'refund') {
            ...
            foreach ($refund_list as $refund_info) {
                ...
                $list['refund_amount'] = imPriceFormat($refund_info['refund_amount']);
                $list['commis_amount'] = imPriceFormat($refund_info['commis_amount']);
            ...
            foreach ($store_cost_list as $store_cost_info) {
                ...
                $list['cost_price'] = imPriceFormat($store_cost_info['cost_price']);
            ...
            foreach ($order_list as $order_info) {
                ...
                $list['order_amount'] = imPriceFormat($order_info['order_amount']);
                $list['shipping_fee'] = imPriceFormat($order_info['shipping_fee']);
                $list['deposit_amount'] = imPriceFormat($tmp_book[$order_info['order_id']]['book_real_pay']);
            ...
            foreach ($order_list as $order_info) {
                ...
                $list['order_amount'] = imPriceFormat($order_info['order_amount']);
                $list['shipping_fee'] = imPriceFormat($order_info['shipping_fee']);
                $list['commis_amount'] = imPriceFormat($commis_list[$order_info['order_id']]['commis_amount']);
                $list['add_time'] = date('Y-m-d',$order_info['add_time']);
    ...
    public function export_billOp(){
        if (!empty($_GET['os_month']) && !preg_match('/^20\d{4}$/',$_GET['os_month'])) {
            exit();
        }
        $model_bill = Model('bill');
        $condition = array();
        if (preg_match('/^[\d,]+$/', $_GET['ob_no'])) {
            $_GET['ob_no'] = explode(',',trim($_GET['ob_no'],','));
            $condition['ob_no'] = array('in',$_GET['ob_no']);
        }
        ...
        $data = $model_bill->getOrderBillList($condition,'*','','ob_no desc',$limit);
        ...
        $csv->filename = 'bill-'.$_GET['os_month'];
    ...
    public function export_bookOp(){
        ...
        foreach ($data as $v) {
            ...
            $deposit_amount += $export_data[$k+1][] = imPriceFormat($tmp_book[$v['order_id']]['book_real_pay']);
            $export_data[$k+1][] = date('Y-m-d',$v['add_time']);
    ...
    public function export_refund_orderOp(){
        ...
        foreach ($data as $v) {
            ...
            $commis_totals += $export_data[$k+1][] = imPriceFormat($v['commis_amount']);
        ...
        $csv = new Csv();
        $export_data = $csv->charset($export_data,CHARSET,'gbk');
        $csv->filename = $_GET['ob_no'].'-refund';
        $csv->export($export_data);
    }
    ...
    public function get_statis_xmlOp(){
        ...
        foreach ($bill_list as $bill_info) {
            ...
            $list['os_order_totals'] = imPriceFormat($bill_info['os_order_totals']);
            $list['os_shipping_totals'] = imPriceFormat($bill_info['os_shipping_totals']);
            $list['os_commis_totals'] = imPriceFormat($bill_info['os_commis_totals']);
            $list['os_order_return_totals'] = imPriceFormat($bill_info['os_order_return_totals']);
            $list['os_commis_return_totals'] = imPriceFormat($bill_info['os_commis_return_totals']);
            $list['os_store_cost_totals'] = imPriceFormat($bill_info['os_store_cost_totals']);
            $list['os_result_totals'] = imPriceFormat($bill_info['os_result_totals']);
    ...
    public function get_bill_xmlOp(){
        ...
        foreach ($bill_list as $bill_info) {
            ...
            if (in_array($bill_info['ob_state'],array(2,3))) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?act=bill&op=show_bill&ob_no={$bill_info['ob_no']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a class=\"btn green\" href=\"index.php?act=bill&op=show_bill&ob_no={$bill_info['ob_no']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }

            $list['ob_no'] = $bill_info['ob_no'];
            $list['ob_order_totals'] = imPriceFormat($bill_info['ob_order_totals']);
            $list['ob_shipping_totals'] = imPriceFormat($bill_info['ob_shipping_totals']);
            $list['ob_commis_totals'] = imPriceFormat($bill_info['ob_commis_totals']);
            $list['ob_order_return_totals'] = imPriceFormat($bill_info['ob_order_return_totals']);
            $list['ob_commis_return_totals'] = imPriceFormat($bill_info['ob_commis_return_totals']);
            $list['ob_store_cost_totals'] = imPriceFormat($bill_info['ob_store_cost_totals']);
            $list['ob_result_totals'] = imPriceFormat($bill_info['ob_result_totals']);
            ...
            $data['list'][$bill_info['ob_no']] = $list;
        }
        exit(Tpl::flexigridXML($data));
    }
    ...
    private function _get_bill_condition($condition) {
        ...
        $sort_fields = array('ob_no','ob_start_date','ob_end_date','ob_order_totals','ob_shipping_totals','ob_commis_totals','ob_order_return_totals','ob_commis_return_totals','ob_store_cost_totals','ob_result_totals','ob_create_date','ob_state','ob_store_id');
