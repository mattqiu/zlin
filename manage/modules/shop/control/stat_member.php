...
class stat_memberControl extends SystemControl{
        ...
        if (!empty($list) && is_array($list)){
            foreach ($list as $k => $v){
                ...
                if ($stat_field == 'orderamount') {
                    $out_array['orderamount'] = imPriceFormat($out_array['orderamount']);
                }
    ...
    public function showmemberOp(){
        ...
        if ($this->search_arr['exporttype'] == 'excel'){
            ...
            //data
            foreach ($member_list as $k => $v){
                $excel_data[$k+1][] = array('data'=>$v['member_name'].'('.L('member_index_true_name,im_colon').$v['member_truename'].')');
                ...
                $excel_data[$k+1][] = array('data'=>L('member_index_available,im_colon').$v['available_predeposit'].L('currency_zh').'('.L('member_index_frozen,im_colon').$v['freeze_predeposit'].L('currency_zh').')');
            }
            $excel_data = $excel_obj->charset($excel_data,CHARSET);
            $excel_obj->addArray($excel_data);
    ...
    public function get_member_xmlOp(){
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                $out_array['available_predeposit'] = imPriceFormat($v['available_predeposit']);
                $out_array['freeze_predeposit'] = imPriceFormat($v['freeze_predeposit']);
    ...
    public function scaleOp(){
        ...
        if ($_GET['exporttype'] == 'excel'){
            ...
            $excel_data[0][] = array('styleid'=>'s_title','data'=>'增云币');
            $excel_data[0][] = array('styleid'=>'s_title','data'=>'减云币');
            //data
