...
class store_classControl extends SystemControl{
    ...
    public function store_classOp(){
        ...
        if (chksubmit()){
            if (!empty($_POST['check_sc_id']) && is_array($_POST['check_sc_id']) ){
                ...
                if ($result) {
                    $this->log(L('im_del,store_class').'[ID:'.implode(',',$_POST['check_sc_id']).']',1);
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $this->log(L('im_add,store_class').'['.$_POST['sc_name'].']',1);
                    showMessage($lang['im_common_save_succ'],$url,'html','succ',1,5000);
                }else {
    ...
    public function store_class_editOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $this->log(L('im_edit,store_class').'['.$_POST['sc_name'].']',1);
                    showMessage($lang['im_common_save_succ'],'index.php?act=store_class&op=store_class');
                }else {
                    showMessage($lang['im_common_save_fail']);
                }
            }
    ...
    public function store_class_delOp(){
        ...
        if (intval($_GET['sc_id']) > 0){
            ...
            if ($result) {
                 $this->log(L('im_del,store_class').'[ID:'.$_GET['sc_id'].']',1);
                 showMessage($lang['im_common_del_succ'],getReferer());
            }
        }
        showMessage($lang['im_common_del_fail'],'index.php?act=store_class&op=store_class');
    }
