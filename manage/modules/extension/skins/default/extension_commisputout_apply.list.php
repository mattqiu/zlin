  ...
  <table class="table tb-type2">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="tc">
          <a href="javascript:void(0)"  onclick="ajax_form('my_apply_info','查看申请人信息','<?php echo urlAdminExtension('extension_commisputout','apply_info',array('id'=>$v['ai_id']));?>',280);" title="查看申请人信息"><span>查看</span></a>|       
		  <?php if ($v['ai_dispose']==0){?>
          <a href="javascript:void(0)"  onclick="ajax_form('my_apply_edit','提佣申请审核','<?php echo urlAdminExtension('extension_commisputout','apply_edit',array('id'=>$v['ai_id']));?>',480);" title="审核提佣申请"><span>审核</span></a>
          <?php }else{?>
          <a href="javascript:void(0)" onclick="javascript:ajax_get_confirm('真的要删除吗?','<?php echo urlAdminExtension('extension_commisputout', 'apply_del',array('id'=>$v['ai_id']));?>')">删除</a>
          <?php }?>      
        </td>
      </tr>
      <?php }?>
