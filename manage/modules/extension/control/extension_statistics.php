...
class extension_statisticsControl extends BaseExtensionControl {
	private $links = array(
	    array('url'=>'act=extension_statistics&op=commis_statistics','lang'=>'commis_statistics'),
        array('url'=>'act=extension_statistics&op=commis_paihang','lang'=>'commis_paihang'),
    );
	 ...
	 * @return 
	 */
	public function statistics($title='') {
		if($_GET['type'] == 'today'){	
		...
		$where = $flow_tablename.".store_id = '".GENERAL_PLATFORM_EXTENSION_ID."' and ".$flow_tablename.".give_status = ".$give_status." and ".$flow_tablename.".saleman_type < 3 ";
		if ($request_date_str !=''){
		...
		Tpl::output('top_link',$this->sublink($this->links,'commis_statistics'));
		Tpl::setDirquna('extension');
		Tpl::showpage('extension_statistics.statistics');
	}

	
	public function commis_paihangOp() {
		self::paihang('平台','','commis_paihang');
	}
	...
	public function paihang($title='',$type='',$menukey='') {
		...
		Tpl::output('top_link',$this->sublink($this->links,'commis_paihang'));
		Tpl::setDirquna('extension');
		Tpl::showpage('extension_statistics.paihang');
	}
}
