...
class extension_commisputoutControl extends BaseExtensionControl {
	private $links = array(
	    array('url'=>'act=extension_commisputout&op=index','lang'=>'commisputout_list'),
		array('url'=>'act=extension_commisputout&op=balance_list','lang'=>'commisputout_balance'),
        array('url'=>'act=extension_commisputout&op=apply_list','lang'=>'commisputout_apply'),
    );
    ...
    public function __construct() {
		...
		$this->appay_count = Model('apply_information')->getApplyCount(array('ai_target'=>GENERAL_PLATFORM_EXTENSION_ID,'ai_type'=>4,'ai_dispose'=>0));
    ...
    public function indexOp() {
		$where = "extension_commis_detail.store_id = '".GENERAL_PLATFORM_EXTENSION_ID."' and extension_commis_detail.give_status = 0 and extension_commis_detail.saleman_type < 3";			
        ...
        $flow_list = $this->calculateCommission(GENERAL_PLATFORM_EXTENSION_ID, C('gl_promotion_level'), $where);
		...
		Tpl::showpage('extension_commisputout.index');
    }
    ...
    public function balance_editOp(){
		...
		//提佣金额
		$where = "extension_commis_detail.store_id = '".GENERAL_PLATFORM_EXTENSION_ID."' and extension_commis_detail.give_status = 0 and extension_commis_detail.saleman_type < 3".$promotion_id;
        $flow_list = $this->calculateCommission(GENERAL_PLATFORM_EXTENSION_ID, C('gl_promotion_level'), $where);
		$pay_commis = 0;
		$promotion_name = '';
        ...
        Tpl::setDirquna('extension');
        Tpl::showpage('extension_commisputout_balance.edit','null_layout');		
	}
    ...
    public function balance_saveOp(){		
        ...
        showDialog('佣金结算操作成功!',urlAdminExtension('extension_commisputout', 'index'),'succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
	}
			...
			foreach($flow_list as $k => $info) {
				...
				if ($info['mc_real']==10){					
				    if ($rate_perfor>0 && !empty($perfor_award)){						
					    $award_perfor_totals = $this->calculate_perforaward($perfor_award,GENERAL_PLATFORM_EXTENSION_ID,$info['saleman_id'],$info['mc_id'],$info['mc_level']);
				    }
				}
				...
				if ($info['mc_id']==1 || $info['mc_id']==2 || $info['mc_id']==3 || $info['mc_id']==4 || $info['mc_id']==5){
				    if ($rate_manage>0 && !empty($manage_award)){
						$award_manage_value = $this->calculate_manageraward($manage_award,GENERAL_PLATFORM_EXTENSION_ID,$info['saleman_id'],$info['mc_id'],$info['mc_level']);
    ...
    public function commisputout_detailOP() {
		...
		$this->links[] = array('url'=>'act=extension_commisputout&op=commisputout_detail','lang'=>'commisputout_detail');
		Tpl::output('top_link',$this->sublink($this->links,'commisputout_detail'));
		
		Tpl::setDirquna('extension');
        Tpl::showpage('extension_commisputout.detail');
    } 
    ...
    public function balance_listOp() {
		...
		$where = array();
		$where['pde_store_id'] = GENERAL_PLATFORM_EXTENSION_ID;
		//时段
		...
		Tpl::output('top_link',$this->sublink($this->links,'balance_list'));
		Tpl::setDirquna('extension');
        Tpl::showpage('extension_commisputout_balance.list');		 
	}
    ...
    public function extcommis_infoOp() {
		...
		Tpl::setDirquna('extension');
        Tpl::showpage('extension_commisputout.extcommisinfo');
    }
    ...
    public function apply_listOp() {
		...
		$apply_list = $model_apply->getCommisputOutApplyList(GENERAL_PLATFORM_EXTENSION_ID);		
        ...
        Tpl::setDirquna('extension');
        Tpl::showpage('extension_commisputout_apply.list');
    }
    ...
    public function apply_editOp() {
		...
		Tpl::output('apply_info',$apply_info);
		//提佣金额
		$where = "extension_commis_detail.store_id = '".GENERAL_PLATFORM_EXTENSION_ID."' and extension_commis_detail.give_status = 0 and saleman_id=".$apply_info['ai_from'];
        $flow_list = $this->calculateCommission(GENERAL_PLATFORM_EXTENSION_ID, C('gl_promotion_level'), $where);
		$pay_commis = 0;
		if (!empty($flow_list) && is_array($flow_list)){
			foreach($flow_list as $k => $info) {
		...
		Tpl::setDirquna('extension');
        Tpl::showpage('extension_commisputout_apply.edit','null_layout');
    }
    ...
    public function apply_saveOp() {
		...
		if (empty($ai_id) || $ai_id<=0 || $verify<0 || $verify>2){
			showDialog('参数错误!',urlAdminExtension('extension_commisputout', 'apply_list'));
		}		
		...
		if ($verify==2){				  
			...
			//提佣金额
		    $where = "extension_commis_detail.store_id = '".GENERAL_PLATFORM_EXTENSION_ID."' and extension_commis_detail.give_status = 0 and saleman_id=".$member_id;
            $flow_list = $this->calculateCommission(GENERAL_PLATFORM_EXTENSION_ID, C('gl_promotion_level'), $where);
		    $pay_commis = 0;
		    if (!empty($flow_list) && is_array($flow_list)){
			    foreach($flow_list as $k => $info) {
        ...
        showDialog('审核操作成功!',urlAdminExtension('extension_commisputout', 'apply_list'),'succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
    }	
    ...
    public function apply_delOp() {
		...
		showDialog('申请信息删除成功!',urlAdminExtension('extension_commisputout', 'apply_list'));
	}	
    ...
    public function extractCommission($flow_list = array(),$pay_commis = 0){	
		if (!empty($flow_list) && is_array($flow_list)){
			...
			foreach($flow_list as $k => $info) {
				...
				$param['pde_store_id']      = GENERAL_PLATFORM_EXTENSION_ID; //店铺ID
				...
				$param['pde_admin']         = $this->admin_info['name']; //结算操作
				...
				$param['pde_payment_admin'] = $this->admin_info['name']; //支付操作
		        ...
		        $predeposit['admin_name'] = $this->admin_info['name'];
