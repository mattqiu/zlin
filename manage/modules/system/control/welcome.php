...
class welcomeControl extends SystemControl{
	...
	public function statisticsOp(){
		...
		if (OPEN_STORE_EXTENSION_STATE==10){
            ...
            // 推广员人数
            $statistics['extension_total'] = $model_extension->getPromotionCount(GENERAL_PLATFORM_EXTENSION_ID);	
            // 本周新增
            $statistics['week_active_extension'] = $model_extension->getExtensionCount(array('createdtime' => array('egt', $tmp_time)));
            // 待审核
            $statistics['extension_apply'] = Model('apply_information')->getApplyCount(array('ai_target'=>GENERAL_PLATFORM_EXTENSION_ID,'ai_type'=>2,'ai_dispose'=>0));
			// 提佣申请
            $statistics['commisputout_apply'] = Model('apply_information')->getApplyCount(array('ai_target'=>GENERAL_PLATFORM_EXTENSION_ID,'ai_type'=>4,'ai_dispose'=>0));
		}		
