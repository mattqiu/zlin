...
class accountControl extends SystemControl{
    ...
    public function qqOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result === true){
                    $this->log(L('im_edit,qqSettings'),1);
                    showMessage(Language::get('im_common_save_succ'));
                }else {
                    $this->log(L('im_edit,qqSettings'),0);
	...
	public function wxOp(){
		...
		if (chksubmit()){
			$obj_validate = new Validate();
			if (trim($_POST['wx_isuse']) == '1'){
				$obj_validate->validateparam = array(
					array("input"=>$_POST["wx_appid"], "require"=>"true","message"=>Language::get('wx_appid_error')),
					array("input"=>$_POST["wx_appkey"], "require"=>"true","message"=>Language::get('wx_appkey_error'))
				);
			}
			$error = $obj_validate->validate();
			if ($error != ''){
				...
				$update_array = array();
				$update_array['wx_isuse'] 	= $_POST['wx_isuse'];
				$update_array['wx_appid'] 	= $_POST['wx_appid'];
				$update_array['wx_appkey'] 	= $_POST['wx_appkey'];
				$result = $model_setting->updateSetting($update_array);
				if ($result === true){
					$this->log(L('im_edit,wxSettings'),1);
					showMessage(Language::get('im_common_save_succ'));
				}else {
					$this->log(L('im_edit,wxSettings'),0);
					showMessage(Language::get('im_common_save_fail'));
				}
			}
		}
    ...
    public function sinaOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                $result = $model_setting->updateSetting($update_array);
                if ($result === true){
                    $this->log(L('im_edit,sinaSettings'),1);
                    showMessage(Language::get('im_common_save_succ'));
                }else {
                    $this->log(L('im_edit,sinaSettings'),0);
                    showMessage(Language::get('im_common_save_fail'));
                }
            }
        }
