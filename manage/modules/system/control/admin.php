...
class adminControl extends SystemControl{
    ...
    public function admin_delOp(){
        if (!empty($_GET['admin_id'])){
            ...
            $this->log(L('im_delete,limit_admin').'[ID:'.intval($_GET['admin_id']).']',1);
    ...
    public function admin_addOp(){
        if (chksubmit()){
            ...
            if ($rs){
                $this->log(L('im_add,limit_admin').'['.$_POST['admin_name'].']',1);
                showMessage(L('im_common_save_succ'),'index.php?act=admin&op=admin');
            }else {
                showMessage(L('im_common_save_fail'));
            }
        }
    ...
    public function admin_editOp(){
        if (chksubmit()){
            ...
            if ($result){
                $this->log(L('im_edit,limit_admin').'[ID:'.intval($_GET['admin_id']).']',1);
    ...
    public function gadminOp(){
        ...
        if (chksubmit()){
            ...
            if (!empty($_POST['del_id'])){
                ...
                $this->log(L('im_delete,limit_gadmin').'[ID:'.implode(',',$_POST['del_id']).']',1);
                showMessage(L('im_common_del_succ'));
            }else {
                showMessage(L('im_common_del_fail'));
            }
    ...
    public function gadmin_addOp(){
        if (chksubmit()){
            ...
            if ($model->insert($data)){
                $this->log(L('im_add,limit_gadmin').'['.$_POST['gname'].']',1);
                showMessage(L('im_common_save_succ'),'index.php?act=admin&op=gadmin');
            }else {
                showMessage(L('im_common_save_fail'));
            }
        }
    ...
    public function gadmin_editOp(){
        ...
        if (chksubmit()){
            ...
            if ($update){
                $this->log(L('im_edit,limit_gadmin').'['.$_POST['gname'].']',1);
                showMessage(L('im_common_save_succ'),'index.php?act=admin&op=gadmin');
            }else {
    ...
    public function gadmin_delOp(){
        if (is_numeric($_GET['gid'])){
            ...
            $this->log(L('im_delete,limit_gadmin').'[ID'.intval($_GET['gid']).']',1);
            showMessage(Language::get('im_common_op_succ'),'index.php?act=admin&op=gadmin');
        }else {
