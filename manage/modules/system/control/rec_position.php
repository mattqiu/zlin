...
class rec_positionControl extends SystemControl{
    ...
    public function get_xmlOp(){
        ...
        foreach ($list as $k => $info) {
            ...
            $operation_detail .= "<li><a href=\"javascript:void(0);\" rec_id=\"{$info['rec_id']}\" imtype=\"jscode\"></i>调用代码</a></li>";
            ...
            if (is_array($_GET['rec_id'])) {
                foreach($_GET['rec_id'] as $rec_id) {
                    ...
                    if ($result){
                        ...
                        $this->log(L('im_del,rec_position').'[ID:'.$rec_id.']',1);
                    }else{
                        exit(json_encode(array('state'=>false,'msg'=>'删除失败')));
                    }
                }
    ...
    public function rec_saveOp(){
            ...
            if (is_array($_FILES['pic']['tmp_name'])){
                foreach($_FILES['pic']['tmp_name'] as $k=>$v){
                    ...
                    if (in_array($ext,array('jpg','jpeg','gif','png'))){
                        ...
                        if ($_FILES['pic']['error'][$k] != 0) showMessage(Language::get('im_common_op_fail'));
                        $c = count($array['body']);
        ...
        $model = Model('rec_position');
        $model->insert($data);
        $this->log(L('im_add,rec_position').'['.$_POST['rtitle'].']',1);
        showMessage(Language::get('im_common_save_succ'),'index.php?act=rec_position&op=rec_list');
    }
    ...
    public function rec_edit_saveOp(){
            ...
            if (is_array($_FILES['pic']['tmp_name'])){
                foreach($_FILES['pic']['tmp_name'] as $k=>$v){
                    ...
                    if (in_array($ext,array('jpg','jpeg','gif','png','bmp'))){
                        ...
                        if ($_FILES['pic']['error'][$k] != 0) showMessage(Language::get('im_common_save_fail'));
        ...
        if ($result){
            ...
            showMessage(Language::get('im_common_save_fail'),'index.php?act=rec_position&op=rec_list');
        }
    }
