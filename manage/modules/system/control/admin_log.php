...
class admin_logControl extends SystemControl{
    ...
    public function list_delOp(){
        ...
        if (!Model('admin_log')->where($condition)->delete()){
            $this->log(L('im_del,im_admin_log'),0);
            exit(json_encode(array('state'=>false,'msg'=>'删除失败')));
        }else{
            $this->log(L('im_del,im_admin_log'),1);
    ...
    private function createExcel($data = array()){
        ...
        $excel_obj->addWorksheet($excel_obj->charset(L('im_admin_log'),CHARSET));
        $excel_obj->generateXML($excel_obj->charset(L('im_admin_log'),CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
    }
