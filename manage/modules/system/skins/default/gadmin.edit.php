...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?act=admin&op=gadmin" title="返回列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
        <h3><?php echo $lang['im_limit_manage'];?> - <?php echo $lang['im_edit'];?>权限组“<?php echo $output['ginfo']['gname'];?>”</h3>
        <h5><?php echo $lang['im_limit_manage_subhead'];?></h5>
      </div>
    </div>
  </div>
  <!-- 操作说明 -->
  ...
  <form id="add_form" method="post" name="adminForm">
    ...
    <div class="imap-form-all">
      ...
      <dl class="row">
        <dt class="tit">
          <span><input class="checkbox" type="checkbox" imtype="modulesAll">
          <?php echo $value['name'];?>模块功能</span></dt>
        <dd class="opt nobg nopd nobd nobs">
          <?php foreach ($value['child'] as $ke => $val) {?>
          <div class="imap-account-container">
            <h4>
              <input class="checkbox" type="checkbox" imtype="groupAll">
              <?php echo $val['name'];?>操作</h4>
            <ul class="imap-account-container-list">
      ...
      <?php } ?>
      <div class="fix-bot"><a href="JavaScript:void(0);" class="imap-btn-big imap-btn-green" onclick="document.adminForm.submit()"><?php echo $lang['im_submit'];?></a></div>
    </div>
  </form>
</div>
<script>
...
$(function(){
	...
	$("#add_form").validate({
        ...
        messages : {
            gname : {
                required : '<i class="fa fa-exclamation-circle"></i><?php echo $lang['im_none_input'];?>',
