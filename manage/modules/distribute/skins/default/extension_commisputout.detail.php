...
<div class="page">
  ...
  <table class="table tb-type2">
    <thead>
      <tr>
        <th class="w12"></th>
        <th class="w36 tc">编号</th>
        <th class="w100 tl">推广员</th> 
        <th class="w150 tc">订单编号</th>      
        <th class="w80 tc">总佣金</th>
        <th class="w80 tc">抽佣率(%)</th>
        <th class="w80 tc">抽佣金额</th>
        <th class="w120 tc">完成时间</th>
    ...
    <tbody>
      ...
      <tr class="bd-line">
        <td></td>
        <td class="tc"><?php echo $v['mcd_id'];?></td>
        <td class="tl"><?php echo $v['extension_name'];?><a href="<?php echo urlShop('member_snshome', 'index',array('mid'=>$v['extension_id']));?>" target="_blank">(查看)</a></td> 
        <td class="tc"><?php echo $v['order_sn'];?><a href="<?php echo urlAdminshop('order', 'show_order',array('order_id'=>$v['order_id']));?>" target="_blank">(查看订单)</a></td>
...
$(function(){
	...
	$('#today_flow').click(function(){
	    window.location.href = 'index.php?act=extension_commisputout&op=commisputout_detail&type=today&promotion_id=<?php echo $output['promotion_id'];?>';
	})
	$('#week_flow').click(function(){
	    window.location.href = 'index.php?act=extension_commisputout&op=commisputout_detail&type=week&promotion_id=<?php echo $output['promotion_id'];?>';
	})
	$('#month_flow').click(function(){
	    window.location.href = 'index.php?act=extension_commisputout&op=commisputout_detail&type=month&promotion_id=<?php echo $output['promotion_id'];?>';
	})
	$('#year_flow').click(function(){
	    window.location.href = 'index.php?act=extension_commisputout&op=commisputout_detail&type=year&promotion_id=<?php echo $output['promotion_id'];?>';
	})
});
</script>