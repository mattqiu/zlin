...
$(document).ready(function(){
    ...
    $('#template_module_list').on('click', '[imtype="btn_add_page_module"]', function() {
        var $btn_add = $(this);
        var page_module = {};
        ...
        $.post("index.php?act=cms_index&op=add_page_module", { module_title: page_module.module_title, module_name: page_module.module_name, module_type: page_module.module_type }, function(data) {
            if(data.result) {
                ...
                $('.page-module').last().find('span[im_type="module_title"]').inline_edit_confirm({act: 'cms_index', op:'update_page_module_title'});
    ...
    }, '[imtype="btn_drop_module"]');

    //模块删除
    $('#template_module_list').on('click', '[imtype="btn_drop_module"]', function() {
        var $btn_drop = $(this);
        if(confirm('确认删除?')) {
    ...
    //启用页面模块
    $('#page_view').on('click', "[imtype='btn_show_page_module']", function() {
        var $btn = $(this);
        ...
        $.post("index.php?act=cms_index&op=update_page_module_show", { module_id: module_id }, function(data) {
            if(data.result) {
                ...
                $btn.attr('imtype', 'btn_hide_page_module');
                $btn.text('关闭');
            } else {
                showError(data.message);
            }
    ...
    //启用页面模块
    $('#page_view').on('click', "[imtype='btn_hide_page_module']", function() {
        var $btn = $(this);
        var module_id = $btn.attr('data-module-id');
        $.post("index.php?act=cms_index&op=update_page_module_hide", { module_id: module_id }, function(data) {
            if(data.result) {
                ...
                $btn.attr('imtype', 'btn_show_page_module');
                $btn.text('启用');
            } else {
                showError(data.message);
            }
    ...
    //删除页面模块
    $('#page_view').on('click', "[imtype='btn_drop_page_module']", function() {
        if(confirm('确认删除')) {
            var $btn_drop = $(this);
    ...
    $('span[im_type="module_title"]').inline_edit_confirm({act: 'cms_index', op:'update_page_module_title'});
    ...
    $('#btn_add_module').click(function() {
        ...
        $('#dialog_add_module').im_show_dialog({
            width: 640,
            title: '添加自定义模块'
        });
    });

    $('#module_frame_list').on('click', '[imtype="btn_select_module_frame"]', function() {
    ...
    $('#module_frame').on('click', '[imtype="btn_select_module_frame_block"]', function() {
        $('[imtype="btn_select_module_frame_block"]').removeClass('module-frame-current').addClass('module-frame-normal');
        $(this).addClass('module-frame-current').removeClass('module-frame-normal');
        $current_module_frame = $(this);
    });

    $('#module_assembly_list').on('click', '[imtype="btn_select_module_assembly"]', function() {
    ...
    $('#btn_save_module').click(function() {
        ...
        var frame_block = {};
        $('[imtype="btn_select_module_frame_block"]').each(function(index, item) {
    ...
    //主题选择
    $('#page_view').on('click', "[imtype='btn_module_style_change']", function() {
        $(this).parents('.handle').find('.module-style-change').show();
    });

    //取消主题选择
    $('#page_view').on('click', "[imtype='btn_module_style_change_cancel']", function() {
        $(this).parents('.module-style-change').hide();
    });

    //主题修改
    $('#page_view').on('click', "[imtype='btn_module_style_select']", function() {
        var $btn = $(this);
        var module_id = $btn.attr('data-module-id');
    ...
    //后台显示样式修改
    $('#page_view').on('click', "[imtype='btn_module_view_change']", function() {
        var $btn = $(this);
        var module_id = $btn.attr('data-module-id');
        var $page_module = $btn.parents('.page-module');
        var module_view;
...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <div class="subject">
        <h3><?php echo $lang['im_cms_index_manage'];?></h3>
        <h5><?php echo $lang['im_cms_index_manage_subhead'];?></h5>
      </div>
    </div>
  </div>
  <div class="fix-bot"><a class="imap-btn-big imap-btn-orange" href="index.php?act=cms_index&op=cms_index_preview" target="_blank">页面预览</a> <a class="imap-btn-big imap-btn-blue ml10" id="btn_index_build" href="Javascript: void(0)"><?php echo $lang['cms_index_build'];?></a> <a class="imap-btn-big imap-btn-green ml10" href="<?php echo CMS_SITE_URL;?>" target="_blank"><?php echo $lang['cms_index_go'];?></a></div>
...
<div id="dialog_add_module" style="display:none;">
  ...
  <div id="module_frame_edit" style="display:none;">
    ...
    <form id="from_new_module" class="module-frame-btns">
      ...
      <div class="bot"><a id="btn_show_module_frame_list" href="JavaScript:;" class="imap-btn-big imap-btn-green"><span>重新选择框架</span></a> <a id="btn_save_module" href="JavaScript:;" class="imap-btn-big imap-btn-green"><span>保存</span></a></div>
    </form>
    <div class="clear"></div>
  </div>
</div>
...
<div id="page_module_<%=module_id%>" data-module-id="<%=module_id%>" data-module-name="<%=module_name%>" class="page-module <%=module_css%> <%=module_state_css%> module-css-<%=module_type%> module-style-<%=module_style%>">
    <div class="title">
    <span im_type="module_title" column_id="<%=module_id%>" title="<?php echo $lang['im_editable'];?>"><i></i><%=module_title%></span>
    </div>
    <div class="handle">
    <div class="module-style-change" style="display:none;">
    <ul>
        <li imtype="btn_module_style_select" data-module-id="<%=module_id%>" data-module-style="style1" class="style1">style1</li>
        <li imtype="btn_module_style_select" data-module-id="<%=module_id%>" data-module-style="style2" class="style2">style2</li>
        <li imtype="btn_module_style_select" data-module-id="<%=module_id%>" data-module-style="style3" class="style3">style3</li>
        <li imtype="btn_module_style_select" data-module-id="<%=module_id%>" data-module-style="style4" class="style4">style4</li>
        <li imtype="btn_module_style_select" data-module-id="<%=module_id%>" data-module-style="style5" class="style5">style5</li>
        <li imtype="btn_module_style_select" data-module-id="<%=module_id%>" data-module-style="style6" class="style6">style6</li>
        <li imtype="btn_module_style_select" data-module-id="<%=module_id%>" data-module-style="style7" class="style7">style7</li>
        <li imtype="btn_module_style_select" data-module-id="<%=module_id%>" data-module-style="style8" class="style8">style8</li>
    </ul>
    <a imtype="btn_module_style_change_cancel" href="JavaScript:;" class="cancel" title="取消">取消</a>
    </div>
    <a imtype="btn_module_style_change" href="JavaScript:;" class="style" title="主题">主题</a>
    <a href="index.php?act=cms_index&op=module_edit&module_id=<%=module_id%>" class="edit" title="<?php echo $lang['im_edit'];?>"></a>
        <a imtype="<%=module_state_operate%>" data-module-id="<%=module_id%>" class="onoff" href="JavaScript:;" title="<%=module_state_operate_text%>"></a>
        <a imtype="btn_drop_page_module" data-module-id="<%=module_id%>" class="del" href="JavaScript:;" title="删除"></a>
        <a imtype="btn_module_view_change" data-module-id="<%=module_id%>" class="move" href="JavaScript:;" title="<%=module_view_text%>"></a>
    </div>
</div>
</script>
...
<div class="template-module module-css-<%=module_type%>" >
  ...
  <a imtype="btn_add_page_module" class="add" data-module-title="<%=module_title%>" data-module-name="<%=module_name%>" data-module-type="<%=module_type%>" href="JavaScript:;">添加</a>
  <%if(module_class == 2) {%>
  <a imtype="btn_drop_module" class="delete" data-module-id="<%=module_id%>" href="JavaScript:;">删除</a>
  <%}%>
</div>
</script>
<!-- 模块框架列表模板 -->
...
<%for(i = 0; i < list.length; i ++) {%>
    <div imtype="btn_select_module_frame" data-frame-name="<%=list[i].frame_name%>" href="JavaScript:;" data-frame-structure='<%=list[i].frame_structure%>' class="module-frame frame-css-<%=list[i].frame_name%>" >
    <%=list[i].frame_title%>
</div>
<%}%>
</script>
...
<div class="cms-module-frame">
    ...
    <%if(!list[block].child) {%>
    <div imtype="btn_select_module_frame_block" data-block-name="<%=block%>" class="cms-module-frame-<%=list[block].name%> module-frame-normal"></div>
    <%} else {%>
    <div class="cms-module-frame-<%=list[block].name%>">
        <%for(var block_child in list[block].child) {%>
        <div imtype="btn_select_module_frame_block" data-block-name="<%=block_child%>" class="cms-module-frame-<%=list[block].child[block_child].name%> module-frame-normal"></div>
        <%}%>
    </div>
    <%}%>
    <%}%>
...
<%for(i = 0; i < list.length; i ++) {%>
    <div imtype="btn_select_module_assembly" data-assembly-name="<%=list[i].assembly_name%>" class="module-assembly assembly-css-<%=list[i].assembly_name%>" >
    <i></i>
