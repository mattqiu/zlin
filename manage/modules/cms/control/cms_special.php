...
class cms_specialControl extends SystemControl{
    ...
    public function cms_special_saveOp() {
        ...
        if($result) {
            ...
            showMessage(Language::get('im_common_save_succ'), self::LINK_CMS_SPECIAL);
        } else {
            $this->log(Language::get('cms_log_special_save').$result, 0);
            showMessage(Language::get('im_common_save_fail'), self::LINK_CMS_SPECIAL);
        }
    }
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'special_list'=>array('menu_type'=>'link','menu_name'=>Language::get('im_manage'),'menu_url'=>'index.php?act=cms_special&op=cms_special_list'),
            'special_add'=>array('menu_type'=>'link','menu_name'=>Language::get('im_new'),'menu_url'=>'index.php?act=cms_special&op=cms_special_add'),
