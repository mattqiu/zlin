...
Tpl::showpage('cms_index_module.edit');
    ...
    public function save_page_moduleOp() {
        ...
        if($result) {
            ...
            showMessage(Language::get('im_common_save_succ'), 'index.php?act=cms_index&op=cms_index');
        } else {
            $this->log(Language::get('cms_log_index_edit').$_POST['module_id'], 0);
            showMessage(Language::get('im_common_save_fail'), '', '','error');
        }
    }
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'cms_index'=>array('menu_type'=>'link','menu_name'=>Language::get('im_manage'),'menu_url'=>'index.php?act=cms_index&op=cms_index'),
        );
        if($menu_key == 'module_edit') {
            $menu_array['module_edit'] = array('menu_type'=>'link','menu_name'=>Language::get('im_edit'),'menu_url'=>'###');
        }
