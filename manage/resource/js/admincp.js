...
//布局换色设置
        ...
        $("div.bgSelector").toggle(300, function() {
            if ($(this).html() == '') {
                $(this).sColor({
                    ...
                    curImg: ADMIN_SKINS_URL + '/images/cur.png',  //必填，颜色选择对象图片路径
	...
	$("#login_out").click(function(){
		window.location.href="index.php?act=index&op=logout";		
	}); 
    ...
    // 顶部各个模块切换
    $('.im-module-menu').find('a').click(function(){
        if ($('.admincp-container').hasClass('unfold')) {
            $('.sub-menu').hide();
        }
        $('.im-module-menu').find('li').removeClass('active');
    ...
    //if ($.cookie('workspaceParam') == null || $.cookie('workspaceParam') == '') {
        //默认选择第一个菜单
        $('.im-module-menu').find('li:first > a').click();
    //} else {	
    //    openItem($.cookie('workspaceParam'));
    //}	

    // 导航菜单  显示
    $('a[imtype="map_on"],a[class="add-menu"]').click(function(){
        $('div[imtype="map_nav"]').show();
    });
    // 导航菜单 隐藏
    $('a[imtype="map_off"]').click(function(){
        $('div[imtype="map_nav"]').hide();
    });
    // 导航菜单切换
        ...
        if ($this.parent().hasClass('selected')) {
            $.getJSON('index.php?act=common&op=common_operations', {type : 'del', value : _value}, function(data){
                if (data) {
                    ...
                    $('ul[imtype="quick_link"]').find('a[onclick="openItem(\'' + _value + '\')"]').parent().remove();
                }
            });
        } else {
            ...
            $.getJSON('index.php?act=common&op=common_operations', {type : 'add', value : _value}, function(data){
                if (data) {
                    ...
                    $('ul[imtype="quick_link"]').append('<li><a onclick="openItem(\'' + _value + '\')" href="javascript:void(0);">' + _name + '</a></li>');
                }
            });
        }
    ...
    if (_commonPendingMatters > 0) {		
        $('li[imtype="pending_matters"]').show().find('em').html($.cookie('commonPendingMatters'));
    }
});

// 点击菜单，iframe页面跳转
function openItem(param) {	
    $('.sub-menu,.im-module-menu').find('li').removeClass('active');
