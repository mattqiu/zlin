...
//唯品会商品推荐相关
function show_vip_recommend_dialog(id) {//弹出框
	recommend_show = id;
...
function show_recommend_dialog(id) {//弹出框
	...
	//初始化颜色控件
	$('#recommend_pic_bgcolor'+id).colorpicker({showOn:'both'});//初始化广告位背景颜色控件
    $('#recommend_pic_bgcolor'+id).parent().css("width",'');
    $('#recommend_pic_bgcolor'+id).parent().addClass("color");
	//初始化背景图像上传控件	
	var obj = $("#recommend_list_form");	
	var imgid = obj.find("input[name='web_id']").val()+'-'+obj.find("input[name='code_id']").val()+'-'+id;
	/* 商品图片ajax上传 */
    obj.find("input[name='bg_image"+id+"']").fileupload({
        dataType: 'json',
        url: 'index.php?act=web_config&op=bgimage_upload',
        formData: {name:'bg_image'+id,imgid: imgid},
        add: function (e,data) {
            data.submit();
        },
            ...
            if (typeof(param.error) != 'undefined') {
                ...
                $('input[name="recommend_list['+id+'][recommend][bgimg]"]').val(param.name);
                //$('#bg_image_pre'+id).attr('href',param.fullname);
				$('#bg_image_load'+id).attr('onMouseOver',"toolTip('<img src="+param.fullname+">')");
            }
        }
    });
}
...
function add_recommend() {//增加商品推荐
	for (var i = 1; i <= recommend_max; i++) {//防止数组下标重复
		if ($(".middle dl[recommend_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			$("#add_recommend_list").before('<div class="imap-form-default" select_recommend_id="'+i+'"><dl class="row"><dt class="tit">商品推荐模块标题名称</dt>'+
    			...
    			'<div class="imap-form-all" select_recommend_id="'+i+'"><dl class="row"><dt class="tit">推荐商品</dt><dd class="opt"><ul class="dialog-goodslist-s1 goods-list">'+
    			'</ul></dd></dl></div>');
			$("#recommend_list_form dl dd ul").sortable({ items: 'li' });
			break;
		}
...
function add_vip_recommend() {//增加唯品会商品推荐
	for (var i = 1; i <= recommend_max; i++) {//防止数组下标重复
		if ($(".middle dl[recommend_id='"+i+"']").size()==0) {//编号不存在时添加
			var add_html = '';
			var del_append = '';
			del_append = '<a href="JavaScript:del_recommend('+i+');"><i class="fa fa-trash"></i>删除</a>';//删除
			add_html = '<dl recommend_id="'+i+'"><dt><h4>商品推荐</h4>'+del_append+
    			'<a href="JavaScript:show_vip_recommend_dialog('+i+');"><i class="fa fa-shopping-cart"></i>商品块</a><a href="JavaScript:show_vip_recommend_pic_dialog('+i+');"><i class="icon-lightbulb"></i>广告块</a></dt>'+
    			'<dd><ul class="goods-list"><li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li>'+
    			'<li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li>'+
    			'<li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li></ul></dd></dl>';
			$("#btn_add_list").before(add_html);
...
function select_recommend_goods(goods_id) {//商品选择
	...
	var consumer_amount = goods.attr("consumer_amount");
	var promotion_amount = goods.attr("promotion_amount");
	var goods_jingle = goods.attr("goods_jingle");
	var have_gift = goods.attr("have_gift");
	var goods_freight = goods.attr("goods_freight");
	var goods_commend = goods.attr("goods_commend");
	var goods_advpic = goods.attr("goods_advpic");
	...
	text_append += '<input name="recommend_list['+id+'][goods_list]['+goods_id+'][consumer_amount]" value="'+consumer_amount+'" type="hidden">';
	text_append += '<input name="recommend_list['+id+'][goods_list]['+goods_id+'][promotion_amount]" value="'+promotion_amount+'" type="hidden">';
	text_append += '<input name="recommend_list['+id+'][goods_list]['+goods_id+'][goods_jingle]" value="'+goods_jingle+'" type="hidden">';
	text_append += '<input name="recommend_list['+id+'][goods_list]['+goods_id+'][have_gift]" value="'+have_gift+'" type="hidden">';
	text_append += '<input name="recommend_list['+id+'][goods_list]['+goods_id+'][goods_freight]" value="'+goods_freight+'" type="hidden">';
	text_append += '<input name="recommend_list['+id+'][goods_list]['+goods_id+'][goods_commend]" value="'+goods_commend+'" type="hidden">';
	text_append += '<input name="recommend_list['+id+'][goods_list]['+goods_id+'][goods_advpic]" value="'+replace_url(goods_advpic)+'" type="hidden">';	
...
//背景图片上传
function update_bg_pic(id,pic) {//更新图片
	var obj = $("#picture_"+id);
	obj.html('<img src="'+UPLOAD_SITE_URL+'/'+pic+'" />');
	DialogManager.close("upload_bg");
}

//中部唯品会推荐图片上传
function show_vip_recommend_pic_dialog(id) {//弹出框
    ...
    if(obj.find("div[select_recommend_pic_id='"+id+"']").size()==0) {//不存在时添加
        var text_append = '<div select_recommend_pic_id="'+id+'" class="middle-banner">'+
            	  ...
            	  '</div>';
        obj.find("#vip_recommend_pic").append(text_append);
	}
	...
	//初始化颜色控件
	$('#recommend_pic_bgcolor').colorpicker({showOn:'both'});//初始化广告位背景颜色控件
    $('#recommend_pic_bgcolor').parent().css("width",'');
    $('#recommend_pic_bgcolor').parent().addClass("color");
	//初始化背景图像上传控件	
	var imgid = obj.find("input[name='web_id']").val()+'-'+obj.find("input[name='code_id']").val()+'-'+obj.find("input[name='key_id']").val();
	/* 商品图片ajax上传 */
    obj.find("input[name='bg_image']").fileupload({
        dataType: 'json',
        url: 'index.php?act=web_config&op=bgimage_upload',
        formData: {name:'bg_image',imgid: imgid},
        add: function (e,data) {
            data.submit();
        },
        done: function (e,data) {
            ...
            if (typeof(param.error) != 'undefined') {
                ...
                $('input[name="recommend_list[recommend][bgimg]"]').val(param.name);
                //$('#bg_image_pre'+id).attr('href',param.fullname);
				$('#bg_image_load').attr('onMouseOver',"toolTip('<img src="+param.fullname+">')");
            }
        }
    });
}
...
function add_slide_adv() {//增加图片
	for (var i = 1; i <= slide_pic_max; i++) {//防止数组下标重复
		if ($("#upload_adv_form ul li[slide_adv_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			');"></span><span class="thumb size-106x106" onclick="select_slide_adv('+i+');"><img src="'+ADMIN_SKINS_URL+
...
function del_slide_adv(pic_id) {//删除图片
    if ($("#upload_adv_form ul li").size()<1) {
         return;//保留一个
    }
...
function del_sale_list(id){//删除商品推荐
    if ($("dl[sale_id]").size()<1) {
         return;//保留一个 //zhang 一个不留
    }
	if(confirm('您确定要删除吗?')){
...
function add_sale_list() {//增加商品推荐
	for (var i = 1; i <= sale_max; i++) {//防止数组下标重复
		if ($("dl[sale_id='"+i+"']").size()==0) {//编号不存在时添加
			var add_html = '';
			add_html = '<dl sale_id="'+i+'"><dt class="title"><h4>商品推荐</h4><a href="JavaScript:del_sale_list('+i+');" class="imap-btn-mini del"><i class="fa fa-trash"></i>删除</a><a href="JavaScript:show_sale_dialog('+i+
    			');" class="imap-btn-mini edit"><i class="fa fa-pencil-square-o"></i>编辑</a><input name="sale_list['+i+'][recommend][name]" value="" type="hidden"></dt>'+
    			'<dd><ul class="goods-list"><li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li>'+
    			'<li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li></ul></dd></dl>';
			$("#add_list").before(add_html);
			$("#select_sale_list").before('<div class="imap-form-default" select_sale_id="'+i+'"><dl class="row"><dt class="tit">商品推荐模块标题名称</dt>'+
    			...
    			'</dd></dl></div><div class="imap-form-all" select_sale_id="'+i+'"><dl class="row"><dt class="tit">已选商品</dt><dd class="opt"><ul class="dialog-goodslist-s1 goods-list">'+
    			'</ul></dd></dl></div>');
...
function select_sale_goods(goods_id){//商品选择
	...
	var goods_name = goods.attr("goods_name");
	var goods_price = goods.attr("goods_price");
	var goods_salenum = goods.attr("goods_salenum");
	var market_price = goods.attr("market_price");
	var promotion_amount = goods.attr("promotion_amount");
	var consumer_amount = goods.attr("consumer_amount");
	var goods_jingle = goods.attr("goods_jingle");
	...
	text_append += '<span class="thumb size-72x72">';
	text_append += '<i></i>';
  	text_append += '<img select_goods_id="'+goods_id+'" title="'+goods_name+'" goods_name="'+goods_name+'" src="'+goods_pic+'" goods_price="'+goods_price+'" market_price="'+market_price+'" promotion_amount="'+promotion_amount+'" consumer_amount="'+consumer_amount+'" goods_salenum="'+goods_salenum+'" goods_jingle="'+goods_jingle+'" have_gift="'+have_gift+'" goods_freight="'+goods_freight+'" goods_commend="'+goods_commend+'" goods_advpic="'+goods_advpic+'" onload="javascript:DrawImage(this,72,72);" />';
	text_append += '</span></div>';
	text_append += '<div class="goods-name">';
...
function update_sale(){//更新
    ...
    obj.find("img").each(function(){
		...
		text_append += '<input name="sale_list['+id+'][goods_list]['+goods_id+'][promotion_amount]" value="'+promotion_amount+'" type="hidden">';
		text_append += '<input name="sale_list['+id+'][goods_list]['+goods_id+'][consumer_amount]" value="'+consumer_amount+'" type="hidden">';
		text_append += '<input name="sale_list['+id+'][goods_list]['+goods_id+'][goods_salenum]" value="'+goods_salenum+'" type="hidden">';	
		text_append += '<input name="sale_list['+id+'][goods_list]['+goods_id+'][goods_jingle]" value="'+goods_jingle+'" type="hidden">';
		text_append += '<input name="sale_list['+id+'][goods_list]['+goods_id+'][have_gift]" value="'+have_gift+'" type="hidden">';
		text_append += '<input name="sale_list['+id+'][goods_list]['+goods_id+'][goods_freight]" value="'+goods_freight+'" type="hidden">';
		text_append += '<input name="sale_list['+id+'][goods_list]['+goods_id+'][goods_commend]" value="'+goods_commend+'" type="hidden">';
		text_append += '<input name="sale_list['+id+'][goods_list]['+goods_id+'][goods_advpic]" value="'+goods_advpic+'" type="hidden">';
    	text_append += '</li>';
    });
