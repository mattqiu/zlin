$(document).ready(function(){
	...
	//给需要修改的位置添加修改行为
	$('span[im_type="inline_edit"]').click(function(){
		...
		$('span[im_type="inline_edit_textarea"]').click(function(){
		var s_value  = $(this).text();
		var s_name   = $(this).attr('fieldname');
		var s_id     = $(this).attr('fieldid');
		var req      = $(this).attr('required');
	...
	//给需要修改的图片添加异步修改行为
	$('img[im_type="inline_edit"]').click(function(){
	...
	$('a[im_type="inline_edit"]').live('click', function() {
		var i_id    = $(this).attr('fieldid');
		var i_name  = $(this).attr('fieldname');
		var i_src   = $(this).attr('src');
		var i_val   = ($(this).attr('fieldvalue'))== 0 ? 1 : 0;
    ...
    //给每个可编辑的小图片的父元素添加可编辑标题 $('img[im_type="inline_edit"]').parent().attr('title','可编辑');

    //给列表有排序行为的列添加鼠标手型效果
    $('span[im_type="order_by"]').hover(function(){$(this).css({cursor:'pointer'});},function(){});
...
//新的inline_edit调用方法
//javacript
//$('span[im_type="class_sort"]').inline_edit({act: 'microshop',op: 'update_class_sort'});
//html
//<span im_type="class_sort" column_id="<?php echo $val['class_id'];?>" title="<?php echo $lang['im_editable'];?>" class="editable tooltip"><?php echo $val['class_sort'];?></span>
//php
//$result = array();
