$(document).ready(function(){
    ...
    $("input[imtype='cms_image']").live("change", function(){
    ...
    $("#picture_image_upload").fileupload({
        dataType: 'json',
            url: "index.php?act=web_special&op=special_image_upload",
            add: function(e,data) {
                ...
                if(result.status == "success") {
					...
					var image_handle = '<a imtype="btn_show_image_insert_link" image_url="'+result.file_url+'" class="insert-link tooltip" title="以图片链接模式插入专题页">&nbsp;</a>';
                    image_handle += '<a imtype="btn_show_image_insert_hot_point" image_name="'+result.file_name+'" image_url="'+result.file_url+'" class="insert-hotpoint tooltip " title="以热点链接模式插入专题页">&nbsp;</a>';
                    image_handle += '<a imtype="btn_drop_special_image" image_name="'+result.file_name+'" class="delete tooltip " title="删除该图片">&nbsp;</a>';
    ...
    //图片删除
    $("[imtype='btn_drop_special_image']").live("click",function(){
        var image_object = $(this).parents("li");
        var image_name = $(this).attr("image_name");
        $.getJSON("index.php?act=web_special&op=special_image_drop", { image_name: image_name }, function(result){
                if(result.status == "success") {
                    image_object.remove();
                } else {
    ...
    //插入图片链接对话框
    $("[imtype='btn_show_image_insert_link']").live('click', function(){
    ...
    //插入图片链接
    $("[imtype='btn_image_insert_link']").live('click', function(){
        var html = $("#special_content").val();
        var item = $(this).parents("div");
        var link = item.find("[imtype='_image_insert_link']").val();
    ...
    //插入热点图
    $("[imtype='btn_show_image_insert_hot_point']").live('click', function(){
        ...
        image_object = dialog_object.find("[imtype='img_hot_point']");
        var x1 = dialog_object.find("[imtype='x1']");
        var y1 = dialog_object.find("[imtype='y1']");
        var x2 = dialog_object.find("[imtype='x2']");
        var y2 = dialog_object.find("[imtype='y2']");
        var w = dialog_object.find("[imtype='w']");
        var h = dialog_object.find("[imtype='h']");
        var url = dialog_object.find("[imtype='url']");
        ...
        $("[imtype='delete_hot_point']").live("click",function(){
            var key = $(this).attr("name");
            $("#"+key).remove();
            $("."+key).remove();
        });

        $("[imtype='select_hot_point']").live("click",function(){
        ...
        dialog_object.find("[imtype='btn_hot_point_commit']").click(function(){
            count++;
            ...
            dialog_object.find("[imtype='list']").append('<li class="'+key+'">热点区域'+count+'<span>('+url.val()+')</span><a imtype="select_hot_point" name="'+xy+'" href="javascript:void(0);" class="imap-btn imap-btn-blue mr5"><i class="fa fa-location-arrow"></i>选中</a><a imtype="delete_hot_point" class="imap-btn imap-btn-red" name="'+key+'" href="javascript:void(0);"><i class="fa fa-trash"></i>删除</a></li>');
    ...
    //插入图片链接
    $("[imtype='btn_image_insert_hot_point']").live('click', function(){
        var html = $("#special_content").val();
    ...
    //图片删除
    $("[imtype='btn_drop_special_image']").click(function() {
    ...
    //选择商品
    $("[imtype='btn_special_goods']").live('click', function(){
        var goods_list = $(this).parents("div").find("[imtype='_special_goods_list']");
        var link_item = $(this).parents("div").find("[imtype='_input_goods_link']");
        var link = link_item.val();
        link_item.val('');
        if(link != '') {
        var url = encodeURIComponent(link);
        $.getJSON("index.php?act=web_special&op=goods_info_by_url", { url: url}, function(data){
            if(data.result == "true") {
                ...
                temp += '<dd class="taobao-item-delete" imtype="btn_special_goods_delete" title="删除添加的商品">&nbsp;</dd>';
                temp += '</dl></li>';
    ...
    //删除商品
    $("[imtype='btn_special_goods_delete']").live('click', function(){
        $(this).parent().parent().remove();
    });

    //插入商品列表
    $("[imtype='btn_special_insert_goods']").live('click', function(){
        var html = $("#special_content").val();
        html += "<div class='special-content-goods-list'>";
        var goods_list = $("<div />").append($(this).parents("div").find("[imtype='_special_goods_list']").clone());
        goods_list.find("i").remove();
        goods_list.find("[imtype='btn_special_goods_delete']").remove();
...
function preview(img, selection) {
    ...
    var dialog_object = $("#fwin_dialog_image_insert_hot_point");
    dialog_object.find("[imtype='x1']").val(selection.x1);
    dialog_object.find("[imtype='y1']").val(selection.y1);
    dialog_object.find("[imtype='x2']").val(selection.x2);
    dialog_object.find("[imtype='y2']").val(selection.y2);
    dialog_object.find("[imtype='w']").val(selection.width);
    dialog_object.find("[imtype='h']").val(selection.height);
}
