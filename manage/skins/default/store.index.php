...
<div class="page">
  <div class="fixed-bar">
	...
	<!-- 分销选入店铺 start zhang -->
	<li><a href="index.php?act=store&op=fx" ><span>分销店铺</span></a></li>
      </ul>
    </div>
  </div>
  ...
  <tbody>
    <tr><th><label><?php echo $lang['belongs_level'];?></label></th>
      ...
      <td><input type="text" value="<?php echo $output['store_name'];?>" name="store_name" id="store_name" class="txt"></td>
        <td><a href="javascript:void(0);" id="ncsubmit" class="btn-search " title="<?php echo $lang['im_query'];?>">&nbsp;</a>
        <?php if($output['owner_and_name'] != '' or $output['store_name'] != '' or $output['grade_id'] != ''){?>
        <a href="index.php?act=store&op=store" class="btns " title="<?php echo $lang['im_cancel_search'];?>"><span><?php echo $lang['im_cancel_search'];?></span></a>
        <?php }?></td>
    </tr></tbody>
  </table>
  </form>
    ...
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <td class="align-center w200">
            <a href="index.php?act=store&op=store_joinin_detail&member_id=<?php echo $v['member_id'];?>">查看</a>&nbsp;&nbsp;<a href="index.php?act=store&op=store_edit&store_id=<?php echo $v['store_id']?>"><?php echo $lang['im_edit'];?></a>&nbsp;&nbsp;
        ...
        <?php }else { ?>
        <tr class="no_data">
          <td colspan="15"><?php echo $lang['im_no_record'];?></td>
        </tr>
        <?php } ?>
      </tbody>
      <tfoot>
