...
var ADMIN_SKINS_URL = '<?php echo ADMIN_SKINS_URL;?>';
...
<body>
<div class="admincp-map ui-widget-content" imtype="map_nav" style="display:none;" id="draggable" >
  <div class="title ui-widget-header" >
    <h3><?php echo $lang['im_admin_navigation'];?></h3>
    <h5><?php echo $lang['im_admin_navigation_subhead'];?></h5>
    <span><a imtype="map_off" href="JavaScript:void(0);">X</a></span> 
  </div>
		...
		(
			{
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['im_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?act=common&op=pic_cut&type=admin_avatar&x=100&y=100&resize=1&ratio=1&url='+data.url,690);
					}else{
						alert(data.msg);
					}
...
function call_back(picname){
    $.getJSON('index.php?act=index&op=save_avatar&avatar=' + picname, function(data){
        if (data) {
            $('img[imtype="admin_avatar"]').attr('src', '<?php echo UPLOAD_SITE_URL . '/' . ATTACH_ADMIN_AVATAR?>/' + picname);
        }
    });
}
</script> 
...
<div class="admincp-header">
  ...
  <div class="admincp-header-r">
    <div class="manager">
      ...
      <img alt="" imtype="admin_avatar" src="<?php if (cookie('admin_avatar') != '') { echo UPLOAD_SITE_URL . '/' . ATTACH_ADMIN_AVATAR . '/' . cookie('admin_avatar'); } else {echo ADMIN_SKINS_URL.'/images/login/admin.png';}?>"> </span><i class="arrow" id="admin-manager-btn" title="显示快捷管理菜单"></i>
      <div class="manager-menu">
        <div class="title">
          <h4>最后登录</h4>
          <a href="javascript:void(0);" onClick="CUR_DIALOG = ajax_form('modifypw', '修改密码', '<?php echo urlAdmin('index', 'modifypw');?>');" class="edit-password">修改密码</a> 
        </div>
        ...
        <ul class="im-row" imtype="quick_link">
    ...
    <ul class="operate im-row">
      <li style="display: none !important;" imtype="pending_matters"><a class="toast show-option" href="javascript:void(0);" onClick="$.cookie('commonPendingMatters', 0, {expires : -1});ajax_form('pending_matters', '待处理事项', '<?php echo urlAdmin('common', 'pending_matters');?>', '480');" title="查看待处理事项">&nbsp;<em>0</em></a></li>
      <li><a class="sitemap show-option" imtype="map_on" href="javascript:void(0);" title="查看全部管理菜单">&nbsp;</a></li>
      ...
      <li><a class="login-out show-option" id="login_out" href="javascript:void(0);" title="安全退出管理中心">&nbsp;</a></li>
    </ul>
  </div>
  <div class="clear"></div>
</div>
