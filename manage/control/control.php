...
class SystemControl{
    ...
    protected final function systemSetKey($user, $avatar = '', $avatar_compel = false) {
        setIMCookie('sys_key',encrypt(serialize($user),MD5_KEY),3600,'',null);
        if ($avatar_compel || $avatar != '') {
            setIMCookie('admin_avatar',$avatar,86400 * 365,'',null);
        }
    }
    ...
    protected final function checkPermission($link_nav = null) {
        ...
        //以下几项不需要验证
        if (!defined('MODULE_NAME') || in_array($act, array('index', 'login', 'common'))) return true;
        if (is_array($permission[MODULE_NAME]) && in_array($act, $permission[MODULE_NAME])){
            return true;
        }
        showMessage(Language::get('im_assign_right'),'','html','succ',0);
    }
    ...
    protected final function sublink($links = array(), $actived = '', $file='index.php') {
        ...
        return "<ul class=\"tab-base im-row\">{$linkstr}</ul>";
    }
...
class BaseExtensionControl extends SystemControl {
	...
	public function __construct()
    {
        ...
        if (OPEN_STORE_EXTENSION_STATE != 10){
			showMessage('平台未开启全网推广功能！',urlAdmin('dashboard','welcome'),'html','error');
        }
    }
}