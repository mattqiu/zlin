...
class orderControl extends SystemControl{
	...
	public function tcOp(){
		...
		if($til == 1){
			...
			$store_id = $_POST['store_idl'];
			$up_id = $_POST['up_idl'];	//店铺ID
			$ti = $_POST['til'];
			$zt = $_POST['ztl'];
			...
			$arr = array(
				'member_id'=>$member_id,
				'available_predeposit'=>array('exp','available_predeposit-'.$ti.'')
			);
			
			//$arr['member_id'] = $member_id;			//分销用户退款
			$model_member = Model('member');	
			$member = $model_member->update($arr);
			...
			$srr = array(
				'member_id'=>$member_idl,
				'available_predeposit'=>array('exp','available_predeposit-'.$zt.'')
			);
			$member = $model_member->update($srr);
