...
// 				$save_file2 = BASE_UPLOAD_PATH.'/'.$_POST['filename'];
    ...
    private function get_pending_matters() {
		...
		if ($this->admin_info['sp'] == 1 || is_array($permission['shop'])){
            // 积分提现
         ...
         * 资讯
         */
        if (C('cms_isuse') && ($this->admin_info['sp'] == 1 || is_array($permission['cms']))) {
            // 文章审核
         ...
         * 圈子
         */
        if (C('circle_isuse') && ($this->admin_info['sp'] == 1 || is_array($permission['circle']))) {
