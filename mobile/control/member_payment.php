...
class member_paymentControl extends mobileMemberControl {
    ...
    private function _api_pay($order_pay_info) {
		...
		$param['buyer_id'] = $order_pay_info['buyer_id'];
		$param['subject']  = $order_pay_info['subject'];
		
		$reciver_info = array();
		$buyer_info = array();
		foreach($order_pay_info['order_list'] as $key=>$v){
		    if ($v['extend_order_common']['reciver_info']){
				$reciver_info['receive_name'] = $v['extend_order_common']['reciver_name'];
				$reciver_info['receive_address'] = $v['extend_order_common']['reciver_info']['address'];
				$reciver_info['receive_zip'] = $v['extend_order_common']['reciver_info']['zip'];
				$reciver_info['receive_phone'] = $v['extend_order_common']['reciver_info']['phone'];
				$reciver_info['receive_mobile'] = $v['extend_order_common']['reciver_info']['mob_phone'];	
					
				$reciver_info['reciver_province_id'] = $v['extend_order_common']['reciver_info']['province_code'];
				$reciver_info['reciver_city_id'] = $v['extend_order_common']['reciver_info']['city_code'];				
			}
			if ($v['extend_member']){
				$buyer_info['member_id'] = $v['extend_member']['member_id'];
				$buyer_info['member_mobile'] = empty($v['extend_member']['member_mobile'])?$reciver_info['receive_mobile']:$v['extend_member']['member_mobile'];	
				$buyer_info['member_time'] = $v['extend_member']['member_time'];				
				break;
			}				
		}
		$param['reciver_info'] = $reciver_info;
    ...
    private function _get_real_order_info($pay_sn) {
        ...
        if (!empty($result['data']['order_list'])) {
            foreach ($result['data']['order_list'] as $order_info) {
                if ($order_info['order_state'] == ORDER_STATE_NEW) {
                    $pay_amount += $order_info['order_amount'] - $order_info['pd_amount'] - $order_info['rcb_amount'] + ($order_info['usable_points'] - $order_info['points_amount']);//-$points_amount;
                    $pay_order_id_list[] = $order_info['order_id'];
                }
            }
        }
