...
class member_chatControl extends mobileMemberControl {
	...
	public function get_node_infoOp(){
		...
		$user_info = $model_chat->getMember($user_id);
		
		$chat_goods_id = intval($_GET['chat_goods_id']);
		$goods_info = $model_chat->getGoodsInfo($chat_goods_id);
		
		$node_info = array();
		$node_info['node_chat'] = C('node_chat');
		$node_info['node_site_url'] = NODE_SITE_URL;
		$node_info['resource_site_url'] = RESOURCE_SITE_URL;
		$node_info['member_info'] = $member_info;
		$node_info['user_info'] = $user_info;
		$node_info['chat_goods'] = $goods_info;
		
		output_data($node_info);		
	...
	public function get_msg_countOp(){
		...
		$model_chat	= Model('web_chat');
		$msg_count = $model_chat->get_newmsg_count($member_id);
		
		output_data($msg_count);		
	}	
}
