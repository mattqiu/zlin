...
class member_fundControl extends mobileMemberControl {
    ...
    public function pdrechargelistOp(){
        ...
        $model_pd = Model('predeposit');
        $list = $model_pd->getPdRechargeList($condition,$this->page,'*','pdr_id desc');
     ...
     * 充值卡列表
     */
    public function rechargecard_vipOp(){
    	$condition = array();
    	$condition['state'] = 0;
    	$model_rc = Model('rechargecard');
    	$page   = new Page();
    	$vip_list = $model_rc->getVipCardList($condition,'*','',$page,'id desc','denomination');
    	
    	$page_count = $model_rc->gettotalpage();
    	output_data(array('vipcard_list' => $vip_list), mobile_page($page_count));
    }
    /**
     * 代金券列表
     */
