...
class member_walletControl extends mobileMemberControl {
    ...
    public function vipcard_addOp()
    {
    	...
    	if(!empty($card['state'])&&!empty($card['member_id'])){
    		...
    		$card_list = $rechargecard_model->getRechargeCardList($rcb_where);
    		if(empty($card_list)){
    			showMessage('VIP充值卡已经用完，请联系店长', '', 'html', 'error');
    			exit;
    		}
    		//取第一个卡号
    	...
    	try {
    		$model_pdr = Model('predeposit');
    		$model_pdr->addRechargeCard($cart_sn, $_SESSION);
    		if($cart_amount>0){
    			$data = array();
    			$data['pdr_sn'] = $pay_sn = $model_pdr->makeSn();
    			$data['pdr_member_id'] = $_SESSION['member_id'];
    ...
    public function my_rechargeOp(){
        ...
        $out_data = array();
		$out_data['predepoit'] = $this->member_info['available_predeposit'];
		$out_data['list'] = $list;
		$page_count = $model_pd->gettotalpage();
		
        output_data(array('wallet' => $out_data), mobile_page($page_count));
    }	
	...
	public function recharge_delOp(){
		...
		if ($result){
			showDialog(Language::get('im_common_del_succ'),'reload','succ','CUR_DIALOG.close()');
		}else {
	...
	private function profile_menu($menu_type,$menu_key=''){
		$menu_array	= array(
			array('menu_key'=>'loglist','menu_name'=>Language::get('im_member_path_predeposit_loglist'),	'menu_url'=>'index.php?act=predeposit&op=pd_log_list'),
			array('menu_key'=>'recharge_list','menu_name'=>Language::get('im_member_path_predeposit_rechargelist'),	'menu_url'=>'index.php?act=predeposit&op=index'),
			array('menu_key'=>'cashlist','menu_name'=>Language::get('im_member_path_predeposit_cashlist'),	'menu_url'=>'index.php?act=predeposit&op=pd_cash_list'),
            array(
		...
		switch ($menu_type) {
			case 'rechargeinfo':
				$menu_array[] = array('menu_key'=>'rechargeinfo','menu_name'=>Language::get('im_member_path_predeposit_rechargeinfo'),	'menu_url'=>'');
				break;
			case 'recharge_add':
			...
			case 'log':
				break;
			case 'cashadd':
			    $menu_array[] = array('menu_key'=>'cashadd','menu_name'=>Language::get('im_member_path_predeposit_cashadd'),	'menu_url'=>'index.php?act=predeposit&op=pd_cash_add');
				break;
			case 'cashinfo':
				$menu_array[] = array('menu_key'=>'cashinfo','menu_name'=>Language::get('im_member_path_predeposit_cashinfo'),	'menu_url'=>'');
				break;
		}
