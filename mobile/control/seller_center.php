...
class seller_centerControl extends mobileSellerControl {
    ...
    public function indexOp() {
        ...
        $store_info['seller_name'] = $this->seller_info['seller_name'];
        $store_info['store_avatar'] = getStoreLogo($this->seller_info['store_avatar']);
        // 店铺等级信息
        ...
        // 出售中的商品
        $store_info['goods_online'] = $goods_online = $model_goods->getGoodsCommonOnlineCount(array('store_id' => $_SESSION['store_id']));
        // 违规下架的商品
        $store_info['goods_lockup'] = $goods_lockup = $model_goods->getGoodsCommonLockUpCount(array('store_id' => $_SESSION['store_id']));
        // 仓库待上架的商品
        $store_info['goods_offline'] = $goods_offline = $model_goods->getGoodsCommonOfflineCount(array('store_id' => $_SESSION['store_id']));
        ...
        $store_info['order_await_pay'] = $model_order->getOrderStateNewCount($condition);//待付款
        $store_info['order_await_ship'] = $model_order->getOrderStatePayCount($condition);//等发货
        $store_info['order_shipped'] = $model_order->getOrderStateSendCount($condition);//等收货
        $store_info['order_finished'] = $model_order->getOrderStateEvalCount($condition);//等评价
        $store_info['order_return'] = $model_return->getRefundReturnCount(array('store_id'=>$this->seller_info['store_id'],'refund_state'=>array('lt',3)));//退款 退货
