...
//    $(document).on('change','span[nctype="pv_name"] > input',function(){
...
//    });
    ...
    /* AJAX添加规格值 */
    //选择商品分类后 动态加载后 需要通过定位父级静态部分才可找到
    $('#goods_spec_group').on('click', 'a[imtype="specAdd"]', function(){
    ...
    // 取消
    $('#goods_spec_group').on('click', 'a[imtype="specAddCancel"]', function(){
        var _parent = $(this).parents('li:first');
    ...
    // 提交
    $('#goods_spec_group').on('click', 'a[imtype="specAddSubmit"]', function(){
        var _parent = $(this).parents('li:first');
    ...
    // 修改规格名称
    $('#goods_spec_group').on('change', 'input[imtype="spec_name"]', function(){
    ...
    // 修改规格名称
    $('#goods_spec_group').on('click', 'input[type="checkbox"]', function(){
        pv = $(this).parents('li').find('span[nctype="pv_name"]');
        var sced = $(this).parents('li').find('input[type="checkbox"]');
        //后面的判定可以不要 zhangc
        if(typeof(pv.find('input').val()) == 'undefined' && sced.is(':checked')){
            pv.html('<input type="text" maxlength="20" class="text" value="'+pv.html()+'" />');
        }else{
            pv.html(pv.find('input').val());
        }
        ajax_into_array();// 将选中的规格放入数组
        ajax_goods_stock_set();// 生成库存配置
    });
    
    //修改 规格名称 离开pv_name的时候触发
    $('#goods_spec_group').on('change','span[nctype="pv_name"] > input',function(){
        change_img_name($(this));       // 修改相关的颜色名称
        into_array();           // 将选中的规格放入数组
        goods_stock_set();      // 生成库存配置
    });
    ...
    $('.batch-input').on('click', '[data-type]',function(){
        ...
        } else if(_type == 'alarm' || _type == 'stock'){
            _value = parseInt(_value);
        }
        if (_type == 'alarm' && _value > 255) {
            _value = 255;
        }
        if (isNaN(_value)&&(_type != 'sku'&&_type != 'barcode')) {
            _value = 0;
        }	
        ...
        if (_type == 'sku') {
        	$('input[data_type="g_serial"]').val(_value);
        	$('input[name="g_serial"]').val(_value);
        }
    });
...
function change_img_name(Obj){
	...
	var sval = _parent.find('input[type="checkbox"]').val();
	if(sval != Obj.val()){
		eval('var data_str = ' + _parent.attr('data-param'));
		if(confirm("确定要修改商品规格名称吗？可能会影响到其他已上传的商品")){
			$.getJSON(data_str.url, {gc_id : data_str.gc_id , sp_value_id : data_str.sp_value_id, name : Obj.val()}, function(data){
		        if (data.done) {
		        	var S = _parent.find('input[type="checkbox"]');
		            S.val(Obj.val());
		            _parent.find('span[nctype="pv_name"]').html(Obj.val());
		        }
		    });
		}else{
			_parent.find('span[nctype="pv_name"]').html('<input type="text" maxlength="20" class="text" value="'+sval+'">');
		}
	}
    
     
     
     //var V = $('li[nctype="spec_group_li_'+S.attr('nc_type')+'"]');
