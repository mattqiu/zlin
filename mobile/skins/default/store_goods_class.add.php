...
<div class="eject_con">
    ...
    <form id="category_form" method="post" target="_parent" action="<?php echo urlMobile('store_goods_class', 'goods_class_save');?>">
        <?php if ($output['class_info']['stc_id']!=0) { ?>
            <input type="hidden" name="stc_id" value="<?php echo $output['class_info']['stc_id']; ?>" />
        <?php } ?>
        <dl>
            <dd><i class="required">*</i><?php echo $lang['store_goods_class_name'].$lang['nc_colon'];?>
                <input class="form-control" type="text" name="stc_name" id="stc_name" value="<?php echo $output['class_info']['stc_name']; ?>" />
            </dd>
        </dl>
        <?php if ($output['class_info']['stc_id']==0) { ?>
            <dl>
                <dd><?php echo $lang['store_goods_class_sup_class'].$lang['nc_colon'];?>
                    <select name="stc_parent_id" id="stc_parent_id" class="form-control">
        ...
        <?php } ?>
        <dl>
            <dd><?php echo $lang['store_goods_class_sort'].$lang['nc_colon'];?>
                <input class="form-control" type="text" name="stc_sort" value="<?php echo intval($output['class_info']['stc_sort']); ?>"  />
            </dd>
        </dl>
        <dl>
            <dd><?php echo $lang['store_goods_class_display_state'].$lang['nc_colon'];?>
                <label>
...
<script type="text/javascript">
    $(function(){
        ...
        setTimeout(function(){
            $('.dialog_wrapper').css('top',50);
            $('.dialog_wrapper').css('left',0);
        },1);
