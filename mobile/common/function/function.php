<?php
/**
 * mobile公共方法
 *
 * 公共方法
 *
 * @package    function
 * @copyright  Copyright (c) 2007-2016 zlin-e Inc. (http://www.zlin-e.com)
 * @license    http://www.SrpingWater.net
 * @link       http://www.zlin-e.com
 * @author	   zlin-e Team
 * @since      File available since Release v1.1
 */
defined('InIMall') or exit('Access Invalid!');

function output_data($datas, $extend_data = array()) {
    $data = array();
    $data['code'] = 200;
	$data['login'] = ($_SESSION['is_login']	== '1')?1:0;
	//返回信息
	$status = array();
	$status['succeed'] = 1;
	$status['error_code'] = 0;
	$status['error_desc'] = '';
		
	$data['status'] = $status;	
	//推广处理
	$data['extension'] = '';	
	if (OPEN_STORE_EXTENSION_STATE > 0){
		$extension_id = cookie('iMall_extension');
	    if (!empty($extension_id)){
			$data['extension_id'] = $extension_id;
			$data['extension'] = "&extension=".$extension_id;
	    }else{
	    	$model_mb_user_token = Model('mb_user_token');
	    	$model_member = Model('member');
	    	$mb_user_token_info = $model_mb_user_token->getMbUserTokenInfoByToken($_COOKIE['key']);
			//$member_info = $model_member->getMemberInfoByID($mb_user_token_info['member_id']);
	    	//缓存推广员、导购员ID
	    	//if ($member_info['mc_id'] ==1 || $member_info['mc_id']==2){
	    	if(!empty($mb_user_token_info['member_id'])){
		    	$extension_id=urlsafe_b64encode($mb_user_token_info['member_id']);
		    	setIMCookie('iMall_extension',$extension_id,3600*24,'/');
		    	$data['extension_id'] = $extension_id;
		    	$data['extension'] = "&extension=".$extension_id;
	    	}	else {
	    		$data['extension_id'] = 0;
	    		$data['extension'] = "";
	    	}
	    	//}
	    }
	}
	$data['OPEN_STORE_EXTENSION_STATE'] = OPEN_STORE_EXTENSION_STATE;
	
	$data['datas'] = $datas;

    output_json($data, $extend_data);
}

function output_error($message, $extend_data = array()) {
	$data = array();
	$data['error'] = $message;
	//$data['login'] = ($_SESSION['is_login']	== '1')?1:0;
	//返回信息
	$status = array();
	$status['succeed'] = 0;
	$status['error_code'] = $extend_data['error_code']?$extend_data['error_code']:0;
	$status['error_desc'] = $message;
	
	$data['status'] = $status;	
	
	$data['datas'] = array();
	
    output_json($data, $extend_data);
}

function output_json($datas, $extend_data = array()) {
	$data = $datas;
	if(!empty($extend_data)) {
        $data = array_merge($data, $extend_data);
    }

    if(!empty($_GET['callback'])) {
        echo $_GET['callback'].'('.json_encode($data).')';
		die;
    } else {
        echo json_encode($data);
		die;
    }	
}

function mobile_page($page_count) {
    //输出是否有下一页
    $extend_data = array();
    $current_page = intval($_GET['curpage']);
    if($current_page <= 0) {
        $current_page = 1;
    }
    if($current_page >= $page_count) {
        $extend_data['hasmore'] = false;
    } else {
        $extend_data['hasmore'] = true;
    }
    $extend_data['page_total'] = $page_count;
    return $extend_data;
}

function APP_page($page_count,$page_nums) {
    //输出是否有下一页
    $extend_data = array();
    $current_page = intval($_GET['curpage']);
    if($current_page <= 0) {
        $current_page = 1;
    }
    if($current_page >= $page_count) {
        $extend_data['hasmore'] = false;
    } else {
        $extend_data['hasmore'] = true;
    }
    $extend_data['page_total'] = $page_count;
	$extend_data['page_nums'] = $page_nums;
    return array('paginated'=>$extend_data);
}

//转换成图片数组
function getPhoto_array($small_img = '', $thumb_img = '', $url = '') {
    $photo = array();
	$photo['small'] = $small_img;
	$photo['thumb'] = $thumb_img;
	$photo['url'] = $url;
    return $photo;
}

function get_server_ip() {
    if (isset($_SERVER)) {
        if($_SERVER['SERVER_ADDR']) {
            $server_ip = $_SERVER['SERVER_ADDR'];
        } else {
            $server_ip = $_SERVER['LOCAL_ADDR'];
        }
    } else {
        $server_ip = getenv('SERVER_ADDR');
    }
    return $server_ip;
}

function http_get($url) {
    return file_get_contents($url);
}

function http_post($url, $param) {
    $postdata = http_build_query($param);
    $opts = array('http' =>
        array(
            'method'  => 'POST',
            'header'  => 'Content-type: application/x-www-form-urlencoded',
            'content' => $postdata
        )
    );
    $context  = stream_context_create($opts);
    return @file_get_contents($url, false, $context);
}

function http_postdata($url, $postdata) {
    $opts = array('http' =>
        array(
            'method'  => 'POST',
            'header'  => 'Content-type: application/x-www-form-urlencoded',
            'content' => $postdata
        )
    );
    $context  = stream_context_create($opts);
    return @file_get_contents($url, false, $context);
}