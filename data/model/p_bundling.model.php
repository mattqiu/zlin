...
class p_bundlingModel extends Model {
    ...
    public function addBundling($insert, $replace = false) {
        return $this->table('p_bundling')->insert($insert);
    }
     ...
     * @return array
     */
    public function getBundlingGoodsCount($condition) {
    	return $this->table('p_bundling_goods')->where($condition)->count();
    }
                    ...
                    if (!empty($b_goods_list) && count($b_goods_list) > 1) {
                        ...
                        foreach ( $b_goods_list as $val ) {
                            if (isset($goods_list[$val['goods_id']])) {
                                ...
                                $b_goods_array[$val['bl_id']][$k]['shop_price'] = imPriceFormat($goods_list[$val['goods_id']]['goods_price']);
                                $b_goods_array[$val['bl_id']][$k]['price'] = imPriceFormat($val['bl_goods_price']);
