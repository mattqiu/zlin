...
class cartModel extends Model {
	...
	private function _addCartDb($goods_info = array(),$quantity) {
		...
		//<!-- 分销选入店铺 start zhang -->
		//$array['up_id']	  = $goods_info['up_id'];
		//$array['up_name'] = $goods_info['up_name'];
		//$array['baifen'] = $goods_info['baifen'];
	 ...
	 * @return unknown
	 */
	//yangbaiyan//private function _addCartCache($goods_info = array(), $quantity = null) {
	...
	private function _addCartCookie($goods_info = array(), $quantity = null) {
		...
		setIMCookie('cart',encrypt(base64_encode(serialize($cart_array))),24*3600);
		return true;
	}
	...
	public function listCart($type, $condition = array(), $limit = '') {
		...
		if(is_array($cart_list)) {
			foreach ($cart_list as $val) {
				...
				$typeBy_id = Model('member')->getMemberTypeByID($buyer_id);
				if(!empty($typeBy_id)){
					//Vip 使用云币抵扣 start zhangc
					$goodsVip = Logic('goods')->goodsVip($val['goods_id']);
					$vip_price = $goodsVip['vip_price'];
					$vip_points = $goodsVip['vip_points'];
					$cart_all_price	+= $vip_price * $val['goods_num'];
				}else{
					$cart_all_price	+= $val['goods_price'] * $val['goods_num'];
				}
			}
		}
        $this->cart_all_price = imPriceFormat($cart_all_price);
	...
	public function getCartNum($type, $condition = array()) {
	    if ($type == 'db') {
    		...
    		if(!empty($cart_goods) && is_array($cart_goods)) {
    			foreach ($cart_goods as $val) {
    				//Vip 使用云币抵扣 start zhangc
    				$typeBy_id = Model('member')->getMemberTypeByID($val['buyer_id']);
    				if(!empty($typeBy_id)){
    					$goodsVip = Logic('goods')->goodsVip($val['goods_id']);
    					$vip_price = $goodsVip['vip_price'];
    					$vip_points = $goodsVip['vip_points'];
	    ...
	    @setIMCookie('cart_goods_num',$this->cart_goods_num,2*3600);
	...
	public function getXianshiInfo($buy_goods_info, $quantity) {
	    ...
	    if (!empty($xianshi_info)) {
			$goods_price = $buy_goods_info['goods_price'];
			...
			$buy_goods_info['xianshi_info']['lower_limit'] = $xianshi_info['lower_limit'];
            $buy_goods_info['xianshi_info']['xianshi_price'] = $xianshi_info['xianshi_price'];
            $buy_goods_info['xianshi_info']['down_price'] = imPriceFormat($goods_price - $xianshi_info['xianshi_price']);
	    }
	 ...
	 * @return array,如果该商品未正在进行抢购，返回空数组
	 */
	...
	public function getXianshiCartList($cart_list) {
        ...
        foreach ($cart_list as $key => $cart_info) {
            ...
            if (!empty($xianshi_info)) {
                ...
                $cart_list[$key]['xianshi_info']['down_price'] = imPriceFormat($cart_info['goods_price'] - $xianshi_info['xianshi_price']);
            }
        }
	    return $cart_list;
	}
        ...
        foreach ($cart_list as $key => $cart_info) {
            ...
            foreach ($cart_list[$key]['bl_goods_list'] as $k => $goods_info) {
                if (array_key_exists($goods_info['goods_id'],$goods_online_list)) {
					...
					$cart_list[$key]['bl_goods_list'][$k]['promotion_cid'] = $goods_online_info['promotion_cid'];
					$cart_list[$key]['bl_goods_list'][$k]['saleman_cid'] = $goods_online_info['saleman_cid'];
                } else {
                    //商品已经下架
                    $cart_list[$key]['state'] = false;
                    $cart_list[$key]['storage_state'] = false;
	...
	public function calcCartList($store_cart_list) {
	    ...
	    foreach ($store_cart_list as $store_id => $store_cart) {
	        ...
	        foreach ($store_cart as $key => $cart_info) {
	            $store_cart[$key]['goods_total'] = imPriceFormat($cart_info['goods_price'] * $cart_info['goods_num']);
	        ...
	        $store_goods_total[$store_id] = imPriceFormat($tmp_amount);
	    }
        ...
        foreach ($store_goods_total as $store_id => $goods_total) {
            ...
            if (is_array($rule_info) && !empty($rule_info)) {
                ...
                if (!empty($rule_info['mansong_goods_name']) && !empty($rule_info['goods_storage'])) {
                    ...
                    $premiums_list[$store_id][$goods_id] = $data;
                }
            }
        }
	 ...
	 * @return string
	 */
	private function _parseMansongRuleImg($rule_info) {
	    if (empty($rule_info) || !is_array($rule_info)) return;
	    $goods_img = (!empty($rule_info['mansong_goods_name']) && !empty($rule_info['goods_storage'])) ?
	    "<a href='".urlShop('goods','index',array('goods_id'=>$rule_info['goods_id']))."' target='_blank' class='imcr-store-gift' title='{$rule_info['mansong_goods_name']}'><img src='{$rule_info['goods_image_url']}' alt='{$rule_info['mansong_goods_name']}'/></a>" : '';
	     return $goods_img;	
	}
