...
class goodsModel extends Model{
    ...
    public function getGoodsListByColorDistinct($condition, $field = '*', $order = 'goods_id asc', $page = 0, $limit = 0) {      
        ...
        $goods_list = array();
        if (C('dbdriver') == 'mysqli') {
            $goods_list = $this->getGoodsOnlineList($condition, $field.','.$_field.' as '.$_distinct, $page, $order, $limit, $_distinct, false, $count);
        } elseif (C('dbdriver') == 'oracle') {
	...
	/**
     * 出售中的商品SKU列表（只显示不同颜色的商品，前台商品索引，店铺商品列表等使用）
     * @param array $condition
     * @param string $field
     * @param string $order
     * @param number $page
     ...
     * @return array
     */
    public function getGoodsCommonStubbsList($condition, $field = '*', $page = 10, $order = "goods_commonid desc") {
        $condition['goods_verify']  = self::VERIFY1;
		$condition['goods_lock']  = 0;
		$condition['goods_state']   = array('neq',self::STATE10);
     ...
     * @return array
     */
	public function getGoodsStoreThemeInfo($goods_id){
		$store_id = $this->where(array('goods_id'=>$goods_id))->get_field('store_id');
		$store_info = Model('store')->field('store_id,store_theme,store_decoration_switch,show_own_copyright')->where(array('store_id'=>$store_id))->find();
		return $store_info;
	}
    ...
    public function getGoodsInfoAndPromotionById($goods_commonid) {
		...
		//商品规格 待修改
		$commonInfo = $this->getGoodsCommonInfoByID($goods_info['goods_commonid'],'spec_name');
		$goods_info['spec_name'] = $commonInfo['spec_name'];
        ...
        if (!empty($goods_list)) {
            ...
            foreach ($goods_list as $val) {
                ...
                @unlink(BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.$val['store_id'].DS.$val['goods_id'].'.png');
				 //unlink(BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.'goodQRcode'.DS.$val['store_id'].DS.$val['goods_id'].'.png');
    ...
    public function checkIsGeneral($goods) {
		if ($goods['is_virtual'] == 1 || $goods['is_fcode'] == 1 || $goods['is_presell'] == 1 || $goods['is_appoint'] == 1 || $goods['is_book'] == 1) {
            return false;
        }
        return true;
    }
    ...
    public function getGoodsDetail($goods_commonid) {
        ...
        $gc_info = $model_goods_class->getGoodsClassList(array('gc_id'=>$goods_info['gc_id'],'commis_rate'));
        $goods_info['commis_rate'] = $gc_info[0]['commis_rate'];
	   ...
	   // 新版放大镜开始
        ...
        if ($goods_info['is_book'] != '1') {
            ...
            if (!empty($goods_info['xianshi_info'])) {
                ...
                $goods_info['promotion_price'] = $goods_info['xianshi_info']['xianshi_price'];
                $goods_info['down_price'] = imPriceFormat($goods_info['goods_price'] - $goods_info['xianshi_info']['xianshi_price']);
            ...
            if (!empty($goods_info['groupbuy_info'])) {
                ...
                $goods_info['promotion_price'] = $goods_info['groupbuy_info']['groupbuy_price'];
                $goods_info['down_price'] = imPriceFormat($goods_info['goods_price'] - $goods_info['groupbuy_info']['groupbuy_price']);
		...
		$rebate_info = array();
		$rebate_info['name'] = '';
		$rebate_info['rebate_value'] = 0;
		$rebate_cid = 0;
		if ($_SESSION['M_mc_id'] == 1 && $goods_info['promotion_cid'] >0 && $goods_info['store_id'] == $_SESSION['M_store_id']){
			//推广员
			$rebate_cid = $goods_info['promotion_cid'];
			$rebate_info['name'] = '推广分佣';
			if ($rebate_cid>0){
			   
			  $commis_arr = Model('extension_commis_class')->getRebateValueByID($rebate_cid,$goods_info['goods_price'],$goods_info['goods_costprice']);
			  //返佣积分
			  $rebate_info['rebate_value'] = $commis_arr['commis_price'];
			  //返佣云币
			  $rebate_info['rebate_points'] = $commis_arr['commis_points'];
			}
		}elseif ($_SESSION['M_mc_id'] == 2 && $goods_info['saleman_cid'] >0){
			//导购员
            $rebate_cid = $goods_info['saleman_cid'];
			$rebate_info['name'] = '导购提成';
			if ($rebate_cid>0){
			  $commis_arr = Model('extension_commis_class')->getRebateValueByID($rebate_cid,$goods_info['goods_price'],$goods_info['goods_costprice']);
			  // 导购员没有积分和云币概念，全部都返佣积分
			  $rebate_info['rebate_value'] = empty($commis_arr['commis_price'])?$commis_arr['commis_points']:$commis_arr['commis_price'];			  
			}
		}	
		$goods_info['rebate_info'] = $rebate_info;   	
		...
		//店铺商品服务
		$contractlist = array();
		$goods_info['contractlist'] = $contractlist;
     ...
     * @param string $table 表名
     */
    public function addGoodsBarcode($insert) {
    	return $this->table('goods_barcode')->insert($insert);
    }
    /**
     * 更新商品条码库数据
     * @param array $update 更新数据
     ...
     * @return boolean
     */
    public function editGoodsBarcode($update, $condition) {
    	$barcode_list = $this->getGoodsBarcodeList($condition, 'goods_barcode', 0);
    	if (empty($barcode_list)) {
    		return false;
    	}
    	$barcode_array = array();
    	foreach ($barcode_list as $val) {
    		$barcode_array[] = $val['goods_barcode'];
    	}
    	return $this->editGoodsByBarcode($update, $barcode_array);
    }
    
    /**
     * 更新商品条码库数据
     * @param array $update
     * @param int|array $barcodeid_array
     * @return boolean|unknown
     */
    public function editGoodsByBarcode($update, $barcode_array) {
    	if (empty($barcode_array)) {
    		return true;
    	}
    	$condition['goods_barcode'] = array('in', $barcode_array);
    	$result = $this->table('goods_barcode')->where($condition)->update($update);
    	return $result;
    }
     ...
     * @return boolean|unknown
     */
    public function delGoodsBarcode($barcode_array) {
    	if (empty($barcode_array)) {
    		return true;
    	}
    	$result = $this->table('goods_barcode')->where(array('goods_barcode' => array('in', $barcode_array)))->delete();
    	return $result;
    }
     ...
     * @return array
     */
    public function getGoodsBarcodeList($condition, $field = '*', $page = 0, $order = 'bid desc', $limit = '') {
    	return $this->table('goods_barcode')->field($field)->where($condition)->order($order)->limit($limit)->page($page)->select();
    }
