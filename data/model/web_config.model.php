...
class web_configModel extends Model{
	 ...
	 * @return array 数组格式的返回结果
	 */
	public function getWebStyle($web_id){
		$web_style=0;
		$param = array();
		$param['web_id']	= $web_id;
		$web_info	= $this->table('web')->where($param)->find();
		if (!empty($web_info) && is_array($web_info)){
			$web_style=intval($web_info['web_style']);
		}
		return $web_style;
	}
	/**
	...
	public function updateWebHtml($web_id = 1,$web_array = array()){
        ...
        if(!empty($code_list) && is_array($code_list)) {
			...
			$output['web_style'] = $web_array['web_style'];
            ...
            switch ($web_page) {
                case 'index':
				    $layout_name = 'default';
				    if (!empty($web_array['web_layout'])){
						$layout_name = $web_array['web_layout'];
					}
                    $style_file = BASE_DATA_PATH.DS.'resource'.DS.'web_config'.DS.'plates.'.$layout_name.'.php';
                    break;
                case 'index_pic':
                ...
                case 'channel_fl':
                    ...
                    $style_file = BASE_DATA_PATH.DS.'resource'.DS.'web_config'.DS.'channel_'.$layout_name.'.php';
                    break;
				case 'brand':
				    $style_file = BASE_DATA_PATH.DS.'resource'.DS.'web_config'.DS.'brandfocus.php';
					break;
				case 'special':
				    $style_file = BASE_DATA_PATH.DS.'resource'.DS.'web_config'.DS.'specialfocus.php';
					break;
                default:
                    $style_file = BASE_DATA_PATH.DS.'resource'.DS.'web_config'.DS.'plates.default.php';
                    break;
            }
	...
	public function getGoodsList($condition = array(),$order = 'goods_id desc',$page = ''){
	    ...
	    $field = 'goods_id,goods_commonid,goods_name,goods_jingle,goods_image,goods_advpic,goods_price,goods_marketprice,store_type,goods_salenum,goods_collect,promotion_amount,have_gift,goods_freight,goods_commend';
	    $goods_list = $model_goods->getGoodsListByCommonidDistinct($condition,$field,$order,$page);
	    if (!empty($goods_list) && is_array($goods_list)) {
	        ...
	        foreach ($goods_list as $key => $value) {
	            ...
	            $value['goods_type'] = 1;
				$value['goods_advpic']=empty($value['goods_advpic'])?$value['goods_image']:$value['goods_advpic'];
	            $list[$goods_id] = $value;				
	        }
