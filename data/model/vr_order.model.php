...
//         $order_code[$i-1]['pay_price'] += $diff_amount;
    ...
    /**
     * 生成兑换码
     * 长度 =  只留8位 //3位 + 4位 + 2位 + 3位  + 1位 + 5位随机  = 18位
    ...
    private function _makeVrCode($perfix, $store_id, $member_id, $num) {
		...
		*/
        $code_list = array();
        for ($i = 0; $i < $num; $i++) {
            $code_list[$i] = $perfix. sprintf('%01d', (int) $i % 10) . random(4,1);
        }
        return $code_list;
    }
    ...
    public function getOrderOperateState($operate, $order_info){
        ...
        switch ($operate) {
            ...
            case 'system_receive_pay':
                $state = in_array($order_info['order_state'],array(ORDER_STATE_NEW,ORDER_STATE_CANCEL));
                $state = $state && TIMESTAMP - $order_info['add_time'] < 604800;
                break;
