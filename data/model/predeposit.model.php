...
class predepositModel extends Model {
    ...
    public function addRechargeCard($sn, array $session)
    {
        ...
        if ($memberId < 1 || !$memberName) {
            throw new Exception("当前为未登录状态，不能使用充值卡");
        }
    ...
    public function changeRcb($type, $data = array())
    {
		...
		case 'order_book_cancel':
            ...
            break;
       	case 'order_stages_discount':
        	$freeze = $data['amount'];
            $desc = '周年庆活动，订单金额分期返还，订单号: '.$data['order_sn'];            
            break;
    	case 'order_whole_discount':
        	$available = $data['amount'];
            $desc = '周年庆活动期间，订单金额全额返还，订单号: '.$data['order_sn'];
            break;
   		case 'order_whole_notused':
           	$available = $data['amount'];
           	$desc = '参加周年庆活动，多充值部分金额按折扣存入，订单号: '.$data['order_sn'];            
            break;
        default:
            throw new Exception('参数错误');
        }
...
file_put_contents("test.log","保存充值卡数据：".json_encode($log).PHP_EOL,FILE_APPEND);
        ...
        $msg = array(
            ...
            'param' => array(
                ...
                'available_amount' => imPriceFormat($available),
                'freeze_amount' => imPriceFormat($freeze),
                'description' => $desc,
            ),
        );
    ...
    public function changePd($change_type,$data = array()) {
        ...
        switch ($change_type){
            case 'order_pay':
                ...
                $data_log['lg_desc'] = '下单，支付积分，订单号: '.$data['order_sn'];
            ...
            case 'order_freeze':
                ...
                $data_log['lg_desc'] = '下单，冻结积分，订单号: '.$data['order_sn'];
            ...
            case 'order_cancel':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_freeze_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '取消订单，解冻积分，订单号: '.$data['order_sn'];
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit-'.$data['amount']);
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
            ...
            case 'order_comb_pay':
                $data_log['lg_freeze_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '下单，支付被冻结的积分，订单号: '.$data['order_sn'];
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit-'.$data['amount']);
            ...
            case 'vr_refund':
                ...
                $data_msg['desc'] = $data_log['lg_desc'];
                break;
            case 'refund_apply'://订单状态必须是客户已经确定收货
                	$data_log['lg_av_amount'] = -$data['amount'];
                	$data_log['lg_freeze_amount'] = $data['amount'];
                	$data_log['lg_desc'] = '申请退货款，冻结返利积分，提现单号: '.$data['order_sn'];
                	$data_pd['available_predeposit'] = array('exp','available_predeposit-'.$data['amount']);
                	$data_pd['freeze_predeposit'] = array('exp','freeze_predeposit+'.$data['amount']);
                	$data_msg['av_amount'] = -$data['amount'];
                	$data_msg['freeze_amount'] = $data['amount'];
        	...
        	case 'refund_cancel':
                	$data_log['lg_av_amount'] = $data['amount'];
                	$data_log['lg_freeze_amount'] = -$data['amount'];
             		$data_log['lg_desc'] = '取消申请退货款，解冻返利积分，提现单号: '.$data['order_sn'];
                	$data_log['lg_admin_name'] = $data['admin_name'];
                	$data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
                	$data_pd['freeze_predeposit'] = array('exp','freeze_predeposit-'.$data['amount']);
                	$data_msg['av_amount'] = $data['amount'];
                	$data_msg['freeze_amount'] = -$data['amount'];
               		$data_msg['desc'] = $data_log['lg_desc'];
                	break;
			case 'refund_confirm': //在用户申请退款的时候已经扣减了用户的实际积分，so只需要释放冻结部分即可
				$data_log['lg_av_amount'] = 0;
				$data_log['lg_freeze_amount'] = $data['amount'];
				$data_log['lg_desc'] = '确定申请退货款，解冻返利积分即可，提现单号: '.$data['order_sn'];
				$data_log['lg_admin_name'] = $data['admin_name'];
				$data_pd['freeze_predeposit'] = array('exp','freeze_predeposit+'.$data['amount']);
        		$data_msg['av_amount'] = 0;
           		$data_msg['freeze_amount'] = $data['amount'];
			...
			case 'return_goods':
				...
				$data_log['lg_desc'] = '订单退款成功，单号: '.$data['order_sn']."退款金额是：".$data['amount'];
				$data_log['lg_admin_name'] = $data['admin_name'];
				$data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
				$data_msg['av_amount'] = $data['amount'];
				$data_msg['freeze_amount'] = 0;
            ...
            case 'cash_apply':
                $data_log['lg_av_amount'] = -$data['amount'];
                $data_log['lg_freeze_amount'] = $data['amount'];
                $data_log['lg_desc'] = '申请提现，冻结积分，提现单号: '.$data['order_sn'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit-'.$data['amount']);
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit+'.$data['amount']);
            ...
            case 'cash_del':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_freeze_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '取消提现申请，解冻积分，提现单号: '.$data['order_sn'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit-'.$data['amount']);
			...
			case 'commis':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = '提佣，提佣单号: '.$data['pde_sn'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
			...
			case 'commis_pay':
                $data_log['lg_av_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '发放佣金，单号: '.$data['pde_sn'];
				$data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit-'.$data['amount']);
			...
			case 'rebate':
			    $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = '返利，返利单号: '.$data['pde_sn'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
			...
			case 'order_book_cancel':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = '取消预定订单，退还积分，订单号: '.$data['order_sn'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
      		...
      		case 'order_stages_discount':
                	$data_log['lg_freeze_amount'] = $data['amount'];
                	$data_log['lg_desc'] = '周年庆活动，订单金额分期返还，订单号: '.$data['order_sn'];
                	$data_pd['freeze_predeposit'] = array('exp','freeze_predeposit+'.$data['amount']);
                	$data_msg['av_amount'] = 0;
                	$data_msg['freeze_amount'] = $data['amount'];
                	$data_msg['desc'] = $data_log['lg_desc'];
          	...
          	case 'order_whole_discount':
          		$data_log['lg_av_amount'] = $data['amount'];
           		$data_log['lg_desc'] = '周年庆活动期间，订单金额全额返还，订单号: '.$data['order_sn'];
        		$data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
             	$data_msg['av_amount'] = $data['amount'];
           		$data_msg['desc'] = $data_log['lg_desc'];
             	break;
             case 'order_whole_notused':
             	$data_log['lg_av_amount'] = $data['amount'];
             	$data_log['lg_desc'] = '参加周年庆活动，多充值部分金额按折扣存入，订单号: '.$data['order_sn'];
             	$data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
             	$data_msg['av_amount'] = $data['amount'];
             	$data_msg['desc'] = $data_log['lg_desc'];
             	break;
        ...
        if (!$update) {
            throw new Exception('会员相关数据更新失败');
        }
        if(!empty($data_log['lg_av_amount'])){
        $insert = $this->table('pd_log')->insert($data_log);
        if (!$insert) {
            throw new Exception('变更积分操作失败');
        	}
        }
        ...
        $data_msg['av_amount'] = imPriceFormat($data_msg['av_amount']);
        $data_msg['freeze_amount'] = imPriceFormat($data_msg['freeze_amount']);
