...
class mb_specialModel extends Model{
	 ...
	 *
	 */
	public function getMbSpecialList($condition, $store_id = self::INDEX_SPECIAL_ID, $page='', $order='special_id desc', $field='*') {
     ...
     *
	 */
    public function addMbSpecial($param, $store_id = self::INDEX_SPECIAL_ID){
	...
	/**
     * 专题项目列表（用于商家后台编辑显示所有项目）
	 * @param int $special_id
     *
     */
    public function getMbSpecialItemListByStoreID($store_id) {
        $condition = array();
        $condition['store_id'] = $store_id;
     ...
     * 商家首页专题
     */
    public function getStoreMbSpecialIndex($store_id) {
		$condition = array();
        $condition['store_id'] = $store_id;
        $result = $this->table('mb_special')->where($condition)->find();
		if($result) {
            return $this->getMbSpecialItemUsableListByID($result['special_id']);
        } else {
            return array();
        }
        ...
        switch ($item_type) {
            ...
            case 'goods1':
           	...
           	case 'custom':
                	...
                	break;
           	case 'custom':
             		$item_data['content'] = $item_data['content'];
               		break;
            default:
                $new_item = array();
     ...
     *
     */
    public function isMbSpecialItemExist($condition, $store_id = self::INDEX_SPECIAL_ID) {
		$condition['store_id'] = $store_id;
    ...
    private function _initMbSpecialItemData($item_data, $item_type) {
        if(!empty($item_data)) {
            ...
            if($item_type == 'goods'||$item_type == 'goods1') {
                $item_data = $this->_initMbSpecialItemGoodsData($item_data, $item_type);
            }else if($item_type == 'custom'){
            	//$item_data['content'] = html_entity_decode($item_data['content']);
            }
        } else {
    ...
    private function _initMbSpecialItemGoodsData($item_data, $item_type) {
        ...
        if(!empty($item_data['item'])) {
            ...
            $goods_list = $model_goods->getGoodsList($condition, 'goods_id,goods_name,goods_price,goods_promotion_price,goods_marketprice,goods_image,promotion_amount');
     ...
     *
     */
    public function addMbSpecialItem($param,$store_id = self::INDEX_SPECIAL_ID) {
    ...
    public function getMbSpecialModuleList() {
        $module_list = array();
        $module_list['custom'] = array('name' => 'custom' , 'desc' => '自定义版块');
        ...
        //$module_list['goods1'] = array('name' => 'goods1' , 'desc' => '市场选货');
        return $module_list;
    }
