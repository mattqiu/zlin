...
class store_decorationModel extends Model {
    ...
    //装修块类型数组
    private $block_type_array = array('html', 'slide', 'hot_area', 'goods', 'brand');
	
	//商品装修块类型数组
    private $block_type_goods_array = array('title', 'slide', 'goods', 'brand');
    ...
    public function getStoreDecorationInfoDetail($decoration_id, $store_id) {
        ...
        if(!empty($store_decoration_info)) {
            $data = array();
            $data['decoration_type'] = $store_decoration_info['decoration_type'];
            //处理装修背景设置
	 ...
	 * @return bool
	 */
    public function delStoreDecoration($condition_id){        
		$this->table('store_decoration_block')->where(array('decoration_id'=>$condition_id))->delete();
		return $this->table('store_decoration')->where(array('decoration_id'=>$condition_id))->delete();
    }
     ...
     * 返回商品装修块模块类型数组
     */
    public function getStoreDecorationGoodsBlockTypeArray() {
        return $this->block_type_goods_array;
    }
}
