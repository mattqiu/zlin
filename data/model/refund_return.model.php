...
class refund_returnModel extends Model{
	...
	public function editOrderRefund($refund) {
		...
		if ($refund_id > 0) {
			...
			$field = 'order_id,buyer_id,buyer_name,store_id,order_sn,order_amount,payment_code,order_state,refund_amount,rcb_amount,mb_commis_totals';
    		$model_order = Model('order');
    	    ...
    	    try {
    	        ...
    	        $predeposit_amount = $order_amount-$order['refund_amount']-$rcb_amount;//可退积分金额
				
				$mb_commis_totals = $order['mb_commis_totals'];//推广系统佣金金额
    	        ...
    	        if ($predeposit_amount > 0) {//退积分
                    $log_array = array();
                    $log_array['member_id'] = $order['buyer_id'];
                    $log_array['member_name'] = $order['buyer_name'];
                    $log_array['order_sn'] = $order['order_sn'];
    	            $log_array['amount'] = $refund['refund_amount'];//退积分金额
    	            if ($refund['refund_amount'] > $predeposit_amount) {
    	                $log_array['amount'] = $predeposit_amount;
    	            }
                    $state = $model_predeposit->changePd('refund', $log_array);//增加买家可用积分金额
                }
				
				//清退推广系统佣金金额
				if (OPEN_STORE_EXTENSION_STATE>0 && $mb_commis_totals > 0) {
				    $model_extension = Model('extension');			
					$model_extension->RemoveRebate($order['order_id'],$order['order_sn']);
				}
    			...
    			if ($state) {
    			    ...
    			    $order_array['refund_amount'] = imPriceFormat($refund_amount);
