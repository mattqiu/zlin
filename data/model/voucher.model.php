...
class voucherModel extends Model {
	 ...
	 * @return string
	 */
	public function getCurrentAvailableVoucherTemplate($condition = array(),$goods_total = 0,$goods_gaintotal=array()) {
		$condition['voucher_end_date'] = array('gt',TIMESTAMP);
		$condition['voucher_state'] = 1;
		$voucher_list = $this->table('voucher')->where($condition)->key('voucher_t_id')->select();
		foreach ($voucher_list as $key => $voucher) {
			if ($goods_total < $voucher['voucher_limit'] && !empty($voucher['voucher_limit'])) {
				unset($voucher_list[$key]);
			} else {
				$vt_where['voucher_t_id'] = $voucher['voucher_t_id'];
				$vt_info = $this->getVoucherTemplateInfo($vt_where,'vc_goods_range,vc_goods_gain');
				if($vt_info['vc_goods_range'] == 1){ //商品范围是利润空间
					//计算商品的利润  商品的售价- 商品的批发价
					if($goods_gaintotal['store_goods_gaintotal'] < $vt_info['vc_goods_gain']){
						unset($voucher_list[$key]);
						return $voucher_list;
					}
				}elseif($vt_info['vc_goods_range'] == 2){//指定的商品范围
					$goods_com_arr = array();
					$voucherGoodsList = $this->getVoucherGoodsList(array('voucher_t_id'=>$voucher['voucher_t_id']),'goods_commonid');
					foreach ($voucherGoodsList as $gkey => $goods){
						$goods_com_arr[] = $goods['goods_commonid'];
					}
					$is_intersect = array_intersect($goods_com_arr,$goods_gaintotal['goods_commonid']);
					if(empty($is_intersect)){
						unset($voucher_list[$key]);
						return $voucher_list;
					}
				}
     ...
     * @return array
     */
    public function getVoucherGoodsCount($condition) {
    	return $this->table('voucher_goods')->where($condition)->count();
    }
     ...
     * @return array
     */
    public function getVoucherGoodsList($condition, $field = '*', $order = 'vc_goods_id asc', $group = '') {
    	return $this->table('voucher_goods')->field($field)->where($condition)->group($group)->order($order)->select();
    }
     ...
     * @return boolean
     */
    public function addVoucherGoodsAll($insert) {
    	$result = $this->table('voucher_goods')->insertAll($insert);
    	if ($result) {
    		foreach ((array)$insert as $v) {
    			if (isset($v['goods_commonid'])) $this->_dGoodsVoucherCache($v['goods_commonid']);
    		}
    	}
    	return $result;
    }
     ...
     * @return boolean
     */
    public function delVoucherGoods($condition) {
    	$list = $this->getVoucherGoodsList($condition, 'goods_commonid');
    	if (empty($list)) {
    		return true;
    	}
    	$result = $this->table('voucher_goods')->where($condition)->delete();
    	if ($result) {
    		foreach ($list as $v) {
    			$this->_dGoodsVoucherCache($v['goods_commonid']);
    		}
    	}
    	return $result;
    }
    ...
    /**
     * 根据商品id查询套餐数据
     * @param unknown $goods_commonid
     */
    public function getVoucherCacheByGoodsId($goods_commonid) {
    	$array = $this->_rGoodsVoucherCache($goods_commonid);
    	if (empty($array)) {
    		$voucher_array = array();
    		$v_goods_array = array();
    		// 根据商品id查询voucher_t_id
    		$v_g_list = $this->getVoucherGoodsList(array('goods_commonid' => $goods_commonid, 'vc_appoint' => 1), 'voucher_t_id');
    		if (!empty($v_g_list)) {
    			$v_id_array = array();
    			foreach ($v_g_list as $val) {
    				$v_id_array[] = $val['voucher_t_id'];
    			}
    
    			// 查询套餐列表
    			$voucher_list = $this->getCurrentAvailableVoucher(array('voucher_t_id' => array('in', $v_id_array)));
    			// 整理
    			if (!empty($voucher_list)) {
    				foreach ($voucher_list as $val) {
    					$voucher_array[$val['voucher_t_id']]['id'] = $val['voucher_t_id'];
    				}
    				$vcid_array = array_keys($voucher_array);
    
    				$v_goods_list = $this->getVoucherGoodsList(array('voucher_t_id' => array('in', $vcid_array)));
    				if (!empty($v_goods_list) && count($v_goods_list) > 1) {
    					$goodsid_array = array();
    					foreach ($v_goods_list as $val) {
    						$goodsid_array[] = $val['goods_commonid'];
    					}
    					$goods_list = Model('goods')->getGoodsCommonList(array('goods_commonid' => array('in', $goodsid_array)), 'goods_commonid,goods_name,goods_price,goods_image');
    					$goods_list = array_under_reset($goods_list, 'goods_commonid');
    					foreach ( $v_goods_list as $val ) {
    						if (isset($goods_list[$val['goods_commonid']])) {
    							$k = (intval($val['goods_commonid']) == $goods_commonid) ? 0 : $val['goods_commonid'];    // 排序当前商品放到最前面
    							$v_goods_array[$val['voucher_t_id']][$k]['id'] = $val['goods_commonid'];
    							$v_goods_array[$val['voucher_t_id']][$k]['image'] = thumb($goods_list[$val['goods_commonid']], 240);
    							$v_goods_array[$val['voucher_t_id']][$k]['name'] = $goods_list[$val['goods_commonid']]['goods_name'];
    							$v_goods_array[$val['voucher_t_id']][$k]['shop_price'] = imPriceFormat($goods_list[$val['goods_commonid']]['goods_price']);
    							$v_goods_array[$val['voucher_t_id']][$k]['price'] = imPriceFormat($val['vc_goods_price']);
    							$voucher_array[$val['voucher_t_id']]['cost_price'] += $goods_list[$val['goods_commonid']]['goods_price'];
    						}
    					}
    				}
    			}
    		}
    		$array = array('goods_array' => serialize($voucher_array), 'v_goods_array' => serialize($v_goods_array));
    		$this->_wGoodsVoucherCache($goods_commonid, $array);
    	}
    	return $array;
    }
     ...
     * @return array
     */
    private function _rGoodsVoucherCache($goods_commonid) {
    	return rcache($goods_commonid, 'goods_voucher');
    }
     ...
     * @return boolean
     */
    private function _wGoodsVoucherCache($goods_commonid, $array) {
    	return wcache($goods_commonid, $array, 'goods_voucher');
    }
     ...
     * @return boolean
     */
    private function _dGoodsVoucherCache($goods_commonid) {
    	return dcache($goods_commonid, 'goods_voucher');
    }
}