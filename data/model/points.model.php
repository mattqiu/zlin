...
class pointsModel extends Model{
	/**
	 * 操作积分
	 * @author zlin-e Develop Team
	 * @param  string $stage 操作阶段 regist(注册),login(登录/签到),comments(评论),order(下单),system(系统),other(其他),pointorder(积分礼品兑换),app(同步积分兑换)
	...
	function savePointsLog($stage,$insertarr,$if_repeat = true){
		...
		switch ($stage){
			...
			case 'signin':
				...
				$points_signin = $this->getMemberSignPoints($insertarr['pl_memberid'],$insertarr['member_sign_num']);
				$insertarr['pl_points'] = $points_signin['member_sign_points'];
				$insertarr['pl_signnum'] = $points_signin['member_sign_num'];
			...
			case 'order':
				...
				if($insertarr['pl_points']>0){//大于0 就是送积分
					if (!$insertarr['pl_desc']){
						$insertarr['pl_desc'] = '订单'.$insertarr['order_sn'].'购物消费';
					}
					
					$points_orderrate = C('points_orderrate');
					if ($insertarr['orderprice'] && !empty($points_orderrate)){
						$insertarr['pl_points'] = 0;
						$insertarr['pl_points'] = @intval($insertarr['orderprice']/$points_orderrate);
						$points_ordermax = intval(C('points_ordermax'));
						if ($insertarr['pl_points'] > $points_ordermax && $points_ordermax>0){
					...
					//小于0就是抵扣积分
					if (!$insertarr['pl_desc']){
						$insertarr['pl_desc'] = '订单'.$insertarr['order_sn'].'购物消耗积分';
					}
				}
				break;
			case 'system':
		...
		if($value_array['pl_points'] != '0'){
			$result = $this->table('points_log')->insert($value_array);
	 ...
	 * @param array $field   查询字段
	 */
	public function getLastSignInfo($member_id,$field='*'){
		//得到条件语句
		$condition =array();
	 ...
	 * @param array $field   查询字段
	 */
	public function getMemberSignPoints($member_id,$member_sign_num){
		$member_sign_points = 0;
		$sign_num = 0;
		if (C('points_isuse')){
		    $last_sign_info = $this->getLastSignInfo($member_id);
		    if(!empty($last_sign)) {			
		        $sign_num = $member_sign_num?intval($member_sign_num):0;
		        $sign_days = timediff(strtotime(date('Y-m-d',$last_sign_info['pl_addtime'])),strtotime(date('Y-m-d')));
			    if ($sign_days>1){
				    $sign_num = 0;
			    }
		    }
		    ...
		    $points_sign = C('points_sign')?unserialize(C('points_sign')):array();
		    if (intval($points_sign[$sign_num])>0){
			    $member_sign_points = intval($points_sign[$sign_num]);
		    }
		}		
		return array('member_sign_points'=>$member_sign_points,'member_sign_num'=>$sign_num);		
	}
