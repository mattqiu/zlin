...
Dialog.prototype = {
    ...
    'init'          : function(){
        ...
        /* 创建内容区域 */
        this.dom.content        = $('<div id="dialog_content" class="dialog_content"></div>').get(0);
...
//        }
    ...
    'hide'          : function(){
		...
		var element_height=$(this.dom.wrapper).outerHeight();//获取元素高度:height+paelement_heighting+margin
		var ee=-element_height;//元素隐藏时的top值
		$(this.dom.wrapper).stop(true).animate({top:ee,opacity:0},600,"linear",function(){self.close();});
		
        //$(this.dom.wrapper).hide();
    },
	/* 显示 */
	'showdialog'    : function(){
		$(this.dom.wrapper).css({"top":$(this.dom.wrapper).outerHeight()*-1+"px","opacity":0});//载入页面时将元素置于页面可见区域的顶部达到自上而下显示效果（PS：有隐藏效果）
        /* 显示对话框 */
		var browser_visible_region_height=document.documentElement.clientHeight;//获取浏览器可见区域高度
		var element_height=$(this.dom.wrapper).outerHeight();//获取元素高度:height+paelement_heighting+margin
		//计算元素显示时的top值
		//var element_show_top=(browser_visible_region_height-element_height)/2;  原来的
		//我修改的
		var element_show_top = $(window).scrollTop()  + (browser_visible_region_height - element_height) / 2; 

		$(this.dom.wrapper).stop(true).animate({top:element_show_top,opacity:1},800);
            ...
            case 'message':
                ...
                switch (type){
                    ...
                    case 'warning':
                        ...
                        _ok_button = $('<input type="button" class="btn1 btnGreen" value="' + button_name + '" />');
    ...
    'setPosition'   : function(pos){
        ...
        if (typeof(pos) == 'string')
        {
		    ...
		    ks_height = $(window).height();
		    if(ks_height<dialog_height){
...
/*common2.2.js*/
DIALOGIMGDIR  = SITEURL+'data/resource/js/dialog/images';
...
function showSucc(msg,url) {
	var p = /<script[^\>]*?>([^\x00]*?)<\/script>/ig;
	var aURL = isUndefined(url) ? null : function(){window.location = url;};
	msg = msg.replace(p, '');
	if(msg !== '') {
		showDialog(msg, 'succ', '提示信息', aURL, true, null, '', '', '', 3);
	}
}
...
function showError(msg,url) {
	...
	msg = msg.replace(p, '');
	if(msg !== '') {
		showDialog(msg, 'alert', '错误信息', aURL, true, null, '', '', '', 3);
	}
}
