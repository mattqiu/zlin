$(function(){
	//加关注
	$("[im_type='followbtn']").live('click',function(){
        ...
        $.getJSON('index.php?act=member_snsfriend&op=addfollow&mid='+data_str.mid, function(data){
        	if(data){
        		...
        		obj.find('[im_type="signmodule"]').children().hide();
        		if(data.state == 2){
        			obj.find('[im_type=\"mutualsign\"]').show();
        		}else{
        			obj.find('[im_type=\"followsign\"]').show();
        		}
    			showSucc('关注成功');
        	}else{
        		showError('关注失败');
	...
	//取消关注
	$("[im_type='cancelbtn']").live('click',function(){
		var data_str = $(this).attr('data-param');
        eval( "data_str = "+data_str);
	...
	// 批量关注
	$('*[imtype="batchFollow"]').live('click', function(){
