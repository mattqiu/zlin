...
//关注品牌js
function follow_brand(brand_id,jstype,jsobj){
     $.get('index.php?act=index&op=login', function(result){
        if(result=='0'){
            login_dialog();
        }else{
            var url = 'index.php?act=member_brandfollow&op=addbrandfollow';
            $.getJSON(url, {'bid':brand_id}, function(data){
                if (data.done)
                {
                    showDialog(data.msg, 'succ','','','','','','','','',2);					
					var total=parseInt($(jsobj).parent().find(".box-num").find(".j_SubjectCountNum").html())+1;
			        $(jsobj).parent().find(".box-num").find(".j_SubjectCountNum").html(total);
                }
                else
                {
                    showDialog(data.msg, 'notice');
...
//点赞活动专场js
function follow_activity(activity_id,jstype,jsobj){
     $.get('index.php?act=index&op=login', function(result){
        if(result=='0'){
            login_dialog();
        }else{
            var url = 'index.php?act=activity&op=addactivityfollow';
            $.getJSON(url, {'bid':activity_id}, function(data){
                if (data.done)
                {
                    showDialog(data.msg, 'succ','','','','','','','','',2);					
					var total=parseInt($(jsobj).find(".followCountNum").html())+1;
			        $(jsobj).find(".followCountNum").html(total);
                }
                else
                {
                    showDialog(data.msg, 'notice');
	    ...
	    if(result){
	       	...
	       	if(result.cart_goods_num >0){
	            for (var i = 0; i < result['list'].length; i++){
	            	...
	            	html+='<dl ncTpye="cart_item_'+goods['goods_id']+'"><dt class="goods-name"><a href="'+goods['goods_url']+'">'+goods['goods_name']+'</a></dt>';
    ...
    $.getJSON(SITEURL+'/index.php?act=cart&op=del&cart_id='+cart_id+'&goods_id='+goods_id+'&callback=?', function(result){
        if(result.state){
            ...
            if(result.quantity == 0){
				...
				$('div[imtype="rtoolbar_total_price"]').html("");
				$("#rtoobar_cart_count").html("").hide()
            }else{
                $('dl[ncTpye="cart_item_'+goods_id+'"]').remove();
				$('li[ncTpye="cart_item_'+goods_id+'"]').remove();
				html="共<i>"+result.quantity+"</i>种商品&nbsp;&nbsp;总计金额：<em>&yen;"+result.amount+"</em>";
				e.find(".total-price").html(html);
				e.find(".addcart-goods-num").html(result.quantity);
				e.find(".incart-goods-box").perfectScrollbar("destroy");
				e.find(".incart-goods-box").perfectScrollbar();
				
				$('div[imtype="rtoolbar_total_price"]').html("共计金额：<em class='goods-price'>&yen;"+result.amount+"</em>");
...
function drop_topcart_item_NEW(t){
	$.getJSON(SITEURL+"/index.php?act=cart&op=del&cart_id="+t+"&callback=?",function(a){
		if(a.state){
			...
			if(a.quantity==0){
				...
				$('div[imtype="rtoolbar_total_price"]').html("共计金额：<em class='goods-price'>&yen;"+a.amount+"</em>");
...
(function($) {
    $.show_im_login = function(options) {
        ...
        var dialog_html = '<div class="quick-login">';
		dialog_html += '<div class="register"><h4>簦到有礼,享受会员特权<a href="index.php?act=login&op=register" class="forget">立即注册</a></h4></div>';
        dialog_html += '<form class="bg" method="post" id="ajax_login" action="'+APP_SITE_URL+settings.action+'">';
		dialog_html += '<input type="hidden" value="ok" name="form_submit">';
		dialog_html += '<input type="hidden" value="'+settings.formhash+'" name="formhash">';
		dialog_html += '<input type="hidden" value="'+settings.imhash+'" name="imhash">';
		dialog_html += '<dl><dt>用户名：</dt><dd><input type="text" name="user_name" autocomplete="off" class="text"></dd></dl>';
		dialog_html += '<dl><dt>密&nbsp;&nbsp;&nbsp;码：</dt><dd><input type="password" autocomplete="off" name="password" class="text"></dd></dl>';
		dialog_html += '<dl><dt>验证码：</dt><dd><input type="text" size="10" maxlength="4" class="text fl w60" name="captcha"><img border="0" onclick="this.src=\''+APP_SITE_URL+'/index.php?act=seccode&amp;op=makecode&amp;imhash='+settings.imhash+'&amp;t=\' + Math.random()" name="codeimage" title="看不清，换一张" src="'+APP_SITE_URL+'/index.php?act=seccode&amp;op=makecode&amp;imhash='+settings.imhash+'" class="fl ml10"><span></span></dd></dl>';
		dialog_html += '<div class="enter"><input type="hidden" name="ref_url" value="'+ref_url+'"><input type="submit" name="Submit" value="登&nbsp;&nbsp;录" class="submit"></div>';
		dialog_html += '</form>';
		if (connect_qq_isuse == '1' || connect_sn_isuse == '1' || connect_wx_isuse == '1'){
			dialog_html += '<div class="other" id="demo-form-site"><h4>使用合作帐号登录<a href="'+SHOP_SITE_URL+'/index.php?act=login&op=forget_password" class="forget">忘记密码?</a></h4>';
		    if (connect_qq_isuse == '1'){
                dialog_html += '<a href="'+SHOP_SITE_URL+'/api.php?act=toqq" title="QQ账号登录" class="qq"><i></i>QQ</a>';
            }
            if (connect_sn_isuse == '1'){
                dialog_html += '<a href="'+SHOP_SITE_URL+'/api.php?act=tosina" title="新浪微博账号登录" class="sina"><i></i>新浪微博</a>';
            }
            if (connect_wx_isuse == '1'){
                dialog_html += '<a href="javascript:void(0);" onclick="ajax_form(\'weixin_form\', \'微信账号登录\', \''+SHOP_SITE_URL+'/api.php?act=towx\', 360);" title="微信账号登录" class="wx"><i></i>微信</a>';
            }
			dialog_html += '</div>';
		}		
		dialog_html += '</div>';
		
		var login_dialog_html = $(dialog_html);
 ...
 */
(function($) {
    $.fn.im_show_dialog = function(options) {
 ...
 *
 * HTML part
 * <a href="javascript" imtype="mcard" data-param="{'id':5}"></a>
 *
 * JAVASCRIPT part
 * <script type="text/javascript" src="<?php echo RESOURCE_SITE_URL;?>/js/qtip/jquery.qtip.min.js"></script>
 * <link href="<?php echo RESOURCE_SITE_URL;?>/js/qtip/jquery.qtip.min.css" rel="stylesheet" type="text/css">
 * $('a[imtype="mcard"]').membershipCard();
 */
(function($){
	$.fn.membershipCard = function(options){
		                ...
		                success: function(data) {
		                	if(data){
		                		...
		                		if(data.follow != 2){
		                			...
		                			var _a;
		                			if(data.follow == 1){
		                				$('<div class="follow-handle" imtype="follow-handle'+data.id+'" data-param="{\'mid\':'+data.id+'}"></div>')
		                					.append('<a href="javascript:void(0);" >已关注</a>')
		                					.append('<a href="javascript:void(0);" imtype="nofollow">取消关注</a>').find('a[imtype="nofollow"]').click(function(){
		                						onfollow($(this));
		                					}).end().appendTo(_bottom);
		                			}else{
		                				$('<div class="follow-handle" imtype="follow-handle'+data.id+'" data-param="{\'mid\':'+data.id+'}"></div>')
		                					.append('<a href="javascript:void(0);" imtype="follow">加关注</a>').find('a[imtype="follow"]').click(function(){
		                						follow($(this));
		                					}).end().appendTo(_bottom);
		                			}
		...
		function follow(o){
			...
			$.getJSON(SHOP_SITE_URL+'/index.php?act=member_snsfriend&op=addfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
					$('[imtype="follow-handle'+data_str.mid+'"]').html('<a href="javascript:void(0);" >已关注</a> <a href="javascript:void(0);" imtype="nofollow">取消关注</a>').find('a[imtype="nofollow"]').click(function(){
						onfollow($(this));
					});
				}
			});
		...
		function onfollow(o){
			...
			$.getJSON(SHOP_SITE_URL+'/index.php?act=member_snsfriend&op=delfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
					$('[imtype="follow-handle'+data_str.mid+'"]').html('<a href="javascript:void(0);" imtype="follow">加关注</a>').find('a[imtype="follow"]').click(function(){
						follow($(this));
					});
				}
			});
    ...
    $.fn.im_region = function(options) {
        ...
        function loadArea($area, area_id){
            ...
            $area.on('change', function() {
                ...
                if(area_id > 0) {
                    if(im_a[area_id] && im_a[area_id].length > 0) {
