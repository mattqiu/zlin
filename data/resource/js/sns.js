$(function(){
	//修改权限模块
	$("[im_type='privacydiv']").live('mouseover',function(){
		$(this).find("[im_type='privacytab']").show();
	});
	$("[im_type='privacydiv']").live('mouseout',function(){
		$(this).find("[im_type='privacytab']").hide();
	});
	$("[im_type='privacyoption']").live('click',function(){
		var obj = $(this);
	...
	//表单权限模块
	$("[im_type='formprivacydiv']").live('mouseover',function(){
		$(this).find("[im_type='formprivacytab']").show();
	});
	$("[im_type='formprivacydiv']").live('mouseout',function(){
		$(this).find("[im_type='formprivacytab']").hide();
	});
	//选择权限
	$("[im_type='formprivacyoption']").die('click').live('click',function(){
		var data_str = $(this).attr('data-param');
	    eval( "data_str = "+data_str);
	    ...
	    //$("[im_type='formprivacytab']").find('span').removeClass('selected');
	    $(this).parent().find('span').removeClass('selected');
	    $(this).find('span').addClass('selected');
	    $("#"+hiddenid).val(data_str.v);
	});
	//分享单个商品
	$("[im_type='sharegoods']").bind('click',function(){
		var data_str = $(this).attr('data-param');
	    eval( "data_str = "+data_str);
	...
	$("#weibobtn_goods").die('click').live("click",function(){
		if($("#sharegoods_form").valid()){
			...
			if(cookienum >= max_recordnum && $("#sg_seccode").css('display') == 'none'){
				//显示验证码
				$("#sg_seccode").show();
				var imhash = $("#sg_seccode").find("[name='imhash']").val();
				$("#sg_seccode").find("[name='codeimage']").attr('src','index.php?act=seccode&op=makecode&imhash='+imhash+'&t=' + Math.random());
	...
	//分享单个店铺
	$("[im_type='sharestore']").bind('click',function(){
		var data_str = $(this).attr('data-param');
	    eval( "data_str = "+data_str);	    
	    ajaxget('index.php?act=member_snsindex&op=sharestore_one&dialog=1&sid='+data_str.sid);
	});
	//删除分享和喜欢的商品
	$("[im_type='delbtn']").die('click').live('click',function(){
		var data_str = $(this).attr('data-param');
        eval( "data_str = "+data_str);
	...
	//喜欢操作
	$("[im_type='likebtn']").die('click').live('click',function(){
		var obj = $(this);
		var data_str = $(this).attr('data-param');
        eval( "data_str = "+data_str);
    ...
    //展示和隐藏评论列表
	$("[im_type='fd_commentbtn']").die('click').live('click',function(){
    ...
    //删除动态
	$("[im_type='fd_del']").die('click').live('click',function(){
		var data_str = $(this).attr('data-param');
        eval("data_str = "+data_str);
	...
	//转发提交
	$("[im_type='forwardbtn']").die('click').live('click',function(){
		var data = $(this).attr('data-param');
        eval("data = "+data);
		if($("#forwardform_"+data.txtid).valid()){
			...
			if(cookienum >= max_recordnum && $("#forwardseccode"+data.txtid).css('display') == 'none'){
				...
				var imhash = $("#forwardseccode"+data.txtid).find("[name='imhash']").val();
				$("#forwardseccode"+data.txtid).find("[name='codeimage']").attr('src','index.php?act=seccode&op=makecode&imhash='+imhash+'&t=' + Math.random());
	...
	//展示和隐藏转发表单
	$("[im_type='fd_forwardbtn']").die('click').live('click',function(){
		var data = $(this).attr('data-param');
        eval("data = "+data);
        //隐藏评论模块
        $('#tracereply_'+data.txtid).hide();
	...
	// 查看大图
	$('[im_type="thumb-image"]').die().live('click',function(){
		src = $(this).find('img').attr('src');
		max_src = src.replace('_small.', '_max.');
		$(this).parent().hide().next().children('[im_type="origin-image"]').append('<img src="'+max_src+'" />').end().show();
	});
	$('[im_type="origin-image"]').die().live('click',function(){
