...
(function($) {
	$.fn.brandinit = function(options){
		...
		//根据首字母查询
		$(this).find('.letter[imtype="letter"]').find('a[data-letter]').click(function(){
		...
		// 根据关键字查询
		$(this).find('.search[imtype="search"]').find('a').click(function(){
			var _url = $(this).parents('.brand-index:first').attr('data-url');
		...
		// 选择品牌
		$(this).find('ul[imtype="brand_list"]').on('click', 'li', function(){
			$('#b_id').val($(this).attr('data-id'));
			$('#b_name').val($(this).attr('data-name'));
		});
		//搜索品牌列表滚条绑定
		$(this).find('div[imtype="brandList"]').perfectScrollbar();
	}
	$.fn.insertBrand = function(options) {
		//品牌搜索容器
		var dataContainer = $(this);
		$(dataContainer).find('div[imtype="brandList"]').show();
		$(dataContainer).find('div[imtype="noBrandList"]').hide();
		var _ul = $(dataContainer).find('ul[imtype="brand_list"]');
		_ul.html('');
		if ($.isEmptyObject(options.param)) {
			$(dataContainer).find('div[imtype="brandList"]').hide();
			$(dataContainer).find('div[imtype="noBrandList"]').show().find('strong').html(options.search);
			return false;
		}
		...
		//搜索品牌列表滚条绑定
		$(dataContainer).find('div[imtype="brandList"]').perfectScrollbar('update');
    };
})(jQuery);