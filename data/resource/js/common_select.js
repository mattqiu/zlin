...
function regionInit(divId){
	var area_id = 0;	
	getArea(function(){
		if(typeof(im_a[area_id]) == 'object' && im_a[area_id].length > 0){//数组存在
...
function areaInit(area_select,area_id){//初始化地区
	getArea(function(){
		if(typeof(area_select) == 'object' && im_a[area_id].length > 0){
			var areas = new Array();
			areas = im_a[area_id];
...
if(typeof(jQuery.validator.addMethod) == 'function'){//添加自动检测是否是最后一级地区
	jQuery.validator.addMethod("checkarea", function(value, element) {
		return this.optional(element) || (typeof(im_a[value]) == 'undefined');//当数组不存在时确定选到最后
	}, "请选择所在地区");
} 
function getArea(callback){
	if(typeof(im_a) == 'undefined'){//加载地区数据
...
function show_gc_1(depth,gc_json){
	var html = '<select name="search_gc[]" id="search_gc_0" im_type="search_gc" class="querySelect">';;
...
function show_gc_2(chooseid,gc_json){
	if(gc_json && chooseid > 0){
		...
		if(childid){
			var html = '<select name="search_gc[]" id="search_gc_'+gc_json[chooseid].depth+'" im_type="search_gc" class="querySelect">';;
			html += ('<option value="0">请选择...</option>');
...
function init_gcselect(chooseid_json,gc_json){
	...
	//商品分类select绑定事件
	$("[im_type='search_gc']").live('change',function(){
        $(this).nextAll("[im_type='search_gc']").remove();
