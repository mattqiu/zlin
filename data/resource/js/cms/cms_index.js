...
$(document).ready(function(){
    ...
    //标题修改
    $("[imtype='btn_module_title_edit']").click(function(){
        object_module_edit = $(this).parent().parent().find("[imtype='object_module_edit']");
        var title = object_module_edit.html();
        $('#dialog_module_title_edit').im_show_dialog({width: 640, title: '编辑标题'});
    ...
    $("#btn_module_tag_edit").click(function(){
        ...
        tag_list.find("li").append('<i imtype="btn_module_tag_select_drop" class="cms-index-tag-select-drop" class="删除所选"></i>');
        $('#dialog_module_tag_edit').im_show_dialog({width: 640, title: '编辑标签'});
        $("#article_tag_selected_list").html(tag_list.html());
    });
    //标签选择
    $(".article-tag-list [imtype='btn_tag_select']").live('click', function(){
    ...
    $(".article-tag-selected-list [imtype='btn_tag_select']").live('click', function(){
        $(this).remove();
        return false;
    });
    //标签保存
    $("#btn_module_tag_save").click(function(){
        ...
        tag_list.find("[imtype='btn_module_tag_select_drop']").remove();
    ...
    //图片修改
    $("[imtype='btn_module_image_edit']").click(function(){
        object_module_edit = $(this).parent().parent().find("[imtype='object_module_edit']");
        ...
        if(image_list.find("li.picture").length > 0) {
            image_list.find("li").each(function(){
                ...
                $(this).append('<a imtype="btn_module_drop_image" image_name="'+image_name+'" class="handle-del" title="删除该图片"><i></i>&nbsp;</a>');
        ...
        $('#dialog_module_image_edit').im_show_dialog({width: 640, title: '编辑图片'});
        if(image_list.find("li.picture").length > 0) {
            ...
            add: function(e,data) {
                data.submit();
                    ...
                    if(result.status == "success") {
                        ...
                        image_content += '<a imtype="btn_module_drop_image" image_name="'+result.file_name+'" class="handle-del" title="删除该图片"><i></i>&nbsp;</a>';			
    ...
    //图片删除
    $("[imtype='btn_module_drop_image']").live("click", function(){
    ...
    //文章修改
    $("[imtype='btn_module_article_edit']").click(function(){
        object_module_edit = $(this).parent().parent().find("[imtype='object_module_edit']");
        ...
        article_list.find('[imtype="article_view"]').remove();
        article_list.find("li").each(function(){
            var title = $(this).find("span.title");
            var article = $(this).find("span.title a");
            $(this).prepend('<span class="article-image" imtype="cms_index_not_display"><p><img src="'+article.attr("article_image")+'"></p></span>');
            title.prepend('<em class="class-name" imtype="cms_index_not_display">['+article.attr("class_name")+']</em>');
            title.append('<em class="publish-time" imtype="cms_index_not_display">('+article.attr("article_publish_time")+')</em>');
            $(this).append('<span title="'+article.attr("article_abstract")+'" class="article-abstract" imtype="cms_index_not_display">文章摘要：'+article.attr("article_abstract")+'</span>');
            $(this).append('<a imtype="btn_article_select" href="JavaScript:void(0);" class="delete" title="选择删除"></a>');
        });
        $('#dialog_module_article_edit').im_show_dialog({width: 640, title: '编辑文章'});
    ...
    //文章选择
    $("#article_search_list [imtype='btn_article_select']").live("click", function(){
        var current_count = $("#article_selected_list li").length;
        if(current_count < limit_count || limit_count === 0) {
            var temp = $(this).parent().clone();
            temp.find("[imtype='btn_article_select']").attr("title","删除");
            $("#article_selected_list").append($("<div />").append(temp).html());
        }
    });
    //文章删除
    $("#article_selected_list [imtype='btn_article_select']").live("click", function(){
        $(this).parent().remove();
    });
    //文章列表保存
    $("#btn_module_article_save").click(function(){
        $("#article_selected_list").find("[imtype='btn_article_select']").remove();
        $("#article_selected_list").find("[imtype='cms_index_not_display']").remove();
    ...
    function_list['article_type_1_save'] = function() {
        var count = 1;
        $("#article_selected_list li").each(function(){
            $(this).prepend('<span imtype="article_view" class="cms-index-count_'+count+'">'+count+'</span>');
            $(this).append('<span imtype="article_view" class="cms-index-click-count">'+$(this).find('[imtype="article_item"]').attr("article_click")+'人关注</span>');
            object_module_edit.append($(this));
            count++;
        });
    };
    ...
    function_list['article_type_2_save'] = function() {
        var count = 1;
        $("#article_selected_list li").each(function(){
            var article = $(this).find('[imtype="article_item"]');
            if(count > 1) {
                $(this).attr("class", "cms-index-article-normal");
                $(this).prepend('<span imtype="article_view" class="cms-index-article-class">'+article.attr("class_name")+'</span>');
                $(this).append('<span imtype="article_view" class="cms-index-article-date">'+article.attr("article_publish_time").substring(8)+'日</span>');
            } else {
                $(this).attr("class", "cms-index-article-focus");
            }
            object_module_edit.append($(this));
    ...
    function_list['article_type_3_save'] = function() {
        $("#article_selected_list li").each(function(){
            var article = $(this).find('[imtype="article_item"]');
            $(this).prepend('<div class="cms-thumb" imtype="article_view"><a imtype="article_view" href="'+article.attr("href")+'"><img class="t-img" src="'+article.attr("article_image")+'" /></a></div');
            object_module_edit.append($(this));
        });
    };
    //文章列表1-1保存
    function_list['article_type_4_save'] = function() {
        var count = 1;
        $("#article_selected_list li").each(function(){
            $(this).append('<span imtype="article_view" class="cms-index-article-abstract">'+$(this).find('[imtype="article_item"]').attr("article_abstract")+'</span>');
            object_module_edit.append($(this));
            count++;
        });
    };
    ...
    //商品修改
    $("[imtype='btn_module_goods_edit']").click(function(){
        object_module_edit = $(this).parent().parent().find("[imtype='object_module_edit']");
        var goods_list = object_module_edit.clone();
        goods_list.find("dl").append('<dd class="taobao-item-delete" imtype="btn_goods_delete" title="删除添加的商品">&nbsp;</dd>');
        limit_count = parseInt($(this).attr("limit_count"), 10);
        if(!limit_count) {
            limit_count = 0;
        }
        $('#dialog_module_goods_edit').im_show_dialog({width: 640, title: '编辑商品'});
        ...
        if($("#goods_selected_list li").length < limit_count || limit_count === 0) {
            if(link != '') {
                ...
                $.getJSON('index.php?act=cms_base&op=goods_info_by_url', { url: url}, function(data){
                    if(data.result == "true") {
                        var temp = '<li imtype="btn_goods_select"><dl>'; 
                        ...
                        temp += '<dd class="taobao-item-delete" imtype="btn_goods_delete" title="删除添加的商品">&nbsp;</dd>';
                        temp += '</dl></li>';
    ...
    //删除商品
    $("[imtype='btn_goods_delete']").live('click', function(){
        $(this).parent().parent().remove();
    });
    //保存商品列表
    $("#btn_module_goods_save").live('click', function() {
        var goods_list = $("#goods_selected_list").clone();
        goods_list.find("[imtype='btn_goods_delete']").remove();
    ...
    //品牌修改
    $("[imtype='btn_module_brand_edit']").click(function(){
        object_module_edit = $(this).parent().parent().find("[imtype='object_module_edit']");
        var brand_list = object_module_edit.clone();
        brand_list.find("li").append('<div imtype="btn_brand_select" class="add-brand"><i></i></div>');
        limit_count = parseInt($(this).attr("limit_count"), 10);
        if(!limit_count) {
            limit_count = 0;
        }
        ...
        $('#dialog_module_brand_edit').im_show_dialog({width: 640, title: '编辑品牌'});
    ...
    //品牌选择
    $("#brand_search_list [imtype='btn_brand_select']").live("click", function(){
    ...
    //品牌删除
    $("#brand_selected_list [imtype='btn_brand_select']").live("click", function(){
        $(this).parent().remove();
    });
    //品牌列表保存
    $("#btn_module_brand_save").click(function(){
        var brand_list = $("#brand_selected_list").clone();
        brand_list.find("[imtype='btn_brand_select']").remove();
    ...
    //商品分类修改
    $("[imtype='btn_module_goods_class_edit']").click(function(){
        object_module_edit = $(this).parent().parent().find("[imtype='object_module_edit']");
        ...
        $('#dialog_module_goods_class_edit').im_show_dialog({width: 640, title: '编辑分类'});
    ...
    $("#btn_module_circle_edit").live('click', function(){
        var module_item = $(this).parent().parent().find("[imtype='object_module_edit']");
    ...
    //店铺修改
    $("[imtype='btn_module_store_edit']").click(function(){
        object_module_edit = $(this).parent().parent().find("[imtype='object_module_edit']");
        var store_list = object_module_edit.clone();
        store_list.find("li").each(function(){
            $(this).find('dl').append('<dd imtype="btn_store_select" class="handle-button" title="删除"></dd>');
        });
        $('#dialog_module_store_edit').im_show_dialog({width: 640, title: '编辑店铺'});
    ...
    //店铺选择
    $('#div_store_select_list').on('click', '[imtype="btn_store_select"]', function(){
        var current_count = $("#article_selected_list li").length;
        var temp = $(this).parents('li').clone();
        temp.find("[imtype='btn_store_select']").attr("title","删除");
        $("#store_selected_list").append($("<div />").append(temp).html());
    });
    //店铺删除
    $('#store_selected_list').on('click', '[imtype="btn_store_select"]', function(){
        $(this).parents('li').remove();
    });
    //店铺列表保存
    $("#btn_module_store_save").click(function(){
        $("#store_selected_list").find("[imtype='btn_store_select']").remove();
        object_module_edit.html("");
    ...
    //用户修改
    $("[imtype='btn_module_member_edit']").click(function(){
        object_module_edit = $(this).parent().parent().find("[imtype='object_module_edit']");
        var member_list = object_module_edit.clone();
        member_list.find("li").each(function(){
            $(this).find('dl').append('<dd imtype="btn_member_select" class="handle-button" title="删除"></dd>');
        });
        $('#dialog_module_member_edit').im_show_dialog({width: 640, title: '编辑会员'});
    ...
    //用户选择
    $('#div_member_select_list').on('click', '[imtype="btn_member_select"]', function(){
        var current_count = $("#article_selected_list li").length;
        var temp = $(this).parents('li').clone();
        temp.find("[imtype='btn_member_select']").attr("title","删除");
        $("#member_selected_list").append($("<div />").append(temp).html());
    });
    //用户删除
    $('#member_selected_list').on('click', '[imtype="btn_member_select"]', function(){
        $(this).parent('li').remove();
    });
    //文章列表保存
    $("#btn_module_member_save").click(function(){
        $("#member_selected_list").find("[imtype='btn_member_select']").remove();
        object_module_edit.html("");
    ...
    //FLASH修改
    $("[imtype='btn_module_flash_edit']").click(function(){
        object_module_edit = $(this).parent().parent().find("[imtype='object_module_edit']");
        $('#dialog_module_flash_edit').im_show_dialog({width: 640, title: '编辑FLASH'});
    });
    //FLASH保存
    ...
    //自定义块修改
    $("[imtype='btn_module_html_edit']").click(function(){
        object_module_edit = $(this).parent().parent().find("[imtype='object_module_edit']");
        $('#dialog_module_html_edit').im_show_dialog({width: 680, title: '编辑自定义块'});
    ...
    $("#btn_module_save").click(function(){
        $("[imtype='object_module_edit']").each(function(){
            $("#add_form").append('<input name="'+$(this).attr("id")+'" type="hidden" value="" />');
            //去掉无用内容
            $(this).find("[imtype='cms_index_not_save']").remove();
