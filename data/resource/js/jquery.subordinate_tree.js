$(document).ready(function(){
	...
	$('img[im_type="flex"]').click(function(){
		...
		if(status == 'open'){
			...
			//ajax
			$.ajax({
				url: 'index.php?act=member_extension&op=my_subordinate&ajax=1&parent_id='+id+'&deep='+deep,
				dataType: 'json',
				success: function(data){
					...
					for(var i = 0; i < data.length; i++){
						...
						//操作
						src += "<td>";
						src += "<a href='javascript:void(0)' im_type='dialog' dialog_title='查看推广员信息' dialog_id='my_promotion_info' dialog_width='480' uri='index.php?act=member_extension&op=promotion_info&promotion_id="+data[i].member_id+"' title='查看推广员信息'>查看</a>|";
						src += "<a href='index.php?act=member_extension&op=promotion_detail&promotion_id="+data[i].member_id+"' title='查看业绩明细'>明细</a>";
						src += "</td>";
						src += "</tr>";
					}
					//插入
