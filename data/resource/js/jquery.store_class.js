				...
				success: function(data){
					...
					for(var i = 0; i < data.length; i++){
						...
						if(data[i].have_child == 1){
							src += "<img fieldid='"+data[i].sc_id+"' status='open' im_type='flex' src='"+ADMIN_SKINS_URL+"/images/tv-expandable.gif' />";
						}else{
							src += "<img fieldid='"+data[i].sc_id+"' status='none' im_type='flex' src='"+ADMIN_SKINS_URL+"/images/tv-item.gif' />";
						}
						//图片
						src += "</td><td class='w48 sort'>";
						//排序
						src += "<span title='可编辑' ajax_branch='store_class_sort' datatype='number' fieldid='"+data[i].sc_id+"' fieldname='sc_sort' im_type='inline_edit' class='editable'>"+data[i].sc_sort+"</span>";
						//名称
						src += "<td class='name'>";
						...
						if(data[i].have_child == 1){
							src += " <img fieldid='"+data[i].sc_id+"' status='open' im_type='flex' src='"+ADMIN_SKINS_URL+"/images/tv-item1.gif' />";
						}else{
							src += " <img fieldid='"+data[i].sc_id+"' status='none' im_type='flex' src='"+ADMIN_SKINS_URL+"/images/tv-expandable1.gif' />";
						}
						src += "<span title='可编辑' required='1' fieldid='"+data[i].sc_id+"' ajax_branch='store_class_name' fieldname='sc_name' im_type='inline_edit' class='node_name editable'>"+data[i].sc_name+"</span>";
						//新增下级
						if(data[i].deep < 2){
