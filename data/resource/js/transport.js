$(function(){
	...
	TransTpl += "元"; //<i class=\"icon-renminbi\"><\/i>
	TransTpl += "<\/em>， 每增加\n";
	...
	TransTpl += "全选<\/label>\n";
	TransTpl += "&nbsp;<a class=\"J_BatchSet imsc-btn-mini\" href=\"JavaScript:void(0);\">批量设置<\/a>\n";
	TransTpl += "<a class=\"J_BatchDel imsc-btn-mini\" href=\"JavaScript:void(0);\">批量删除<\/a>\n";
	TransTpl += "<\/div>\n";
	TransTpl += "<div class=\"tbl-attach\">\n";
	TransTpl += "<div class=\"J_SpecialMessage\"><\/div>\n";
	TransTpl += "<a class=\"J_AddRule imsc-btn-mini ml5\" href=\"JavaScript:void(0);\">";
	TransTpl += "<i class=\"fa fa-map-marker\"></i>为指定地区城市设置运费<\/a>\n";
	TransTpl += "<a class=\"J_ToggleBatch imsc-btn-mini\" style=\"display:none\" href=\"JavaScript:void(0);\">";
	...
	//头模板
	RuleHead += "<table cellspacing=\"0\" class=\"imsc-table-style\" cellpadding=\"0\" border=\"0\">\n";
	RuleHead += "<colgroup>\n";
...
//	RuleCell += "<a class=\"imcu-btn2\" title=\"编辑运送区域\" area-haspopup=\"true\" area-controls=\"J_DialogArea\" entype=\'J_EditArea\' data-acc=\"event:enter\" href=\"JavaScript:void(0);\">编辑<\/a>\n";
	...
	RuleCell += "元";//<i class=\"icon-renminbi\"><\/i>\n
	RuleCell += "<\/em>\n";
	RuleCell += "<\/td>\n";
	RuleCell += "<td>\n";
	...
	RuleCell += "<i class=\"fa fa-pencil-square-o\"><\/i><p>编辑<\/p>\n";
	RuleCell += "<\/a>\n";	
	RuleCell += "<\/span>\n";
	RuleCell += "<\/td>\n";
	RuleCell += "<\/tr>\n";
...
//		SelectArea = new Array();
		...
		//首先找市被全部选择的省份
		$('#J_CityList').find('.imsc-province-tab').each(function(){
		...
		$('#J_CityList').find('.imsc-province-list').find('input[type="checkbox"]').each(function(){
			if ($(this).attr('checked')){
				CityValue += $(this).val()+',';
			}
		});
...
/*	省份点击事件*/
	$('#dialog_areas').on('click','.J_Province',function(){
		if ($(this).attr('checked')){
			//选择所有未被disabled的子地区
			$(this).parent().find('.imsc-citys-sub').eq(0).find('input[type="checkbox"]').each(function(){
			...
			//计算并显示所有被选中的子地区数量
			num = '('+$(this).parent().find('.imsc-citys-sub').eq(0).find('input:checked').size()+')';
			if (num == '(0)') num = '';
			...
			$(this).parent().find('.imsc-citys-sub').eq(0).find('input[type="checkbox"]').attr('checked',false);
			//取消大区域选择
...
/*	大区域点击事件（华北、华东、华南...）*/
	$('#dialog_areas').on('click','.J_Group',function(){
		if ($(this).attr('checked')){
			...
			//循环显示每个省下面的市级的数量
			$(this).parent().parent().parent().find('.imsc-province-list').find('.imsc-province').each(function(){
				//显示该省下面已选择的市的数量
				num = '('+$(this).find('.imsc-citys-sub').find('input:checked').size()+')';
				//如果是0，说明没有选择，不显示数量
				if (num != '(0)'){
...
/*	省份下拉事件*/
	$('#dialog_areas').on('click','.trigger',function () {
		...
		if ($(this).next().css('display') == 'none'){
			...
			$('.ks-contentbox').find('.imsc-province').removeClass('showCityPop');
...
//		curTransType = $(this).next().next().attr('name').substring(6,8);
		...
		try{
			...
			//初始化已选中的checkbox
			$('#J_CityList').find('.imsc-province').each(function(){
				var count = 0;
