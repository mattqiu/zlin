$(function(){
    ...
    //展示和隐藏评论列表
	$("[im_type='sd_commentbtn']").live('click',function(){
		var $this = $(this);
	...
	//评论提交
	$("[im_type='scommentbtn']").live('click',function(){
		var data = $(this).attr('data-param');
        eval("data = "+data);
	...
	//删除评论
	$("[im_type='scomment_del']").live('click',function(){
		var obj = $(this);
	...
	//展示和隐藏转发表单
	$("[im_type='sd_forwardbtn']").live('click',function(){
		var $this = $(this);
		$.get('index.php?act=index&op=login', function(result){
		    if(result=='0'){
		    	login_dialog();
	...
	//转发提交
	$("[im_type='s_forwardbtn']").live('click',function(){
		var data = $(this).attr('data-param');
		...
		if(form.valid()){
			...
			if(!isNaN(cookienum) && cookienum >= MAX_RECORDNUM){
			    if (seccode.css('display') == 'none') {
	                //显示验证码
	                seccode.show();
	                var imhash = seccode.find("[name='imhash']").val();
	                seccode.find("[name='codeimage']").attr('src','index.php?act=seccode&op=makecode&imhash='+imhash+'&t=' + Math.random());
    ...
    //删除动态
	$("[im_type='sd_del']").live('click',function(){
