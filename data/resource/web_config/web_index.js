...
function show_recommend_dialog(id) {//弹出框
    ...
    obj.find("input[name='bg_image"+id+"']").fileupload({
        dataType: 'json',
        url: ADMINURL + '/index.php?act=web_api&op=bgimage_upload',
        formData: {name:'bg_image'+id,imgid: imgid},
        add: function (e,data) {
        	$('img[imtype="bg_image_load'+id+'"]').attr('src', ADMIN_SKINS_URL + '/images/loading.gif');
            data.submit();
        },
        done: function (e,data) {
            var param = data.result;
			$('img[imtype="bg_image_load'+id+'"]').attr('src',ADMIN_SKINS_URL + '/images/preview.png');
            if (typeof(param.error) != 'undefined') {
                alert(param.error);                
            } else {
                $('input[name="recommend_list['+id+'][recommend][bgimg]"]').val(param.name);
                $('img[imtype="bg_image_pre'+id+'"]').attr('src',param.fullname);
            }
        }
    });
...
function add_recommend() {//增加商品推荐
	for (var i = 1; i <= recommend_max; i++) {//防止数组下标重复
		if ($(".middle dl[recommend_id='"+i+"']").size()==0) {//编号不存在时添加
			var add_html = '';
			var del_append = '';
			del_append = '<a href="JavaScript:del_recommend('+i+');"><i class="fa fa-times-circle "></i>删除</a>';//删除
			add_html = '<dl recommend_id="'+i+'"><dt><h4>商品推荐</h4>'+del_append+
    			'<a href="JavaScript:show_recommend_dialog('+i+');"><i class="fa fa-shopping-cart"></i>商品块</a><a href="JavaScript:show_recommend_pic_dialog('+i+');"><i class="fa fa-lightbulb-o"></i>广告块</a></dt>'+
    			'<dd><ul class="goods-list"><li><span><i class="fa fa-gift"></i></span></li><li><span><i class="fa fa-gift"></i></span></li><li><span><i class="fa fa-gift"></i></span></li>'+
    			'<li><span><i class="fa fa-gift"></i></span></li><li><span><i class="fa fa-gift"></i></span></li><li><span><i class="fa fa-gift"></i></span></li>'+
    			'<li><span><i class="fa fa-gift"></i></span></li><li><span><i class="fa fa-gift"></i></span></li></ul></dd></dl>';
...
function update_recommend() {//更新
	...
	if (get_text=='1') {
		...
		obj.find("dd").find("img").each(function() {
			var goods = $(this);
			var goods_pic = goods.attr("src");
			var goods_name = goods.attr("title");
...
function show_recommend_pic_dialog(id) {//弹出框
	...
	var recommend_bgcolor = $("input[name='recommend_list["+id+"][recommend][bgcolor]']").val();
	obj.find("input[name='recommend_list[recommend][bgcolor]']").val(recommend_bgcolor);
	
	var recommend_bgimg = $("input[name='recommend_list["+id+"][recommend][bgimg]']").val();
	obj.find("input[name='recommend_list[recommend][bgimg]']").val(recommend_bgimg);
	
	var recommend_bgimgpre = $("img[id='bg_image_pre"+id+"']").attr('src');
	obj.find("img[imtype='bg_image_pre']").attr('src',recommend_bgimgpre);
    ...
    obj.find("input[name='bg_image']").fileupload({
        ...
        add: function (e,data) {
        	$('img[imtype="bg_image_load"]').attr('src', ADMIN_SKINS_URL + '/images/loading.gif');
            data.submit();
        },
        done: function (e,data) {
            var param = data.result;
			$('img[imtype="bg_image_load"]').attr('src',ADMIN_SKINS_URL + '/images/preview.png');
            if (typeof(param.error) != 'undefined') {
                alert(param.error);                
            } else {
                $('input[name="recommend_list[recommend][bgimg]"]').val(param.name);
                $('img[imtype="bg_image_pre"]').attr('src',param.fullname);
            }
        }
    });
	...
	for (var i = 1; i <= sale_max; i++) {//防止数组下标重复
		if ($("dl[sale_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			add_html = '<dl sale_id="'+i+'"><a href="JavaScript:del_sale_list('+i+');" class="del">X</a><dt><h4>商品推荐</h4><a href="JavaScript:show_sale_dialog('+i+
    			');"><i class="fa fa-pencil-square-o"></i>编辑</a><input name="sale_list['+i+'][recommend][name]" value="" type="hidden"></dt>'+
    			'<dd><ul class="goods-list"><li><span><i class="fa fa-gift"></i></span></li><li><span><i class="fa fa-gift"></i></span></li><li><span><i class="fa fa-gift"></i></span></li>'+
    			'<li><span><i class="fa fa-gift"></i></span></li><li><span><i class="fa fa-gift"></i></span></li></ul></dd></dl>';
