...
class buy_virtualLogic {
    ...
    public function getBuyStepData($goods_id, $quantity, $member_id) {
        ...
        $goods_info['goods_total'] = imPriceFormat($goods_info['goods_price'] * $goods_info['quantity']);
        $goods_info['goods_image_url'] = cthumb($goods_info['goods_image'], 240, $goods_info['store_id']);
        $goods_info['vr_invalid_refund'] = empty($goods_info['vr_invalid_refund'])?0:$goods_info['vr_invalid_refund'];
    ...
    public function buyStep3($post, $member_id) {
        ...
        //应付总金额计算
        $pay_total = $goods_info['goods_price'] * $post['quantity'];
        ...
        $input['payment_code'] = $post['payment_code'];
    ...
    private function _createOrder($input, $goods_info, $member_info) {
        ...
        $order['payment_code'] = $input['payment_code']; //支付方式
        ...
        $order['goods_image'] = $goods_info['goods_image_url'];
        $order['commis_rate'] = 0;
        $order['gc_id'] = $goods_info['gc_id'];
        $order['vr_indate'] = $goods_info['virtual_indate'];
        $order['vr_invalid_refund'] = $goods_info['vr_invalid_refund'];
        ...
        if ($goods_info['ifgroupbuy'] == 1) {
        	...
        	$order['promotions_id'] = 0;
        }

        $order_id = $model_vr_order->addOrder($order);

        if (!$order_id) {
            throw new Exception('订单保存失败'.implode('--',$order));
        }
    ...
    /**
     * 云币支付
     * 如果云币足够就单独支付了该订单，如果不足就彻底扣除用户所有云币 //最后走退换货流程给返回
     */
    private function _jfPay($order_info, $input, $buyer_info) {
    	if ($order_info['order_state'] == ORDER_STATE_PAY) return;
    
    	$available_jf_amount = floatval($buyer_info['member_points']*C("points_trade"));
    	if ($available_jf_amount <= 0) return;
    	...
    	$data_points = array();
    	$data_points['pl_memberid'] = $buyer_info['member_id'];
    	$data_points['pl_membername'] = $buyer_info['member_name'];
    	...
    	if ($available_jf_amount >= $order_amount) {
    		//立即支付，订单支付完成
    		$points = $order_amount / C("points_trade");//计算回云币数 
    		$data_points['pl_points'] = -$points; //扣除应该为负数
    		$model_points->savePointsLog('order',$data_points,true);
    		$available_jf_amount -= $order_amount;
    		...
    		$data_order['payment_time'] = TIMESTAMP;
    		$data_order['payment_code'] = 'predeposit';
    		$order_info['points_amount'] = $data_order['points_amount'] = $order_amount;
    		$result = $model_vr_order->editOrder($data_order,array('order_id'=>$order_info['order_id']));
    		if (!$result) {
    			throw new Exception('订单更新失败');
    		}
    		...
    		//暂冻结积分,后面还需要 API彻底完成支付
    		//立即支付，扣除用户所有云币
    		$points = $available_jf_amount / C("points_trade");//计算回云币数
    		$data_points['pl_points'] = -$points; //扣除应该为负数
    		$model_points->savePointsLog('order',$data_points,true);
    		
    		//积分支付金额保存到订单
    		$data_order = array();
    		$order_info['points_amount'] = $data_order['points_amount'] = $available_jf_amount;
    		$result = $model_vr_order->editOrder($data_order,array('order_id'=>$order_info['order_id']));
    		if (!$result) {
    			throw new Exception('订单更新失败');
    		}
    ...
    private function _pdPay($order_info, $input, $buyer_info) {
        ...
        $model_pd = Model('predeposit');
	//总金额 需要减去前面两种支付掉的金额
        $order_amount = floatval($order_info['order_amount'])-floatval($order_info['rcb_amount'])-floatval($order_info['points_amount']);
        $data_pd = array();
        $data_pd['member_id'] = $buyer_info['member_id'];
        $data_pd['member_name'] = $buyer_info['member_name'];
        $data_pd['amount'] = $order_amount;
    ...
    /**
     * 虚拟商品不存在商品库的购买 步骤
     * @param array $post 接收POST数据，必须传入 实际支付金额pay_price（可能会涉及商家促销暂时不涉及）， store_id 店铺ID
     * @param int $member_id 会员ID
     * @return array
     */
    public function vrpayStep($post, $member_id) {
    	
    	$store_info = Model('store')->getStoreInfoByID($post['store_id']);
    	...
    	//应付总金额计算
    	$pay_total = $post['total_price'] * $quantity;
    	$store_id = $post['store_id'];
    	//需要支付支付
    	$input['jf_price'] = $jf_price = $post['jf_price'];
    	//整理数据
    	$goods_info = array();
    	$goods_info['store_id'] = $store_id;
    	$goods_info['store_name'] = $store_info['store_name'];
    	$goods_info['goods_id'] = 1;
    	$goods_info['goods_name'] = $store_info['store_name']."_实际消费_".$post['pay_price'];
    	$goods_info['goods_price'] = $post['pay_price'];
    	$goods_info['goods_image'] = '';
    	$goods_info['gc_id'] = 0;
    	$goods_info['virtual_indate'] = TIMESTAMP + 60; //有效期比当前支付时间多一分钟
    	$goods_info['vr_invalid_refund'] = 0; //过期后不允许退款
    	$goods_info['ifgroupbuy'] = 0; //是否促销 如果商家有促销需开启即可
    	$goods_info['goods_storage_alarm'] = 0; //库存预警
    	$goods_info['goods_storage'] = 999;//库存数
    	...
    	$input['pay_total'] = $pay_total;
    	$input['order_from'] = $post['order_from'];
    	if($pay_total==0.00||$pay_total==0||empty($pay_total)){
    		$post['payment_code'] = "predeposit";
    	}else{
    		if($post['payment_code'] == "predeposit"){
    			$post['payment_code'] = "wxpay";
    		}
    	}
    	$input['payment_code'] = $post['payment_code'];
    	$input['payment_method'] = $store_info['payment_method'];
    	...
    	try {
    		...
    		if(!empty($jf_price) && $jf_price>0){
    			$model_points = Model('points');
    			$data_points = array();
    			$data_points['pl_memberid'] = $member_info['member_id'];
    			$data_points['pl_membername'] = $member_info['member_name'];
    			$data_points['orderprice'] = floatval($order_info['order_amount']);
    			if(!empty($order_info['order_sn'])){
    				$data_points['order_sn'] = $order_info['order_sn'];
    			}
    			$data_points['pl_desc'] = '会员在'.$order_info['store_name'].'--'.$order_info['order_sn'].'使用了云币抵扣';
    			//立即支付，订单支付完成
    			$data_points['pl_points'] = -$jf_price; //扣除应该为负数
    			$model_points->savePointsLog('order',$data_points,true);
    		}
    		if (!empty($post['password'])) {
    			if ($member_info['member_paypwd'] != '' && $member_info['member_paypwd'] == md5($post['password'])) {
    				...
    				if (!empty($post['rcb_pay'])) {
    					$order_info = $this->_rcbPay($order_info, $post, $member_info);
    					//因为互斥的关系
    					$order_info['points_amount'] = 0.00;
    				}elseif(!empty($post['jf_pay'])){
    					//因为互斥的关系
    					$order_info['rcb_amount'] = 0.00;
    					$order_info = $this->_jfPay($order_info, $post, $member_info);
    				}
    				//积分支付
    				if (!empty($post['pd_pay'])) {
    					$this->_pdPay($order_info, $post, $member_info);
