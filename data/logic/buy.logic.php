...
class buyLogic {
    ...
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id) {
        ...
        //得到页面所需要数据：收货地址、发票、代金券、积分、商品列表等信息
    ...
    public function getCartList($cart_id, $member_id) {
        ...
        if (count($buy_items) > 99) {
            return callback(false, '一次最多只可购买99种商品');
        }
        ...
        //是否存在跨境淘商品
        if(!empty($cart_list[0]['is_ikjtao'])){
        	$store_other_total[0]['is_ikjtao'] = 1;
        }
        ...
        return callback(true, '', array('goods_list' => $goods_list, 'store_cart_list' => $store_cart_list,'store_other_total'=>$store_other_total));
    ...
    public function getGoodsList($cart_id, $member_id, $store_id) {
        ...
        $goods_list = array();
        //是否存在跨境淘商品
        if($goods_info['up_id']==C('ikjtao_store_id')){
        	$store_other_total[$goods_info['store_id']]['is_ikjtao'] = 1;
        }
    ...
    public function getBuyStep1Data($member_id, $data) {
        ...
        //定义返回数组
        $result = array();
        //商品金额计算(分别对每个商品/优惠套装小计、每个店铺小计,可用亲读币购买小计、可用云币抵扣小计、计算其他商品店铺的相关数据)
        list($store_cart_list,$store_goods_total,$store_goods_qctotal,$store_goods_pointtotal,$store_other_total) = $this->_logic_buy_1->calcCartList($store_cart_list);
        $result['store_cart_list'] = $store_cart_list;
        $result['store_goods_total'] = $store_goods_total;		
        $result['store_point_total'] = $store_goods_pointtotal; //使用云币
        $result['store_other_total'] = $store_other_total;//店铺利润
        ...
        //代金券改造后需要验证商品的范围
        $store_voucher_list = $this->_logic_buy_1->getStoreAvailableAppointVoucherList($store_cart_list,$store_goods_total,$store_other_total, $member_id);
        ...
        if (floatval($buyer_info['member_points']) > 0) {
        	...
        	$result['available_points'] = $buyer_info['member_points']*C("points_trade");
        }
    ...
    private function _createOrderStep3() {
        ...
        //商品金额计算(分别对每个商品/优惠套装小计、每个店铺小计,可用亲读币购买小计、可用云币抵消小计)
        list($store_cart_list,$store_goods_total,$store_goods_qctotal,$store_goods_pointtotal,$store_other_total) = $this->_logic_buy_1->calcCartList($store_cart_list);
        
		//使用云币抵扣 需要考虑云币是否充足
        //Vip 使用云币抵扣 start zhangc
        //$member_info = Model('member')->getMemberInfoByID($this->_member_info['member_id'],'mc_id,member_points,member_exppoints');
        //$store_final_goods_total = $this->_logic_buy_1->reCalcGoodsTotal($store_goods_total,$store_mansong_rule_list,'points');
        ...
        $this->_order_data['store_goods_pointtotal'] = $store_goods_pointtotal;
        $this->_order_data['store_final_qcbpay_total'] = $store_final_qcbpay_total;
        ...
        $this->_order_data['input_voucher_list'] = $input_voucher_list;
        $this->_order_data['store_other_total'] = $store_other_total;
    }
    ...
    private function _createOrderStep4() {
        ...
        $store_other_total = $this->_order_data['store_other_total'];
        $store_goods_pointtotal = $this->_order_data['store_goods_pointtotal'];
		...
		$pay_sn_total = 0;
		$payment_method  = 1;
		$store_payment_list = array();	
		foreach ($store_cart_list as $store_id => $goods_list) {
			$store_PM =$model_store->getStorePayment_MethodByID($store_id);
			$store_payment_list[$store_id]=$store_PM;
			if ($store_PM['pay_method']==0 || $store_PM['pay_method']==2){
				$payment_method  = 0;
			}
		}
		if ($payment_method  == 1){
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {    
			...
			if (OPEN_STORE_EXTENSION_STATE > 0){		
					$buyer_info	= Model('member')->getMemberInfoByID($member_id,'parent_id');
					if(!empty($buyer_info['parent_id'])){
						$extension_id = $buyer_info['parent_id'];
					}else{
						$extension_id = $member_id;
				}
				$Commisinfo=$this->_getOrderGoodsCommisTotals($goods_list,$extension_id,$store_id);
				//使用代金券后就不给提拥
				if(!empty($order_common['voucher_price'])){
					$Commisinfo['CommisTotals'] = 0.00;
				}
			}						
            ...
            //$order['baifen'] = $goods_list[0]['baifen'];
			...
			$order['pay_method']   = $store_payment_list[$store_id]['pay_method'];
			$order['pay_store_id'] = $store_payment_list[$store_id]['pay_store_id'];
			//推广处理
			if (OPEN_STORE_EXTENSION_STATE > 0){
			    ...
			    if (!empty($Commisinfo) && is_array($Commisinfo)){
				    $order['saleman_id'] =$Commisinfo['saleman_id'];
				    $order['saleman_name'] =$Commisinfo['saleman_name'];
						//$order['mb_commis_totals'] =$Commisinfo['CommisTotals'];
				}
			}
			//此处注意 该金额只是暂存数据库。真正使用云币在后面，确定付款的时候 在给予考虑 没有选择使用云币或账户的云币够不够 再给予扣除 。可使用云币抵扣的现金
			$order['usable_points'] = $store_goods_pointtotal[$store_id]['store_vip_points_total'];
			//返利积分数
			$order['rebate_amount'] = $store_other_total[$store_id]['store_rebate_amount'];
            ...
            //返利云币数
            $order_common['order_pointscount'] = $store_other_total[$store_id]['order_pointscount'];
            ...
            foreach ($goods_list as $goods_info) {
            	//<!-- 分销选入店铺 start zhang -->
            	//$order_goods[$i]['commis_rate'] = $goods_info['baifen'];
                ...
                if (!intval($goods_info['bl_id'])) {
                    ...
                    //<!-- 分销选入店铺 start zhang -->
                    //$order_goods[$i]['baifen'] = $goods_info['baifen'];
    ...
    private function _getOrderGoodsCommisTotals(&$goods_list,$saleman_id='',$store_id='') {		
		...
		//$where=array();
		//$where['member_id']=$saleman_id;
		//$where['mc_id']=array(array('eq',1),array('eq',2),'or');
		...
		if ($saleman_info['mc_id']==10){$goods_commis_id='saleman_cid';}		
		//提成比率库		
        ...
        foreach ($goods_list as $vk=>$goods_info) {
            if (!intval($goods_info['bl_id'])) {				
                ...
                //优惠套装
                foreach ($goods_info['bl_goods_list'] as $ik=>$bl_goods_info) {
					
                    $commis_id=$bl_goods_info[$goods_commis_id];					
                    if ($commis_id==0) continue;//商品佣金代码是否有效					
				    if (empty($commis_info[$commis_id]) || !is_array($commis_info[$commis_id])) continue;//商品佣金模板是否存在
				    ...
				    switch ($commis_class){
				      ...
				      case 1:  //固定佣金
					    ...
					    break;
				      case 2:  //售价佣金
					    $commis_value = ($bl_goods_info['bl_goods_price']*$commis_rate/100)*$goods_info['goods_num'];
					    break;
				      case 3:  //利润佣金
					    $commis_value = (($bl_goods_info['bl_goods_price']-$bl_goods_info['goods_tradeprice'])*$commis_rate/100)*$goods_info['goods_num'];
					    break;
				      default:  //无提成
				        $commis_value = 0;
					    break;
					...
					if ($commis_value<0) $commis_value=0;
				    $commis_totals += $commis_value;								
					$goods_list[$vk]['bl_goods_list'][$ik]['mb_commis_totals']=$commis_value;
                }				
            }						
        }		
    ...
    private function _createOrderStep5() {
        ...
        if (!empty($this->_post_data['rcb_pay'])) {
            $order_list = $this->_logic_buy_1->rcbPay($order_list ? $order_list :$this->_order_data['order_list'], $this->_post_data, $buyer_info);
        }
        //使用充值卡支付
        if (!empty($this->_post_data['jf_pay'])) {
        	$order_list = $this->_logic_buy_1->jfPay($order_list ? $order_list :$this->_order_data['order_list'], $this->_post_data, $buyer_info);
        }
        //使用积分支付
    ...
    private function _getGoodsList($cart_list) {
        ...
        foreach ($cart_list as $key => $cart) {
            ...
            if (!intval($cart['bl_id'])) {
                ...
                $goods_list[$i]['vip_price'] = $cart['vip_price'];
                $goods_list[$i]['vip_points'] = $cart['vip_points'];
                $goods_list[$i]['rebate_price'] = $cart['rebate_price'];//返利积分 zhangc
                $goods_list[$i]['rebate_points'] = $cart['rebate_points'];//返利云币数 zhangc
                $goods_list[$i]['goods_tradeprice'] = $cart['goods_tradeprice'];
                ...
                foreach ($cart['bl_goods_list'] as $bl_goods) {
                    ...
                    //优惠商品不参与优惠 zhangc
                    $goods_list[$i]['vip_price'] = $bl_goods['goods_price'];
                	$goods_list[$i]['vip_points'] = 0;
                	$goods_list[$i]['rebate_price'] = 0.00;
                	$goods_list[$i]['rebate_points'] = 0;
                    $goods_list[$i]['goods_tradeprice'] = $bl_goods['goods_tradeprice'];
