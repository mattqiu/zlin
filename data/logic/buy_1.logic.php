...
class buy_1Logic {
    ...
    public function calcCartList($store_cart_list) {
		...
		//存放每个店铺的可用亲读币支付总额
        $store_goods_qctotal = array();
		//存放每个店铺的可用云币支付总额
        $store_goods_pointtotal = array();
        //存放需要增的支付相关数据 zhangc
        $store_other_total = array();
        //存放本次下单所有店铺商品总金额
        $order_goods_total = 0;
        $is_ikjtao = 0;
        //Vip 使用云币抵扣  --获取会员的云币数和 会员等级 start zhangc
        $model_member = Model('member');
        $member_info = $model_member->getMemberInfoByID($_SESSION['member_id'],'mc_id,member_points,member_exppoints');
        $typeBy_id = $member_info['mc_id']; //会员身份：推广员、导购
        $member_points = $member_info['member_points']; //会员云币
        /**
         * 购物车分三级层：商品(多数量)、店铺(多个商品)、订单(多家店铺)
         */
        foreach ($store_cart_list as $store_id => $store_cart) {
        	//商品层
        	$vip_price_total = 0; //VIP商品层面的 价之和
        	$vip_points_total = 0; //VIP商品价可使用云币之和，因为云币是取整
        	$price_total = 0;	//商品会员价之和
        	$rebate_price_total = 0; //返利积分
        	$rebate_points_total = 0; //返利云币数 
        	
        	//店铺层
        	$store_goods_amount = 0;  //计算店铺的商品总额
			$store_qcamount = 0;	//店铺亲城币
			$store_points_amount = 0;//云币总额
			$store_vip_points_total = 0;//可使用的云币数
			$store_goods_gain_amount = 0; //店铺商品的利润总和  活动可能使用
			$store_commis_amount = 0;
			$store_commis_points = 0;
			$order_pointscount = 0; //返利云币总数
			//商品公共ID组
			$store_goods_comid = array();
            foreach ($store_cart as $key => $cart_info) {
            	$member_id = !empty($cart_info['buyer_id'])?$cart_info['buyer_id']:$_SESSION['member_id'];            	
            	$price_total = imPriceFormat($cart_info['goods_price'] * $cart_info['goods_num']);
            	//商品的批发价 用于计算商家利润 =批发价-成本价 优先商品和券等相关活动可能限制使用
            	$store_cart[$key]['goods_tradeprice'] = imPriceFormat($cart_info['goods_tradeprice']);
            	//显示VIP价 start zhangc
            	$store_cart[$key]['vip_price'] = $vip_price = $cart_info['vip_price'];
            	$store_cart[$key]['vip_points'] = $vip_points = $cart_info['vip_points']; //应付云币取整
            		$vip_price_total = $vip_price * $cart_info['goods_num']; //计算商品金额总和
            	$vip_points_total 	= $vip_points * $cart_info['goods_num'];//应付云币总数        	
            	$rebate_price_total 	= $cart_info['rebate_price']*$cart_info['goods_num'];//返利积分
            	$rebate_points_total 	= $cart_info['rebate_points']*$cart_info['goods_num'];//返利云币
            	$goods_gain_total 	= imPriceFormat($cart_info['goods_tradeprice'] - $cart_info['goods_costprice']);//商品利润
            	//单商品总价
            	$store_cart[$key]['goods_total'] = $vip_price_total;
            	$store_cart[$key]['rebate_amount'] = $rebate_price_total; //商品返利积分总数
            	$store_cart[$key]['order_pointscount'] = $rebate_points_total; //商品返利云币总数            	
                $store_cart[$key]['goods_image_url'] = cthumb($store_cart[$key]['goods_image']);
                
				$store_goods_amount 	+= $vip_price_total;	//店铺商品总和
                $order_pointscount 		+= $rebate_points_total; //店铺返利云币总数
                $store_rebate_price_total += $rebate_price_total;//店铺返利积分总数
                $store_vip_points_total += $vip_points_total; //店铺可用云币数
				$store_goods_gain_amount += $goods_gain_total;  //店铺商品利润总和
				$store_cart[$key]['commis_price'] = $cart_info['commis_price'];
				$store_cart[$key]['commis_points'] = $cart_info['commis_points']; //云币取整
				$store_commis_amount 	+= $cart_info['commis_price']*$cart_info['goods_num'];	//返佣积分
				$store_commis_points 	+= $cart_info['commis_points']*$cart_info['goods_num'];//返佣云币
				$store_cart[$key]['mb_commis_totals'] = $store_commis_amount;//推广佣金
				//临时使用，创建公共ID商品库，检测某个商品是否存在
				$store_goods_comid[] = $cart_info['goods_commonid'];	
				//是否存在跨境淘商品
				if($cart_info['up_id']==C('ikjtao_store_id')){
					$is_ikjtao = 1;
				}
            }
            
            $store_cart_list[$store_id] = $store_cart;
            $store_goods_total[$store_id] = $store_goods_amount;
			$store_goods_qctotal[$store_id] = $store_qcamount;
			$store_goods_pointtotal[$store_id]['store_vip_points_total'] = $store_vip_points_total;
			$store_other_total[$store_id]['store_goods_gaintotal'] = $store_goods_gain_amount;
			$store_other_total[$store_id]['store_rebate_amount'] = $store_rebate_price_total;//店铺返利积分
			$store_other_total[$store_id]['order_pointscount'] = $order_pointscount;//店铺返利云币
			$store_other_total[$store_id]['store_commis_amount'] = $store_commis_amount;//店铺返利积分
			$store_other_total[$store_id]['store_commis_points'] = $store_commis_points;//店铺返利云币
			$store_other_total[$store_id]['goods_commonid'] = $store_goods_comid;
			$store_other_total[$store_id]['is_ikjtao'] = $is_ikjtao;
        }
        return array($store_cart_list,$store_goods_total,$store_goods_qctotal,$store_goods_pointtotal,$store_other_total);
    }
	...
	public function getMansongRuleCartListByTotal($store_goods_total) {
        ...
        foreach ($store_goods_total as $store_id => $goods_total) {
            ...
            if (is_array($rule_info) && !empty($rule_info)) {
                ...
                $rule_info['discount'] = imPriceFormat($rule_info['discount']);
	 ...
	 * @return array
	 */
	public function getStoreAvailableAppointVoucherList($store_cart_list,$store_goods_total,$store_other_total, $member_id) {
		if (!C('voucher_allow')) return $store_cart_list;
		$voucher_list = array();
		$model_voucher = Model('voucher');
		foreach ($store_cart_list as $store_id => $goods_list) {
			...
			$condition['voucher_store_id'] = $store_id;
			$condition['voucher_owner_id'] = $member_id;
			$goods_total = $store_goods_total[$store_id];
			$goods_gaintotal = $store_other_total[$store_id];
			$voucher_list[$store_id] = $model_voucher->getCurrentAvailableVoucherTemplate($condition,$goods_total,$goods_gaintotal);
		}
		return $voucher_list;
	}
	/**
    ...
    public function getXianshiInfo( & $goods_info, $quantity) {
        ...
        $goods_info['xianshi_info']['down_price'] = imPriceFormat($goods_info['goods_price'] - $goods_info['xianshi_info']['xianshi_price']);
    ...
    public function getOfflineGoodsPay($buy_list) {
        ...
        $offline_pay = Model('payment')->getPaymentOpenInfo(array('payment_code'=>'offline'));//支付方式_待修改
        if ($offline_pay) {
    ...
    public function rcbPay($order_list, $input, $buyer_info) {
        ...
        foreach ($order_list as $key => $order_info) {
			...
			//这里订单总金额 = 订单总金额 - 已经使用积分余额抵扣的金额 //由于互斥所以就再次处不减了 应该还需要（ + 可用云币的金额 - 使用云币抵扣的金额）
            $order_amount = floatval($order_info['order_amount']) - floatval($order_info['pd_amount']);
            $data_pd = array();
            ...
            $data_order = array();
            //因为互斥 所以云币抵扣为0
            $order_list[$key]['points_amount'] = $data_order['points_amount'] = 0;
     ...
     * 云币支付,依次循环每个订单
     * 因为云币抵扣不是100%，只允许部分抵扣
     * 所以 如果云币足够 部分抵扣 就单独支付了该订单，
     */
    public function jfPay($order_list, $input, $buyer_info) {
    	$member_id = $buyer_info['member_id'];
    	$member_name = $buyer_info['member_name'];
    	//当前账户云币余额
    	$available_points = $buyer_info['member_points'];
    	if ($available_points <= 0) return;
    
    	$model_order = Model('order');
    	$model_points = Model('points');
    	$do_points = 0; //扣除一单后下一单 还剩多少云币
    	foreach ($order_list as $order_info) {
    		
    		//货到付款的订单、已经支付的订单跳过
    		if ($order_info['payment_code'] == 'offline') continue;
    		if ($order_info['order_state'] == ORDER_STATE_PAY) continue;
    		$order_amount = floatval($order_info['order_amount']) - floatval($order_info['pd_amount']);//因为互斥 所以不需要扣除充值卡抵扣的金额 - floatval($order_info['rcb_amount']);
    		
    		//云币支付的时候需要注意，因为不能接扣除订单的全部金额，只可部分付款
    		//所以 要根据订单中暂存的 可用金额
    		$usable_points_amount = $order_info['usable_points'];//不取整便是实际可抵扣的金额
    		$usable_points_total = ceil($order_info['usable_points']/C("points_trade")); //需要抵扣的云币数

    		$data_points = array();
    		$data_points['pl_memberid'] = $member_id;
    		$data_points['pl_membername'] = $member_name;
    		$data_points['orderprice'] = $order_amount;
    		$data_points['order_sn'] = $order_info['order_sn'];
    		$data_points['pl_desc'] = '会员在'.$order_info['store_name'].'--'.$order_info['order_sn'].'使用了云币抵扣';
    		//此时需要修改订单的云币抵扣额度
    		$data_order = array();
    		//因为互斥 所以充值卡抵扣为0
    		$order_list[$key]['rcb_amount'] = $data_order['rcb_amount'] = 0;
    		//用户剩余的云币 是否大于 
    		if ($available_points >= $usable_points_total) {
    			
    			//立即支付，订单支付还未完成
    			$data_points['pl_points'] = -$usable_points_total; //扣除应该为负数
    			$model_points->savePointsLog('order',$data_points,true);
    			
    			//订单金额为0时，只需要支付云币即可支付成功
    			if($order_amount==0){
    				$order_list[$key]['order_state'] = $data_order['order_state'] = ORDER_STATE_PAY;
    				$data_order['payment_time'] = TIMESTAMP;
    				$data_order['payment_code'] = 'predeposit';
    				$order_list[$key]['points_amount'] = $data_order['points_amount'] = $order_amount;
    			}
    			$order_list[$key]['points_amount'] = $data_order['points_amount'] = $usable_points_amount;
    			
    			$result = $model_order->editOrder($data_order,array('order_id'=>$order_info['order_id']));
    			if (!$result) {
    				throw new Exception('订单更新失败');
    			}
    			$available_points -= $usable_points_total; //扣除该单后 下单剩余云币数
    			// 发送商家提醒 云币抵扣部分
    		} else {
    			//暂冻结积分,后面还需要 API彻底完成支付
    			if ($available_points > 1) {
    				//立即支付，订单支付还未完成
    				//由于客户实际账户里的云币余额不足，所以需要将会员账户剩余的云币余额都给扣除
    				$data_points['pl_points'] = -$available_points; //扣除应该为负数
    				$model_points->savePointsLog('order',$data_points,true);
    				
    				$order_list[$key]['points_amount'] = $data_order['points_amount'] = $available_points * C("points_trade");//实际抵扣的金额，云币转换金额
    				$data_order['rcb_amount'] = 0; //因为互斥所以充值卡为0
    				$result = $model_order->editOrder($data_order,array('order_id'=>$order_info['order_id']));
    				if (!$result) {
    					throw new Exception('订单更新失败');
    				}
    				$available_points = 0; //扣除该单后 下单剩余云币数为0
    			}
    		}
    	}
    	return $order_list;
    ...
    /**
     * 积分支付,依次循环每个订单
     * 如果积分足够就单独支付了该订单，如果不足就暂时冻结，等API支付成功了再彻底扣除
     */
    public function pdPay($order_list, $input, $buyer_info) {
        ...
        foreach ($order_list as $order_info) {
            ...
            if ($order_info['payment_code'] == 'offline') continue;
            if ($order_info['order_state'] == ORDER_STATE_PAY) continue;
            //需实际支付订单的金额 = 订单总额 - 充值卡支付 + （订单可用云币抵扣金额-实际云币抵扣的金额）
			$order_amount = floatval($order_info['order_amount']) - floatval($order_info['rcb_amount']) + (floatval($order_info['usable_points']) - floatval($order_info['points_amount']));
            $data_pd = array();
            $data_pd['member_id'] = $member_id;
            $data_pd['member_name'] = $member_name;
            $data_pd['amount'] = $order_amount;
...
//                 }
    ...
    public function getReciverAddr($address_info = array()) {
        if (intval($address_info['dlyp_id'])) {
        	...
        	$reciver_info['address_id'] = $address_info['address_id'];
            ...
            $reciver_info['member_idcard'] = $address_info['member_idcard'];//身份证
			//添加省级和市级代码
			if ($address_info['city_id']>0){
				$city_info = Model('area')->getAreaInfo(array('area_id'=>$address_info['city_id']),'area_parent_id,area_code');
				$reciver_info['city_code'] = $city_info['area_code'];
				if ($city_info['area_parent_id']>0){
					$province_info = Model('area')->getAreaInfo(array('area_id'=>$city_info['area_parent_id']),'area_code');
					$reciver_info['province_code'] = $province_info['area_code'];
				}
    ...
    public function extendInPay($order_list) {
        ...
        if ($order_list[0]['order_type'] == 2) {
            ...
            }else if(!empty($order_info['points_amount'])) {
                $data['book_points_amount'] = $order_info['points_amount'];
            }
            ...
            //如果未使用站内余额，返回
            if (empty($order_info['rcb_amount']) && empty($order_info['points_amount']) &&  empty($order_info['pd_amount'])) {
	...
	//<!-- 分销选入店铺 start zhang -->
		$new_array['up_id'] = $goods_info['up_id'];
	//$new_array['up_name'] = $goods_info['up_name'];
	//$new_array['baifen'] = $goods_info['baifen'];
		//计算出会员信息 start zhangc
		$vipInfo = Logic('goods')->goodsVip($goods_id,$_SESSION['member_id'],$goods_info);
		$new_array['goods_price'] = $goods_info['goods_price'];
		$new_array['vip_price'] = $vipInfo['vip_price'];
		$new_array['vip_points'] = $vipInfo['vip_points'];//
		$new_array['rebate_price'] = $vipInfo['rebate_price'];//返利积分数
		$new_array['rebate_points'] = $vipInfo['rebate_points'];//返利云币数 zhangc
		$new_array['commis_price'] = $vipInfo['commis_price'];//返拥积分数
		$new_array['commis_points'] = $vipInfo['commis_points'];//返拥云币数 zhangc
		//$new_array['give_points'] = $goods_info['give_points'];//返利云币数 无需从商品中获取
        $new_array['goods_marketprice'] = $goods_info['goods_marketprice'];
		$new_array['goods_tradeprice'] = $goods_info['goods_tradeprice'];
		...
		if (OPEN_STORE_EXTENSION_STATE > 0){
		    $new_array['promotion_cid'] = $goods_info['promotion_cid'];
			$new_array['promotion_amount'] = $goods_info['promotion_amount'];
		    $new_array['saleman_cid'] = $goods_info['saleman_cid'];
			$new_array['saleman_amount'] = $goods_info['saleman_amount'];
		}
    ...
    private function _getOnlineCartList($cart_list) {
        ...
        foreach ((array)$cart_list as $key => $cart_info) {
            ...
            if (in_array($cart_info['goods_id'],array_keys($goods_online_array))) {
				...
				//计算出会员信息 start zhangc
				$vipInfo = Logic('goods')->goodsVip($cart_info['goods_id'],$_SESSION['member_id']);
				$cart_list[$key]['goods_price'] = $vipInfo['vip_price'];
                $cart_list[$key]['vip_price'] = $vipInfo['vip_price'];
                $cart_list[$key]['vip_points'] = $vipInfo['vip_points'];
                $cart_list[$key]['rebate_price'] = $vipInfo['rebate_price'];//返利积分数 
                $cart_list[$key]['rebate_points'] = $vipInfo['rebate_points'];//返利云币数 zhangc
                $cart_list[$key]['up_id'] = $goods_online_info['up_id'];
                //是否有跨境淘商品
                if($goods_online_info['up_id'] == C('ikjtao_store_id')||$goods_online_info['store_id'] == C('ikjtao_store_id')){
                	$is_ikjtao = 1;
                }
                $cart_list[$key]['goods_tradeprice'] = $goods_online_info['goods_tradeprice'];
				//预定信息
				...
				//推广处理
				if (OPEN_STORE_EXTENSION_STATE > 0){
				    $cart_list[$key]['promotion_cid'] = $goods_online_info['promotion_cid'];
					$cart_list[$key]['promotion_amount'] = $goods_online_info['promotion_amount'];
				    $cart_list[$key]['saleman_cid'] = $goods_online_info['saleman_cid'];
					$cart_list[$key]['saleman_amount'] = $goods_online_info['saleman_amount'];
				}
            } else {
                //如果商品下架
        ...
        $cart_list[0]['is_ikjtao'] = $is_ikjtao;
        return $cart_list;
    }
        ...
        foreach ($cart_list as $key => $cart_info) {
            ...
            foreach ($cart_list[$key]['bl_goods_list'] as $k => $goods_info) {
                if (array_key_exists($goods_info['goods_id'],$goods_online_list)) {
					...
					$cart_list[$key]['bl_goods_list'][$k]['goods_costprice'] = $goods_online_info['goods_costprice'];
                    ...
                    //每个商品直降多少
                    $total_down_price += $cart_list[$key]['bl_goods_list'][$k]['down_price'] = imPriceFormat($goods_online_info['goods_price'] - $goods_info['bl_goods_price']);
                } else {
                    //商品已经下架
                    $cart_list[$key]['state'] = false;
                    $cart_list[$key]['storage_state'] = false;
            ...
            $cart_list[$key]['down_price'] = imPriceFormat($total_down_price);
        }
    }
    ...
    private function _parseMansongRuleDesc($rule_info) {
        ...
        if (!empty($rule_info['mansong_goods_name']) && !empty($rule_info['goods_storage'])) {
            if (APP_ID == 'mobile') {
                ...
                $goods_desc .= '<a class="imcr-store-gift" title="'.$rule_info['mansong_goods_name'].'" target="_blank" href="'.urlShop('goods','index',array('goods_id'=>$rule_info['goods_id'])).'"><img alt="'.$rule_info['mansong_goods_name'].'" src="'.cthumb($rule_info['goods_image'], 60).'"></a>';
            }
        } else {
            $goods_desc = '';
        }
        return sprintf('满%s%s',$rule_info['price'],$discount_desc);
    }
