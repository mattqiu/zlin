...
class cartControl extends BaseBuyControl {
	...
	public function __construct() {
        ...
        if (!in_array($op,$op_arr) && !$_REQUEST['member_id'] ){
	...
	public function indexOp() {
        ...
        //购物车列表
        $cart_list	= $model_cart->listCart('db',array('buyer_id'=>$_REQUEST['member_id']));
    ...
    public function ajax_loadOp() {
        ...
        if ($_REQUEST['member_id']){
            ...
            if(!empty($cart_list)){
                foreach ($cart_list as $k => $cart){
                    ...
                    $cart_array['list'][$k]['goods_span'] = $cart['goods_span'];
            ...
            foreach ($cart_list as $key => $cart){
                ...
                $value['goods_span'] = $cart['goods_span'];
                $value['goods_image'] = thumb($cart,60);
                $value['goods_url'] = urlShop('goods', 'index', array('goods_id' => $cart['goods_id']));
                $cart_array['list'][] = $value;
            }
        }
		setIMCookie('cart_goods_num',$model_cart->cart_goods_num,2*3600);
		$cart_array['cart_all_price'] = imPriceFormat($model_cart->cart_all_price);
        ...
        if ($_REQUEST['type'] == 'html') {
            ...
            if (isset($_REQUEST['callback'])) {
                $json_data = $_REQUEST['callback']=='?' ? '('.$json_data.')' : $_REQUEST['callback']."($json_data);";
            }
            exit($json_data);
        }
	...
	public function addOp() {
        ...
        if (is_numeric($_REQUEST['goods_id'])) {
	    	...
	    	//<!-- 分销选入店铺 start zhang -->
		$goods_info['up_id']	= $goods_list[0]['up_id'];
		$goods_info['up_name']	= $goods_list[0]['up_name'];
	    	//$goods_info['baifen']	= $goods_list[0]['baifen'];
        ...
        if($_REQUEST['member_id']) {
            $save_type = 'db';
            $goods_info['buyer_id'] = $_REQUEST['member_id'];
        } 
        //else {
        //    $save_type = 'cookie';
        //}
        ...
        if ($insert) {
            //购物车商品种数记入cookie
            setIMCookie('cart_goods_num',$model_cart->cart_goods_num,2*3600);
            $data = array('state'=>'true', 'num' => $model_cart->cart_goods_num, 'amount' => imPriceFormat($model_cart->cart_all_price));
        } else {
	...
	public function updateOp() {
		...
		$cart_info = $model_cart->getCartInfo(array('cart_id'=>$cart_id,'buyer_id'=>$_REQUEST['member_id']));
		if ($cart_info['bl_id'] == '0') {
		    ...
		    if(empty($goods_info)) {
		        ...
		        $return['msg'] = '商品已被下架';
		        $return['subtotal'] = 0;
		        QueueClient::push('delCart', array('buyer_id'=>$_REQUEST['member_id'],'cart_ids'=>array($cart_id)));
		    ...
		    if(intval($goods_info['goods_storage']) < $quantity) {
		        ...
		        $model_cart->editCart(array('goods_num'=>$goods_info['goods_storage']),array('cart_id'=>$cart_id,'buyer_id'=>$_REQUEST['member_id']));
		        exit(json_encode($return));
		    }
		} else {
        ...
        $update = $model_cart->editCart($data,array('cart_id'=>$cart_id,'buyer_id'=>$_REQUEST['member_id']));
		if ($update) {
		    $return = array();
			$return['state'] = 'true';
			$return['subtotal'] = $goods_info['goods_price'] * $quantity;
	...
	public function delOp() {
		...
		if ($_SESSION['member_id']) {
		    //登录状态下删除数据库内容
			$delete	= $model_cart->delCart('db',array('cart_id'=>$cart_id,'buyer_id'=>$_REQUEST['member_id']));
			if($delete) {
			    $data['state'] = 'true';
