...
class alipay{
    ...
    private $parameter;
    /**
     * 订单类型 product_buy商品购买,predeposit积分充值
     * @var unknown
     */   
    private $order_type;
    ...
    public function __construct($payment_info = array(),$order_info = array()){
    	...
    	if(!empty($payment_info) and !empty($order_info)){
			...
			$this->pay_server  = 'create_direct_pay_by_user';//默认使用即时到帐交易接口
			switch ($this->payment['payment_config']['alipay_server']){
              case '0':
                $this->pay_server = 'trade_create_by_buyer'; //使用标准双接口
                break;
              case '1':
                $this->pay_server = 'create_partner_trade_by_buyer'; //使用担保交易接口
                break;
              case '2':
                $this->pay_server = 'create_direct_pay_by_user'; //使用即时到帐交易接口
                break;
            }
    	}
    }
    ...
    public function get_payurl(){
    	...
    	$this->parameter = array(
			...
			"show_url"	        => SHOP_SITE_URL.'/index.php?act=member_order&op=show_order&order_id='.$this->order_id,	//订单详情
			'receive_name'		=> $this->receive['receive_name'],//收货人姓名
            'receive_address'	=> $this->receive['receive_address'],	//收货人地址
            'receive_zip'		=> $this->receive['receive_zip'],	//收货人邮编
            'receive_phone'		=> $this->receive['receive_phone'],//收货人电话
            'receive_mobile'	=> $this->receive['receive_mobile'],//收货人手机
			'_input_charset'	=> CHARSET,					//网站编码 
            'sign_type'		    => 'MD5',				//签名方式
            'extra_common_param'=> $this->order['order_type'],//product_buy商品购买,predeposit积分充
