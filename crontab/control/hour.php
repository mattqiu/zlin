...
class hourControl extends BaseCronControl {
    ...
    public function xs_createOp() {
        ...
        try {
            ...
            echo 'Total:'.$count."\n";
            $fields = "*,CONCAT(goods_commonid,',',color_id) as im_distinct";
            for ($i = 0; $i <= $count; $i = $i + $step_num){
                $goods_list = $model_goods->getGoodsOnlineList(array(), $fields, 0, '', "{$i},{$step_num}", 'im_distinct');
    ...
    public function _xs_update() {
        ...
        try {
            ...
            $fields = "*,CONCAT(goods_commonid,',',color_id) as im_distinct";
            for ($i = 0; $i <= $count; $i = $i + $step_num){
                $goods_list = $model_goods->getGoodsOnlineList($condition, $fields, 0, '', "{$i},{$step_num}", 'im_distinct');
                $this->_build_goods($goods_list);
            }
            if ($count > 0) {
                sleep(2);
