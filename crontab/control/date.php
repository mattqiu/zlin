...
include(BASE_CORE_PATH.'/lwo.php');
if (!@include(BASE_CORE_PATH.'/lwo.php')) exit('lwo.php isn\'t exists!');
...
class dateControl extends BaseCronControl {
    ...
    private function _add_points() {
        ...
        //24小时之内登录的会员送云币和经验值,每次最多处理5W个会员
       ...
       if (C('points_isuse')) {
        ...
        //24小时之内完成了实物订单送云币和经验值,每次最多处理5W个订单
        $model_order = Model('order');
        $condition = array();
