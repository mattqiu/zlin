...
class monthControl extends BaseCronControl {
    ...
    private function _create_real_order_bill($data){
        ...
        for ($i=0;$i<=$store_count;$i=$i+300){
            //取店铺表列表(因为可能存在无订单，但有店铺活动费用，所以不再从订单表取店铺数量)
    ...
    private function _calc_real_order_bill($data_bill){
        ...
        //本期应结                                订单金额                       退款总额                        佣金金额                         退款佣金                           店铺活动费用
        $update['ob_result_totals'] = $update['ob_order_totals'] - $update['ob_order_return_totals'] - $update['ob_commis_totals'] + $update['ob_commis_return_totals']- $update['ob_store_cost_totals'];
