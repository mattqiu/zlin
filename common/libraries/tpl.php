...
class Tpl{
	...
	public static function showpage($page_name='',$layout='',$time=2000){
		...
		self::getInstance();
			...
			$layout_file = BASE_PATH.'/skins/'.TPL_NAME.DS.$layout;
			if(empty(self::$tpl_quna)){
				$tpl_file = BASE_PATH.'/skins/'.TPL_NAME.DS.$tpl_dir.$page_name.'.php';
			}else{
				$tpl_file = BASE_PATH.'/modules/'.self::$tpl_quna.'/skins/'.TPL_NAME.DS.$tpl_dir.$page_name.'.php';
			}
			...
			if (file_exists($tpl_file)){				
				...
				if ($layout != ''){			
					if (file_exists($layout_file)){
						...
						$error = 'Tpl ERROR:'.'skins'.DS.$layout.' is not exists';
				...
				$error = 'Tpl ERROR:'.'skins'.DS.$tpl_dir.$page_name.'.php'.' is not exists';
				throw_exception($error);
			}
	}
	/**
    ...
    public static function showTrace(){
    	$trace = array();
    	//当前页面
		$trace[Language::get('im_debug_current_page')] =  $_SERVER['REQUEST_URI'].'<br>';
    	//请求时间
        $trace[Language::get('im_debug_request_time')] =  date('Y-m-d H:i:s',$_SERVER['REQUEST_TIME']).'<br>';
        //系统运行时间
        $query_time = number_format((microtime(true)-StartTime),3).'s';
        $trace[Language::get('im_debug_execution_time')] = $query_time.'<br>';
		//内存
		$trace[Language::get('im_debug_memory_consumption')] = number_format(memory_get_usage()/1024/1024,2).'MB'.'<br>';
		//请求方法
        $trace[Language::get('im_debug_request_method')] = $_SERVER['REQUEST_METHOD'].'<br>';
        //通信协议
        $trace[Language::get('im_debug_communication_protocol')] = $_SERVER['SERVER_PROTOCOL'].'<br>';
        //用户代理
        $trace[Language::get('im_debug_user_agent')] = $_SERVER['HTTP_USER_AGENT'].'<br>';
        //会话ID
        $trace[Language::get('im_debug_session_id')] = session_id().'<br>';
        //执行日志
        $log    =   Log::read();
        $trace[Language::get('im_debug_logging')]  = count($log)?count($log).Language::get('im_debug_logging_1').'<br/>'.implode('<br/>',$log):Language::get('im_debug_logging_2');
        $trace[Language::get('im_debug_logging')] = $trace[Language::get('im_debug_logging')].'<br>';
        //文件加载
		$files =  get_included_files();
		$trace[Language::get('im_debug_load_files')] = count($files).str_replace("\n",'<br/>',substr(substr(print_r($files,true),7),0,-2)).'<br>';
        return $trace;
    }
