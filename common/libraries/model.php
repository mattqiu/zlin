...
class Model{
    ...
    public function __call($method,$args) {
		if(in_array(strtolower($method),array('table','order','where','on','limit','having','group','lock','master','distinct','index','attr','key'),true)) {
            ...
            return $this;
		}elseif(in_array(strtolower($method),array('limit'),true)){
			$this->options[strtolower($method)] =   $args[0];

			if(is_numeric($args[1]) && $args[1] >= 0){	
			  $this->options[strtolower($method)] =   $args[0].','.$args[1];	
			}
        	return $this;
			...
			pagecmd('setEachNum',	$args[0]);
			$this->unoptions = false;
			pagecmd('setTotalNum',	$this->get_field('COUNT(*) AS im_count'));
        	return $this;
        }elseif(in_array(strtolower($method),array('min','max','count','sum','avg'),true)){
            $field =  isset($args[0])?$args[0]:'*';
            return $this->get_field(strtoupper($method).'('.$field.') AS im_'.$method);
        }elseif(strtolower($method)=='count1'){
            $field =  isset($args[0])?$args[0]:'*';
            $options['field'] = ('count('.$field.') AS im_count');
...
//            if(isset($pkValue)) $data[$pk]   =  $pkValue;
    ...
    public function add($data=array(), $options=array()) {
		...
		if(is_array($data)){
		  $this->_auto_insert_data($data,$options);
		}
		$result = $this->insert($data,false,$options);
        return $result;
    }
	/**
	 * 插入默认值数据
