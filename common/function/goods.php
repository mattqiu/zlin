...
function thumb($goods = array(), $type = ''){
	if (strpos($goods['goods_image'],'http://')!==false){
...
function cthumb($file, $type = '', $store_id = false) {
    ...
    $fname = basename($file);
    /* 因为有分销的商品 图片取得是供应商的 所以这个直接传店铺ID是不可取
    // 取店铺ID 
...
function orderPaymentName($payment_code) {
    return str_replace(
            array('offline','online','alipay','tenpay','chinabank','predeposit','weixin','wxpay','kuaiqian','llpay','xjpay','skpay'),
            array('货到付款','在线付款','支付宝','财付通','网银在线','余额支付','微信支付','微信支付','快钱支付','连连支付','现金支付','刷卡支付'),
            $payment_code);
}
...
/**----------------------------------------微信部分----------------------------------------
 * 取得素材缩略图的完整URL路径，接收商品信息数组，返回所需的素材缩略图的完整URL
 *
 * @param array $goods 商品信息数组
 * @param string $type 缩略图类型  值为60,160,240,310,1280
 * @return string
 */
function Mthumb($goods = array(), $type = '', $fullpath = false){
    $type_array = explode(',_', ltrim(GOODS_IMAGES_EXT, '_'));
    if (!in_array($type, $type_array)) {
        $type = '240';
    }
	...
	if ($fullpath==false){
	  if ($goods['mat_type']==2){
		return UPLOAD_SITE_URL.DS.ATTACH_MATERIAL.DS.'images/flash.png';
	  }elseif($goods['mat_type']==3){
		return UPLOAD_SITE_URL.DS.ATTACH_MATERIAL.DS.'images/music.png';
	  }elseif($goods['mat_type']==4){
		return UPLOAD_SITE_URL.DS.ATTACH_MATERIAL.DS.'images/movie.png';
	  }
	}else{
	  $type ='';
	}
	...
	if (strpos($goods['goods_image'],'/')!==false){
		return $file;
	}
    if (empty($goods['goods_image'])) {
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage($type);
    ...
    $file = $type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file);
    if (!file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_MATERIAL.'/'.$member_id.'/'.$file)){
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage($type);
    }
    $thumb_host = UPLOAD_SITE_URL.'/'.ATTACH_MATERIAL;
    return $thumb_host.'/'.$member_id.'/'.$file;

}
/**
 * 取得素材缩略图的完整URL路径，接收图片名称与会员ID
 *
 * @param string $file 图片名称
 * @param string $type 缩略图尺寸类型，值为60,160,240,310,1280
 * @param mixed $member_id 店铺ID 如果传入，则返回图片完整URL,如果为假，返回系统默认图
 * @return string
 */
function MCthumb($file, $type = '', $member_id = false,$mat_type = 1, $fullpath = false) {
    ...
    $fname = basename($file);
    // 取店铺ID
    if ($member_id === false || !is_numeric($member_id)) {
        $member_id = substr ( $fname, 0, strpos ( $fname, '_' ) );
    }
    // 本地存储时，增加判断文件是否存在，用默认图代替
    if ( !file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_MATERIAL . '/' . $member_id . '/' . ($type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file)) )) {
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage($type);
    }
    $thumb_host = UPLOAD_SITE_URL . '/' . ATTACH_MATERIAL;
    return $thumb_host . '/' . $member_id . '/' . ($type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file));
}
?>
