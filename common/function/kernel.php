...
//手机号码验证
function CheckMobileValidator($phone){
 ...
 * 字符串部分用*号代替
 */
function replaceStrForStar($str,$Length=3){
 ...
 * 产生验证码
 *
 * @param string $imhash 哈希数
 * @return string
 */
function makeSeccode($imhash){
	$seccode = random(6, 1);
	$seccodeunits = '';

	$s = sprintf('%04s', base_convert($seccode, 10, 23));//10进制转23进制 
	...
	setIMCookie('seccode'.$imhash, encrypt(strtoupper($seccode)."\t".(time())."\t".$imhash,MD5_KEY),3600);
	return $seccode;
}
 ...
 * @return boolean
 */
function checkSeccode($imhash,$value){
	list($checkvalue, $checktime, $checkidhash) = explode("\t", decrypt(cookie('seccode'.$imhash),MD5_KEY));
	$return = $checkvalue == strtoupper($value) && $checkidhash == $imhash;
	if (!$return) setIMCookie('seccode'.$imhash,'',-3600);
	return $return;
}
		...
		foreach ($fields_array as $key => $value ) {
			...
			if (is_array($data) && array_key_exists($k, $data)) {
				...
				if ($format_array && in_array($k, $format_array)) {
					$out_array[$k] = imPriceFormatb($data[$k]);
				}
			}else {
				$out_array[$k] = "--";
			}
 ...
 */
function setZLCookie($name, $value, $expire='3600', $path='', $domain='', $secure=false){
	if (empty($path)) $path = '/';
 ...
 * @param string $secure 是否通过安全的 HTTPS 连接来传输 cookie,默认为false
 */
function setIMCookie($name, $value, $expire='3600', $path='', $domain='', $secure=false){
	if (empty($path)) $path = '/';	
	if (empty($domain)) $domain = SUBDOMAIN_SUFFIX ? SUBDOMAIN_SUFFIX : '';
...
function showMessage($msg,$url='',$show_type='html',$msg_type='succ',$is_show=1,$time=3000){
	...
	switch ($show_type){
		...
		case 'exception':
			...
			echo '<dd><p /><p /><p /><p /></dd>';
			echo '<dd><p /><p /><p /><p />Copyright 2014-2016 '.C('site_name').', All Rights Reserved '.$lang['company_name'].'</dd>';
			echo '</dl>';
			echo '</body>';
			echo '</html>';
			exit;
		...
		case 'msg_dialog':
			 ...
			 */
            Tpl::setDir('');
			Tpl::output('html_title',Language::get('im_html_title'));
			Tpl::output('msg',$msg);
			Tpl::output('url',$url);
			Tpl::output('msg_type',$msg_type);
			Tpl::output('is_show',$is_show);
			Tpl::showpage('msg','msg_dialog_layout',$time);
			break;
		default:
		    /**
		     * 不显示右侧工具条
...
function showEditor($id, $value='', $width='700px', $height='300px', $style='visibility:hidden;',$upload_state="true", $media_open=false, $type='all'){
    ...
    case 'simple':
        $items = "['source', '|', 'fullscreen', 'undo', 'redo', 'cut', 'copy', 'paste', '|',
            ...
            'insertunorderedlist', '|', 'emoticons', 'image'".$media.", 'link', '|', 'about']";
        break;
    default:
	...
	echo '
<script src="'. RESOURCE_SITE_URL .'/kindeditor/kindeditor.js" charset="utf-8"></script>
...
* @return string	$price_format
*/
function imPriceFormatForList($price) {
    if ($price >= 10000) {
 ...
 * @return string 通知内容
 */
function imReplaceText($message,$param){
	if(!is_array($param))return false;
	//yangbaiyan//$param['mail_send_time']	= date('Y-m-d H:i');
 ...
 * 推广员二维码
 *
 * @param $param string 文件名
 * @return string
 */
function GetExtensionQRcode($member_id, $update = 0){
	if($update == 1 || !file_exists(BASE_UPLOAD_PATH.DS.ATTACH_EXTENSION_QRCODE.DS.'qrcode_'.$member_id.'.png')){
		// 生成商品二维码
        include(BASE_RESOURCE_PATH.DS.'phpqrcode'.DS.'index.php');
		$PhpQRCode = new PhpQRCode();
		$PhpQRCode->BuildExtensionQRCode($member_id);	
	}
	return UPLOAD_SITE_URL.DS.ATTACH_EXTENSION_QRCODE.DS.'qrcode_'.$member_id.'.png';
}
 ...
 * @return string
 */
function getMicroShopAvatarForID($id){
	if(file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_MICROSHOP_AVATAR.'/avatar_'.$id.'.jpg')){
		return UPLOAD_SITE_URL.'/'.ATTACH_MICROSHOP_AVATAR.'/avatar_'.$id.'.jpg';
	}else{
		return UPLOAD_SITE_URL.'/'.ATTACH_MICROSHOP_AVATAR.DS.'default_bg.png';
	}
}
...
function getStoreLogo($img, $type = 'store_avatar'){
    if ($type == 'store_avatar') {
        if (empty($img)) {
            ...
            return UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.'LOGO'.DS.$img;
        }
 ...
 * 取得商品二维码
 *
 * @param string $店铺ID及商品ID
 * @return string
 */
function GetGoodsQRCode($store_id,$goods_id){
	if (!file_exists(BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.'goodQRcode'.DS.$store_id.DS.$goods_id.'.png')){
        return UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.'default_qrcode.png';
    }else{
        return UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.'goodQRcode'.DS.$store_id.DS.$goods_id.'.png';
	}
}
 ...
 * @return string
 */
function GetStoreQRCode($store_id){
	if (!file_exists(BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.'storeQRcode'.DS.$store_id.'.png')){
        return UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.'default_qrcode.png';
    }else{
        return UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.'storeQRcode'.DS.$store_id.'.png';
	}
}
 ...
 * @return string
 */
function GetWeiXinQRCode($wx_id){
	if (!file_exists(BASE_UPLOAD_PATH.DS.ATTACH_WEIXIN.DS.'wxQRcode'.DS.$wx_id.'.png')){
        return UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.'default_qrcode.png';
    }else{
        return UPLOAD_SITE_URL.DS.ATTACH_WEIXIN.DS.'wxQRcode'.DS.$wx_id.'.png';
	}
}
 ...
 * 获取EDITOR专题图片
 */
function getAttachEditorImageUrl($image_name='') {
    return UPLOAD_SITE_URL.DS.ATTACH_EDITOR.DS.DS.$image_name;
}
 ...
 * 如果文件名中包含"_"使用"#"代替
 *
 * @example import('cache'); //require_once(BASE_PATH.'/common/libraries/cache.php');
 * @example import('libraries.cache');	//require_once(BASE_PATH.'/common/libraries/cache.php');
 * @example import('function.kernel');	//require_once(BASE_PATH.'/common/function/kernel.php');
...
function template($tplpath){
	if (strpos($tplpath,':') !== false){
		...
		return BASE_PATH.'/skins/'.TPL_NAME.'/'.$tplpath.'.php';
	}
}
...
function chksubmit($check_token = false, $check_captcha = false, $return_type = 'alert'){
	...
	if ($check_captcha){
		if (!checkSeccode($_POST['imhash'],$_POST['captcha'])){
		    setIMCookie('seccode'.$_POST['imhash'],'',-3600);
			if ($return_type == 'alert'){
				showDialog('验证码错误!');
			}else{
				return -12;
...
function write_file($filepath, $data, $mode = null)
{
    ...
    $data = "<?php defined('InIMall') or exit('Access Invalid!'); return ".$data.";";
...
function loadfunc($filename, $file_ext = '.php'){
	if(preg_match('/^[\w\d\/_.]+$/i', $filename.$file_ext)){
		$file = realpath(BASE_PATH.'/common/function/'.$filename.$file_ext);
	}else{
		$file = false;
	}
	if (!$file){
 ...
 * @return obj 对象形式的返回结果
 */
function im_class($classname = null){
	static $_cache = array();
	if (!is_null($classname) && isset($_cache[$classname])) return $_cache[$classname];
	$file_name = BASE_PATH.'/common/libraries/'.$classname.'.class.php';
...
function ubb($ubb){
	$ubb = str_replace(array(
			...
			"<span style=\"color:#$1\">",
			"<img src=\"".CIRCLE_SITE_URL.'/skins/'.TPL_CIRCLE_NAME."/images/smilier/$1.png\">",
...
function snsShareFrom($sign) {
    switch ($sign) {
        ...
        case '2' :
            ...
            break;
        case '3' :
            return L('sns_from') . '<a target="_black" href="' . MICROSHOP_SITE_URL . '">' . L('im_modules_microshop') . '</a>';
            break;
        case '4' :
            return L('sns_from') . '<a target="_black" href="' . CMS_SITE_URL . '">CMS</a>';
            break;
        case '5' :
            return L('sns_from') . '<a target="_black" href="' . CIRCLE_SITE_URL . '">' . L('im_circle') . '</a>';
            break;
    }
}
...
function getChat($layout){
	if (!C('node_chat') || !file_exists(BASE_COMMON_PATH.'/libraries/chat.php')) return '';
...
/**
 * Mobile商城后台使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $act control文件名
 * @param string $op op方法名
 * @param array $args URL其它参数
 ...
 */
function urlMobile($act = '', $op = '', $args = array())
{
	return url($act, $op, $args, false, MOBILE_SITE_URL);
}
/**
...
function urlAdminshop($act = '', $op = '', $args = array()){
    return url($act, $op, $args, false, SHOP_modules_URL);
}
/**
 ...
 * @return stringurlAdminMobile
 */
function urlAdminExtension($act = '', $op = '', $args = array()){
    return url($act, $op, $args, false, EXTENSION_modules_URL);
}
 ...
 * @return string
 */
function urlWeiXin($act = '', $op = '', $args = array()){
    return url($act, $op, $args, false, WEIXIN_SITE_URL);
}

/**
 * 微信小程序使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $act control文件名
 * @param string $op op方法名
 * @param array $args URL其它参数
 * @return string
 */
function urlWxapp($act = '', $op = '', $args = array()){
	return url($act, $op, $args, false, WXAPP_SITE_URL);
}
/**
 * 手机浏览器使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $act control文件名
 * @param string $op op方法名
 * @param array $args URL其它参数
...
function urlWap($act = '', $op = '', $args = array()){
	return url($act, $op, $args, false, WAP_SITE_URL);
}

function urlBaiMap($lat,$lng,$title,$content){
	return "http://api.map.baidu.com/marker?location=".$lat.",".$lng."&title=".$title."&content=".$content."&output=html";
}
/**
 * 验证是否为平台店铺
 *
...
* @return string 
*/
function payment_method_name($payment_method) {	
 ...
 */
function BuildDateQueryStr($type='', $fromdate = '', $enddate = '', $returnArr = false) {
    $request_date_str = '';
	$dataArr = array();
	if($type == 'today'){			
		if ($returnArr == true){
			$dataArr['startdate'] = date('Ymd',time());
			$dataArr['enddate'] = date('Ymd',time());
		}else{
		...
		if ($returnArr == true){
			$dataArr['startdate'] = $year.'-'.$month.'-1';
			$dataArr['enddate'] = $year.'-'.$month.'-'.$daynum;
		}else{
			for($i = 1;$i<=$daynum;$i++){
		...
		$year = date('Y',time());
		if ($returnArr == true){
			$dataArr['startdate'] = $year.'-1-1';
			$dataArr['enddate'] = $year.'-12-31';
		}else{				
			$day31 = array('01','03','05','07','08','10','12');
			for($i=1;$i<=12;$i++){
				$month = $i<10?'0'.$i:$i;
		...
		switch ($day){
			...
			case 'Saturday':
				...
				$dataArr['startdate'] = date('Ymd',time()-86400*5);
			    $dataArr['enddate'] = date('Ymd',time());
				break;
			case 'Sunday':
				...
				$dataArr['startdate'] = date('Ymd',time()-86400*6);
			    $dataArr['enddate'] = date('Ymd',time());
				break;
		}			
	}else {
		if($fromdate != '' && $enddate != ''){
            $fromsp = strtotime($fromdate);
			$tosp   = strtotime($enddate);
			if ($returnArr == true){
				$dataArr['startdate'] = $fromdate;
...
function timediff( $begin_time, $end_time )
{
  ...
  //$secs = $remain % 60;
  $res = $days;//array( "day" => $days, "hour" => $hours, "min" => $mins, "sec" => $secs );
  return $res;
}
 ...
 *@ 要存储的文件名
 */
function DownloadRemoteImg($url = "", $filename = "")
{
    //去除URL连接上面可能的引号
